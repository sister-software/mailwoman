...
3. **The model release version is the NEXT UNIFIED number — verify, don't assume.** A *code-only*
...
- **Pass the explicit semver** to `-f version=` (e.g. `4.13.0`). It becomes release-it's
  `--increment=<value>`. (`--minor` through the local yarn wrapper silently degraded to a *patch* —
