...
- **Model gap (gazetteer-only):** 38 — the lowest-effort tier is a `locale`-recipe extract build:
  **NZ** (2.12M LINZ-derived OA street rows on disk — the best-provisioned), then **CA/MX/BR**
  (Deepparse-trained, OA extracts on disk). NZ is a real extract build, **not** a bare declare — the
