...
so a layout that adds one top-level entry per country would not scale.
...
| `src/mailwoman_train/`   | 17,371 over 48 modules | flat, without subpackages                                                 |
...
`config.py` is excluded from this work. It is 629 lines of typed dataclasses with a strict merge/coerce loader, the
same structure every reference project uses. It moves into `config/` as two modules and is otherwise unchanged.
...
Five deferred imports inside `encode_with_features` exist to avoid the cycle. The cycle exists because `PieceSpan`,
the shared value type, lives in the module that also does the work. Because of the deferral, an `ImportError` inside
`country_lexicon` raises at first call rather than at import, so a broken module reaches a training run instead of
failing at startup.

Their comment claims they keep `tokenizer.py` import-light. Measurement contradicts that: `gazetteer_anchor` adds
2,263 µs and `country_lexicon` 117 µs on top of `tokenizer`'s 36,869 µs, which is 6.5%. Neither module imports
torch. Deferred imports elsewhere in the tree do save real startup time and stay (see §11).
...
normalization has no module of its own, so the first file that needed it became its home: 978 lines of builder that
five modules depend on for four names. A `text/` module should own those names, so the dependency points from the
builders to the utility.

`AGENTS.md` describes this failure for TypeScript under "Before writing a small utility, check whether it already
...
Role directories sit at the top for cross-country code, and the same role names are nested under a country key for
country-specific code. A new country adds one directory holding only the roles it needs.

`corpus-python` adopts that model with one change: the country keys sit under `countries/` rather than at the package
root. Corpus has 14 country and region directories among 7 role directories today, and with every country present
the role directories would be hard to find. One extra path segment keeps the package root at roughly twelve entries
permanently.

The key is `countries/` rather than `locales/`, because the Python per-country code (register readers,
script-specific text normalization) is keyed by country. In this repository, `locale` means the `en-US` form that
keys the weights packages and the `--locale` flag.
...
The tree uses `observability/` rather than `logging/`. Python resolves a subpackage named `logging` correctly inside
a package that also imports the standard library module of that name, but a reader would find it confusing.
`torchtitan` uses the same name.

The tree uses `evaluation/` rather than `eval/` for the same kind of reason, plus a mechanical one. The agent
worktree's write guard refuses any shell command containing that three-letter token, so a directory with exactly that
name could never again be moved, renamed or removed from a worktree. The longer name also avoids confusion with the
builtin.

## 7. The extension work

The extension work has two pieces, both taken from projects that solved the same problem at larger scale.
...
**`train/callbacks/`** follows `allenai/OLMo-core`, which has 22 callback modules, one per concern. `train()`'s 476
lines become a loop plus five callbacks: console logging, the CSV row (`eval_csv_row`), the periodic evaluation
(`_eval_val`), checkpointing (`save_checkpoint`), and trackio. Each becomes independently testable, which none of
them is today.
...
clones follow the runbook. That step must change from "add a function" to "add a row", or the next corpus version
will add clone 58.
...
`head: "modal"` rather than a filename.
...
- Widen `[tool.mypy] files` from `["src"]` to include `launch/`, which this work rewrites anyway. `tests/` stays
  out: `uv run mypy --strict tests` reports 663 errors in 54 of 56 files on this branch, so bringing it in needs its
  own project.
...
1. Add `types.py` and remove the five deferred imports.
2. Add `text/` and repoint the five importers away from `build_jp_slice`.
...
8. Write the launch-table test, rename `modal/` to `launch/`, and collapse the 57 and the 7.
...
- No module in `src/mailwoman_train/` exceeds 500 lines, and no function exceeds 120. Two modules sit near the limit
  and determine where it lands. `config.py`'s dataclass block is 478 lines and moves to `config/schema.py` under the
  limit. `tokenizer.py` is 609 lines and must therefore split further than a rename into `tokenizer/__init__.py`.
- The Python prefix check reports zero groups.
- No intra-package import inside a function body imports a module that transitively imports the module holding it.
  This detects a regression of §3.1, and it flags a cycle rather than every deferred import.
  ...
  every `--help`. Of the 63 deferred intra-package imports in the tree, five exist to avoid a cycle (`tokenizer.py`
  at 541, 550, 566, 583 and 598), and the other 58 reduce startup time.
