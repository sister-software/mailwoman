...
**Prerequisite:** PR #1092 (phase-0 goldens) merged; branch `feat/legacy-excision-swaps` off updated main. Execute on the lab host (weights symlinks + gazetteer present) — the check suites skip in CI by design, so the live receipts in each task are the real check.
...
- Test: `mailwoman/test/v1-parse-check.test.ts` (new — golden check)
...
- [ ] **Step 3: Rewire `createServeEngine`** — `mailwoman/api-engine.ts`: delete the rules block (lines ~222-236: the `createAddressParser()` + old `parse` closure and the `createAddressParser`/`createDiagnosticReport` imports if now unused). Build `parse` from the neural classifier, decoupled from the WOF-data check so parse degrades only on missing weights:

```ts
// Parse needs only the model weights — not the gazetteer. Load them independently of the
// WOF-data check below so `/v1/parse` answers whenever weights resolve, even on a geocode-degraded boot.
...
Reuse the loaded module/classifier for the geocode stack below instead of importing twice (restructure the existing `neuralMod` import + `classifier` construction so weights load ONCE — the geocode path keeps its WOF check and 503 behavior). `decodeAsTuples`/`decodeAsXML` import from `@mailwoman/core`. Update the degraded-branch returns (`{ engine: { parse, health }, … }`) so they carry the new `parse` (or undefined).
...
- [ ] **Step 5: Golden check test** — `mailwoman/test/v1-parse-check.test.ts`:
...
describe.skipIf(!weightsPresent())("v1-parse golden check (neural vs rules baseline)", () => {
	test("pre-registered agreement floors hold", async () => {
		...
		for (const { label, floor } of FLOORS) {
			...
			console.error(`check ${label}: ${hit}/${total} = ${rate.toFixed(4)} (floor ${floor})`)
...
```bash
git add api/engine.ts api/schema.ts mailwoman/api-engine.ts mailwoman/test/api-engine.test.ts mailwoman/test/v1-parse-check.test.ts
git commit -m "feat(api)!: /v1/parse speaks native neural output (components + tree); weights-conditional 501"
```
...
- Test: `libpostal/parse-check.test.ts` (new)
...
- [ ] **Step 2: Golden check test** — `libpostal/parse-check.test.ts`: same skeleton as Task 2's check (license header, `weightsPresent()` via the same symlink probe path, `describe.skipIf`, 300s timeout). Body outline:
...
for (const row of goldenRows) {
	...
	// Wire-shape check: status 200, and every returned label is a mapped libpostal label
	// or a known pass-through (a label outside KNOWN_LABELS fails the check).
...
```bash
git add libpostal/cli.ts libpostal/index.ts libpostal/parse-check.test.ts
git commit -m "feat(libpostal)!: /parse backed by the neural classifier; weights pre-flight; golden check"
```
...
- [ ] **Step 1: Full verification sweep** — `yarn tsc -b` (repo), `yarn vitest --run mailwoman/test/legacy-golden-integrity.test.ts` (goldens untouched), the three check suites (paste all printed rates), `yarn lint` on touched files, `git status` clean.
- [ ] **Step 2: Push + PR** titled "Legacy excision plan 2: production surfaces off the rules parser", body carrying: the three swaps, the check rates per label vs the pre-registered floors, the nominatim classified diff summary, and the note that `/v1/parse`'s wire change is the v7 breaking change (migration guide lands in plan 5). Same attribution trailers as phase 0.
