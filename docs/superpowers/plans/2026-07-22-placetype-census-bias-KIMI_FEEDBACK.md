...
**Sound design — GO, with seven required changes before task expansion.** The core claim (a two-sided register-pair check is precise enough to make a soft decode bias viable) is well-supported by rung 3, and the v385 control is exactly the right experiment to prove the check doesn't manufacture signal. The gaps below are about _where the evidence is circular_, _how the feature plugs into the existing prior stack_, and _three decisions the plan treats as open that the codebase already answers_.
