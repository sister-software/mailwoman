...
**Gate (pre-registered): bare-char-JP coord acceptability ≥ 0.70 (bare-Latin floor).** PASS → Phase 2 (JP-seven activation, 33→47 head w/ own param-group LR per #727) + Phase 3 (full extract + channels re-aligned per-unit). Run Leg-2 same session.
