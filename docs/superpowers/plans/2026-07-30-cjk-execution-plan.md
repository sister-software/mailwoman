...
**Resolves:** ROAD_TO_MAILWOMAN_V8_3_0 §4 D5 — and upgrades Arc 2 from "reduce-time scope call" to
...
- Shared surfaces, both additive: the release train (HF/R2/preflight gain the second artifact +
  ...
  construction).
- **D5 resolution:** the _work_ is committed now; whether the JP model ships inside the 8.3.0 reduce
  or as an 8.4 fast-follow is decided at the Leg-1 verdict by ordinary check arithmetic — with the
