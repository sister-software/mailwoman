...
2. **Probe 0b — CRF transitions (CPU):** confirm decode ignores learned transitions today; fit a transition matrix on (base + fragment extract) labels with the encoder frozen; decode Viterbi-with-transitions; re-run parity. Cheapest possible sequence-prior lever.
