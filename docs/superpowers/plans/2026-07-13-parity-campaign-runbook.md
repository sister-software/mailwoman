...
**Context:** the v7 excision is blocked on `mailwoman eval parity` floors (house_number ≥ 0.97, postcode ≥ 0.97, street ≥ 0.90). Baseline after the $0 splice (v242-multisplice, staged on the data root): house_number 0.7273, postcode 0.9861 PASS, street 0.4033. Residual = label-pattern knowledge (fragments, trailing-number, AU units), proven not tokenizer-bound.
**Consult:** DeepSeek pro ×3 (session 019f590a, 2026-07-13). Structural advice adopted with corrections below; embedded numbers (ratios, step counts, thresholds) are STARTING HYPOTHESES to adjudicate, never checks — per the standing calibration policy.
