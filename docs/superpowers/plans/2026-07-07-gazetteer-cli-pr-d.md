...
### Task 1: `gazetteer-pipeline/postcode/` — the extract build with fills folded
...
- Create: `mailwoman/gazetteer-pipeline/postcode/index.ts` — `buildPostcodeExtract(opts)`:
...
- Create: `mailwoman/commands/gazetteer/build/postcode-extract.tsx` — thin command (`--country us`, `--out`, source-dir overrides)
- Delete: `scripts/fill-zcta-centroids.ts`, `scripts/backfill-postcode-centroids.ts`
- Modify: RELEASING.md postcode mentions; manifest `postcode_build*` sections gain a "superseded by `gazetteer build postcode-extract`" note

**Steps:**

- [ ] Move lib + test; fix imports; existing zcta tests stay green.
- [ ] `buildPostcodeExtract` + fills (lazy resolver-wof-sqlite imports); fixture test: tiny staging DB with a `(0,0)` postcode row → fill from a fixture centroid map → sealed output.
- [ ] Command + compile + `gazetteer build postcode-extract --help` smoke.
- [ ] Delete the two mutators; straggler `rg`; docs touch-ups; typecheck + tests green; commit.

### Task 2: drawer classification sweep

- [ ] `git mv scripts/audit-po-box-cedex-extract.ts scripts/eval/` (an audit is an eval; fix any `./lib` depth).
