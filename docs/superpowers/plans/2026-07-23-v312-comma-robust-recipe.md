...
**Date:** 2026-07-23 · **Status:** proposal rather than launched · **Context:** the v3.11.x lineage is closed for shipping (stop rule executed): no checkpoint passes the gauntlet metamorphic layer; the terminal break — comma-free US address → total resolution loss — is byte-stable under damped consolidation (8k ≡ 10k), i.e. a learned behavior rather than churn. Five-whys record: the dep-loc extracts are ~100% comma-structured; commas were promoted toward required boundary evidence; comma-free robustness paid the bill.
...
- **Probe-level blocking at 2k** (the process fix, now mandatory): the invariance mini-suite (`mailwoman eval invariance --baseline v385`) runs at every checkpoint grade rather than at ship time. A 2k read showing the comma-drop class regressing = stop at probe cost.
