...
**Architecture:** Everything stays one-directional: route tables → emitted documents → generated clients, with no artifact committed downstream of the spec. Spec emission = an `openapi` subcommand on each drop-in CLI (each already owns its app; zero new deps) + a `mailwoman openapi` command for the native surface (`mailwoman` deps `@mailwoman/api` already). The docs site's static spec copies become build products (docs prebuild runs the emitters), killing the staleness class permanently. Client generation salvages the abandoned branch's layout decisions (`mailwoman_client` Python package with per-surface modules; progenitor compile-time Rust crate) — the operator authored those names; the difference is the specs now come from the emitters, and no generated artifact lands in git.
