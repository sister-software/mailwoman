...
- Tests: the three `mailwoman/test/*-router.test.ts` files port to the wired engine (same skip-if-stack gates: error paths unconditional via `createMailwomanAPI(wiredEngine)`… note error paths need NO stack — use the engine factory's preflight-bypassed form or fixture engines; success paths gate on WOF+extracts as today). `server/metrics.test.ts` dies with its module (api-kit's port owns the coverage — verify api-kit/metrics.test.ts covers every assertion the old file had before deleting; anything missing moves over).
