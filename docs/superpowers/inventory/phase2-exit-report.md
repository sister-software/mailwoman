...
arriving there, though it doesn't link _forward_ to those guides; logged as a gap (§7) rather than fixed,
since fixing it means restructuring a table page rather than a 1–3 line add.
...
2026-05-26)" block calling the WOF prepare pipeline stalled; logged as a gap rather than fixed (a
paragraph-level rewrite rather than a link fix).

## 7. Remaining gaps (not fixed — logged for the next phase/author pass)

**In-scope findings from this audit rather than touched (exceed the 1–3-line fix bar):**
...
- The record-matching cluster (`cross-registry-linking.mdx`, `dedup-entity-truth.mdx`,
  ...
  Adding one is a real (if small) content addition rather than a 1–3-line audit fix.
- `how-mailwoman-compares.mdx` — inventory marks it `role: reference`, but it carries no
  ...
  switching-from-* guides, doesn't link forward to any of them (the guides link back to it rather than the
...
**Queued externals (per brief, included verbatim — confirmed still pending rather than touched by this
audit):**
