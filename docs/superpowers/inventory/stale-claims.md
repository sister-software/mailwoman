...
- Training has been cross-entropy-only since model v0.5.0. `crf_loss_weight: 0.0` in **every** training config from `corpus-python/src/mailwoman_train/configs/v0_5_0-classifier-ce-only-full.yaml` (first CE-only config) through the current/latest `v2.9.0-country-counterweight.yaml:89` (uncommitted, today's work). `model.py:708` runs the entire CRF forward+backward pass only when `crf_loss_weight > 0`. When the weight is 0 (every current config), `model.py:748` falls through to `loss = ce_loss`, so the CRF module gets **no gradient**. The shipped model card states this explicitly at `neural-weights-en-us/model-card.json:33-34`: `"crf_at_training": false, "crf_at_inference": true`.
- The shipping JS runtime decoder (`neural/viterbi.ts`) implements Viterbi over a **frozen, hand-coded BIO-structural transition mask** (`buildBIOTransitionMask`) and never uses learned transitions. Its own header (lines 12-20) says the learned-transition mode is "Currently not exported from the training-side ONNX bundle." `package_weights.py:104-115` (`export_crf_transitions`) confirms this by returning `None` whenever `crf_loss_weight == 0.0`. No `crf-transitions.json` exists anywhere in the repo or in `neural-weights-en-us/` (confirmed by `find`).
...
| 2   | v3.0.0 CRF instability "fixed" in v0.4.0, without mention of the permanent CE-only fallback since v0.5.0 | `concepts/crf-decoder.mdx:102-112`                           | misleads-reader |
...
**Ground truth:** Training has been CE-only (`crf_loss_weight: 0.0`) in every config since v0.5.0. The branch at `model.py:708` (`if self.crf is not None and attention_mask is not None and self.crf_loss_weight > 0:`) computes the CRF NLL and backpropagates into the transition matrix, and it never runs under the shipped recipe. The branch that runs is `model.py:748`, `else: loss = ce_loss`. The shipped model card states this at `neural-weights-en-us/model-card.json:33`: `"crf_at_training": false`.
...
**Severity:** misleads-reader. The page presents an inert module (present in the graph but never trained) as an active learning mechanism.
...
**Ground truth:** CRF training was reactivated after v0.4.0 and diverged three more times before being permanently disabled. The comment at `corpus-python/src/mailwoman_train/configs/v0_6_0-stage3.yaml:66-76` reads: "`crf_loss_weight: 0.0` after three divergence attempts: attempt 1 (lr=1.5e-4, crf=0.5): NaN at step 950 / attempt 2 (lr=1.0e-4, crf=0.1): NaN at step 1700 ... v0.5.4 used crf_loss_weight=0.0 (CE-only) and trained fine — match that." A later fp32 diagnostic (`v0_6_2-crf-fp32-diagnostic.yaml`) tested whether bf16 precision was the cause. The next config in the lineage, `v0_6_3-house-venue.yaml:122`, returns to `crf_loss_weight: 0.0`, and every config after it stays at 0.0 through the current `v2.9.0`.
...
**Severity:** misleads-reader. The omission turns "abandoned after repeated divergence" into "fixed."
...
**Ground truth:** The CRF NLL is computed only when `crf_loss_weight > 0` (`model.py:708`), and the weight is `0.0` in the shipped recipe. See Finding 1.
...
**Ground truth:** The current active label set has 33 labels (`16 tags × {B-,I-} + O`), not 21. `corpus-python/src/mailwoman_train/labels.py:14` says "`STAGE3_BIO_LABELS` (33) — v0.6.0 ship, the CURRENT active set", `labels.py:84-135` defines it, and `neural-weights-en-us/model-card.json:31` records `"num_labels": 33`. The correct transition-matrix size is 33×33+33+33 = 1155 scalars. The parameter count at `model-card.json:32` is `"params": "33.9M"`, not 8.87M. A sibling doc already has the correct label count: `concepts/how-the-model-reasons.mdx:160` says `raw emission logits per token × 33 labels`.
...
**Severity:** misleads-reader. The claim is cheap to falsify, and a sibling page in the same directory contradicts it.
...
**Ground truth:** Same as Finding 4: 33 labels and 16 tags. The doc's own tag table (lines 40-51) lists only 10 base tags (country, region, locality, dependent_locality, postcode, subregion, cedex, venue, street, house_number) and is missing 6 tags present in the shipped label set: `street_prefix`, `street_suffix`, `unit`, `po_box`, `intersection_a`, `intersection_b`.
...
**Ground truth:** `neural-weights-en-us/model-card.json:25,27-28,31-32` records `hidden_size: 384`, `num_attention_heads: 6`, `intermediate_size: 1536`, `num_labels: 33`, and `params: "33.9M"`. All five geometry numbers in the doc's table are wrong, beyond the label count covered in Findings 4-5. The model has grown considerably since the table was last updated (nsplice vocabulary embedding growth, more heads, and more width).
...
**Ground truth:** The tokenizer in the currently shipped weights bundle is two generations past both versions the doc mentions. `neural-weights-en-us/model-card.json:10,30` records `"tokenizer_version": "0.8.0-fr-nsplice"` and `"vocab_size": 66319`, which is 4x the doc's "shipping" figure and 38% larger than the "validated, pending" A1 figure the doc describes as unshipped. `neural-weights-en-us/model-card.json:110` independently describes the tokenizer as "SentencePiece unigram, byte_fallback=true, vocab_size=66319 (v0.8.0-fr-nsplice = v0.7.1-nsplice + 2406 FR diacritic pieces; ships with the model)."
...
Ground truth restated from `.github/workflows/publish.yml:129-131`: `# fr-fr shares the en-us model for now (locale-specific weights are future work).` / `cp neural-weights-en-us/model.onnx neural-weights-fr-fr/model.onnx` / `cp neural-weights-en-us/tokenizer.model neural-weights-fr-fr/tokenizer.model`. There is no fr-fr-specific training run, and the two npm packages ship byte-identical model files. (`concepts/neural-classification.mdx:85` describes this accurately: "Trained on en-US + fr-FR data, the model has internalized the conventions of both." It is **one** model trained on a combined corpus, which is why the packages can share a binary. The findings below are pages that instead describe two separate per-locale artifacts.)
...
**Ground truth:** "Separate weight packages" is literally true, because two distinct npm packages exist and can be distributed and versioned independently. The sentence does not claim that the _contents_ differ. This finding has lower confidence than 8, 10, and 11. It is flagged because it sits one line below Finding 8 and reinforces the same impression, although the literal words are defensible.
...
**Ground truth:** No independent fr-fr training track exists that could "catch up", because fr-fr's weights are a direct copy of en-us's. The gap the sentence describes (fr-fr lagging en-us in attention) does not exist in that form. What exists is a policy decision to defer training fr-fr separately.
...
**Ground truth:** The weights are one identical binary, copied at publish time. This is the most directly falsifiable instance, because the page uses the false claim as evidence for an architecture argument that a reader might repeat.
...
**Ground truth:** The sentence is literally true in isolation, because both packages are published and both work. In a conference talk proposal about the neural architecture, however, it invites the inference that two independently trained, locale-specialized models exist, and they do not.
...
**Ground truth:** The shipped int8 bundle is ~36.8 MB. The page's 50 KB figure is off by roughly three orders of magnitude. `neural-weights-en-us/model.onnx` is 36,787,564 bytes on disk, and `neural-weights-en-us/model-card.json:114` records `"int8_size_mb": 36.8`. This is not strictly a locale-sharing claim, because it would be wrong even if fr-fr had its own model. It repeats the same "per locale" framing on the same page as Finding 12, so it is flagged alongside it.
...
- `docs/articles/plan/phases/PHASE_2_training.mdx:161` and `PHASE_6_japan.mdx:57` discuss per-locale vs. shared-model tradeoffs as **open roadmap questions**. They make no claim about current state, so they are not flagged.
...
- `docs/articles/concepts/how-the-model-reasons.mdx:134` hedges learned CRF transitions correctly ("If learned CRF transitions are present (v0.6.4+ with `crf_fp32=true`)...") and already uses the correct 33-label count (line 160). It is cited above as the accurate contrast and is not a finding.
- `docs/articles/concepts/attention-and-bidirectional-context.mdx:83-84` and `concepts/what-mailwoman-is.mdx:30` mention BiLSTM/LSTM only as comparisons with historical NLP lineage ("the CLOSEST formal analog in classical NLP is... BiLSTM-CRF → BERT token classification → this"), not as claims about Mailwoman's own architecture. Not flagged.
- `evals/` and `retrospectives/` are excluded per task scope, because they are dated, point-in-time records. The task did not exclude `reviews/`, but it is also dated and point-in-time by construction. Its few "locale-specific weight packages" mentions were reviewed and judged low-signal because they refer to a feature name and make no architecture claim, so they were not written up.
- The "677-million-row corpus" figure (`understanding/our-approach/from-pelias-to-mailwoman.mdx:41`) was not checked. `model-card.json`'s `training` block has no authoritative row-count field to verify it against cheaply, so it fell outside the "cheap to check" bound for the item-3 sweep.
