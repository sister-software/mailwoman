...
:::info[Verified as of 2026-06-11 — release 4.3.0]
Numbers below are sourced from the shipped `model-card.json` and the
[v4.3.0 ship gate](./evals/2026-06-11-v4.3.0-ship-gate.md), which carries the
...
All publishable workspaces release in lockstep; the current npm version is **4.3.0**.
...
| Property             | Value                                                                                                          |
| -------------------- | --------------------------------------------------------------------------------------------------------------- |
| **Lineage**          | `v1.1.0-relabel-consolidation` @ step 40k, from scratch — the consolidation recipe on a label-consistent mix (#511 affix relabel), corpus `v0.4.12` + relabel pass |
...
| **Conventions**      | Locale head exported (`locale_logits`) → address-system conventions mask (#478 slice 1): fr forbids the USPS affix decomposition + pins the 5-digit postcode shape |
...
The shipped weights are **4.3.0** — the relabel run ([#511](https://github.com/sister-software/mailwoman/issues/511), the #492 ladder's data answer) plus the conventions mask. #492 is resolved (capacity falsified; cause was data). The next pipeline step is arbitration ([#478](https://github.com/sister-software/mailwoman/issues/478), slice 1 shipped); the next coverage lever is po_box/cedex. The ordered
