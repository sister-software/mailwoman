...
- Tokenization and whitespace traps ([#3 in the failure catalogue](../why-its-hard/addresses-that-break-geocoders.md#3-tokenization-and-whitespace-traps)) — `"12 1/2 Main St"` and `"12½ Main St"` collapse to a canonical form before the tokenizer sees them.
- Unicode/transliteration encoding half ([#6](../why-its-hard/addresses-that-break-geocoders.md#6-unicode-and-transliteration-traps)) — NFC ensures the resolver does not miss `"São"` because the input was NFD-encoded.
...
- Unicode/transliteration script-handling ([#6](../why-its-hard/addresses-that-break-geocoders.md#6-unicode-and-transliteration-traps)) — picks the right tokenizer and weights for the script.
- Language-switch hybrids ([#7](../why-its-hard/addresses-that-break-geocoders.md#7-language-switch-hybrids)) — when the input mixes scripts, the gate either picks a hybrid-trained weights package or falls back to a multi-locale ensemble.
...
- Street/locality collisions ([#4](../why-its-hard/addresses-that-break-geocoders.md#4-streetlocality-collisions)) — the encoder sees the whole sequence and learns that `"Paris"` near `"Cafe"` is part of a venue, while `"Paris"` between `","` and `"Ontario"` is a locality.
...
- Ambiguous locality names ([#1](../why-its-hard/addresses-that-break-geocoders.md#1-ambiguous-locality-names)) — the re-reader gives the model a second look at `"Paris"` with structural priors that the next token is a region/country.
- Repeated admin names ([#2](../why-its-hard/addresses-that-break-geocoders.md#2-repeated-admin-names)) — BIO structure + re-reader together handle `"New York, New York"` as two adjacent localities.
- Numeric chaos ([#5](../why-its-hard/addresses-that-break-geocoders.md#5-numeric-chaos)) — the CRF prevents structurally-impossible label combinations; the re-reader can revisit `"12345 123rd St"` after the rest of the sequence is labelled.
...
- The consistency portion of numeric chaos ([#5](../why-its-hard/addresses-that-break-geocoders.md#5-numeric-chaos)) — when `"12345"` could be a house number or a postcode, the solver picks based on what makes the rest of the sequence consistent.
...
- Administrative nightmares ([#8](../why-its-hard/addresses-that-break-geocoders.md#8-administrative-nightmares)) — `"Springfield"` with no region produces 41 candidates. The honest answer is to surface the list, not pretend one is correct.
...
- [Addresses that break geocoders](../why-its-hard/addresses-that-break-geocoders.md) — the failure catalogue this article responds to
- [How it works now](./how-it-works-now.md) — the current pipeline in plainer language
- [How it will work](./how-it-will-work.md) — the v0.4.0 work in plainer language
