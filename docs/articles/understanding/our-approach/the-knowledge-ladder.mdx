...
- **The multi-hot encoding gives homograph awareness for free.** A token whose
  ...
  ambiguous by construction_ — the model learns "context is load-bearing here."
...
There is a pleasing symmetry with the resolver: the ancestors table precomputes the
...
address-system detection load-bearing: detected system → the codex conventions row →
