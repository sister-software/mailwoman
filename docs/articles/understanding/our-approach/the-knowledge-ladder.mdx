...
| **3.2. Gazetteer anchor** | **known-surface clues (per-token candidate-tag sets, input layer)** | Partial — postcode anchor shipped; country anchor in flight ([#464](https://github.com/sister-software/mailwoman/issues/464)); general channel designed |
...
multi-hot "this surface form is a known {country}", "{country, region}", "{po_box
designator}" — as input features. The model still decides every tag; the gazetteer
