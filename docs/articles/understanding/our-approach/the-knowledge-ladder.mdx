...
*informs*, it never *overrides*. Think of it as a caching layer of clues: RAG-shaped
...
is the same idea applied at the *emission* layer, and this rung is its input-layer
sibling.
...
- **It is not overfitting — the knowledge lives outside the weights.** Add a place to
  the gazetteer tomorrow and the clue fires with no retrain. The model learns to *use*
...
- **The multi-hot encoding gives homograph awareness for free.** A token whose
  candidate set has more than one entry ("Georgia" → `[country, region]`) is *flagged as
  ambiguous by construction* — the model learns "context is load-bearing here."
...
hierarchy chain so the *resolver* trades disk for speed; the gazetteer anchor
precomputes surface-form membership so the *parser* trades disk for clues. Same
trade, both stages.
...
([#478](https://github.com/sister-software/mailwoman/issues/478)): per-system *transition*
