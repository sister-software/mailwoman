...
**Gazetteer anchor** — an input-layer feature channel that attaches a per-token **candidate-tag set** from the gazetteer/codex ("this surface is a known {country, region}") so the model conditions on lexicon membership without being overruled by it. The knowledge lives outside the weights — extend the gazetteer, no retrain. See [the knowledge ladder](./our-approach/the-knowledge-ladder.mdx), rung 3.2.
