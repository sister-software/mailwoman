...
**Homonymy / homograph** — one surface, many referents: "Georgia" is a US state in `Atlanta, Georgia` and a country in `Tbilisi, Georgia`. Handled by **disambiguation**, split across two stages: the parser resolves the *tag* from in-string context; the resolver late-binds the *referent* with geographic context (concordance). See [`concepts/synonymy-and-homonymy.md`](../concepts/synonymy-and-homonymy.mdx).
...
**Shallow fusion** — blending an external knowledge source (a gazetteer, an FST prior) into a model's decision as a *signal*, rather than retraining the model or overriding its output. Mailwoman applies it at two layers: the [FST prior](../concepts/fst-priors-as-shallow-fusion.mdx) biases emissions at decode time; the **gazetteer anchor** feeds membership features at the input layer. RAG-shaped, but the retrieval lands as feature vectors, not prompt text.
