...
When we ran these against our own live endpoint the day we received them, three of four resolved confidently to the wrong continent — including one to Comer, Georgia, USA, a namesake nobody in the conversation had predicted (the Spanish verb "comer" happens to be a town). We publish that result rather than hide it: these cases are now tracked improvement targets in the release gauntlet, and the bare-`Shinjuku` case is annotated as genuinely ambiguous — a lone toponym query plausibly _does_ mean Tokyo, and only conversational context says otherwise. An honest geocoder's job on that one is low confidence, not a lucky guess.
