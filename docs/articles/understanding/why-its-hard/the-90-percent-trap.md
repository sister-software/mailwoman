...
Reducing the failure rate from 10% to 5% halves the manual-correction cost. Reducing it from 10% to 2% reduces it by 80%. The value is in reducing the number of addresses that need human intervention, not in parsing more addresses correctly. Every percentage point of parser accuracy past 90% is worth thousands of dollars per month in avoided manual work.
