...
"The train station" without a city name implies "the train station where the user is." The geocoder needs the user's location to resolve this. This is a context problem, not a parsing problem. The parser correctly identifies "train station" as a transit query. The resolver needs a location to search near. Without one, the query is unresolvable.
