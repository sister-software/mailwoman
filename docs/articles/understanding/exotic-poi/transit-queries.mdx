...
The parser handles transit queries as a mixed case: a named station like "Grand Central Terminal" looks like a venue query. An unnamed transit query like "bus stop" looks like an amenity query, and the amenity-query path — the `poi_query` kind, the [`@mailwoman/poi-taxonomy`](https://www.npmjs.com/package/@mailwoman/poi-taxonomy) lexicon, `poi.db` resolution — is real infrastructure now (see [Amenity queries](./amenity-queries.mdx)). It just doesn't cover transit yet: the taxonomy's 23-category seed has no `bus_stop`, `train_station`, or `airport` entries, so an unnamed transit query has nowhere in the lexicon to land.
