...
A single physical building can have multiple valid addresses. This is the normal state of affairs for commercial buildings, multifamily housing, corner properties, and any structure that touches more than one administrative system — not an edge case. A parser that assumes "one building = one address" will fail silently on a large fraction of real-world queries.
...
- [The database fallacy](../why-its-hard/the-database-fallacy.md) — why no database captures all address forms for a building
- [How humans break addresses](../why-its-hard/how-humans-break-addresses.md) — the postal city ≠ municipal city failure mode
