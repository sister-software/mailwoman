...
([below](#the-spine-fellegi-sunter-without-labels)) with the **real** per-level
...
- **A gradient-boosted matcher** over the comparison vector once labels exist. A
  ...
  Fellegi-Sunter spine by ~5pp dedup F1 held-out within a state and by ~+22pp on
  ...
  domain, A/B it or drop back to the hand-weighted spine with
  ...
  cross-dataset and reconciliation flows pin the FS spine (`learnedScorer: false`),
