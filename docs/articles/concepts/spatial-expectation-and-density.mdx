...
That line is not squeamishness. It is the correct division of labor for a system that resolves fragmented public data into an analyzable shape. The coverage product (#621) is worth building precisely because it makes that analysis tractable — not because it answers the question for the analyst, but because it asks the question precisely.

The spatial expectation model, at both scales, is how we keep that line exact: given what we know about the population and the data sources, here is what we'd expect to see, and here is how much the observed data departs from it. The rest is theirs.
