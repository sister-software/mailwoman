...
The result, measured on addresses the fit never saw: ECE 0.060 → 0.0055, an eleven-fold tightening. The Brier score, another honesty measure, drops from 0.029 to 0.024. The `conf=` now means roughly what it says.
...
On the left, each dot is a confidence bin; its height is how often those spans were actually right, and a perfectly honest forecaster sits on the dashed diagonal. The amber dots — raw softmax — float above the line in the middle bands (the under-statement), then the teal dots settle back onto it. On the right is the move that makes calibration pay rent: raise the auto-accept bar and the precision of what you keep climbs while coverage falls, so you can pick a point — accept above 0.90, say — and know you're auto-handling ~97% of spans at ~98% precision, routing the rest to review.
