...
**`place_importance`** is built by a separate script (`scripts/build-importance.ts`) after the main DB exists — see [Wikipedia importance scores](../plan/reference/WIKIPEDIA_IMPORTANCE.mdx) for the full ETL this feeds. It:
