...
For nominatim, the source of truth is `openapi.yaml` at the workspace root, for now — the copy above is published as a static site asset. photon's and libpostal's documents are each emitted at runtime from their route tables (`GET /openapi.json`); neither has a checked-in yaml to keep in sync. All three validate against the OpenAPI standard with zero errors under [Redocly CLI](https://redocly.com/docs/cli):

```bash
npx --yes @redocly/cli@latest lint nominatim/openapi.yaml
# @mailwoman/photon has no checked-in yaml — its document is emitted at runtime, GET /openapi.json
# @mailwoman/libpostal has no checked-in yaml — its document is emitted at runtime, GET /openapi.json
```
