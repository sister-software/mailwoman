...
For photon and nominatim, the source of truth is `openapi.yaml` at the workspace root, for now — the copies above are published as static site assets. libpostal's document is emitted at runtime from its route table (`GET /openapi.json`); there's no checked-in yaml to keep in sync. All three validate against the OpenAPI standard with zero errors under [Redocly CLI](https://redocly.com/docs/cli):
