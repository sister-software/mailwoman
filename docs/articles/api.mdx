...
The source of truth is the route table, not a checked-in file. All three drop-ins build their OpenAPI 3.1 document from the same `@hono/zod-openapi` route definitions that serve the requests, and emit it at runtime from a running server — `GET /openapi.json`. There's nothing to keep in sync by hand: change a route, the document changes with it.

To lint a running server's document with [Redocly CLI](https://redocly.com/docs/cli), fetch it to a temp file first (Redocly's `lint` doesn't read stdin):
