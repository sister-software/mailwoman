...
casing rather than the casing you sent. Because the rewrite preserves length, `start` and `end` still index
...
you. Each entry identifies the step and its effect.
...
Run the stage on a deliberately messy string and parse both forms, so you can see what the stage
...
Read the last two lines against each other. Both parses return the same five components — the stage
did not change what the model found — and the difference is in one value: the raw parse keeps the
double space in `street`, the normalized parse does not. So preprocessing is not what rescues this
...
- **Case normalization is pure-ASCII only.** Accented and non-Latin input is left alone, because
  changing case there can change length (`ß` becomes `SS`) and violate the offset requirement everything
