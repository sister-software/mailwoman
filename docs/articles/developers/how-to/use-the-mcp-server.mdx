...
This is a change. Through `8.6.0` this server read the variable and nothing else — it was the one entry
point that did not fall back to the convention path, and a config with only the data root in it left the
model-backed tools without a resolver. `8.7.0` moved the fallback into the shared resolver, so the MCP
server inherited it along with everything else.
