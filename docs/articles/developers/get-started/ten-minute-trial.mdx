...
About ten minutes rather than counting the one-time gazetteer download.
...
Four red crosses and it still exits `0`. That is the interface rather than a bug: the exit code tracks the
...
Ignore the `export` line. The file landed at `<data-root>/db/wof/candidate.db`, and that path is where
...
centroids, so the coordinate is New York's rather than this specific rooftop. Street-level precision
...
produced Texas is the **country filter** rather than the ranking: `--locale` defaults to `en-US`, `en-US`
implies `--default-country US`, and that filter is applied on the older backend and not on this one.
Cross the two changes and the confound comes apart (measured 2026-08-04, this gazetteer, this model):
...
`candidate.db` is on disk at `<data-root>/db/wof/candidate.db`. You also have the command that will
