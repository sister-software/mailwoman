...
- **`/reverse` needs a database this page does not download.** Reverse geocoding on the Photon and
  Nominatim drop-ins runs point-in-polygon over a Who's On First admin distribution at
...
with no admin distribution on disk the reverse geocoder is never constructed, and the route answers
...
| `/reverse` without an admin distribution | Empty collection    | See the prerequisites                                                                                |
...
| `/reverse` without an admin distribution                     | `null`              | See the prerequisites                                                                                                                                      |
