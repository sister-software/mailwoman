...
| **v0.3.0 → v3.0.0** (shipped 2026-05-22)                          | Stage 2 label expansion (`B/I-venue`, `B/I-street`, `B/I-house_number`) + linear-chain CRF decoder + dual loss (CE + 0.05·CRF NLL) + corpus rebuild adding NAD (57.9M) → 677M aligned rows. npm major-bumped from 2.0.6 → 3.0.0.                                                                                                                         | ~1 day (overlap with adapter+demo work) | macro F1 0.32 on golden v0.1.2 (4,535 entries); capability-surface win (`house_number` 0.78, `venue` 0.39, `street` 0.27); **coarse F1 regressed** (region 0.83 → 0.18) — under-trained at step-1800 ship + label-space dilution. CRF makes orphan-`I-*` decode structurally impossible (Saint Petersburg fix verified on the live demo). |
| **v0.4.0** (next)                                                 | Recover the coarse-F1 regression. Candidates: longer training (step-5000+ before judging), CE-loss reweighting toward coarse classes through warmup, per-token CRF NLL normalization (eliminates the hand-tuned `crf_loss_weight`), JS-side Viterbi decode (eval used Viterbi; demo still does per-token argmax — eval-vs-prod drift on orphan-I cases). | ~3-5 days                               | Targets: coarse F1 back to ≥0.6 region/locality + ≥0.7 postcode; `venue` ≥0.6, `street` ≥0.7 (the #57 floors); calibration ≥0.85.                                                                                                                                                                                                         |
| **v0.5.0+**                                                       | Stage 3 organization/POI venue + top-k decoding for Resolver                                                                                                                                                                                                                                                                                             | ~5-7 days                               | Deferred until v0.4.0's coarse recovery lands                                                                                                                                                                                                                                                                                             |
