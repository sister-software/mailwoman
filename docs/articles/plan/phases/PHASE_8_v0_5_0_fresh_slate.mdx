...
- **What:** carry the v0.4.0 sidecars + diagnostic tools into the v0.5.0 process from day one.
  ...
  - Verdict-smoke framework redesigned: **constant LR for the smoke window**, OR `max_steps >= 10000` so the cosine tail doesn't dominate. Documented in [`VERDICT_SMOKES.md`](../reference/VERDICT_SMOKES.mdx). Code enforcement: `--smoke-mode constant|long-tail` on `python -m mailwoman_train train` and `smoke` subcommands; defaults to constant for end-to-end smokes.
...
- **Output:** new [`VERDICT_SMOKES.md`](../reference/VERDICT_SMOKES.mdx) + `--smoke-mode` CLI flag + updated TODO.md.
