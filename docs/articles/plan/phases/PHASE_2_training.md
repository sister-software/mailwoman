...
- **v0.4.0** (next) — recover the coarse-F1 regression. Candidates: longer training (the step-1800 ship was early; reach step-5000+ before judging), CE-loss reweighting toward coarse classes during the warmup, per-token CRF NLL normalization (eliminates the need for `crf_loss_weight` hand-tuning), targeted source weighting against the corpus's NAD-heavy fine-label mix. Also: JS-side Viterbi decode in `@mailwoman/neural` (eval used Viterbi; production demo still does per-token argmax — eval-vs-production drift on orphan-I cases).
