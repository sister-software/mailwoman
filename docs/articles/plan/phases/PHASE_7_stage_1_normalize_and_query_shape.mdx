...
**Goal:** ship two new workspaces, `@mailwoman/normalize` and `@mailwoman/query-shape`, that implement runtime-pipeline stages 1 + the QueryShape boundary per [`STAGES.md`](../reference/STAGES.mdx) and [`QUERY_SHAPE.md`](../reference/QUERY_SHAPE.mdx). Pure functions, no ML, no new runtime dependencies beyond `@mailwoman/core`.

**Branch:** ad-hoc on `main`; ship via incremental commits.

**Depends on:** [`STAGES.md`](../reference/STAGES.mdx) (the interface contract) and [`QUERY_SHAPE.md`](../reference/QUERY_SHAPE.mdx) (the QueryShape sub-system design).
