...
- **BR (v7.2 candidate) — banking.** Overture BR shard built clean (210k rows, house_number 96% after the `SN (CASA N)` skip, country=BR/source=overture-latam), 8k training now → scorecard staged SEPARATELY from v7.1.0. License reconfirmed OK (Overture Addresses = CDLA-Permissive-2.0 uniformly; "0% OA-lineage" ≠ share-alike). After BR the Overture Latin frontier is closed (Fable) — every other populated parquet is an existing locale.
