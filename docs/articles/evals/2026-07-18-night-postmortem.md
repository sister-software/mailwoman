...
**CA/MX/BR — UNBLOCKED, the real v7.1.0 path.** Overture ingest landed clean: **CA 16.8M, MX 30.7M, BR 89.9M** rows, no token. They use the existing `overture` adapter with **populated `locality` + `postcode` (LIVE tags)** — they do NOT hit NZ's dead-tag problem. So a CA+MX+BR multi-locale STAGE-3 run is shippable independent of NZ. Caveats: BR is 0% OA-lineage (source + license audit before trusting); audit MX/BR `locality` semantics like NZ.
