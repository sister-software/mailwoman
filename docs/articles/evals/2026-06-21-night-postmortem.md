...
**DeepSeek-confirmed (2nd consult):** there is no non-GPU / non-canonical-swap lever that improves OOD full-address parsing without violating model-first — pre-normalizer (deterministic rule), tokenizer tweak (= training), low-confidence regex fallback (a 2nd bypass, not a fix), and prompting (wrong model class) all rejected. The non-gated coverage surface is genuinely maximized; the parser is `#148` (GPU, operator HOLD). Useful #148 follow-up (queued, not built tonight): a ground-truth OOD-validation harness (per-locale assembled-coord baseline + a %>100km OOD rate) so the retrained model is measurable the moment it lands.
