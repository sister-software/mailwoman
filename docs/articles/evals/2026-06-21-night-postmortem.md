...
**Candidate-path fold (#741) — design tested on a copy, cloned-row approach falsified.** To reach the demo/CLI candidate default (which the FTS-path #475 lever doesn't touch), I tried the obvious "clone the geo-locality's candidate row under the postal-city `name_key`" on a copy of `candidate-global-20g.db`. Result: a `neg_rank=0` sentinel is bare-name-safe (a bare "Antioch" still → Antioch, CA) BUT then loses to any competing in-region homonym (a TN-bbox "Antioch" returned New Hope, TN, not Nashville) — because a state-level region bbox is too coarse and the candidate B-tree has **no postcode dimension**. No single `neg_rank` is both bare-name-safe and homonym-beating on a name+region table. **Revised #741 design:** a postcode-keyed sibling table `postal_city_candidate(name_key, postcode → spr_id)` the candidate lookup probes first when the query has a postcode — porting the FTS lever as a dedicated side-index, not cloned rows. A negative result that saves the operator from building the wrong shape.
