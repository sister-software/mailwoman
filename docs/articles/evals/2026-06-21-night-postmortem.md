...
2. **It's not even broken in production.** The eval hardcodes `placeCountry: false` (line 669, to isolate arbitration from the #244 coarse prior) — so the eval runs WITHOUT the country inference the real pipeline uses. The production CLI (placeCountry on) geocodes a 10-address FI sample at **7/10 within 50 km** (matching the `--default-country` number), the 3 misses being genuinely ambiguous/unusual names. So **placeCountry already infers the country in production**; EU full-address geocoding is ~70%, not 8%.
