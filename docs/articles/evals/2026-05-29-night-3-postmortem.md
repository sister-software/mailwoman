...
_confidence quality_ but not the _release metric_: the harness ceiling is set by
...
collapse (spurious spans + dropped anchors → stranded fragments). The _pattern_
...
_calibrated confidence_ (−20pp overconfidence — useful downstream for resolver
...
- **Mode:** 396 expected components are MISSED (model emits no such tag) vs 323
  labeled-but-wrong — i.e. ~55% of the gap is _missing labels_, not bad values.
...
_does_ emit; it cannot create the missing street / intersection / foreign-locale
labels that cap the harness — which is exactly why `ls=0.1` left the harness
flat. And `#41` char-level fusion attacks tokenizer _fragmentation_ (postcodes,
...
- **#33 locale pre-classifier — measured, not built (PR #194):** rather than
  ...
  **FR/DE/NL collapse to US** (shared `\d{5}`). Adding lexical features
  ...
  Conclusion: lexical features work and the #33 MLP is warranted _specifically_
...
85.5% overconfident _on its wrong answers_ — so v0.6.x failures are confident,
structurally-coherent, _wrong_ values. That is exactly what label smoothing
targets, which is why #31 is the binding experiment.

**Honest finding:** the postcode fix is decisive for postcode accuracy but
moves the _primary_ harness metric by only +2 assertions — whole-address
...
- Captured all v0.6.0 gate baselines _during_ training, so the calibration
