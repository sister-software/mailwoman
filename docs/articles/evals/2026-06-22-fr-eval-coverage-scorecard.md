...
**Root-cause check — parse gap, not coverage gap (so the lever is the model, not the gazetteer).** PT resolves 52% but its label `locality`-recall is **39%** (equally low), while *where* it resolves the coordinate is tight (1.2 km). If the unresolved half were a coverage miss (locality extracted, gazetteer can't place it), label-recall would be high and resolve-rate low — the opposite. Instead both are low: the model **fails to extract a resolvable locality** for ~half of non-IT EU addresses. So #148 (retrain to lift parse recall) is the justified lever; gazetteer coverage is *not* the bottleneck (the EU candidate is comprehensive — see the #734 retirement). ⚠ *Earlier-in-the-night caveat (verify-before-verdict): the IT-only read (p50 3 km → "median non-US geocodes well") was the BEST case; the panel corrected it — IT is the exception, not the rule.*
