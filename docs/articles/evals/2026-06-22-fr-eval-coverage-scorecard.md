...
| region | 219 | 57.6 | **34.7** | **43.3** | ~ thin | ✓ (admin) |
...
| region | 2956 | 89.6 | ✓ (FR's 219 is the gap) |
...
2. **The real coordinate-relevant FR gap is `region` recall, not `country`.** région F1 43.3 is driven by **recall 34.7%** on OOD région formats (the model emits région 99.6% of the time on the in-distribution `Locality, Département` format it was trained on, but the `fr.jsonl` golden carries varied real-world orders it misses). région reaches the assembled coordinate (admin resolution); country does **not** — the resolver takes country from the coarse-placer, never the model's country tag, in both eval (`defaultCountry`) and production (`placeCountry`).
...
| --- | --- | --- |
| FR région (recall 34.7%) | **parse** (OOD format) — model overfit to `Locality, Département` | more OOD région-bearing FR training data + a held-out OOD région set |
