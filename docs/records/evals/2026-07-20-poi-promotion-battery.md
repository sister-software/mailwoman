...
**Method used: the DIRECT equivalent rather than the full model-eval harness.** The golden-set runner
...
being tuned for category density rather than brand density — not a subject-match or kind-classification
issue. See the v1.1 doc for the full trace.

## Latency (report-only rather than part of the promotion check)
...
faster), confirming this is JIT/cache warm-up ordering noise rather than a real speed-up:
...
done here since the report-only ask was for a first-order signal rather than a tight bound.
...
| ------------ | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Golden 2pp   | flag-on must not misroute/change real address parses | 0/4,507 misroutes, 0/4,507 top-kind deltas (direct classifyKind method rather than the full scoring harness) |
