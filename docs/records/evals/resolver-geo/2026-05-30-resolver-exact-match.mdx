...
- Of neural's 749 eval failures, **100% resolved to a place — just the wrong
  one** (0% were unresolved). So the ceiling is a **resolver-ranking** problem rather than a parser-coverage problem.
- **77% of the wrong resolutions are >100 km off** (median wrong-error 333 km,
  p90 2624 km) — distances that mean _wrong state_ rather than a local near-miss.
...
famous Springfield/New York/Chicago among equally-good name matches); it only
...
The headline: **+8.5pp end-to-end Acc@1 and a 5.2× reduce in p90 coordinate error**
...
Failure attribution shows the implementation effect: neural's resolver-side errors drop
from 749 to 544 (−27%) with parser-side errors still at 0 — the fix moved
wrong-state resolutions into correct ones rather than coverage.

The 211 km p90 that _remains_ is the actual admin-centroid tail (large rural
...
street-level tier rather than for this PR.
