...
address explicitly names — `Vienna, Austria` → Vienna AT rather than Vienna WV. This measures it against the
...
tagging the country correctly (the A0 parser probe confirmed it), the greedy walk wasignoring it.
...
parallel exonym + coverage change already tracked on **#826** (the #823 residual split). The post-#822
breakdown:
...
- **Residual B — gazetteer coverage / disambiguation (13):** Brazil, Belize, Honduras, Madagascar, São Tomé,
  ...
  hint — within-country coverage or disambiguation rather than a name-match problem.
