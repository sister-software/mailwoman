...
**Date:** 2026-06-05
**Scope:** the second CJK locale — South Korea coarse resolution. An _inverted_ build (point-primary, not name-primary) feeding the same resolver strategy, why it lands province + sub-kilometer but only 26% name-confirmed, and where the ceiling is.

Japan landed at 94% name-matched ([the CJK arena writeup](./2026-06-05-cjk-arena.mdx)). We pointed the same implementation at Korea expecting a re-run, and Korea handed back a different shape of data — so the build had to invert, and the direct result is a partial: the coordinate and the province come out clean, the administrative _name_ mostly doesn't, and the reason is a gap in the gazetteer, not in the approach.
