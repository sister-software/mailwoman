...
check for the per-input routing thesis. Headline: the change was **fixing the resolver** rather than routing parsers.
...
- **Ground truth:** `scripts/eval/gen-wof-bootstrap.py` samples real US WOF localities
  ...
  address strings, labeled with the source WOF id + centroid (hierarchy-tolerant:
...
2. **US state abbreviations didn't resolve** — WOF regions carry "Illinois" rather than "IL";
   `findPlace('IL')` returned nothing, killing the parent-constraint the walk depends on.

Fixes (both production keepers):

- **`ResolveOpts.defaultCountry`** — a top-level country hint (set from the locale-hint).
...
## Kill/continue check (full 2,406-row run)
...
- **Arbiter beats neural-only by +3.1pp overall and +4.7pp on noisy input.** Check: all
...
- Oracle shows ~6pp of headroom above the arbiter — a better router (lexical quality
  signal, notresolver score) could capture more.

## Conclusions

1. **The resolver was the bottleneck rather than parser routing.** Fixing country + abbreviation
...
3. **The biggest remaining change is resolver coverage** rather than the parser: more aliases/
