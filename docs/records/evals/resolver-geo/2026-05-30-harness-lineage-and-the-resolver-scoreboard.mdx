...
**resolver end-to-end metric** (address to correct WOF place) is the direct
...
- The neural model learned a _different_ (frequently defensible) segmentation and is
...
actual _disagreements_: a tag the model did not emit, or a value that is neither
...
## The direct scoreboard: resolver end-to-end

The product goal is _address to correct place plus coordinates_ rather than _match
...
1. **Resolver end-to-end Acc@1 is the headline metric.** It matches the product,
   is lineage-neutral, and is where the model's value shows.
2. **The `harness-v0-neural` suite is a regression check rather than a promotion bar.**
...
3. **Coverage, measured on lineage-neutral sets, is the change.** The
   ...
   lineage-neutral — notthe Pelias harness.
...
("the path to 25% is coverage and the resolver rather than another recipe tweak") and
...
comma'd-only forgetting regression check. Hypothesis: the model leans on a comma cue that
...
by at least 1.5pp, revert — the lineage-mismatch result still has that evidence.
