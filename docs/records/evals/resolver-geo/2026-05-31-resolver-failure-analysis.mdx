...
direct non-circular signal. 101 rows had a miss from one or both parsers.

## Where neural's 76 locality-misses go

| bucket                    |  count | what it is                                                                  |
| ------------------------- | -----: | --------------------------------------------------------------------------- |
| **(A) name-form variant** | **40** | resolver found the RIGHT place; the eval's name-matcher rejects the form    |
| (B) wrong place           |     13 | resolved to a real but incorrect admin — the resolver-disambiguation target |
| (C) no locality resolved  |     23 | parser mis-segment, `<Null>` tokens, or WOF coverage gap                    |

> **If bucket (A) were credited, neural locality on these 1500 rows is ≈97.6% rather than 94.9%.**
...
1. **Eval name-match fidelity (HIGH ROI) — ✅ SHIPPED IN THIS PR.** Match OA's expected name
   against the resolved place's full WOF **`names` altname set** (normalized) rather than a single
   ...
   are distinct VT municipalities. This is _correctness_ rather than gaming: WOF records these as the
   same place.
   ...
   **Result (1500 OA rows, real v0.7.2):**
   ...
   all 4 `Saint Albans → St. Johnsbury` actual errors still fail (0 wrongly credited) — the
...
2. **Resolver disambiguation — the `Saint Albans Town/City → St. Johnsbury` cluster (4 rows).**
   An actual ranking bug: for "Saint Albans, VT" the resolver picks the wrong VT town. Check
...
3. **Neural parser robustness (feeds the retrain/MLM track rather than the resolver).** Two concrete
   patterns from (C):
...
with several small villages and Saint-/Mount-/township name variants. This is consistent with
(A) dominating: the failures cluster exactly where canonical-name forms diverge most rather than
where the resolver is structurally weak.

## Where neural already wins (bucket: v0-only failures, 25 rows)

v0/Pelias returns _no locality_ for several that neural gets right: `Des Moines`, `Des Plaines`
