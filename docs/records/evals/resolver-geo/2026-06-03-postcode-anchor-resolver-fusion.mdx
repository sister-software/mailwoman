...
anchor sharpens: where rather than which place. The tables below are emitted verbatim by
...
with a street word reads as a house number rather than a postcode (house numbers sit beside the street,
...
whose postcodes the resolver's extracts do not carry (DE/ES/IT/NL/FR), and it tightens the tail even where
...
and the position-aware confidence means it runs on the anchor's own judgment rather than an eval-side crutch.
...
node scripts/eval/oa-resolver-eval.ts \
  ...
  --postcode-extracts <us.db>,<intl.db> --postcode-anchor
```
