...
## Why this is the direct scoreboard
...
- **Coordinate error** (great-circle from the resolved admin centroid to OA's real
  point) is an actual map-accuracy signal — un-gameable, since OA's point was
...
(admin-centroid tier — median is centroid-to-address distance rather than a geocoding
...
- It **does not** measure street/house precision (the resolver is admin-level;
  coord error reflects centroid-to-point distance rather than a geocoding miss).
- Region-match required a name↔abbrev map: resolved regions carry the gazetteer's
  ...
  USPS abbreviation ("CA", "DC"). An early reduce scored region-match at 30% purely
  from that mismatch — a matcher bug rather than a resolver one; fixed in the runner.
...
could only compare against the parser's own text span rather than the place the resolver
chose — so it couldn't tell a right-name/wrong-place resolution from a
correct one. The name is also generally useful to consumers (display the canonical
name rather than the raw input span).
