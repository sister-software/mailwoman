...
**Sample size:** 3,096 postcode-containing entries, 14 country buckets

Built in response to DeepSeek's turn-12 question: "Are we bad at
...
something else (frequently locality or O).
...
moderately directly (`62701` → `[62, ##701]` or `[6, ##27, ##01]`), but
...
2. **Postcode-aware tokenizer pre-pass.** A regex pass that
   detects-and-guards postcode-shaped substrings (`/[A-Z]\d[A-Z]?\s*\d[A-Z]\d/`,
...
```bash
node scripts/harness-postcode.ts \
...
```bash
node scripts/harness-postcode.ts ... --check --floor 0.9 --min-count 10
```

Exits nonzero on any country with ≥ 10 samples and accuracy below the
floor. v0.6.0's US (80.5%, 1765 samples) fails this check today; v0.7's
target is for it to pass.

## See also

- [v0.6.x cycle retrospective + v0.7 plan](../../retrospectives/v0-6-x-cycle-retrospective.mdx)
- [FST priors as shallow fusion](../../site-2026-08/concepts/fst-priors-as-shallow-fusion.mdx) —
