---
sidebar_position: 40
title: "v0.6.2 step 100K eval — HOLD per check, but v0.6.3 directly addresses the gap"
---
...
reached step 100K and was evaluated against v0.6.0 via the 2D pre-publish check +
v0-vs-neural harness. Verdict: **hold** (1 check violation), but the picture is much
...
- **Corpus changes from v0.6.0:** synth-street weight 2.0 → 0.5, synth-no-street extract added
...
| `street` recall                     | 27.7%           | 22.4%           | **27.1%** (-0.6pp — in practice baseline)        |
...
the v0.6.2b half-weight parallel). The val_loss bouncing was per-batch noise rather than a structural overconfidence pattern — macro_f1 stayed remarkably flat.
...
the val set rather than the golden set) shows how each tag's quality evolved. Note
that val-set F1 is consistently higher than the harsher golden-set recall
numbers in the check report below — the val set is closer to the training
distribution.
...
regression is fully fixed at training time, notat golden-eval time.
...
finding of the early-eval check. By step 100K, it recovered to -0.6pp (27.7% → 27.1%) — within
...
1. **Direct:** synth-no-street's adversarial venues included `5th Avenue Theatre` and
   ...
   [`corpus-poisoning-vulnerability.md`](../../site-2026-08/concepts/corpus-poisoning-vulnerability.mdx) for the broader analysis.)
...
- `synth-no-street-v063` (the rebuilt extract) removes `5th Avenue Theatre` and `7th Street
Bistro` from `ADVERSARIAL_VENUES`. A module-load-time assertion now rejects any future
  digit+ordinal venue addition.
- `synth-house-venue-v063` (new extract, 32K rows) teaches house_number + venue coexistence
...
sidegrade, and combined with the check failure, the strict reading is hold.
...
**v0.6.2 falls in row 4: HOLD.** Reasoning: the check failure is structural (house_number
regression at 5pp, well above the 2pp threshold), and even though several tags improved,
...
parquet extracts (`part-no-street-v063.parquet`, `part-house-venue-v063.parquet`) are too;
...
If v0.6.3 passes the check and beats the harness baseline, it promotes. If it passes the check
...
becomes the priority — we need preventative checks before the next synth extract goes in.
...
- `check-report.md` (2D check verdict)
- `verdict.txt`

## See also

- [v0-vs-neural harness](./2026-05-28-v0-vs-neural-harness.mdx) — the 14.4% baseline
- [Layer 1 morphology FST eval](../experiments/2026-05-28-layer-1-morphology-fst.mdx) — the decoder-only fix
...
- [Street-supplement architecture](../../site-2026-08/concepts/street-supplement-architecture.mdx) — the
  design framework
