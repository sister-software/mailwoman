...
**Berlin is an actual failure — but not the one we were retraining for.** Berlin's PIP equals its
...
The other half of the story is native order — the layout real feeds and users use. By PIP it is
...
international PIP (59.6 off → 56.1 on) — the anchor was never the international change. And native German,
measured directly, beats Pelias comfortably and is done. The actual residual is one thing: Berlin in
international order. Everything else is the name-match artifact.

## Why three retrains "failed"

They optimized an aggregate that was half artifact, half a bug the change couldn't reach. The anchor was
...
2. **Berlin city-state segmentation** — the model needs to learn the `City, City Postcode` layout where
   ...
   but it's a conditional retrain for a future shift rather than tonight. Filed as a follow-up.
...
The corpus and anchor changes are spent; the German story moves to the resolver and a narrow city-state
data fix.
