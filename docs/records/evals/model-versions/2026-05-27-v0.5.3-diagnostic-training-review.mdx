...
29M parameters (h384, 6 layers, 6 heads) reached 0.638 on the v0.5.1 recipe. The model overfits past step 65K on this corpus — a larger model (h512, ~68M) would overfit faster rather than break through the ceiling. The bottleneck is:

1. **Training recipe** — the source weights and LR schedule rather than architecture
2. **Data diversity** — the model sees 1M rows/epoch but the source-weight sampler may starve it of street/venue/house_number tags from structured-address sources (BAN, TIGER, NAD)
3. **Eval granularity** — val_macro_f1 averages across all tags equally. A model that perfects region (F1=0.99) but ignores venue (F1=0.0) can still report 0.6+ macro F1. Per-tag F1 breakdown is needed to diagnose which tags are being sacrificed.
...
Instrument the data loader to log per-tag token counts per source per epoch. The model has 673M rows across 12 sources, but the source-weight sampler controls which sources dominate. If `wof-admin: 2.0` means the model sees 40% bare place names, that explains the frequency dominance — but the fix is to add structured-address examples to wof-admin rather than to starve the model of place-name examples entirely.

### 4. Keep int8 quantization for browser deployment

The 29M model quantizes from 66 MB (fp32) to 17 MB (int8) with negligible accuracy loss. This is correct for the browser cold-load budget (~60 MB total). Do not increase model size. The architecture is sufficient for the task. Fix the recipe rather than the model.

## Demo preset results (v0.5.2 weights + inference enhancements)

:::caution[These results use the currently deployed **v0.5.2 model weights** (which are v0.5.1's `model-stage2-step-001800-int8.onnx` with the v0.1.0 tokenizer) rather than the v0.5.3 checkpoint. The inference-time enhancements (FST prior, QueryShape locality guard, Wikipedia importance, grouper-audit) run on top of whatever model is loaded. Results below may appear better than the raw model warrants because the priors and grouper-audit are compensating for model weaknesses.]
:::
...
**Default pipeline: 1/6 correct.** The grouper-audit fills the gaps the model leaves, but the phrase grouper's proposals are frequently wrong (locality=Pennsylvania, locality=Ave, etc.) because they're structural guesses without semantic understanding.
...
The pipeline path reveals the truth: the grouper-audit fills those gaps with provisional nodes, making the missing tags visible. The grouper-audit is working as designed — it only fills empty spans. The problem is:
...
2. **The phrase grouper's structural proposals are low quality for these gaps.** It proposes `LOCALITY_PHRASE("Pennsylvania")` based on capitalization rather than semantics. When injected at 0.55 × grouper confidence, these wrong provisional nodes appear in the output.
...
### Process: new eval release check
...
| 4   | No new failure-class zero-outs?      | Check the kryptonite and adversarial sets.                                       |
...
This session had measured thrashing. Documenting the confusion points so future sessions avoid them.
...
| 6   | DeepSeek diagnosed wof-admin downweight as "caused tag collapse."                         | Confirmation bias — once the F1 comparison "proved" regression, every observation confirmed it. The downweight was the correct fix. | Cascading effect on all subsequent analysis |
...
1. **Demo presets are a release check.** Run them before writing any verdict on a training run.
...
4. **Print raw BIO coverage, notJSON projection.** `decodeAsJSON` hides the model's gaps.
...
**`eval-model`** — Demo preset release check. Accepts model path + tokenizer, runs 6 presets through neural-only + full pipeline, reports JSON + BIO coverage + source attribution. Flags regressions from baseline. Prevents the "4/6 correct but 4% coverage" blindspot that cost hours today.
...
**Evidence checklist.** The skill instructions should require that every consultation prompt includes: (1) functional test output alongside aggregate metrics, (2) tokenizer version when comparing models, (3) raw BIO output notJSON projection, (4) explicit "what changed" matrix for multi-variable comparisons.
