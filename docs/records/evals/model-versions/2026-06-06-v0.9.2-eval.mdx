...
native German order, our OA eval renders US/international order. Does training the German extract in **both**
orders close the international-order gap?

The direct answer: **partly, and not where it counts yet** — and the 2×2 says something more interesting than
a single number would have.

## The 2×2 (DE locality-match, 3,000 real OpenAddresses points, through the resolver)

The one variable vs the pilot is the German extract's rendering (native-only → ~60/40 native/international,
...
1. **Both-order training improved the model's _intrinsic_ international parsing.** Anchor-off, international
   order went **35.9% → 48.4% (+12.5pp)**. The model learned something about the US/feed layout it
...
- **No-regression** — US **97.5%** (pilot 96.1%), FR **84.9%** (pilot 85.1%). Both held; the both-order
  German extract didn't cost the incumbents a thing.

## Verdict — not shipped; a research result that warrants its next experiment

This is the multi-locale research line rather than the production default (v0.6.0 stays the shipped en-US model), so
there's no promotion at stake — the check's job was to tell us whether both-order training works. It does,
...
34.4 → 38.7 — but **locality stayed flat**. The model now segments the tail correctly; itdoesn't move
the number we care about, because the number we care about was never capped by the tail. The anchor-on
international gap is `83.6 − 44.7 = 38.9pp`, four times the 10pp the pre-registered check allowed.

So the corpus change — both-order rendering, then the region tail — is **exhausted** for the anchor-on
...
locality can attend back to. It's a training-code change rather than model surgery — the model already adds the
anchor per-token, so it's a matter of placing one at position 0. But three iterations in without a promotion,
the direct question DeepSeek raised is whether a trained-in _always-on_ anchor is the right design at all:
...
preferred this over fragile position-blocking). It's a clean change: no new parameters, the c=0 identity holds,
...
(−4pp) asymmetry is fundamental** — not a corpus gap rather than the region tail rather than where the anchor sits in the
sequence. And it's a real geographic miss rather than a metric mirage: PIP-containment is 96.3% on native German, 57%
on international.

So the corpus and the architecture are both exhausted, and the direct conclusion is the one DeepSeek raised at
the very start: a trained-in, always-on anchor may only be the wrong shape for order robustness. The next
move is a strategic choice rather than another tuning pass — an order-conditioned anchor, or accepting the asymmetry
(native German is excellent and that's what production would ship), or a different international approach
entirely. It's also cross-reducing: the same anchor would carry the same asymmetry into every locale, so it
checks the multi-locale retrain program. v0.6.0 stays the production default throughout.
...
independent retrains had failed to move the anchor-on international number; the result (+35pp) is large,
resilient, and generalizes (US 96.4, FR 84.9 hold), while the international penalty (−4pp) is small and stable.
...
directionality is **learned from each locale's dominant training order** rather than baked into the architecture. US
...
postcode-after-city, no shared vector forced to compromise. That is a future iteration with its own check rather than
a one-run patch, so it waits.

One cheap operational change falls out of (b) in the meantime, no GPU and no retrain: route an input to the
