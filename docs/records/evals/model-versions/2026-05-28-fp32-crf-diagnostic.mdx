...
The 2026-05-28 night-2 postmortem left one open
...
Both previously-NaN'd steps (bold) cleared directly. macro_f1 climbing monotonically:
...
2. **v0.6.2 stays CE-only.** Per the NaN protocol's "one variable at a time" rule: v0.6.2
   already changes corpus composition (synth-street weight + new synth-no-street extract).
...
# Clear pyc cache (Modal's documented failure mode — labels.py / model.py changes need this):
...
- 2026-05-28 night-2 postmortem — the open question
