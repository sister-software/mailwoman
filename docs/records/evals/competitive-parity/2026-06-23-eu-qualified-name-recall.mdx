---
sidebar_position: 1
title: "EU qualified-name recall — measuring #734's collision-risky change"
tags:
  - evals
  - resolver
  - international
...
# EU qualified-name recall — measuring #734's collision-risky change
...
First, the headline recall is **90.8% rather than the recorded 88%** — confirming #734's read that the old number was depressed by a Lithuanian eval-extraction artifact rather than a real coverage hole. (Lithuania isn't in this panel, so it can't drag the number here.)

Second, the **risky change is the safe one, and the safe change does nothing.** The structural suffixes (`b.`, `im`) recover zero rows here — those are Austrian/Swiss forms that don't appear in this panel (an AT/CH panel would be needed to measure them). The trailing-token strip, the one flagged collision-risky, recovers **14 Portuguese localities at zero collisions**:
...
- A trailing-token base-name normalization bounded to **≤3-char codes** is a clean, collision-free change worth **+1.4 pp** here — almost entirely Portuguese freguesia codes. It is the resolver-side twin of what #370's span-rescore (PR #777) does parse-side: both recover the same `Santa Eulália Viz` rows.
