...
**Net v8 data-pipeline status: fully green.** Both the alignment concept and the corpus-build infra handle JP directly. The only remaining v8 work is training-side: the char-level encoder/loader/model rework (above) — a scoped day-shift arc, not a data problem.
