# Going multi-locale starts with finding out what's  broken (2026-06-02)
...
tempting move is to grab a pile of German data and retrain. Don't. Not yet. First find out what's
 broken, because "it doesn't work on German" hides at least three different failures, and
...
filter_. Therefore, every German address was restricted to US places before ranking ever ran. German Berlin was never even a candidate. Fix the
...
## What the parser  gets wrong, and why
...
## The check: is the tokenizer the wall?
...
So that's the check, and it's an afternoon's work rather than a GPU run. Feed the v0.6.0-a0 SentencePiece
...
Check passed. The wall is coverage, and coverage is cheap.

## Teaching the order

The fix is a small supplement extract that shows the model German order, and the trick is to not
...
Five thousand of those, weighted at 0.2 (a supplement rather than a flood), continue-trained on top of
v0.7.2. One variable changed against the previous recipe.

## The baseline, and an direct stopping point
...
rather than an autonomous one. Everything is one approval away: the extract is built and
...
than a point (the interference regression check), and the German resolver coordinate error should hold.

If German costs US accuracy, the extract comes back out. That's the whole discipline: you don't get to
