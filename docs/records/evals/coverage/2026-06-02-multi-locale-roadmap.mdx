...
The German coverage extract taught the model one thing: addresses where the house number comes after
the street and the postcode comes before the city. Before generalizing that recipe to more locales,
it's worth knowing which locales  have that problem, because "the parser is bad at European
...
| ------ | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| DE     | house after street, postcode before city            | `Hauptstraße 5` → street=`Hauptstraße 5` (house swallowed)                                                                              | order extract (done)                                  |
| ES     | house after street, postcode before city            | `Calle Mayor 12` → street=`Calle Mayor 12`                                                                                              | **same order extract**                                |
| IT     | house after street, postcode before city            | `Via Roma 12` → street=`Via Roma 12`                                                                                                    | **same order extract**                                |
| NL     | house after street, `1012 LM` postcode before city  | `Damrak 70, 1012 LM Amsterdam` → street tagged venue, `locality: "LM Amsterdam"`                                                        | order extract + Dutch postcode                        |
| GB     | house **first**, alphanumeric postcode last         | `10 Downing Street, London SW1A 2AA` → house+street correct, `locality: "London SW1A 2AA"`                                              | postcode coverage rather than order                    |
| FR     | house first, postcode before city (already trained) | region/venue/po_box weak, but the failing rows are multi-script (`Шом, Creuse`) + coarse `région, France` queries rather than street addresses | multi-script + coarse-query coverage rather than order |
...
model was trained on. Therefore, it tags house and street correctly. What it cannot do is recognize
`SW1A 2AA` as a postcode, so the postcode lands inside the locality. An order extract would do nothing
for GB. It needs postcode coverage, which is closer to the existing postcode-repair work than to the
German order extract.
...
kryptonite (`Шом, Creuse`, permuted to `France, Шом, Creuse`). So the FR change is the broader
multi-script and permutation robustness plus coarse region+country queries, and the German order
extract touches none of it.
...
scripts, which would mean a tokenizer change (a bigger, more expensive change) instead of coverage.
...
subword pieces (and for CJK, per-character pieces) for all of them rather than raw byte soup.

So the non-Latin collapse measured earlier is not the tokenizer shredding the script. The pieces
exist; their embeddings areunder-trained, because the corpus is almost entirely Latin. That is
the same diagnosis as German: coverage rather than tokenizer. The synth-from-real-OA recipe therefore reaches
...
wouldbe three more extracts to pull back out. So this is a map rather than a backlog. Measure the first
one before you commit to the rest.

The spot-check is directional (a few hand-written addresses per locale rather than a labeled set); treat the
