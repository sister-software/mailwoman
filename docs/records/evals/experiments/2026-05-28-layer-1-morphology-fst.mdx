...
Applied the newly-landed [street-morphology FST](../../site-2026-08/concepts/street-supplement-architecture.mdx)
...
- **Model:** `model-v061-step-100000-int8.onnx` (the experimental v0.6.1 weights with synth-street
  extract, weight 2.0, that produced the original 1066 dep_loc regression)
...
[WOF hierarchy gap](../../site-2026-08/concepts/wof-hierarchy-gap.mdx) doc: the admin FST has no street
...
The model's confidence on its synth-street-induced dep_loc predictions is high. Per
the v0.6.1 calibration probe design: when the hallucinations
are high-confidence, **retraining is required, nota decoder-time threshold or bias**.
...
`street_types.txt` dictionaries contain several 1-2 character abbreviations (`a`, `b`, `av`, `bd`,
...
damage back to 213, in practice baseline-equivalent.
...
Per DeepSeek's turn 2 recipe
and the [street-supplement architecture doc](../../site-2026-08/concepts/street-supplement-architecture.mdx):
...
node scripts/eval-morphology-fst.ts \
  ...
  --admin-fst $MAILWOMAN_DATA_ROOT/db/wof/fst-per-locale/fst-en-us.bin \
...
- [Street-supplement architecture](../../site-2026-08/concepts/street-supplement-architecture.mdx) — the design
  this eval validates against
- [WOF hierarchy gap](../../site-2026-08/concepts/wof-hierarchy-gap.mdx) — the structural cause
- [v0.6.1 error analysis](../model-versions/v0.6.1-error-analysis.mdx) — the original regression
- 2026-05-28 night-2 postmortem — the postmortem that drove
  the design consult
