...
with the gotchas already paid for rather than a from-scratch spike._

## What you want

The demo's heavy assets — `model.onnx`, `tokenizer.model`, `wof-hot.db`, `fst-en-US.bin` (~60 MB cold) — are
HTTP-cache-only today. The header comment in `src/pages/demo/index.tsx` states: _"After first load the
...
So the requests a service worker has to cache are **cross-origin to `huggingface.co`** rather than same-origin. That
...
- **If the final GET is CORS-clean** (most likely, since the demo works): cache normal (non-opaque) responses.
  ...
  sizes (the plugin already treats those sizes as authoritative record).
...
self.addEventListener("fetch", (event) => {
	...
	event.respondWith(
		(async () => {
			// cache key = the stable HF request rather than the signed redirect target
...
Playwright drives all five with a real Chromium + service-worker support; that's the check this feature needs,
and it's a session's worth of careful work rather than an end-of-shift add-on.
