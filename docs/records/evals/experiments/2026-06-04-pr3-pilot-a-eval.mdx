...
**Date:** 2026-06-04
**Model:** `v0.9.0-pilot-selfcond-s42`, step-20000 (early check), fp32 ONNX
...
## The pre-registered check, and the result

| Check (from the design doc)              | Target           | Result             | Pass?                          |
| ---------------------------------------- | ---------------- | ------------------ | ------------------------------ |
...
That shape — street and house_number **up**, locality and postcode **down**, exact-match up — is the v0.8.0 Saint-Albans signature exactly. The pilot learned German street order (v0.7.2 mangles German streets under its US-order prior); it paid for it by dropping the trailing city.
...
The pilot is within 1.6pp of v0.7.2 on US locality **and still beats the rules parser** — at 20k from-scratch steps against v0.7.2's 100k. The US street parse-F1 regression (−18pp) doesn't reach the resolver because US locality is position-resilient. So the conditioning architecture didn't poison the dominant locale; the damage is confined to German order. That localizes the problem directly: this is the boundary bug rather than a training blow-up.

## The corpus is correct, so the defect lies downstream of the data
...
1. **`cross_pollution` measured the wrong failure.** It counts gold city/region-start tokens predicted as _postcode_ (B/I-postcode). The actual collapse is city → **O** (dropped) rather than city → postcode. So the metric read 0.00% the whole run while locality recall cratered. A live DE-locality-recall (or city→O) regression check would have fired.
...
- **Export bug fix (`corpus-python/src/mailwoman_train/model.py`).** The FiLM split used `.chunk(2, dim=-1)`, which the dynamo exporter emits as an opset-18 `Split(num_outputs=2)` node. onnxruntime-node — and the WASM/WebGPU web runtime — reject it: `Unrecognized attribute: num_outputs`. v0.7.2 had no FiLM so it never hit this. Replaced with two explicit index expressions (`film[..., :H]`, `film[..., H:]`) — mathematically identical, same trained weights, every runtime loads it. Any future self-conditioned export needs this. 11 PR3 CPU tests stay green.

## Decisions made

- **Did not promote** the pilot to `releases.json` / HF. Check failed by ~45pp on the headline metric; the model is not on disk anywhere a consumer would pick it up.
- **Did not launch the 100k follow-up.** That was conditional on the check passing. At 25.6% (vs a 70% bar), and with the 10k-vs-20k slope flat (German locality pinned — see above), extending to 100k would not cross the bar. Not justified by this data.
...
## The slope: flat rather than rising — the recipe is not salvageable by more steps
...
Doubling the training moved German locality essentially zero — Berlin is pinned at 34.2% at _both_ checkpoints. The collapse is a **converged failure mode** the model settled into by step-10000 and stays in rather than an undertraining artifact. A 100k run of this recipe would not cross the 70% bar. So "just train longer" is off the table, and the fix has to be structural.
...
2. **Does the resolver already paper over this?** With the postcode anchor pinning coordinates at p50 1.3 km even when locality-match is 25.6%, how much does the dropped city cost a production lookup? Worth quantifying before deciding how hard to chase the parser fix.
