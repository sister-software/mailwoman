...
1. **`cross_pollution` measured the wrong failure.** It counts gold city/region-start tokens predicted as _postcode_ (B/I-postcode). The actual collapse is city → **O** (dropped), not city → postcode. So the metric read 0.00% the whole run while locality recall cratered. A live DE-locality-recall (or city→O) regression check would have fired.
