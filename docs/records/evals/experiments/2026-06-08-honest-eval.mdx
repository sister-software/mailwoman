# The direct eval — when the metric graded by name and missed the wrong state
...
will "resolve" it on the eval — but that is recall of a memorized place rather than generalization. And
...
matters — resolving the right _name_ in the wrong _place_. "New York" matches "New York"
...
1. **A leakage-free set.** Evaluate only on OpenAddresses rows whose geography the training
   ...
   no manifest holdout. We report VT and flag the rest directly rather than scoring noise.
...
The first run on the held-out Vermont set:
...
in one line. The resolver was finding the right _name_ and the wrong _place_ nearly nine times in
...
postcode directly. The resolver is where it breaks: WOF stores the state only as "Vermont", and the
...
them into the `names` table, which `build-fts` folds into `place_search`. It was only missing from
the build manifest's post-build step, so the deployed gazetteer was built without it (0 `abbr` rows).

### The fix, measured on the same set
...
the fix makes the resolver pick the right _instance_.
...
they are nearest rather than the town whose boundary encloses them. That is why the scorecard leads with
...
The ancestry gap is a build artifact rather than a source gap. NYC's source geojson carries a full
...
| set                | metric               |       baseline |      abbrev only | abbrev + backfill |
| ------------------ | -------------------- | -------------: | ---------------: | ----------------: |
...
lifts both from 0% region / ~330km coord to ~99.5% / ~3.3km. The bug lived in the gazetteer rather than
the parser. And once the resolver is direct, the neural parser keeps its edge over v0 on geography
it never trained on:
...
beat-Pelias-on-the-hard-cases result we want on a held-out set.
...
3. Broaden the held-out set: a targeted OA re-ingest for FR's held-out départements, and a DE
