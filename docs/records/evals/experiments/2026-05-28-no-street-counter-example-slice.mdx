---
sidebar_position: 37
title: "2026-05-28 No-street counter-example corpus extract"
---

# No-street counter-example corpus extract — 2026-05-28
...
- `corpus/src/synthesize-no-street.ts` — six-template synthesizer:
  - `venue-adversarial` (35% weight) — venues whose names contain street-typing
    ...
    `Highway 61 Diner`, etc.). The critical source: these are exactly the rows
...
- `corpus/src/synthesize-no-street.test.ts` — 8 unit tests covering each
  ...
  interface check and a 1000-iteration template-distribution sanity check.
- `scripts/build-no-street-extract.mjs` — mirrors `build-po-box-extract.mjs`:
  reads `{locality, region, postcode, country}` tuples from JSONL stdin and
  emits aligned `LabeledRow` JSONL ready for the parquet extract routing step.
...
With this extract landed, the full v0.6.2 retrain recipe per DeepSeek turn 2 is
runnable:

1. **Reduce synth-street weight** 2.0 → 0.5
2. **Add synth-no-street extract** at weight 1.0, 50K–100K rows, US-primary with
...
4. **2D pre-publish eval check** before promoting:
...
- **Non-US locale gap.** The harness showed neural at 0% on most non-US/non-FR
  locales. This extract helps with US/FR/DE/GB venue-vs-street confusion but does
...
- [v0-vs-neural harness eval](../model-versions/2026-05-28-v0-vs-neural-harness.mdx) — establishes
  the failure clusters this extract addresses
