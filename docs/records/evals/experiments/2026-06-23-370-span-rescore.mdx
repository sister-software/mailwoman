...
p50 1.8 km. The swap is real coordinate-recall rather than a same-name mirage. Build it. (And note the division of labor the buckets already imply: PL/CZ/AT own the swap tail; IT and FR have **zero** swaps — their unresolved rows are pure coverage gap, a job for more gazetteer rather than a rescore.)
