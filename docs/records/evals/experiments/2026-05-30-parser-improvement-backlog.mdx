...
- postal-standards). Those arenas — unlike our own Pelias-derived 376-assertion
  suite — surface where the neural parser fails. This doc turns those
...
The neural model is the **robustness layer** rather than a worse v0. The backlog
...
The cheapest, lowest-risk change is a **deterministic post-decode repair pass**
...
2. **Absorbed-into-street** — `STE 12` / `STE D` mid- or post-street
   ...
   **coverage** gap (units underrepresented in training), the same lesson as
   intersections. The fix is a unit-containing synth extract (a B-list item) rather than a
   post-decode pass.

Net: the repair patches the locality-mislabel half directly and safely; the
...
on **unit-tag recall** rather than address-level pass. Stays opt-in; promote to
default-on once a golden-set unit-tag delta confirms the +N/0 holds there too.

## B2 — country-repair (scoped rather than built)
...
sees street-less addresses). Folds into the same coverage extract as B5.
...
3. **B3' unit + street-less coverage extract** — synth units mid/post-street
