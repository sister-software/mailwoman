...
A coverage guarantee on _where_ a resolved address is. Companion to the [isotonic
...
When the resolver places an address at a locality, it returns that locality's centroid. The direct
...
no model in the loop. Coverage is conditional on the resolver placing the row; rows it
declined to place are reported as an abstention rate rather than silently dropped.

## Results

Three locales off the night's resolved dumps (seed 20260607, 50/50 split). Realized coverage tracks
the target everywhere, so the guarantee holds in practice and the radii are direct:
...
5.49 km of it nine times in ten — and that "nine times in ten" is measured rather than hoped for (0.893).
...
actual city-centroid-to-doorstep distance for the rows that _do_ resolve rather than a modelling failure.
...
show actual ambiguity ("could be either Berlin") instead of a false-precision pin.
