---
sidebar_position: 38
title: "Pre-publish 2D eval check"
---

# Pre-publish 2D eval check
...
The shape of the check is the lesson the
2026-05-28 night-2 postmortem and the
[Layer 1 eval](./experiments/2026-05-28-layer-1-morphology-fst.mdx) both pointed at: a recall-only check
...
The two-dimensional check catches both halves.
...
Each dimension stands alone but they compose: a regression has to clear both checks to ship.

## Why both dimensions matter — the v0.6.1 retroactive check

Running the check against v0.6.0 → v0.6.1 produces:
...
The first two would be caught by a recall-only check. The third — `dependent_locality`
going 0 → 1066 — would not be caught by a recall-only check (recall there
...
3. **Run the check:**
   ...
   node scripts/eval-check.ts \
     ...
     --out-md /tmp/check-report.md
   ```

   Exit 0 → safe to promote. Exit 1 → block the release-it dispatch.

The check is reusable forever — it doesn't know about specific tags or specific models, only
about the threshold interface. v0.7+ models with `dependent_street`, `unit_designator`, JP
tags will be measured the same way.

## What the check does not replace

- The [v0-vs-neural harness](./model-versions/2026-05-28-v0-vs-neural-harness.mdx) (per-locale + per-file
  pass rates across 376 hand-tuned assertions). The check works on aggregate golden-set
  metrics; the harness works on the legacy rule-based pipeline's harsh acceptance criteria.
  Both should be green.
- Manual demo-preset eval. The `/eval-model` skill remains the human-eyeballs check before
  any push to default.
- Falsehoods catalog regression check
  ([`data/eval/falsehoods/streets.jsonl`](./model-versions/2026-05-28-v0-vs-neural-harness.mdx#falsehoods-extension)).

The check is one part of the pre-publish protocol rather than the whole thing.

## See also

- [Layer 1 eval](./experiments/2026-05-28-layer-1-morphology-fst.mdx) — the eval-script's `--out-json` flag
  emits the format this check consumes
- [v0-vs-neural harness](./model-versions/2026-05-28-v0-vs-neural-harness.mdx) — the complementary breadth
  metric
- 2026-05-28 night-2 postmortem — DeepSeek-turn-1
  rubric this check operationalizes
