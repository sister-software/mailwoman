...
Every one of these is `improvement_target` except **`us-addison-zip-75001`** (`4900 Airport Pkwy, Addison TX 75001`, status `pass`, expects `address_point` within 100 m). The full-string rung answered 32.966059, −96.829856 at the `interpolated` tier, 198 m from the truth; `mwdev_run` over the US `pass` rows on the same tree answers the same coordinate and tier. The ladder's full-string rung equals the board grade here, as the rule requires — and the board grade was a failing `pass` row on this data root. Traced the same afternoon: the Texas register holds `4900 airport parkway` at the truth coordinate under the NAD city key `addi`, and the postcode rung's locality contradiction (#1631, 3e470ff62) read the abbreviation as a different place. The reader now prefers the row whose locality agrees and treats a truncated key as the same place; the row answers at the rooftop again. The board-routed A/B run before 3e470ff62 landed had reported 0 rows differ, because it grades at 1/5/25 km (#2155).
