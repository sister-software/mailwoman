...
- **#323 — both-order German corpus** (merged). `synthesizeLocaleRow({ order })` renders German in native
  AND international layout; `build-german-extract.mjs --intl-fraction`; a reproducible native-order eval
...
- **#325 — order-robustness eval & diag tooling** (merged). `scripts/eval/de-order-eval.sh` (the 2×2
  native/intl × anchor on/off + US/FR check harness), `--model-anchor-lookup` on `per-locale-f1.ts`
  (anchor-aware per-tag F1), and the three diag scripts the finding/memories reference.
- **v0.9.2 both-order retrain** — trained clean (20k, no NaN), evaluated. The production package stayed unchanged because this was a research result rather than a production regression. Full report: `2026-06-06-v0.9.2-eval.md`. Headline: both-order training moved
...
- **DeepSeek consult (3 turns, delegated authority)** — turn 1–2 pressure-tested the anchor-vs-word-order
  ...
  diagnostic (force DE=1.0) that left v0.9.2 intl at 44.5% → the anchor harm is **structural-positional rather than
...
- **#328 / #329 — v0.9.3 region-tail** (merged, trained, evaluated). The production package stayed unchanged. International German
  ...
  intl gap (38.9pp) is the anchor's **positional** harm rather than the corpus (the posterior diagnostic already
  ruled out feature content). **The corpus change is exhausted for the anchor-on intl gap.** → v0.9.4.
- **#334 / #336 — v0.9.4 dual-injection** (merged, trained, evaluated). The production package stayed unchanged. DeepSeek's signed-off
  ...
  number (~44%). The anchor's native-help (+35) / intl-hurt (−4) asymmetry is **fundamental** — not corpus rather than region rather than injection position.
- **#337 — the anchor fork, RESOLVED** (merged). With the operator away, the third DeepSeek turn (delegated
  ...
  next thread is a **country-conditioned anchor vector** rather than another tuning pass. Decision record in
  `2026-06-06-v0.9.2-eval.md` §"accept the asymmetry".
- **Multi-locale tooling (#332 / #333 / #335)** — `build-locale-extract.mjs` rebuilt registry-driven, with a
  ...
  `$MAILWOMAN_DATA_ROOT`). **DE / NL / FR / IT / ES are all extract-ready.** _(Correction: an earlier note here called ES
...
- **Demo-refinement tier (#338 / #339 / #340 / #343, + the S38 design doc #342)** — the operator's "visualizer
  ...
  - **#338 (S34) — span-highlight visualizer.** The raw input rendered as a displaCy-style ribbon, each tagged
    span tinted by its confidence (the table's red→amber→green tiering) with the tag labeled beneath; dropped
  ...
  - **#343 (S35) — containment hierarchy tree.** The parse as the tree it is —
  ...
  - **#342 (S38 design doc)** — the service-worker cache groundwork. We did not ship a SW: the heavy assets are
    ...
    Playwright check, so a focused session starts fast instead of half-shipping at the shift tail.
...
- **Measure-first paid off twice.** The whole shift exists because we re-measured German in its native
  ...
  the anchor's role.
- **The corpus pipeline mirrored v0.4.1-de exactly** — overlay manifest, base extracts by reference, no
  rebuild of 677M rows. Clean, fast, verified on the volume before launch.
- **Pivoting directly when GPU was called off.** DeepSeek's "(b), no more GPU" verdict could have read as a dead
...
- **The generalized builder (S3) hit real walls** I should have anticipated: FR's OA CSV is 2.5 GB (over
  ...
  is proven; the rest is held, directly documented rather than half-shipped.
- **The demo tier serialized on merges.** S34/S37/S40 each touch `ResultPanel.tsx`, so each waited on the
  previous to merge before it could branch directly (the last one rebased onto the prior). It worked, but a
...
- **Did not bump the critical dependabot alert (vitest, dev-scoped).** The exploit needs the Vitest UI
  server listening (never run); bumping the test runner mid-shift risked destabilizing the CI check I depend
  on. Logged for the operator instead. _(Alternative: bump it — rejected as higher-risk-than-reward.)_
- **Held `build-locale-extract.mjs`** rather than ship a generalizer that doesn't yet beat the German builder.
...
- **S1 — the anchor is essential in both orders rather than a US-order band-aid.** v0.9.1 anchor-on model, DE
  ...
  reaches native perf without the anchor (the check's 2×2).
- **S2 — the order artifact is systemic but the anchor is the bigger change.** Re-measured the PR3
  ...
  ceiling is only ~48% (= v0.9.1 anchor-off). Two separable changes: order-training lifts the anchor-OFF
  intl ceiling; the anchor unlocks native 48→83 (and Berlin needs it). Resolved the "not yet re-measured"
  caveat in memory.
- **S-diagnostic — the anchor's intl harm is structural rather than posterior.** Forcing the German postcode
...
- **the fork (#327) — RESOLVED by DeepSeek (delegated authority), ~11:50 UTC → (b) accept the asymmetry.**
  ...
  ~40pp). The anchor's native-help (+35) / international-hurt (−4) asymmetry is **fundamental** — not corpus rather than region rather than injection position. With the operator away, the third DeepSeek turn made the call: **(b)** —
  the result is large/resilient/generalizing (US 96.4, FR 84.9 hold), the intl penalty small/stable, and
  ...
  postcode-before-city → looks right; one shared vector can't serve both). The plan is constrained rather than dead —
  the documented next thread is a **country-conditioned anchor vector** (a future iteration with its own check),
  plus a no-GPU operational change: route international-order inputs to the `c=0` path via a lightweight
...
- **The next anchor thread is the country-conditioned vector** — pick when you're ready. The shift mapped the
  ground and #327 now tracks it; it checks the multi-locale retrain program, so it's the change before any
...
- **The demo's service-worker cache (S38) is the operator's other explicit ask rather than yet started.** Today the
...
- **FR (#330):** the FR gap is venue (0%) + region (19%) — convention gaps rather than basics. Worth a targeted FR
...
- **Country-conditioned anchor (#327):** a per-locale anchor direction (DE specializes on postcode-before-city,
  US on postcode-after-city), so no shared vector is forced to compromise. Its own check via `de-order-eval.sh`.
  The interim no-GPU change (order-check → `c=0` route) is the cheap first move.
- **Multi-locale databases:** DE / NL / FR / IT / **ES** are all extract-ready via `build-locale-extract.mjs` (ES via
  a per-part `conform` map on the raw CNIG upstream). The check before any retrain is the country-conditioned
  anchor (#327) rather than the data.
- **Demo S38 — service-worker precache** for the ~60 MB bundle, keyed by `selectedVersion` (see above).
- **FR (#330):** assemble a venue+region FR extract (real OSM/WOF POIs + FR région forms), measure with
  `per-locale-f1.ts`.
...
| Data acquired           | Italy (468 MB) + Spain (451 MB) OA countrywide; **DE/NL/FR/IT/ES all extract-ready**              |
| NaN incidents           | 0                                                                                                 |
| CI failures             | 0                                                                                                 |
