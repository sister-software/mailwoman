...
- **#334 / #336 — v0.9.4 dual-injection** (merged, trained, evaluated). The production package stayed unchanged. DeepSeek's signed-off
  ...
  number (~44%). The anchor's native-help (+35) / intl-hurt (−4) asymmetry is **fundamental** — not corpus, not region, not injection position.
...
- **the fork (#327) — RESOLVED by DeepSeek (delegated authority), ~11:50 UTC → (b) accept the asymmetry.**
  ...
  ~40pp). The anchor's native-help (+35) / international-hurt (−4) asymmetry is **fundamental** — not corpus, not region, not injection position. With the operator away, the third DeepSeek turn made the call: **(b)** —
