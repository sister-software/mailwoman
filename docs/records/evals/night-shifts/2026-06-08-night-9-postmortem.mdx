...
The direct-eval foundation (#371 leakage-free split + #373 PIP-containment) was built and
**immediately earned its keep**: on the leakage-free US/Vermont set it exposed a real,
...
Held-out set = OA rows in corpus-held-out geography the model never trained on. Only US/Vermont
clears the 1000-row trust floor (1428 rows); FR held-out départements = 16 rows; DE has no manifest
holdout.

1. **Symptom (VT set):** name-match 93.7%, region-match **0%**, coord p50 **326km** — the
   resolver finding the right _name_ in the wrong _state_, invisible to name-match.
...
| set                | metric          | baseline |      abbrev only | abbrev + ancestry backfill |
| ------------------ | --------------- | -------: | ---------------: | -------------------------: |
...
- **#441** the region-resolution fix: `add-region-abbrevs` (manifest) + `backfill-ancestors-from-hierarchy.ts`.
  Validated on the direct harness; cross-locale no-regression (DE/FR identical); neural beats v0 on
  the held-out set (p99 277 vs 2120km); slim/demo propagation analyzed; **+ a regression suite**
  (`region-abbrev.test.ts`, 8 tests) hardening the abbrev→region + constraint path.
  **DB not promoted** (canonical swap is operator-conditional/blocked — see below).
- **#443** `research blog: "The right name in the wrong state"` — the eval-honesty narrative, house voice,
  humanizer-passed.
...
- **#446** `feat(corpus): three new adapters` — **geonames** (CC-BY multi-locale gazetteer),
  **geonames-postal** (CC-BY multi-locale postcode→locality→region, ~80 countries), **usgov-irs-bmf**
  ...
  multi-locale goal. (Note: the `corpus-cli` spawned-CLI "unknown adapter" test is flaky under
...
- **#387 comment** — the city-state retrain recipe + pre-registered check, **deferred** to sign-off
  (no turnkey synth/config; no DE held-out-set check; merge wall; 3 prior reverts).
...
- Kept the abbrev+backfill gazetteer as a **validation copy** (`admin-abbrev-test.db`); did not
  swap the canonical (the swap was classifier-blocked, and matches the operator-conditional posture).
- **Deferred the Modal #387 retrain** (logged a sign-off recipe) rather than launching: merge wall
  blocks model promotion, no turnkey city-state synth/config exists, no DE held-out-set check, and
...
2. **Promotion of the region fix.** When you merge #441 and want it live: run the two build steps
   ...
   Orpromote the validated copy at `$MAILWOMAN_DATA_ROOT/db/wof/admin-abbrev-test.db`
...
6. Corpus growth (Tier 12): three adapters added (#446 — geonames, geonames-postal, usgov-irs-bmf).
   ...
   or need a large-format download to build correctly; military/rural-route formats are schema-conditional.
...
The body above was written mid-shift, before the merge wall lifted. What happened by close:
...
- **All 7 PRs merged** (#437 → #439 → #441 → #438/#443/#445/#446), after a direct operator
  ...
  direct-eval doc (kept the superset). Main green afterward (Test + Docs).
...
- **R2 demo push held — it's a no-op.** Investigating before pushing showed the static demo already
  resolves wrong-state cases: #365's client-side `expandUsRegion` expands `IL→Illinois` _before_ the
...
- **Follow-up shipped same session: #447** (`fix(resolver): widen over-fetch for short queries`).
  A probe of the _real_ resolver exposed that exact-abbrev tiering existed but the exact-abbrev holder
  ...
  `country` hint it's already 15/15 (why the direct-eval never saw it). Merged; main green.
