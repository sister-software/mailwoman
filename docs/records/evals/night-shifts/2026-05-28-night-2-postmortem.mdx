...
**Pipeline reuse.** The PO box synthesis pattern (synthesizer → adapter → JSONL → align → parquet → MANIFEST) was built fresh for v0.6.0 but transferred directly to v0.6.1's street synth. Net new code for the second iteration was minimal because the first was structurally clean. This is the right shape — make the first instance generalizable.
...
The pattern I fell into was: "training launched → schedule check-in → wait → check → wait." That's correct for _monitoring_ but wrong for _building_. Building is parallel to training rather than serialized after it.
...
I uploaded v0.6.1 to HF before running the full error analysis. By the time I saw the locality -8.6pp regression, the artifact was already at `en-us/v0.6.1/*`. I rationalized it as "experimental rather than default" but **the right discipline is: don't upload anything I'd be unhappy to promote.**

Per DeepSeek: the gap isn't that v0.6.1 shipped — it's that the eval-vs-baseline check came after the upload rather than before. A pre-publish check (block on any tag regressing >2pp without explicit override) would have caught it. The `before/after` per-tag table should land in the commit body rather than in a separate doc.
...
> 1. **Pre-publish eval check** — run `eval-model` against the current release baseline; abort if any tag regresses >2pp. Would've caught the v0.6.1 locality drop before it went out.
...
> 3. **Shift artifact** — wrap with a structured handoff note (what shipped, what regressed, open questions, next steps) committed to a known path. You got the data; the skillcodifies _where_ it lands so the next shift doesn't start cold.
