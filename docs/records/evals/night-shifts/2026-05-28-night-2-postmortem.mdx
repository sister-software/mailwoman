...
Per DeepSeek: the gap isn't that v0.6.1 shipped — it's that the eval-vs-baseline check came after the upload, not before. A pre-publish check (block on any tag regressing >2pp without explicit override) would have caught it. The `before/after` per-tag table should land in the commit body, not in a separate doc.
