...
**`label_smoothing=0.1` does not pass the v0.7 ship check.** At 100K (int8, vs
...
pre-publish check **fires on house_number −6.0pp**. Calibration improves
...
## 0. Calibration 20K early-check — PASSED
...
by 100K. Early-kill check did not fire; training continued. (fp32-vs-int8
caveat: the effect dwarfs any quantization artifact; final check uses parity.)

## 0b. Calibration 50K interim full check — MIXED→CONCERNING (under-trained)

Interim check on the step-50K checkpoint (int8, parity with v0.6.0 int8) vs
v0.6.0@100K. **Caveat: calib is half-trained here; 100K is the fair compare.**

| Metric                         | v0.6.0@100K | calib@50K       |                                    |
| ------------------------------ | ----------- | --------------- | ---------------------------------- |
| Overconfidence-on-wrong (≥0.9) | 86.3%       | 65.4%           | ✓ down (but ~65% rather than ~50%) |
...
**Mechanism (under-training alone):** label smoothing flattens the target
distribution, so low-prior tags (dep*locality / subregion, class-weight 0.3)
that were suppressed now leak → hallucination explosion and structural-validity
collapse (spurious spans + dropped anchors → stranded fragments). The \_pattern*
(rare-tag leakage) is smoothing overcorrection rather than uniform under-training.
...
## 0c. Calibration 100K definitive check — DOES NOT PASS
...
| house_number recall                     | 77.7%       | 71.7%      | ✗ −6.0pp (>2pp check)                |
...
(flat-to-down); house*number regresses 6pp. Calibration delivers better-
\_calibrated confidence* (−20pp overconfidence — useful downstream for resolver
thresholding) and a postcode bump, but it is **not** the change for the release
...
step-100000) + local int8 — **not uploaded to HF rather than promoted**.
...
## 0d. Why the harness didn't move — ceiling is COVERAGE rather than confidence
...
- **Mode:** 396 expected components are MISSED (model emits no such tag) vs 323
  labeled-but-wrong — i.e. ~55% of the gap is _missing labels_ rather than bad values.
...
- Only 9/321 failures are structurally invalid → failures are coherent-but-
  incomplete rather than garbled.
...
(0 neural-only harness wins). So the change calibration touches (neural confidence)
isn't even the product bottleneck. (NB: surfacing this required fixing a
release-check bug — `eval-matrix --model-path` decoded STAGE3 checkpoints against
...
already handled by #35) — it does not create those missing labels
either. The dominant harness changes are **corpus coverage** (intersection
...
release metric is coverage-bound rather than calibration-bound.
...
- **Eval tooling (PR #194):** `eval-error-analysis.ts` + `harness-v0-neural.ts`
  gained `--model/--tokenizer/--model-card` + `--postcode-repair` so the check
  can measure the calibration checkpoint on the held-out TEST.
- **#33 locale pre-classifier — measured rather than built (PR #194):** rather than
  build an MLP blind, measured the existing rule-based `@mailwoman/locale-hint`
...
### Baselines captured (v0.6.0, for the #31 check)
...
A push-button check runner (`scripts/eval/v07-calibration-check.sh <calib.onnx>`)
...
**direct finding:** the postcode fix is decisive for postcode accuracy but
...
are the larger gaps. Postcode fix is necessary rather than sufficient.
...
- Captured all v0.6.0 check baselines _during_ training, so the calibration
...
- **CI unblock via PR rather than direct main push.** Auto-classifier (correctly)
...
- **#35 as a decoder-side label repair (not a tokenizer change or emission
  prior).** A true "guard-before-SentencePiece" needs a retrained model;
  the no-retrain, highest-precision change is snapping the decoded span to the
  regex match. Built with precision guards (longest-match-wins, SNAP-only for
  numeric shapes, local smear-clip) → 0 regressions.
- **Calibration base = v0.6.0 rather than v0_6_4.** Acceptance compares to "v0.6.0
...
- **Full 100K run with a 20K early-kill check** (not a short probe). Budget
...
- **Did not self-merge PR #194.** Auto-classifier blocked it (the merge
  ...
  for the operator.
- **Did not promote/ship the calibration model.** It failed the check (harness
  flat, house_number −6pp). Per ship discipline + the pre-publish eval check,
  kept it experimental (volume + local) rather than uploaded to HF. Resisted the pull
  to "ship something" from a long training run.
- **Did not autonomously launch iteration-2 (`ls=0.05`).** Training is
  authorized, but the iteration-2-vs-pivot fork is an actual judgment call at
...
- **Ran the interim 50K check** instead of only waiting for 100K — surfaced the
...
- **the fork — what moves the harness next** (§0c + §0d). `ls=0.1` didn't move
  ...
  **coverage-bound** (missing street/intersection/foreign-locale labels) rather than
...
calibration-check.sh <int8>` at 100K. If structural pivot: scope #41.
...
| Models promoted/shipped | 0 (calibration failed the check — kept experimental)                |
