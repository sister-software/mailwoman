...
- **`eval-model` skill**: Demo preset release check — runs 6 addresses through neural-only + full pipeline, checks for grouper-audit nodes, flags confidence regressions.
...
1. **Demo-assets plugin is the right abstraction.** Model/tokenizer/FST/WOF-slim all staged from a single authoritative record. The tokenizer mismatch that shipped bad output to production can't recur.
...
3. **Per-tag F1 was the fastest fix with the highest use.** The macro F1 comparison that caused hours of wrong analysis in the v0.5.3 session is now impossible — per-tag breakdown is logged at every eval step.
...
- CRF export pipeline has no versioning interface — no `requires_trained_crf` flag. TS side can't distinguish trained from untrained transitions.
