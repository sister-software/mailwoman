...
| -------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| **ls=0.05**                            | postcode +4.2pp, but street −4.1 / venue −4.5 / house_number −2.4 (3 tags >2pp) | +1.6pp (20.2%)                                                                                          | **not promoted** — calibration trades postcode for street/venue; fails the check, not significant |
| **bare-street**                        | street −2.8pp (golden full-address streets shifted)                             | +0.9pp (19.5%); **usa +7pp** (22→29%) but **functional 2/34→1/34 — target missed**; net not significant | **not promoted** — redistributes (US harness up, golden street down), fails the check             |
...
- **175 untrained-locale** (deu 17/17, nzd 22/22, nld 20/22, place.fra 13/13, …) — **BLOCKED.** v0.7.2 trains US+FR only; these locales fail because they're out-of-distribution. Covering them is the multi-locale PARSER problem, which is the known-unsolved German-end-of-string-collapse direction (the v0.8.0 order-extract reverted for exactly this). **Not attempted autonomously** — it needs the anchor-based / collapse-fix work, not a naive retrain.
  ...
  So the honest answer to "can we move the harness tonight?": **the safe change (calibration) can't** (adds no coverage); **the bare-street change can, partially** (the in-distribution cluster); **the big gap (untrained locales) is blocked** on unsolved parser work.
...
- **Launched the bare-street run** (concurrent, second A100) after the harness analysis showed a clean in-distribution change — and committed it (#308) before knowing the result. Honest call: a real, safe shot at the operator's harness goal; it didn't pan out, but the analysis + the `bareProb` feature are durable.
