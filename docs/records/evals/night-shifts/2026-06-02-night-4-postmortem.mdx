...
order extract roughly doubled German street (19.1→41.2) and house_number (14.6→30.9). But the
continue-train recipe destabilized span boundaries. German locality and postcode collapsed, the resolver
fell with them, and US/FR slippedpast the 1pp regression check. The recipe is rejected; the mechanism it
exposed is the prize. Details below.

## RESULTS — German train completed + evaluated (REVERT)

The German continue-train (v0.7.2 → step-140000, `synth-german: 0.2`) finished directly (no NaN, app
...
before, re-triggered at end-of-string by the order extract. The cities were not forgotten; their span
boundaries came apart:

- **the result.** `Prenzlauer Allee 36, 10405 Berlin` → baseline mis-tags `36` as
...
So the change is real, and the failure is a known, nameable boundary bug rather than a dead end. The next
attempt needs the order signal _without_ the boundary damage. Candidates: (a) the Saint-Albans span-merge
decoder fix applied to house_number/locality spans, (b) a larger/cleaner extract so the model sees complete
...
1. **DE-0 — tokenizer check: pass.** `scripts/diag-tokenizer-de.ts`. The v0.6.0-a0 SentencePiece
   ...
   ß-piece boundary rather than a tokenizer ceiling. Coverage is the fix. (commit `97d1ec4`)
2. **DE-1/DE-2 — German synthesizer + extract.** `corpus/src/synthesize-german.ts` (+ 3 passing tests)
   renders REAL OpenAddresses Berlin/Saxony tuples in idiomatic German order via the OpenCage `DE`
   template, teaching house-number-after-street + postcode-before-city. `build-german-extract.mjs`
   pools 1.2M real DE tuples → a 5,000-row labeled extract. (commit `08f85f1`)
3. **DE-3 — config + extract (staged, then run on approval).** `v0_8_0-german.yaml` continue-trained
...
5. **Earlier this session (the day's work, also on this branch):** the `--default-country` flag, the
   per-source bbox + German OA ingest, the per-locale-f1 regression check, the full-stack capability probe.
...
## How the train got launched (and the check it cleared)

The auto-classifier conditional the in-place mutation of the **shared** corpus `MANIFEST.json`, correctly:
my own plan flagged DE-3 as needing your sign-off, and "wide berth" is general autonomy rather than specific
...
```bash
# 1. register the extract (the conditional step):
...
# 3. verify the loader sees the extract, THEN launch (~$3, ~12 min A100):
...
postcode 89.0%. US/FR baseline (the interference regression check): US 76.2% / FR 62.8% micro-F1. German
...
- **Keep** if street + house_number climb materially AND US/FR stay within ~1pp AND German resolver
  coord holds.
- **Revert** (pull the extract) if any existing locale drops > 1pp. You don't get to hope a locale is
  free; measure whether it was.

## What went well

- Check-first paid off. DE-0 is an afternoon's script, and it could have saved a wasted GPU run if the
  tokenizer had been the wall. It wasn't, so we proceeded with evidence instead of hope.
- Reading the data loader before launching caught the `country_weights` failure mode: the loader rejects any
  ...
  never happened.
- Real OA tuples + the OpenCage DE template means the extract carries correct German morphology and
...
- The German extract inherits some noise from the OA `CITY` column (e.g. `Rabenau Sachs`,
...
- **Continue-train (+40k) over a fresh 100k run.** ~$3 vs ~$8; leaves budget buffer; v0.7.2's
  ...
  the result shows). Alternative was a clean separate dir, which `train_remote` can't init-from without a
  volume-side checkpoint copy.
- **Sourced the extract from real OA tuples rather than synthesized German streets.** German morphology is too
  easy to fake wrong.
- **Stopped at the MANIFEST check rather than working around it.** The denial was correct.
...
1. **Which fix for the next German attempt?** The order signal works; the boundary fragmentation is
   ...
   it's the "one more rule" change you've pushed back on; here it's a decode-time span join rather than a
   hand-written parse rule); (b) bigger/cleaner extract + retrain; (c) fresh-with-German run instead of
...
3. The de golden eval is synthetic (real OA tuples, German-order rendered). It directly separated the
...
- (held) ES/IT/NL extension: same recipe would replicate the boundary damage; blocked behind the German
  round-2 fix landing.
- (cheap follow-up, still valid) CITY-column cleaning in `build-german-extract.mjs`: the OA `CITY`
...
| models trained        | 1 (v0.7.2 → step-140000, German order extract)          |
| Modal spend           | ~$3-4 (40k continue-train + fp32 export, A100)          |
| model promoted        | 0 (REVERT verdict, recipe rejected)                     |
| NaN incidents         | 0                                                       |
| CI failures           | 0                                                       |
| classifier checks hit | 1 (shared MANIFEST mutation — respected, then approved) |
