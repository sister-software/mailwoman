...
- **scorer wiring** — folds a postcode's postal-city aliases into the EXISTING `softNameScore` alias implementation in `#findLocalityCoordFirst`. A user-typed postal city becomes a name-match alias for the geographic locality the postcode sits in → the right place tiers over a same-named distractor, and the false postcode/city mismatch flag stops firing.
- **`resolver-backend.ts`** — opt-in via `MAILWOMAN_POSTAL_CITY_ALIAS_DB` (FTS path).

**OPT-IN / DEFAULT-OFF, byte-stable** (every alias path blocked on the reader; the unchanged coord-first suite + a byte-stability test pin it). Verified: 7 new tests incl. the decisive before/after on the **real** antioch→nashville edge; resolver-wof-sqlite 249 passed / 0 failed; tsc clean.

**MEASURED on the production resolver (notunit-tested).** Built `postcode-locality-us.db` (the missing US coord-first extract — 45,902 locality polygons PIP'd against 42,318 postcode centroids; 19,560 postcodes get a containing locality, the rural/PO-box rest don't) and ran a real-resolver `findPlace` spot-check (real admin DB + the new US extract + the real alias DB), aliases off vs ON:
...
| coord p90 — setting-active (2,444) | 1080.3 km | **8.8 km**     |

p50 barely moves (3.2→3.1 km) because most divergent postcodes already resolve near-right; the change fixes the **catastrophic tail** (p90 278 → 10 km) with **zero regressions** — exactly the behavior the opt-in/byte-stable design promises. This is non-circular (truth = postcode centroid rather than the alias table) and a strong promote signal for default-on (operator's call).

**Why not the standard OA eval:** the OA US sample's inputs use the GEOGRAPHIC locality name (matching `expected`), so the postal-city change never fires on it — running it would read flat, a misleading non-result (the verify-before-verdict trap). The faithful measurement is the postal-city-input spot-check above; a full aggregate eval would need a postal-city-input corpus graded against independent geography — the operator follow-up alongside the candidate-path build-time fold.

**Candidate-path fold (#741) — falsified the obvious design, then BUILT + measured the right one.** To reach the demo/CLI candidate default (which the FTS-path #475 change doesn't touch), I first tried the obvious "clone the geo-locality's candidate row under the postal-city `name_key`" on a copy. **Falsified:** a `neg_rank=0` sentinel is bare-name-safe (bare "Antioch" → Antioch, CA) BUT loses to any in-region homonym (TN-bbox "Antioch" → New Hope, TN rather than Nashville) — a state-level region bbox is too coarse and the candidate B-tree has **no postcode dimension**. No single `neg_rank` is both safe and homonym-beating.

So I built the right shape: a **postcode-keyed side-index** `postal_city_candidate(name_key, postcode → spr_id)` that `WofCandidateTableLookup` probes first when the query carries a postcode (exact `(name_key, postcode)`, short-circuit, bypassing population/region ranking). **Conditional** on table-present + postcode + locality-tier, so bare-name safety is structural (no postcode → no probe → untouched). shipped (`320958d0`, on PR #740): schema + builder (`build-postal-city-candidate.ts`) + lookup probe + 6 unit tests; resolver suite 255✓/0✗.

**Measured on a copy** (6,413 edges built from the full US alias × postcode*locality bridge): Antioch/37013→Nashville, Cactus/85032→Phoenix, Woodbridge/22191→the VA locality (base mis-picked Woodbridge **NJ** — population-first), bare "Antioch"→Antioch CA. Aggregate over 9,688 divergent edges: **2,221 fixed, 3 regressed (0.03%)**, coord **p90 1635km→768km**, p50 7.4→3.4km. (4× the FTS path's fixes — the candidate backend's bare population ranking had a \_worse* postal-city problem. Residual 768km = the ~30% of divergent postcodes with no containing WOF locality, unfixable by any method. The 3 regressions are postcodes whose containing-locality centroid sits >50km from the postcode centroid.) OPT-IN: the side-index only exists if built; absent it, byte-stable.

**Browser probe done too — the change now spans all three resolver implementations.** Mirrored the postcode probe into the demo's `httpvfs-resolver.ts` `WofCandidateTableLookup` (`b7e19618`), memoized-existence-conditional so it's inert on today's production candidate.db (byte-stable). 4 unit tests via the stubWorker pattern. So postal-city resolution is now wired in **Node FTS (#475) + Node candidate (#741) + browser candidate (#741)** — all opt-in, all byte-stable, all unit-tested. **The only remaining step is operator-conditional:** fold `build-postal-city-candidate` into the canonical candidate build + rebuild + R2 republish, at which point the demo activates it (Node + browser together).

## #734 EU-recall — diagnosed into three distinct per-country changes (no rebuild tonight)

Ran the candidate-recall harness on the AT/FI/SK holdout against the promoted `-20g` DB, then verified the buckets. Findings sharpen #734 from a lumped "coverage depth + bilingual aliases" into three separable changes — and correct one number:

| country | exact recall | +strip-fallback   | misses absent / wrong-pt | change                                  |
| ------- | ------------ | ----------------- | ------------------------ | --------------------------------------- |
...
**Operator follow-up:** this is the build-on-a-copy MVP that proves + measures the change (EU recall tail closed, coord-validated, zero GPU). The provenance-clean path is to fold GeoNames into `build-unified-wof` (so it flows through the canonical candidate build with proper provenance), then rebuild + R2 republish.
...
This reframes the day-shift "-20g world coverage" promote: it was _97-country_ coverage rather than world. **Highest-value operator follow-up of the shift** — fold GeoNames into `build-unified-wof`, rebuild, republish. Filed as #742.

**Full-depth run (definitive scope).** Beyond the cities15000 MVP, ran the full `allCountries.txt` (13.4M places, quality-filtered to drop historical/abandoned `PPLH/PPLQ/PPLW/PPLCH`): **4,642,285 town-tier localities added across 151 newly-registered countries → 248 countries with coverage; candidate ~13M rows.** Town-tier sanity: 200/200 sampled rows each for AF/HK/AL land inside the country bbox (no mis-mapping) — and the major-city tier was coord-spot-checked 9/9 within 30km, so the coverage is verified at both tiers. **Demo-size tradeoff (operator's call):** the full-depth candidate is 1.26 GB vs 811 MB — heavier for the byte-range demo. So the likely shipped split is `cities15000` (major cities, light) to the demo + `allCountries` (full depth) to the CLI/server, or a population threshold; the builder takes whatever depth the operator chooses. The proof is that the change scales directly to full global depth.

## ⚠️ Pipeline-level investigation — a false alarm caught twice by verify-before-verdict (DeepSeek-prompted)

A cautionary tale worth recording. Graded the assembled pipeline coordinate on FI address strings via `oa-resolver-eval` and got **8% locality-match, coord p50 7,044 km** (wrong continent). I initially read this as "the parser is OOD on EU" and even ran it past DeepSeek as a GPU-conditional wall. **Both reads were wrong**, and the corrections matter more than the alarm:

1. **It's not the parser.** Forcing `--default-country` per locale snapped the assembled coord from ~7000 km to **~1–2 km** (FI 8%→68%/2.2km, ES→79%/1.3km, IT→89%/2.2km, NL→95%/0.1km, PL→67%/1.6km). The parser extracts EU localities fine; the failure was the candidate backend resolving **country-agnostic** (population-first) on addresses that carry no explicit country, landing on a same-name homonym off-continent.
2. **It's not even broken in production.** The eval hardcodes `placeCountry: false` (line 669, to isolate arbitration from the #244 coarse prior) — so the eval runs without the country inference the real pipeline uses. The production CLI (placeCountry on) geocodes a 10-address FI sample at **7/10 within 50 km** (matching the `--default-country` number), the 3 misses being ambiguous/unusual names. So **placeCountry already infers the country in production**; EU full-address geocoding is ~70% rather than 8%.

**The real, useful findings:** (a) `oa-resolver-eval` understates EU accuracy by ~60pp because it disables `placeCountry` — it needs a production-representative mode (placeCountry-on) before anyone reads its EU/non-US numbers as real; (b) the residual ~30% is ambiguous-name mis-resolution (a same-name homonym wins without a hard country constraint) — a candidate-backend refinement (a postcode-derived country constraint when a postcode is parsed) rather than a parser problem, and an actual non-GPU follow-up. The "#148 parser is the EU enable" framing in earlier drafts is **retracted** — the parser is not the EU bottleneck; the country-constraint in the candidate path is. (My thanks to verify-before-verdict; this is exactly the trap the discipline exists for.)
...
- **address-point extracts** (`address-point-schema.ts`) — the #735 national-rooftop tier's data path. Reader (`AddressPointSqliteLookup`) projects `Pick<AddressPointTable, …>`; writer (`build-address-point-extract.ts`) derives its DDL + index DDL + INSERT column list from the shared module (the hot positional INSERT stays for throughput, per the candidate convention — only its column list is shared); the interpolation test fixture builds off the shared DDL too. tsc clean, resolver suite 249✓/0✗.
...
- **5-digit-HN repair** (3.76pts) — `5977ce4d`, `neural/postcode-repair.ts::repairLeadingHouseNumber`, US-conditional, wired at `classifier.ts:484`, tested. And done the _model-first-respecting_ way — a post-decode parse repair rather than a resolver override (the false postcode never enters the tree, so the postcode-anchor sees the true trailing postcode; zero anchor interference).

Combined ~6.1pts of the 12% admin tail, already in. **Remaining open:** the spelled-ordinal fold (0.44pts — marginal, and needs the situs canonical-form checked before a fold can normalize toward it) and the NAD→OpenAddresses situs theme-reselect (3.69pts — a multi-state extract rebuild, **partially in-flight via PR #738's new `--oa-csv` extract mode**). Decision: no new #723 code tonight — the cheap changes are banked and the big remaining one is data-pipeline work riding #738.
...
- **`il` vs `il-cook`.** The national build split Cook County out (OOM avoidance), but `il.db` (4.86M rows) already contains Cook/Chicago (632k Chicago rows) while `il-cook.db` (1.46M, 612k Chicago) is a separate, overlapping build — merging would dup. Decision: **host `il.db` alone** (complete state incl. Cook); `il-cook` is a stale/ambiguous artifact to investigate rather than a rollout blocker.
- **Stale `out/` broke the CLI locally.** After checking out main for the shift (without recompiling), `out/commands/tiger/fetch.js` (from my prior tiger-branch work rather than on main) was orphaned in `out/` and pastel still loaded it → `fetchTIGER` import crash. `tsc -b` doesn't delete orphaned outputs. Fixed by removing the stale dir. Local-tree only (a fresh clone of main is fine) — but a second instance of the "stale compiled artifact" class this week (cf. the demo `core/out` misuse hazard). Worth a `compile:clean` after any branch switch.
...
- **#531 (typo-tolerant retrieval) — scoped rather than built.** The FTS path already has trigram-Jaccard fuzzy; the candidate path (now the demo/CLI default) has none, and its `WITHOUT ROWID` B-tree clusters alphabetically (not by edit-distance), so a fuzzy fallback needs a new trigram/spellfix side-index + a candidate rebuild + a browser fetch-cost measurement — bigger than a night-shift item. Design follow-up. **#530 (the parser-side half — teach the MODEL to tolerate typos in the surface) is now shipped** (above); #531 is the retrieval-side half (recover the gazetteer hit when the _parsed_ token is itself misspelled). They're complementary rather than substitutes.

## Concrete next steps (operator)

All the big changes are built + measured on a copy; the _apply_ (one `build-unified-wof` + candidate rebuild + R2 republish) is the only conditional part — and it activates all of them at once.
...
2. **Merge PR #740** (25 commits). Carries the #530 default-OFF correction — **merge before any corpus build** so the default distribution stays byte-stable. #738's `build-address-point-extract.ts` edit may need a trivial merge with the #175 change there.
...
| verify-before-verdict saves | 2 (--default-country test: EU failure is country-constraint rather than the parser; CLI test: EU not broken, the 7000km was an eval `placeCountry:false` artifact) |
...
| diagnosed / filed           | #734 (3-change→fixed) · #741 (filed→built) · #742 (global coverage)                                                                                                |
...
| new data artifact           | `postcode-locality-us.db` (US coord-first extract)                                                                                                                 |
| DeepSeek consults           | 1 (next-change direction — drove the #734 EU win)                                                                                                                  |
| Modal $ / GPU               | $0 (no-GPU plan)                                                                                                                                                   |
| regressions shipped         | 0                                                                                                                                                                  |
| canonical-DB swaps          | 0 (all conditional work built-on-a-copy + flagged)                                                                                                                 |
