...
- The proven recipe (codex synth + format-diverse + real-OOD gate + int8 value_info-strip) carried two more changes cleanly through build+gate.
...
- The affix gate's first read was a false 0% (fold) — a per-tag affix scorer should have existed before the extract (now it does).
...
- **Consolidation v1.0 (#466)** once affix is promoted: make the affix extract **multi-locale** (clears its own FR-postcode gate without bundling the now-deterministic country), prove it solo, then **weight-merge** the affix delta into the v4.1.0 base rather than re-stacking extracts (the dilution is step-budget-bound). DeepSeek-recommended; gated on #462 + operator go + GPU budget.
