...
- **Berlin = actual city-state bug** (955/1500 unresolved; locality span dropped in `…, Berlin, Berlin PC`
...
notunit-tested — the conditional re-measure happened this shift:
...
- **#387 city-state recovery: Berlin PIP-containment 36.3% → 80.9% (+44.6pp)**, overall 56.1% → 78.4%,
  ...
  actual city-states, zero collateral damage. ~669 rows recovered.
- **#386 Saxony credit: Sachsen name-match 51.1% → 65.5% (+14.4pp)** on real resolved output.
- The DE re-measure was not symlink-conditional (#397) — it runs against the explicit v0.9.4 model in
...
- **#390** `--anchor-rerank` flag on `oa-resolver-eval` (#369 S8): measured the re-ranker via real
  `resolveTree` + PIP — it HURTS (uniform posterior), so it stays blocked on a sharp posterior.
- **#391** per-locale calibration tables (#368): NL ECE 0.047→0.010, DE 0.089→0.041 vs the global table.
- **#392** calibration drift guard (#368): CI regression check on ECE regression >0.02.
...
- **#399** per-locale F1 floor check (#375 S48): non-blocking CI regression check, self-tested.
...
- **Both halves of the German finding shipped + measurable** (#395 + #396 + #398); the symlink-conditional live
  re-measure is the only remaining step, tracked in #397.

## What went well

- The pre-registered checks + DeepSeek's decision trees made every retrain verdict mechanical, including the
...
- Three docs-build CI misses (MDX bare `<` ×1, blog broken-link `/articles` vs `/docs` ×1, plus tag
  warnings) — a local `yarn ci:docs` before pushing docs would've caught them.
- Completion monitors false-positived twice on stale prior-run `step-020000` checkpoints → switched to
  app-task detection.
- **The eval loads compiled `core/out/` rather than source.** The first #387 before/after showed _zero_ effect —
...
- No MODEL promoted (no retrain cleared its check); the deployed model default is unchanged. But two RESOLVER
...
- **#387 residual (~12.5% Berlin) is a PARSER bug rather than the resolver:** the model mangles `Straußstraße`→
...
- **#368 L2** calibrate the DEPLOYED multi-locale model (per-locale table — DE/NL under-served). The
  per-locale fitter (#391) + drift guard (#392) are in place; thispoints them at the multi-locale run.
...
- The shift's second half (post the German pivot) shipped, all off the A100: the full calibration program
  ...
  per-locale F1 floor check, split-conformal coordinate intervals, two blogs, and an eval report. No GPU
