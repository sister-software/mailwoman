...
when the neural parser is out of distribution on a locale, the anchor still geolocates the address.
...
- **#253 — the browser resolver, dual-target complete.** `PostcodeBinaryResolver` in `@mailwoman/neural`:
  ...
  the same `lookup()` boundary as the SQLite resolver. `build-postcode-binary.ts` emits per-country `.bin`
...
- **IT placement attempted, then dropped.** Built `admin-it.db` and ran the backfill, but a functional
  ...
  in Liguria. IT stays membership-only; placement is blocked on WOF data quality rather than on our pipeline. The
  experiment was reverted (the extract is back to the clean DE/FR state; nothing IT-related reached a PR).

## What went well

- The gazetteer was nearly free. `build-unified-wof` already accepted `--placetypes postalcode`, and the
  resolver already resolved postcodes for the US extract. Most of the anchor was assembly rather than invention.
...
- The tests seed their own fixture extracts, so the suite runs in CI without the data volume.
- A functional spot-check rather than an aggregate metric, caught the bad IT data. The `27% placed` number alone
...
- The WOF German postcode repo is rougher than expected. Roughly a third of its records are bare stubs
  ...
  direct fix crosses a policy line (see open questions).
...
- **The coarse ancestor fallback is a flag rather than a default.** It lifts the Berlin/Saxony sample from 34% to
