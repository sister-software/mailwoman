...
description: Changes-not-retrains night — eval/CI guardrails + query-shape/resolver changes, promotion walled.
---

# Night 34 — 2026-07-05

**Finalized at hand-off.** Window: 06:10 → 15:35 UTC (~9.5 h). Scope: changes-not-retrains, no-GPU-first.
...
`postcodeCountryPrior` change (GB 63→90%, CA 42→67%, staged), the metamorphic gauntlet taken to 35/35
...
- **#928 GB namesake FIX** (PR #928-change, staged, the night's biggest coverage win) — the GB tail
  diagnostic revealed a contained change: the coarse placer conflates GB/US (mis-routes `London E4 9AZ`
  ...
  Measured: GB 190→**271 ok (63%→90%)**, abroad 73→19, with the change on + GB safelisted. US-safe by
  construction. The diagnostic-then-change loop is why the "characterization" slack-time approach beat
...
- **Changes beat GPU**: #829 was slated for a possible fine-tune probe; the preprocessing change cleared
  the whole class for free.
- **The plan finished at ~12% of the shift**, so the bonus was real product: two more no-retrain
  changes (trailing-punct → metamorphic 35/35), a reusable OSM→golden panel builder, and two first-ever
...
- **#928 GB namesake tail characterized** (from the new panel) — 24% of GB rows resolve to US
  ...
  wiring), a focused-session change to the country-determination path — not a contained night change.
...
| Models trained      | 0                                                                                                                        |
| Modal $             | **$0 of $30** (every win a no-GPU change / measurement)                                                                  |
