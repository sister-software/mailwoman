...
implementation is fragile in a way that recipe tuning cannot stabilize.
...
- **Postcode-only harness**
  ...
  — per-country postcode accuracy check. Created as the v0.7 regression
...
ship recommendation was wrong because it was conditional against per-tag
...
crisis-level — the model is decisively wrong rather than uncertain. Two
implications:
...
calibrated against the same set we've been reading. Some check
...
- **Model capacity was not the binding constraint.** 29.3M params, 6
  ...
  is unknown — but the failures we measured were calibration
  failures rather than capacity ceilings.
...
- Early-eval checks against DEV; release-check against TEST (touched once).
...
2. **Postcode-aware tokenizer pre-pass** — regex detects and guards
...
- **Synthesis-as-supplement rather than synthesis-as-primary.** Small targeted
  extracts, weight < 0.25, one-and-done. Real data is the primary fix for
...
it — was the single highest-changeage intervention of the cycle.
