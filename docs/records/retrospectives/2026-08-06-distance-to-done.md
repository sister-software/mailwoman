...
Three numbers in the brief that opened this review did not match their primary artifacts. They are
corrected in place below and collected in [Corrections](#corrections-to-numbers-in-circulation).
The same pattern runs through the whole document: the receipts are accurate, and several of the
summaries built on top of them have drifted.
...
coupled to a gazetteer. The claim has four required terms. Three are demonstrably true today. The
fourth, "calibrated", needs a qualification. The isotonic calibrators ship in the weights bundle, but `calibrate` is
...
time"). A one-line documentation fix or a checked flag flip would resolve it. As written, the claim
and the default disagree.
...
Under the corrected key the candidate reads 75.2 against the 7.0.0 baseline's 71.7, a lead of
3.5pp. The check spec argues this at length and re-anchors the floor instead of ratcheting it. The reasoning is
...
ledger and nowhere else.** It does not appear in the release row, the Run B record, or the model card.
...
number is defensible. Its absence from every narrative surface is not. It is the most important
unrecorded finding in this review.
...
the repo is #1525's: **92/94 conditional, 177 tracked**, run twice identically. The repo has no
results artifact for that run, either as JSON or as a verdict file. The shared `regression.db` is
unstamped and, per #1525's own operator note, stale.

Each demotion is defensible on its own terms. #1526's rationale is correct: _"a 'pass' status on a
row that fails at HEAD is a mis-status rather than a check"_. The aggregate effect is that
...
has been scored against Pelias.** The founding objective's status is therefore _unknown_, and the
last measurement had mailwoman behind. Re-running it is the most valuable measurement available to
this project right now, and it needs no GPU.
...
photon.komoot.io". A person checked it by eye once, and no executable test verifies it.
...
are in better shape. mcp's `server.ts` and `cli.ts` contain the stdio transport, arg parsing, and lazy
weight load, which make up the whole operational claim. The test header states that they have
**zero coverage**.
...
The project has 74 open issues. The classification below marks each call that is a judgment rather than a reading of the record.
...
shipped without it, and ROAD_TO_V9 §8 parks it explicitly. The epic has outlived two majors of its
own name. **#598 (record matching)** is parked by an operator decision with the evidence recorded.
That is a deliberate park rather than a stall.
...
**Two issues look closed and are not.** #1539 and #1519 look closed by their commit subjects. `f2c9810b7` touches
...
note in that issue warns about the same drift in the other direction. The drift has built up again.

---

## 4. The gap list, ranked

This list gives no time estimates. It classifies each gap by the shape of the work: **single-lane** (one engineer, one thread), **multi-lane**
...
| #   | Gap                                                                                                                           | Shape       | Kind                                                                     |
| --- | ----------------------------------------------------------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------ |
| b1  | Re-run the three-way benchmark on 9.0.0; the standing result has Pelias ahead 88 to 80 @25 km                                 | single-lane | bounded engineering — **the most valuable single measurement available** |
...
| c8  | Starter kits — 22 unchecked boxes and zero checked. No artifacts exist                                                                                                  | single-lane                                        | bounded, plus two operator checks (org repo creation, npm first-publish) |
...
"this address is missing its postcode" back to the postcode, even though the gazetteer contains
it. This finding shapes the suggestion layer's entire design. Any claim that "mailwoman repairs
degraded input" is currently false for the postcode, the most valuable component.

### 5.2 The substitution hazard

A worse outcome also occurs: **16 of 139 rows emit a different token as the postcode.**
...
quirk. The v9 sub-venue extract is the first training-side fix for it. The class is not resolved.

### 5.4 The instruments have been lying, and some still are

Most of the last week's receipts record checks that did not measure what they claimed:
...
- The invariance runner grades the raw classifier with en-US for every row. It scored Run B's
  _gained_ GB capability as a loss. **This is not fixed.**
- `regression.db` can be rebuilt from a stale compiled tree. #1525 added stamping, but the shared
  artifact is currently unstamped.
...
The fixes are landing quickly, but more defects of this kind are likely.
...
- `README.md` links `docs/articles/plan/` and `docs/articles/evals/`, and neither exists. It says
  **"33 published packages"** where the release list holds 48, and it contains the **"Drop-in
  servers" section twice**.
...
This section states what the record shows, without extrapolation.

**The method.** The project pre-registers bars, reverts falsified changes instead of shipping them,
and writes verdicts down with the numbers that produced them. Twelve preregistration documents in
`docs/superpowers/plans/` carry graded verdicts, including three that read NEGATIVE and stopped. The
sp-vocab-prune arc was stopped by its own premise check. This practice is uncommon, and it made this
review possible. Someone had already measured almost everything this review needed and written the
measurement down.

**The v9 anchor fix.** The #1467 root cause was that every training config used a 67,708-key anchor
lookup with _zero letter-containing keys_, so the GB slot never received gradient. The fix was
verified with Fisher mass on the GB slot at 11.28% against CA/JP exact-zero controls. gb-golden went 294 → 318
...
at pack time with a guard that refuses to publish a missing target. Each of those safeguards exists
because something broke once and was recorded. The release process works.

**The gazetteer and layer discipline.** Gazetteer artifacts are sealed, read-only, provenance-tracked,
and license-tagged. A meaning-of-zero rule distinguishes absence from zero. The NI postcode extract
ships ancestry only, with zero coordinates, because inventing coordinates would be false data.

**The corpus of failures.** The project has 306 gauntlet cases across 115 countries, 400
oracle-verified sweep candidates, per-country JSONL, ablation ladders, and a degradation map. Most
projects at this stage know what works. This one has a written, structured, growing account of what
does not. 71% of the gauntlet does not check because someone refused to grade a failing row as a
pass.

**JP.** The JP model scores 0.9928 at 15 km against a pre-registered 0.70, on a board that holds out
whole municipalities. It uses a 2,237-character vocabulary and trained from scratch in about 80
minutes. This answers the architecture question that blocked non-Latin script for the project's
whole life. Only the serving path is missing.
...
Precision at 1 km is the real gap (26 against 71), and it comes from address-point coverage rather
than parsing. Re-running that benchmark on 9.0.0 costs no GPU and would tell this project more about
its position than any other single task.
...
needs engineers assigned to it.

This review's first finding is the gap between what the receipts show and what the summaries say.
