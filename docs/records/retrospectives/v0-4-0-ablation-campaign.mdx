...
The bio_slip class (6% of postcode FN) is a decoder bug, not a model bug. The model's BIO tag attribution is correct; the emitted span includes leading/trailing punctuation tokens. Fixed in `core/decoder/build-tree.ts` (commit `c72ab4c` on main):
