...
We hold no user accounts or query history. For a data-subject request (access, deletion, portability), we have nothing on file to return or delete beyond the short-lived operational logs described above. Send requests and questions to the contact address.
...
- **CCPA/CPRA (California) and similar regimes:** we collect no personal information and sell none, so there is nothing to opt out of.
