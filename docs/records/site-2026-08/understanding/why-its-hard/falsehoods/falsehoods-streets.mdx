...
`Piccadilly, London, W1J 9PN` has no type suffix. Neither does `Broadway, New York, NY` nor `The Strand, London`. several of the world's most famous streets lack the suffix pattern that address validators demand.
...
French addresses place the descriptor at the beginning: `rue de Rivoli`, `avenue des Champs-Élysées`, `place de la Concorde`. Spanish: `Calle Mayor`. Italian: `Via Roma`. German: `Hauptstraße`. The Anglophone "Street at the end" pattern is the exception rather than the rule.
...
Mannheim, Germany uses a grid system: `R 5, 6-13, D-68161 Mannheim` — block R, row 5, buildings 6-13, with no street name or street type at all.
