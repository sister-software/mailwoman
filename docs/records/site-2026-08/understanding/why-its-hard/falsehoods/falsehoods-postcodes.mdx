...
**The resolver (Stage 6)** treats the postcode as a search-space constraint rather than a primary locator. When the postcode is present, the resolver limits gazetteer lookups to the postcode's rough administrative area. When the postcode is absent, the resolver searches the full gazetteer and returns lower-confidence candidates. The postcode is supplementary evidence. The resolver does not drop to postcode-only geocoding unless nothing else is available.
