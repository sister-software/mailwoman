...
_"Close enough" is a statement about your use case rather than about the coordinate. A geocode that is correct for statistical aggregation may be catastrophically wrong for emergency dispatch. And the coordinate itself answers a question — "the front door" — that nobody bothered to define._
...
| The emergency entrance    | the result shows marked, accessible, fire department key box | Sub-meter                                                                    |
| The parcel centroid       | Tax assessor's coordinate for the land parcel                | frequently the geometric center of the lot rather than the building          |
...
| ------------------------------------------------- | ----------------------- | ----------------------------------------------- |
| Statistical aggregation (census, market analysis) | Block group (~250m)     | You're counting people rather than finding them |
...
A building at the corner of Main St and Elm St may have the address `123 Main St` but the only public entrance is on Elm St. The address's street name is a label rather than a statement about entrance orientation.
...
The reverse geocode is answering "what place is at this coordinate?" not "what addresses resolve to this coordinate?" The latter question requires a forward geocode of every candidate address and comparison of the resulting coordinates. The former question is what most geocoders answer.

## How traditional geocoders handled these

**libpostal** does not produce coordinates — it produces labeled text spans. The precision question is downstream. This is architecturally correct: the parser's confidence is about label correctness rather than spatial accuracy. The resolver owns the spatial question.
...
**The resolver (Stage 6) returns candidates with placetypes rather than a single coordinate.** A WOF record has a `placetype` and optionally a bounding box. The resolver returns the placetype alongside the coordinate, so the downstream system knows whether it received a building-level coordinate or a locality centroid. But WOF does not distinguish between "building entrance" and "building centroid" — both are the same coordinate in WOF's data model.
...
**[Confidence is per-component rather than spatial](../../../concepts/confidence-calibration.mdx).** The parser's confidence score is about the label — "I'm 95% sure this token is a locality" — not about the coordinate — "I'm 95% sure the locality is within 500 meters of this point." The resolver's score is about gazetteer match quality rather than spatial accuracy. Separating these into different signals (label confidence, resolver match quality, spatial precision metadata) is future work.
...
- **Reverse geocoding.** The resolver forward-geocodes — text → coordinate. Reverse geocoding (coordinate → text) is a different problem with different infrastructure (spatial index rather than text index). Mailwoman does not do reverse geocoding.
...
- [How mail delivery works](../../the-problem/how-mail-delivery-works.mdx) — the delivery chain that consumes these coordinates
