...
The categories overlap — several real-world addresses hit two or three at once. That compounding is part of the difficulty.

## 1. Ambiguous locality names

several city names are reused across regions. The string `"Paris"` could plausibly mean Paris, France or Paris, Texas. Without a region or country signal, no parser can disambiguate; without a population prior, no resolver can either.
...
A subclass of #1: the locality and one of its parent admins share a name. frequently legitimate (the city is the namesake of the country/state/province), but it confuses both regex-based parsers and naive resolvers that deduplicate by string match.
...
Mailwoman's defence: both rule and neural classifiers can emit multiple proposals on the same token sequence — the solver expects spans rather than unique strings. `B-locality I-locality , B-region I-region` is the BIO sequence that handles this.
...
- **NFC vs NFD normalization.** `"São"` can be encoded as one codepoint (`S + ã + o`) or two (`S + a + ̃ + o`). Same string, different bytes. Resolvers that index in one form and query in the other miss matches.
...
Addresses that mix two languages mid-string. frequently because of translation artifacts, sometimes because of bilingual cities (Montreal, Brussels), sometimes because of expat communities re-translating.
...
Place names that are popular enough to exist in dozens of jurisdictions. Even with a region qualifier, ambiguity frequently remains.
