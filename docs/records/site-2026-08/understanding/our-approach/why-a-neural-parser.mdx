...
Richard Sutton's 2019 essay "The Bitter Lesson" argues that over the history of AI, general methods that use computation have consistently outperformed hand-crafted domain-specific approaches. The lesson is bitter because the result — encode your knowledge into rules — almost always loses to the result — build a system that learns from data.
...
## What "neural" means: a small transformer rather than a large one

Mailwoman's neural classifier is a **9-million-parameter encoder-only transformer** trained from scratch on address data. It is about the same size as a 2018 BERT-mini rather than a 2026 GPT-6. This is deliberate.
...
- **Cheap to retrain.** When you add training data for a new failure mode, you can retrain in hours rather than days.

### Why from scratch

The model is trained from scratch on Mailwoman's corpus rather than fine-tuned from a pretrained general-English model. This is an explicit choice that is currently under empirical validation as of May 2026 — the training stability experiments described in the [v0.5.0 blog post](pathname:///research/2026-05-24-v0-5-0-c-train-bisect) are testing whether this choice is correct.
...
Rules are being augmented rather than replaced.
...
- `whos_on_first` — gazetteer lookup for country, region, locality, and neighborhood names.
- `street_prefix` / `street_suffix` — dictionaries of known directionals and street types.

These classifiers are deterministic, fast, and correct for the cases they cover. A US 5-digit postcode is a postcode — a model gains no benefit by re-learning that pattern.
...
Default mode is `rule_only`. The neural classifier warrants each component one at a time, blocked on golden-set metrics. If the neural model regresses, flipping back to `rule_only` is a one-line config change with no retraining.
...
The model conditions on the surrounding tokens because the transformer's [self-attention mechanism](../../concepts/attention-and-bidirectional-context.mdx) gives every token access to every other token's representation. "NY" next to "Steakhouse" is venue. "NY" next to "10001" is region. The model learns this distinction from corpus co-occurrence patterns.
...
This is the **graceful failure** that the operator's Paris, Texas talk described as the target behavior: "The mail carrier squinting at bad handwriting and figuring it out anyway." Unlike the mail carrier, the model doesn't figure out ambiguous input; it signals when it can't, and that signal is more useful downstream than a confident wrong answer.
...
:::info[Historical planning note — May 2026]

This section is a roadmap snapshot from the training-stability crisis of May 2026, superseded by everything that has shipped since. For current state, see [the scope declaration](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/SCOPE.mdx) and [Releases & capabilities](../../releases.mdx). Kept for the historical record.

:::

As of May 2026, Mailwoman is working through training stability challenges described in the [v0.5.0 retrospective](pathname:///research/2026-05-24-v0-5-0-c-train-bisect). The architecture — the Knowledge Ladder, the phrase grouper, the reconciler, the staged pipeline — is in place. The training recipe is being refined to produce stable model weights on the expanded corpus.

The architecture direction holds. What's open is whether the current training approach can reach its potential, a question the training log is answering in real time.
