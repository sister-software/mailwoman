...
Mailwoman [evaluates itself](../../concepts/quality-and-evaluation.mdx) by running against a set of 4,535 hand-labeled addresses (the "golden set") and measuring how well each pipeline mode does. This article explains the four modes, the metrics, and what the v0.5.0 results mean — in plain terms.
...
These are simplifications of the same [staged pipeline](./the-staged-pipeline.mdx). Each "mode" is a different composition of the same underlying stages rather than four separate parsers.

## The metrics

**Exact match** — did the parser get _every single component_ of the address right? House number, street, city, region, postcode — all must match the human-labeled answer exactly. This is harsh. Getting 4 out of 5 components right scores zero.
...
**Empty-parse rate** — how frequently does the parser give up entirely and return no result? Lower is better. A parser that always guesses something (even if wrong) scores 0% here.

**Overconfident-wrong rate** — how frequently does the parser say "I'm very sure" (confidence above 90%) but get the parse wrong? This is the most dangerous failure mode for downstream consumers: a geocoder that's confidently wrong will silently return the wrong coordinates with no signal that something went amiss.
...
### The reconciler fixes the overconfidence problem

Hybrid-joint mode (the reconciler) drops overconfident-wrong from 54.5% to 0.1%. How? By checking whether the parsed components form a consistent real-world hierarchy. "Is there a city called Houston in a state called NY?" If not, the parse is rejected or rewritten.

The reconciler also eliminates empty parses entirely (0.0%) — it always produces _something_, even if conservative.

The trade-off: exact match drops from 30.8% (rule-only) to 6.0% (hybrid-joint). The reconciler is better calibrated but less precise on well-formed addresses. This is a calibration-vs-accuracy trade-off that the next iteration will address by re-adding class weights to the training recipe.

### The architecture is working, the quality isn't there yet

The staged pipeline — rules for structure, neural for ambiguity, reconciler for calibration — is producing the behaviour it was designed for. Each layer adds value:
...
- Reconciler contributes calibration (0.1% overconfident-wrong vs 54.5%).
...
- [The staged pipeline](./the-staged-pipeline.mdx) — how the four modes compose
- [The knowledge ladder](./the-knowledge-ladder.mdx) — why each layer exists
- [Dual-loss curvature conflict](../../concepts/dual-loss-curvature-conflict.mdx) — why the training was unstable before and what fixed it
- [Training-run charts](../../concepts/training-charts.mdx) — the loss curves behind these numbers
- [v0.5.0 — as shipped](https://github.com/sister-software/mailwoman/blob/main/docs/records/plan/v0-5-0-shipped.mdx) — what the six threads delivered
