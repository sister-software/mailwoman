...
This is a roadmap snapshot, current as of May 2026. For current state, see [the scope declaration](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/SCOPE.mdx), [How Mailwoman parses an address](../../concepts/how-mailwoman-parses-an-address.mdx), [How Mailwoman resolves a place](../../concepts/how-mailwoman-resolves-a-place.mdx), and [How it works now](./how-it-works-now.mdx).

:::

This article describes where Mailwoman is heading. The work is tracked in GitHub issues and the [`plan/`](https://github.com/sister-software/mailwoman/blob/main/docs/records/plan/README.mdx) directory. Status is current as of May 2026.
...
The May 2026 diagnostic work identified that the **CRF-NLL term dominates the CE term** by 8-20× in gradient magnitude. This is the opposite of what the v0.4.0 campaign assumed (it hypothesized CRF gradient collapse rather than CRF gradient dominance). The current experiment is **CE-only training**: remove the CRF loss term entirely, train on cross-entropy alone, keep the CRF as an inference-time structural decoder with the frozen mask.

Resolution path:

1. **CE-only smoke (in progress).** 2,000-step constant-LR run with `crf_loss_weight=0.0`. Check: no loss climb past step 2,000 AND val_macro_f1 ≥ 0.35.
2. **If CE-only stable → full 50K C-train.** Quality ceiling TBD — the result is stability rather than quality. Quality gains come from recipe knobs (class weights, source weights, longer training) that were unsafe under dual-loss.
...
4. **Parallel: reconcile integration.** Wire joint decode as the default Stage 5 path behind a feature flag. Evaluate against kryptonite catalogue + golden v0.1.2. Check: +15pp exact-match on kryptonite, ≤1pt macro_F1 regression on golden.
...
- **Rule classifiers are not replaced.** They stay deterministic, fast, and reliable. The neural classifier warrants each component one at a time.
- **The model stays small.** The transformer is staying at roughly 9 million parameters. the result comes from better architecture (phrase grouper, joint decode) and better training rather than from scaling up.
...
- **Replacing upstream projects.** Mailwoman uses Pelias, libpostal, OSM, WOF, and other open data as sources and inspiration. It is a complementary project rather than a competitor.

## See also

- [v0.4.0 blog post](pathname:///research/2026-05-23-v0-4-0-ablation-campaign) — the ablation campaign retrospective
- [v0.5.0 blog post](pathname:///research/2026-05-24-v0-5-0-c-train-bisect) — the C-train divergence and bisect
- [The knowledge ladder](./the-knowledge-ladder.mdx) — the staged decomposition design
- [Implementation plan](https://github.com/sister-software/mailwoman/blob/main/docs/records/plan/README.mdx) — the full phase roadmap
