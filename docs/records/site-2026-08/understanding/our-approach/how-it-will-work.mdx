...
2. **If CE-only stable → full 50K C-train.** Quality ceiling TBD — the result is stability, not quality. Quality gains come from recipe knobs (class weights, source weights, longer training) that were unsafe under dual-loss.
...
- **The model stays small.** The transformer is staying at roughly 9 million parameters. the result comes from better architecture (phrase grouper, joint decode) and better training, not from scaling up.
