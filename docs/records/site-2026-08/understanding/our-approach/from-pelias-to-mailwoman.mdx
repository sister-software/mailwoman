...
- **Parsing** — turn the string into labeled parts (`house_number=350`, `street=5th Ave`, `locality=New York`, `region=NY`, `postcode=10118`). For this, Pelias uses [`libpostal`](https://github.com/openvenues/libpostal), a C library trained on OpenStreetMap data.
- **Resolving** — look up the parsed place in a gazetteer (a large place database, frequently [Who's On First](https://whosonfirst.org/)) and return coordinates plus IDs.
...
The neural classifier is a small transformer model (about 29M parameters in the current weights — see [`concepts/how-mailwoman-parses-an-address.md`](../../concepts/how-mailwoman-parses-an-address.mdx)) trained on a corpus built from several open data sources and rebuilt several times over — see [`concepts/training-pipeline.md`](../../concepts/training-pipeline.mdx) for how it's constructed. It ships in two pieces:
...
- **Multi-locale packaging.** Mailwoman ships a separate weights package per locale (en-us, fr-fr) — a packaging boundary for a locale to get its own trained weights without a breaking change, even though today both packages carry the same model (see [`plan/reference/ARCHITECTURE.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/ARCHITECTURE.mdx)).
...
design: spans that don't fit the winning interpretation are demoted to
"accessory" information and punted to the resolver. If your only goal is "resolve to a
point", that's a reasonable economy. But it forecloses the geocoder-_adjacent_ jobs —
