...
flowchart TD
    ...
    S1 --> S15["Stage 1.5: Coarse-placer<br>separate small model — guesses the country (on by default)"]
    S15 --> S2["Stage 2: Locale check<br>script class + postcode-format detection"]
    ...
    S27 --> S3["Stage 3: Token classify<br>rule + neural classifier — one forward pass"]
    ANCH["Anchors: soft hints retrieved from the databases<br>postcode → country; gazetteer place-name + homograph flags"]
    ANCH -.->|"informs, never overrides"| S3
    S3 --> S4["Stage 4: Sequence correct<br>CRF Viterbi, frozen structural mask"]
    S4 -->|"default: argmax"| S6["Stage 6: Resolve<br>WOF gazetteer lookup + disambiguation"]
    S4 -.->|"opt-in"| S5["Stage 5: Reconcile<br>joint decoding (off by default)"]
    S5 -.-> S6
    S15 -.->|"country prior"| S6
...
Unicode normalization (NFD-decomposed accents → composed), locale-aware case folding, whitespace collapsing. Carries no address-structure knowledge. Cleans bytes so downstream layers see canonical text.

### Stage 1.5 — Coarse-placer

A separate, small model (#244) reads the whole normalized string and guesses its country, with a confidence. It is **on by default,** and it is the second model in the system. It has exactly one job: a fast, coarse country guess. It does _not_ feed the main classifier. Its guess is held back and handed to the resolver (Stage 6), where it gently boosts in-country candidates when two places share a name. A confident "this looks French" never forces a French reading; it only tips the scale at resolution time. This is the same discipline as the anchors below: knowledge informs, it never overrides.

### Stage 2 — Locale check

Detects whether input is en-US, fr-FR, ja-JP, etc. Rule-based today: script-class detection (CJK → ja-JP, Cyrillic → ru, Arabic → ar) plus known-format hits (5-digit ZIP → en-US, FR postcode → fr-FR). The `@mailwoman/locale-hint` workspace ships the logic; it exists as a workspace but the factory default still falls back to caller-trust. Wiring it as the default is a near-term TODO.
...
The v0.5.0 classifier (Thread C-s) adds two architectural changes blocked behind config flags:
...
Both are scaffolded and tested in `main`. The full training run that exercises them (C-train) is in progress as of May 2026 — see the [v0.5.0 C-train blog post](pathname:///research/2026-05-24-v0-5-0-c-train-bisect).

### How outside knowledge reaches the model — anchors

The classifier never memorizes the world's place names; that is the gazetteer's job. Instead, knowledge from the databases is retrieved at inference time and fed in as soft input features — **anchors** — alongside the tokens:

- **Postcode-anchor.** A postcode is looked up and the result ("this is probably in the United States") is fed in as a soft signal on the surrounding tokens.
- **Gazetteer-anchor.** Each token is flagged with what the gazetteer knows about it — whether it matches a known country, region, PO box, or _homograph_ place-name (a place whose name also looks like something else, like "Paris" or "Reading").

Both are optional and both are soft: they shift the model's probabilities, they never replace its reading. This is the one discipline that keeps the system from sliding back into a rule engine. We let a lookup table _override_ the model exactly once, for `country`, and reversed it within a day when a homograph eval caught the regression. Knowledge informs; it never overrides. For the mechanism, see [FST priors as shallow fusion](../../concepts/fst-priors-as-shallow-fusion.mdx) and [What Mailwoman is](../../concepts/what-mailwoman-is.mdx).

The forward pass itself runs **once** per address. The anchors and the phrase prior are folded into the per-token scores, then a single decode (Stage 4) picks the labels. There is no loop, and no second neural read of the model's own output.
...
Joint decode is **opt-in**, off by default. It was briefly the default and then retired on 2026-06-14: joint reconcile broke the street + house_number precondition on [77–84% of clean US addresses](https://github.com/sister-software/mailwoman/blob/main/docs/records/evals/experiments/2026-06-14-reconcile-retirement.md) while fixing none, and the damage stayed invisible because the evals graded the raw neural labels rather than the assembled parse a user receives. The default is now plain argmax (spans sorted by start position); reconcile stays behind a flag until it can be graded on the assembled coordinate. The successor mechanism for rule-vs-neural disagreement is the opt-in **arbitration** layer (#478), which edits the neural tree in a containment-preserving way and is also off by default.
...
This is where "which Paris?" gets answered — never by the model, which only tagged "Paris" as a place. The resolver walks the parsed tree top-down and narrows each name with everything around it: a resolved parent constrains its children (a locality must sit inside the region that resolved above it), the country constrains the search, an exact-name match outranks a fuzzy one, and the coarse-placer's country guess (Stage 1.5) re-ranks candidates within a tier. Only when all of that still leaves a tie does population break it — "Paris" with no other signal resolves to France because more people mean that one, the same shortcut a person on the street uses. See [the resolver and WOF](../../concepts/resolver-and-wof.mdx) and [importance vs population](../../concepts/importance-vs-population.mdx).
...
Default mode is `rule_only`. The neural classifier warrants each component one at a time, blocked on golden-set metrics. If a neural model regresses, flipping back to `rule_only` is a one-line config change with no retraining.
...
- **CE-only training resolved the divergence problem.** The v0.5.2 model ships from a stable 100K-step CE-only training run on an A100. The dual-loss CRF-NLL + CE divergence that blocked v0.4.0 and v0.5.0 was resolved by setting `crf_loss_weight: 0.0` — CRF is used at inference only rather than training. v0.5.3 diagnostic training is in progress with per-tag F1 visibility.
...
- **Joint decode is retired as the default (2026-06-14).** It stays behind `forceJointReconcile`; the default is argmax. Reconcile broke the street + house_number precondition on most clean US addresses while the label-level evals showed it improving — which is the whole reason we now grade the assembled parse rather than the raw labels. See Stage 5 above.
- **Locality/region confusion persists on some inputs.** "New York, NY" is fixed (region-aware guard). "Washington, DC" is partially fixed (FST correctly biases locality) but the model's high B-street confidence for "Washington" after a street phrase resists the prior. Fix is in training data rather than more priors.
