...
Default mode is `rule_only`. The neural classifier earns each component one at a time, blocked on golden-set metrics. If a neural model regresses, flipping back to `rule_only` is a one-line config change with no retraining.
