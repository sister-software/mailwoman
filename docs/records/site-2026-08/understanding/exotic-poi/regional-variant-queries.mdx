...
| liquor store               | off-license           | UK, Ireland                 |
...
**Google** handles regional variants through its search model, trained on global query logs. "Servo" returns gas stations in Australia because Google's Australian users search for "servo" and click on gas station results. The mapping is learned from user behavior rather than hand-curated.
...
**Nominatim** does not handle regional variants. The search is against OSM tags and names, which use standard English categories. "Servo" returns no result unless an Australian mapper added `alt_name=servo` to a gas station — which almost none have.
...
2. **A locale signal.** The geocoder needs to know where the user is to disambiguate variants. "Chemist" means pharmacy in the UK and Australia but not in the US. The locale check (Stage 2) provides this signal for Mailwoman — it detects the script and format of the query, which is a proxy for the user's region.
