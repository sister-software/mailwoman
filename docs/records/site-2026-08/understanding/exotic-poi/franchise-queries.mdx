...
**Franchise queries are still deferred.** Mailwoman has no brand lexicon yet. No table maps "Walmart," "Macca's," or `マクド` to a chain identifier, and nothing joins Overture's `brand.wikidata` field to a name. The location-constraint half (`near Springfield, IL`) already works through the same anchor-parsing path that amenity queries use. The brand-lookup half doesn't exist. See [the series index](./exotic-point-of-interest-queries.mdx#whats-still-ahead) for what's built and what isn't across the whole POI arc.
