...
A franchise query asks for a specific chain or brand. The user wants the nearest **McDonalds** rather than the nearest restaurant. The brand name is the query, and the geocoder must resolve it to one or more specific locations.
...
Franchise queries have three components: a **brand name**, an optional **sub-brand** or **feature filter**, and an optional **location constraint**. The brand name is the hard part because it may not match what the franchise calls itself, and it varies by region.
...
The alias table is not a translation dictionary. It [maps every name people use for a chain onto one brand identifier](../../concepts/synonymy-and-homonymy.mdx), so "Macca's" in Australia, "Mickey D's" in the US, and "マクド" in Kansai all resolve to the same brand.

## Sub-brand problem

Franchises have sub-brands that differ in meaningful ways:

- **Walmart** has Walmart Supercenter (full grocery), Walmart Discount Store (general merchandise without grocery), Walmart Neighborhood Market (grocery-only, smaller footprint), and Sam's Club (warehouse membership).
...
The location constraint is an address or place name, which is the kind of query Mailwoman already handles. The franchise lookup is new. Parsing the location constraint and then querying the franchise index near that location is a **two-stage resolver** query: resolve the place, then find POIs near it.
...
**Nominatim** handles franchise queries by matching the brand name against OSM's `name` and `brand` tags. OSM has good coverage of franchise locations in well-mapped areas. However, OSM's brand tagging is inconsistent: some locations are tagged `brand=McDonald's`, others `name=McDonald's`, and the relationship between franchise brand and individual location name varies by mapper. Nominatim does not handle slang or regional variants. `Macca's` returns no result unless an Australian mapper tagged a location with `alt_name=Macca's`.

**Pelias** indexes franchise locations as part of its OSM and OpenAddresses gazetteers. The same OSM tagging inconsistency applies. Pelias's search matches "McDonalds" against name and brand fields, and its `categories` parameter can filter by broad types (e.g., `food`), but it has no brand-alias resolution. Slang and variants don't match unless the exact text appears in the data. Pelias does not model sub-brand classification.

## What Mailwoman does today

The category infrastructure that amenity queries use (the `poi_query` kind, the anchor split, and `poi.db` resolution) exists now, but brand matching isn't wired into it. Mailwoman's parser still sees `Walmart` and tries to classify it as an address component. A lone brand name is a one-token address, and the parser may read it as `locality` (there are towns named Walmart) or `venue`. The venue classifier exists but is weak, as [Landmark queries](./landmark-queries.mdx).

**Franchise queries are still deferred.** Mailwoman has no brand lexicon yet. No table maps "Walmart," "Macca's," or `マクド` to a chain identifier, and no lookup joins Overture's `brand.wikidata` field to a name. The location-constraint half (`near Springfield, IL`) already works through the same anchor-parsing path that amenity queries use. The brand-lookup half doesn't exist. See [the series index](./exotic-point-of-interest-queries.mdx#whats-still-ahead) for what's built and what isn't across the whole POI arc.
