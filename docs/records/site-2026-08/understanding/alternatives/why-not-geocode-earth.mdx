...
If Google's API is the proprietary default, [geocode.earth](https://geocode.earth) is the open-source default. It runs Pelias — the geocoder Mailwoman forked — as a hosted service with global coverage, open data, and transparent pricing. For several applications, geocode.earth is the right answer. For applications that need to own their parser, it has the same fundamental limitation as every hosted API: you do not own the parsing layer.
...
Geocode.earth's core selling point is that you can inspect the data, run it yourself, or pay them to run it for you. The hosted service is a convenience rather than a lock-in.
...
For geocode.earth's use case, these are acceptable tradeoffs: Pelias on hosted infrastructure is fast, reliable, and good enough for most address formats. They are also why Mailwoman exists as a separate project, built to learn from data rather than encode every address shape as a rule.
...
- [Why not use Google's API?](./why-not-google-api.mdx) — the proprietary alternative
- [From Pelias to Mailwoman](../our-approach/from-pelias-to-mailwoman.mdx) — the fork history
- [The tokenization tautology](../why-its-hard/the-tokenization-tautology.mdx) — the Pelias parser's structural ceiling
- [How it used to work](../our-approach/how-it-used-to-work.mdx) — Mailwoman v1 (pre-fork Pelias) in detail
