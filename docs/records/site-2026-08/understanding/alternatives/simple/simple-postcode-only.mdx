...
That's it. Three steps, each deterministic, each trivially updateable when a new postcode is assigned. The rest of the address — the street, the building number, the apartment — is for the delivery driver rather than the geocoder.
...
- **You're in a country with precise postcodes.** UK postcodes cover ~15 addresses on average. Irish Eircodes are unique per delivery point. Canadian postcodes cover one side of one block face. In these countries, postcode-only geocoding is nearly building-level.
...
- **You have the rest of the address for non-geographic use.** The street and building number are for printing shipping labels rather than for geocoding. The geocoder only needs to answer "which delivery zone?" and the postcode answers that.
...
- **Addresses without postcodes.** Ireland before 2015 (Eircode launch). Hong Kong. Rural routes in developing countries. Military addresses (APO/FPO). The regex extracts no field, the lookup returns no result, the geocoder fails.
...
- **The user's expectation.** A user who types their full address and gets a coordinate expects sub-building accuracy. A ZIP centroid feels like a wrong answer even when it's the best the system can do. The geocoder has no way to signal "this is ZIP-level accuracy rather than address-level."

## Where Mailwoman fits

Mailwoman's resolver treats the postcode as **supplementary evidence rather than a primary locator.** When the postcode is present, the resolver limits gazetteer lookups to the postcode's rough administrative area — the same information the postcode-only approach uses. But Mailwoman also uses the parsed locality, region, and street to narrow the candidate set further. A postcode + locality + region match is more precise than a postcode alone.

The postcode-only approach and Mailwoman are not competitors. A system that starts with postcode-only geocoding and later needs finer resolution can add Mailwoman as a **second pass**: postcode-only for the 90% of addresses where ZIP-level accuracy is sufficient, Mailwoman for the 10% where a specific building matters. The postcode extracted by the regex feeds directly into Mailwoman's locale check and resolver constraints.
