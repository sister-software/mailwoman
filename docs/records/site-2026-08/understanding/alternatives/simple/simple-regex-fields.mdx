...
- **Non-standard ordering.** `75008 Paris, 12 rue de la République` — the postcode is at the beginning, the street number is after the street type, the city is after the postcode. Every positional assumption is wrong. The regexes extract no field or extract the wrong thing.
...
Mailwoman's locale check (Stage 2) and kind classifier (Stage 2.5) serve the same function as the positional anchor: "this is a US address in standard ordering." The difference is that Mailwoman learns the ordering from data rather than hardcoding it per country.
