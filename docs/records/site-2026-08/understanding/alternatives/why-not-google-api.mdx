---
sidebar_position: 3
title: Why not use Google's API?
tags:
  - domain
  - hubris
  - motivation
...
# Why not use Google's API?

Google's Geocoding API is the default choice for geocoding. It is fast, globally complete, and returns results in milliseconds. For several applications, calling Google is the right move. For applications that need to own their address data, audit their results, or operate at scale, it is the expensive kind of cheap.

## What you get

Google's Geocoding API converts an address string into a latitude/longitude, a formatted address, and a set of address components (street number, route, locality, administrative area, country, postcode). The API does parsing, resolving, and formatting in a single call. There is no separate parser or gazetteer — Google's model handles everything.
...
| India, Brazil, Southeast Asia        | Good — municipality or neighborhood level rather than building-level |
| Sub-Saharan Africa, Central Asia     | Variable — region or locality level, measured gaps in rural areas    |

The uneven coverage is not Google's fault — it reflects the quality of the underlying data sources in each country. But it means the API's "global" label masks measured regional variation.
...
- **Prototypes and MVPs.** You need geocoding working today rather than in six months. The API cost is negligible at prototype scale.
...
- **You operate in regions where Google is weak.** If a measured fraction of your addresses are in countries where Google's coverage is at the locality level, a regional gazetteer may outperform Google.
...
- **You handle guarded addresses.** If privacy regulations or internal policy prohibit sending addresses to third-party APIs, you need an in-house solution.

Mailwoman targets the "own your parser" niche — an alternative for applications where Google's terms, pricing, or opacity make the API unsuitable rather than a Google competitor.

## See also

- [The 90% trap](../why-its-hard/the-90-percent-trap.mdx) — the economic argument for owning your parser
- [The database fallacy](../why-its-hard/the-database-fallacy.mdx) — why even Google's database has gaps
- [Why not use geocode.earth?](./why-not-geocode-earth.mdx) — the open-source hosted alternative
