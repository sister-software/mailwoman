...
Intersection addresses show that **the "housenumber + street" format is one convention among many**, not a universal requirement. A parser that assumes every address has a building number will fail on intersection queries. A parser that treats `intersection_a` and `intersection_b` as first-class components can handle both conventional and intersection addresses without architectural change.
...
- [How mail delivery actually works](./how-mail-delivery-works.mdx) — the delivery system that handles intersections
