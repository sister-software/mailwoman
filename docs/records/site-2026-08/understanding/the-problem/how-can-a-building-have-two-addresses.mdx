...
## The address is a protocol rather than a property

An address is a **social protocol** for directing a courier rather than a geographic fact — different couriers use different protocols, and the same building can participate in several at once.
...
- **Postcode boundary**: A building whose units on different floors map to different postcodes because the postcode boundary follows the building's vertical axis (rare but real in dense urban cores).
...
And let the resolver return candidates for "Empire State Building" in New York City — which will include the 350 5th Ave location with high confidence. The parser does not need to know that "Empire State Building" and "350 5th Ave" are the same place. The the system that because WOF stores both as names for the same `wof:id`.

When the input is `350 5th Ave, New York, NY 10118` and the resolver returns the WOF record for the Empire State Building, the output should include the WOF ID and coordinates, but also preserve the raw parsed address. The user might need the mailing address form rather than the building name.
...
The `unit` tag is optional — several addresses do not have units. But its presence in the schema means the parser can recognize and extract unit information when it exists, and flag its absence when the building is known to be multi-unit (from the resolver's building metadata).

## See also

- [What is a postcode?](./what-is-a-postcode.mdx) — why mailing postcodes and geographic postcodes diverge
- [The database fallacy](../why-its-hard/the-database-fallacy.mdx) — why no database captures all address forms for a building
- [How humans break addresses](../why-its-hard/how-humans-break-addresses.mdx) — the postal city ≠ municipal city failure mode
- [Resolver and Who's On First](../../concepts/resolver-and-wof.mdx) — how the gazetteer stores alternate names
