...
The hierarchy is for sorting mail rather than for describing geography. A US 5-digit ZIP gets a letter to the right post office — that's it. The carrier who delivers it knows which streets are in their route. The ZIP doesn't encode that information explicitly.

## Why postcodes are not polygons

The US Postal Service does not publish ZIP code boundaries. The ZIP code is a route rather than a shape. A single ZIP can contain:
...
When you see a ZIP code displayed as a polygon on a map, you are almost certainly looking at a **ZIP Code Tabulation Area** (ZCTA) — a statistical approximation produced by the US Census Bureau. ZCTAs are generalized from census blocks rather than from USPS delivery routes. USPS explicitly disclaims them. The Census Bureau's own documentation says ZCTAs "are not exact representations of USPS ZIP Code service areas."
...
- **PO boxes cluster at post offices.** A ZIP code used primarily for PO boxes (common in small towns) will produce a centroid at the post office building rather than at the residences the PO box holders live in.

## What to do with a postcode in a geocoder

A geocoder should treat a postcode as **supplementary evidence** rather than as a primary locator:
...
Postcodes are designed for sorting machines and carrier routes rather than for maps. Every design choice follows from this:
...
A postcode is built to move mail rather than to locate things. The geocoder's job is to treat it as a spatial hint for the resolver rather than as a spatial fact on its own.

## See also

- [What is a ZIP Code and how is it structured?](./what-is-a-zip-code.mdx) — the US system in detail
- [The database fallacy](../why-its-hard/the-database-fallacy.mdx) — why there is no master list of postcode-to-place mappings
- [How mail delivery works](./how-mail-delivery-works.mdx) — the system postcodes were designed for
