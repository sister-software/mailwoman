...
Addresses look obvious — you see one every day — but parsing them turns out to be one of the harder problems in natural language processing, because a single string can mean several places, and a single place can be written several ways.
...
| `locality`                         | `"New York"`, `"Paris"`                      | typically         |
| `dependent_locality`               | `"Greenpoint"` (neighborhood)                | no                |
...
A parsed result is therefore a **flat dictionary** with optional repeated tags (rare; typically each tag appears at most once), not a tree. The exact shape:
...
"Buffalo" can be a locality (Buffalo, NY), a venue (Buffalo Wild Wings has it under), an animal name, or even a verb. Context decides which. A rule that hardcodes "Buffalo = locality" is wrong in two of the three cases.
...
A parser that labels one token at a time has to decide that "Saint" and "Petersburg" go together. The first version of Mailwoman's neural classifier (v0.2.0) got "Saint Petersburg" wrong because the second token's label was independent of the first — see [BIO labels](../../concepts/bio-labels.mdx) for the fix.
...
- [Tokenization](../../concepts/tokenization.mdx) — how strings become tokens before parsing
- [BIO labels](../../concepts/bio-labels.mdx) — how the neural classifier marks component boundaries
- [Resolver and Who's On First](../../concepts/resolver-and-wof.mdx) — turning parsed components into coordinates
