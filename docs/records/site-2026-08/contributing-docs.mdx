...
that touches the site); the editorial parts are reviewer discipline. Run the check locally with
`yarn workspace @mailwoman/docs lint:structure`.

## Declare the page's role

Every maintained page declares what kind of page it is in frontmatter, and each role has a interface:
...
[Joint consistency resolution](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/design/2026-06-29-joint-consistency-resolution.mdx) for the rendered
...
Interfaces belong in Reference; a duplicated explanation is a future contradiction.
...
- [ ] Don't touch publicness. Reorganization never changes what's published: nothing excluded from
      ...
      detail, and evidentiary value (a maintained page wants to cite them). Demotion is for mistakes rather than age: a page leaves the
...
- **Name the property.** The word "direct" (and its cluster) stays out of prose — say what the
  page claims: the eval holds out the training data, the coverage number excludes
