...
[Closed-vocab fields: model-first](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/closed-vocab-fields-model-first.mdx) for why
...
A node typically plays one role (its `tag`). A **dual-role place** plays several on one span: `Berlin` is both a region and a locality. The extra roles ride in `interpretations` (each a `{ tag, placeId, lat, lon, … }`), and the flat projections surface every role — `decodeAsJSON` emits both `region` and `locality`, `decodeAsXML` lists `roles="region locality"`. See [concepts/dual-role-places.md](./concepts/dual-role-places.mdx).
...
The demo at [`/demo`](https://mailwoman.ai/demo) is the reference browser integration.

## Component tags

The full [`ComponentTag` union](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/SCHEMA.mdx) (authoritative record: `core/types/component.ts`):
...
generated clients, with no hand-maintained artifact downstream of the spec. `mailwoman clients generate`
...
both build: `uv build` the Python package + import-check the wheel in an ephemeral
environment, `cargo check --examples` the crate. No generated artifact is committed — regenerate any time
the specs change.

**Not yet published.** Both packages build and verify directly on every `mailwoman clients generate`
run, but neither has shipped to a registry yet — that's blocked behind an operator step (provisioning a
PyPI account and a crates.io account, each with a publish token) documented in the repo's
`RELEASING.md`. Once provisioned, the same generation pipeline runs inside a conditional CI job: it always
...
Forward-geocode against the hosted Photon trial endpoint (`https://photon.mailwoman.ai`, no local
server needed):
