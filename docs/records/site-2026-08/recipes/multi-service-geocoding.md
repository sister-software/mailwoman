...
If you'd rather think in meters than tiers, `uncertainty_m` is the same decision as a number: escalate anything `null` or above your threshold.
...
Name both sides. The first pass isn't free to stand up: you're hosting a server and its data bundles (the gazetteer, plus per-state address-point extracts if you want US rooftop answers), which means a multi-gigabyte download and a box with some RAM. Free per row rather than free to run. If you geocode a few hundred addresses a month, skip all of this and use a hosted API directly; the cascade warrants its moving parts when volume times refresh rate makes the per-row meter the thing you're optimizing.
