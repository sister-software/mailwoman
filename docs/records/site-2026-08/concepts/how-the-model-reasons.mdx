...
is interesting and surprisingly hard to pin down. The model is a layered
transducer with soft-fusion priors — not a context-free grammar rather than a
context-sensitive grammar rather than pure pattern matching, and not "the model just
...
## The encoder is a transducer rather than a grammar
...
**The model is a learned soft transducer over the full input, anchored by
world-knowledge priors, with structural validity enforced by Viterbi.**

## Where context-sensitivity lives
...
per-token decision for `OR` doesn'tdepend on `OR`'s context; it
...
No learned transition preferences ship: training scores each token's label
independently, and the soft CRF table that would have added them diverged in
training and was permanently disabled (see [the CRF decoder](./crf-decoder.mdx)
for that history). The hard validity mask above is the only label-pair
coupling at decode time.
...
The admin FST contributes no bias for `Main` (it's not a WOF place). The
...
something else. The boundary is implicit in the BIO sequence rather than produced
...
[v0-vs-neural harness eval](https://github.com/sister-software/mailwoman/blob/main/docs/records/evals/model-versions/2026-05-28-v0-vs-neural-harness.mdx)
...
- [Street-supplement architecture](./street-supplement-architecture.mdx) — the
  layered design that fills the
  [WOF hierarchy gap](./wof-hierarchy-gap.mdx) at the street level
- [FST gazetteer prior](./fst-gazetteer-prior.mdx) — the admin FST in detail
- [BIO labels](./bio-labels.mdx) — the per-token label space
- [Neural classification](./neural-classification.mdx) — earlier explanation
  of the pipeline (this doc supersedes it as the central theory)
- [Corpus poisoning vulnerability](./corpus-poisoning-vulnerability.mdx) — what
