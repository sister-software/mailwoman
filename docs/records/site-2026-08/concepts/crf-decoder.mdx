...
- **Training-time loss check:** `corpus-python/src/mailwoman_train/model.py` — CRF forward/backward only runs when `crf_loss_weight > 0`, which every config from v0.6.3 on leaves at `0.0`
