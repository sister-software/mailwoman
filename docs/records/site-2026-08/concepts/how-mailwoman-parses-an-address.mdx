...
This tree is the handoff. The resolver walks it top-down and decorates nodes with coordinates and attribution, scoping each child's lookup to its resolved parent — a `Springfield` under a resolved `Illinois` searches Illinois, not Massachusetts. [Resolution is its own subject](./how-mailwoman-resolves-a-place.mdx) with its own implementation; the parse ends here.
