---
sidebar_position: 19
title: DeepSeek — max_tokens covers reasoning, notoutput
tags:
  - concepts
  - corpus
  - training
...
# DeepSeek — max_tokens covers reasoning, notoutput
...
DeepSeek's chat-completions API takes a `max_tokens` parameter. The natural reading is "stop after this several output tokens." The actual semantic is "stop after this several **reasoning + output** tokens." With `reasoning_effort=low`, the model's internal reasoning consumes 75% of the budget before any output is emitted.
...
The single best signal is the rejection-pipeline log. If the substring validator (see [Synthetic corpus — alignment validation](./synthetic-corpus-validation.mdx)) starts rejecting rows in batches, with reasons like `not-in-raw:postcode` clustered at end-of-row positions, the LLM responses are being truncated. The validator is doing the right thing. It would silently corrupt the corpus without it.
...
DeepSeek's documentation does state this, in a sentence buried under the reasoning-effort overview. If you discover the trap by reading the docs, well done. If you discover it by watching your corpus get silently truncated, you are in good company — Thread B2's first run was the first time we hit it inside the lab workflow, and the patch took longer than the original generation would have if we had set the budget right at the start.

## Carry forward

When writing the next DeepSeek-driven worker:

1. Default `max_tokens` to 3× the natural output ceiling rather than 1×.
...
- [Synthetic corpus — alignment validation is essential](./synthetic-corpus-validation.mdx) — why the validator caught this even when the worker silently accepted truncated responses
- [`CORPUS_V0_4_0_GENERATION.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/CORPUS_V0_4_0_GENERATION.mdx) — the operational record, including B2's actual prompt-engineering decisions and rate parameters
