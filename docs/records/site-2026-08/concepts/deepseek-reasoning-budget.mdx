...
DeepSeek's chat-completions API takes a `max_tokens` parameter. The natural reading is "stop after this many output tokens." The actual semantic is "stop after this many **reasoning + output** tokens." With `reasoning_effort=low`, the model's internal reasoning consumes 75% of the budget before any output is emitted.
