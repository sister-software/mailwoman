...
Layer 4 (brand FST for franchise venues) is parallel, not subordinate. Layer 5 (locality-conditional hierarchy blocking) is meta — it modulates which layers are active per country.
...
**Locale handling:** Build a single combined FST containing all locales — the morphology data is small enough (60 locales × ~30 affix lines × ~5 variants = ~9K entries) that per-locale extract routing is unnecessary. Position (prefix vs suffix) is encoded in the `PlaceEntry` payload, not derived per-entry at query time.
...
**Status:** Spec'd, not built. Lands after Layer 1 results inform whether candidacy provides additional headroom.
