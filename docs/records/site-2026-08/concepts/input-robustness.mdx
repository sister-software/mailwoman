...
and the checks that verify each layer holds.
...
The first pass is pure code, no model, in [`@mailwoman/normalize`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/STAGES.mdx).
...
That last one warrants its keep: a stray period glued onto the final token
...
leaves it off (`normalize/compute.ts`). That's a choice rather than an oversight.
...
Turkish dotted and dotless I) and violate the required offset relationship Layer 1 worked
...
(`corpus/src/extract-recipes/intersection.ts`, `locale.ts`), abbreviation variants in
...
to the model. No rule rewrote one into the other; the model saw both forms
...
Absorption you can't measure is absorption you can't trust. Three checks watch these
...
(`mailwoman/eval-harness/gauntlet/metamorphic.ts`), layer 2 of
...
confidence in it. It also grades the **assembled geocode**, coordinate and tier rather than
...
- **INV (invariance, ≤1m).** A label-preserving perturbation of a base address
  ...
  coordinate more than a meter (`INV_EPSILON_KM = 0.001`) or change the resolution
...
- **BAND (tolerance, ≤5km).** A _corrupting_ perturbation (a single-character transpose
  ...
  parse, so demanding byte-identical output would be the wrong interface. It still has
...
a tracked note: visible, non-blocking, so the check fails only on _new_ regressions. The
bookkeeping reduces the other way too: an xfail that starts passing gets flagged for
removal, so the list can't rot into the Pelias pass-list trap it was built to avoid.

Casing and spacing are green, cleared by the two normalizer changes above (#829's
...
floor rather than a verdict on production.
...
cases. They're a stress test with expected components attached rather than a coordinate
check: the parse should survive with lower confidence, and the labels say what
"survive" means.

### `perturb-golden.ts` — the one-off arena

`packages/mailwoman/lib/dev-tools/perturb-golden.run.ts` takes golden rows and applies rule-defeating
...
It's a generator you run when you want that arena rather than a standing check.

## The coverage matrix

Put the perturbation classes against the layers that absorb them and the checks that
watch them, and the full picture falls out, thin spots included.
...
to be blind spots. They run through the metamorphic check now, and the surprise was that
several of them already hold. The EN suffix abbreviations pass the strict meter
...
- [Eval discipline](./eval-discipline.mdx) — the full Gauntlet, and why the
  coordinate is graded rather than the name string
- [What Mailwoman is](./what-mailwoman-is.mdx) — why knowledge stays outside the
  learner, the principle behind Layer 1's abstention
- [The pipeline interface](./staged-pipeline-interface.mdx) — where the normalize stage
