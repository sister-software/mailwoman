...
**Attempts at column-weighted BM25** (`bm25(place_search, 1, 10, 1)` — name 10×, alt_names 1×) **don't fix this**: FTS5's doc-length normalization uses the row's total content length across all columns, the unweighted ones included. A row with bloated `alt_names` is penalized everywhere, even when scoring the `name` column in isolation.
