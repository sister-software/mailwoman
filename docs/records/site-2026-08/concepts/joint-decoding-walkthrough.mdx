...
[The knowledge ladder](../understanding/our-approach/the-knowledge-ladder.mdx) explains _why_ the v0.5.0 pipeline grew two new information layers (Stage 2.7 phrase grouper, expanded Stage 5 reconcile). This article walks through _what they do_ on one concrete input, end-to-end.
...
If you have not read [The staged pipeline](../understanding/our-approach/the-staged-pipeline.mdx), do that first — this article assumes you know the six stages by name.

## Stage 1 + 2 + 2.5 (warm-up)

Stage 1 normalizes the bytes. Nothing interesting happens here for our input — no NFC fix-ups, no abbreviation expansion. The string passes through unchanged.

Stage 2 is the locale check. For Latin-only input like this, the default is `en-US` with a moderate confidence. The downstream resolver weights will tilt toward US gazetteer hits.

Stage 2.5 is the kind classifier. It looks at the shape of the string (commas, capitalization, the trailing two-letter token after a final comma) and decides this is a `structured_address` — not a bare postcode rather than a single locality. The full pipeline runs.
...
Notice though that the `venue` interpretation is _not_ far behind. The classifier knows it could be a venue; itthinks region is slightly more likely _in isolation_. The top-k output preserves that information so Stage 5 can use it.

## Stage 4 — sequence corrector

Stage 4 is the CRF decoder. It enforces the BIO grammar (no orphan `I-*` without a preceding `B-*`) on the tag sequence. For our top-k input, the CRF passes the candidates through unchanged — there are no orphan BIO labels to fix in this example. (See [CRF decoder](./crf-decoder.mdx) for a case where Stage 4 _does_ change the output, e.g. `Saint Petersburg` → `Petersburg`.)

## Stage 5 — reconcile picks the joint-coherent interpretation

This is where the new joint decoder warrants its keep. Stage 5 receives:
...
This is what every Mailwoman version before v0.5.0 returned. Two regions in one address (`NY` and `TX`), one locality. The the system satisfy this and fails — it cannot find a `parent_id` chain where Houston is a locality in both New York and Texas. The output is incoherent.
...
The `concordance_bonus` is the new piece. For each candidate parse tree, Stage 5 looks up the `parent_id` chain in the Who's On First gazetteer (see [Resolver and WOF](./resolver-and-wof.mdx)) and asks: does the chain agree? Concretely:

- Interpretation X says `region: NY` and `locality: Houston`. WOF says `Houston → Texas` rather than `Houston → New York`. The chain disagrees. The bonus drops to ~0 (a near-fatal penalty).
...
The confidence is the product of the winning interpretation's per-stage scores. It is modest (0.42) because the underlying signals are modest — `Houston` only had 0.65 from the phrase grouper, the venue tag was a second-best from the classifier, etc. But the parse is _correct_, which is the result for v0.5.0.
...
This is the [knowledge-ladder](../understanding/our-approach/the-knowledge-ladder.mdx) point made concrete. Each stage knows its own kind of information; the joint decoder is what composes them. Removing the joint decoder is what makes geocoders fail on adversarial inputs. Adding it back is most of what v0.5.0 set out to do.

## See also

- [The knowledge ladder](../understanding/our-approach/the-knowledge-ladder.mdx) — the conceptual frame for why these layers exist
...
- [`reconcile-empty-parse-bonus.md`](./reconcile-empty-parse-bonus.mdx) — the multiplicative-score failure mode (link will resolve once Doc D lands)
