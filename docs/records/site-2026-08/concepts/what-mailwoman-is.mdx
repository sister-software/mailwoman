...
source-of-truth: engineering/SCOPE.mdx, engineering/reference/closed-vocab-fields-model-first.mdx, engineering/reference/ARCHITECTURE.mdx
...
project its name, its lineage, and the two failure modes it guards against.
It's the page to read when the component count starts to feel like complexity
instead of architecture — because the system is layered rather than tangled, and the
layers have a logic.
...
- **Sequence labeler.** The model is a small transformer encoder doing BIO
  ...
  no part of it is generative. For a closed label set over short strings,
  this is the most boring, battle-proven architecture in NLP. Boring is a
  feature.
- **Microlanguage.** Addresses are a language in the strict sense: a
...
- **Retrieval-augmented.** The model never memorizes the atlas. Postcode and
  ...
  ambiguous decisions, and no external signal overrides the learner. (We re-derived this
...
- **Calibrated.** The confidences are isotonic-calibrated against held-out
  data, so "0.6" means right-about-60%-of-the-time rather than "my rule was moderately
...
retrain. Asking a model to memorize the world's place names burns capacity it doesn't
have to spend; asking a database to understand "Friedrichstraße 43-45, 10117
Berlin-Mitte" written four different ways asks it to do what rule systems have
always failed at. Each side does the job the other is structurally bad at.

Spatial layers — `poi.db` is the first — extend the same discipline (sealed,
...
resolved against a database instead of guessed by a model that was never going
to memorize every coffee shop. The consumer here is frequently an agent rather than
a person typing into a search box, so mailwoman ships the layers and the tools
(`@mailwoman/mcp`) and leaves the planning and the narrative to whatever's
driving. The model still owns the grammar; the atlas gains layers.
...
systems call confidence is _rule specificity_: how tightly the template
matched rather than how likely the reading is to be correct. A sieve cannot say "I'm not sure." Novel
...
uncertainty becomes an actionable measurement: the resolver can
abstain on it, soft-score with it, or fall back to rules because of it. The
capability difference shows up exactly where the
[arena evals](https://github.com/sister-software/mailwoman/blob/main/docs/records/evals/competitive-parity/parity-scorecard-2026-06-09.md) say it does: on noisy,
degraded, real-user input — the queries a geocoder receives — the
...
says: no knowledge engineering _inside the learner_. The pre-model
...
or a React Native app. This started as a distribution decision — no step to
...
1. **It's why the iteration loop exists.** Small model → ~1-hour single-GPU
   retrains → single-variable change experiments with same-day checks. The
...
2. **It forced the correct architecture.** A model too small to memorize the
   ...
   masked those bugs rather than fixed them.

The cost: the _data_ doesn't fit the pocket even though the model
...
true because it's scoped rather than because we stopped checking. There is also
headroom: the model could grow severalfold before challenging a
conservative mobile budget — the constraint that binds is iteration
cost, and that's the one worth defending.

## Two ways to un-make it

Two failure modes can un-make this architecture, and both have names
in our history:
...
explain Mailwoman by analogy, that's why: no other system combines all three,
so there's no reference implementation to match it against.
