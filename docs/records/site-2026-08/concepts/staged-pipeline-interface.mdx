...
- [The staged pipeline](../understanding/our-approach/the-staged-pipeline.mdx) — the narrative for _why_ the stages exist
- [STAGES.md (reference)](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/STAGES.mdx) — the full per-stage interface with type definitions, error semantics, and edge cases
...
Every stage is **optional**. When you omit one, the coordinator either substitutes a no-op stub (`normalize`, `computeQueryShape`, `detectLocale`, `classifyKind`) or skips the stage entirely (`classifier`, `placeCountry`, `resolver`).

### Decode-time constraint layers (v4.3.0+)

The neural classifier carries two constraint layers configured at construction rather than as pipeline
stages — they run inside `classifier.parse` and are part of the SHIP CONFIG interface (the model
card's notes name the required settings per release):

- **Conventions mask** (`addressSystemConventions: "auto" | SystemCode`, v4.3.0+): the model's
  exported locale head detects the address system; the codex conventions row removes that
  system's ungrammatical tags from the Viterbi vocabulary and arms the postcode-shape snap
  repair. Threshold-conditional (0.8) — never fires on a guess; undetected systems parse unmasked.
- **Span bridge** (`bridgePunctuationGaps: true`, REQUIRED from v4.4.0): merges same-tag
  fragments split at unlabeled intra-token punctuation ("P.O. Box" decoding as `P`+`O`+`Box`) —
  the corpus label format cannot express punctuation inside spans, so the bridge is decode-side
  containment. Separator punctuation (commas) never bridges; space-only gaps never bridge.

Running a v4.4.0+ model without these settings reproduces measured check failures (po_box 60.4 vs
89.1 with the bridge; the FR digit-split class without conventions) — they are not optional
enhancements for those weights, they are the decode interface.
...
| Stage                  | If it throws                                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `normalize`            | Propagates. Input preprocessing is a interface; a crash is a bug.                                                                                  |
| `computeQueryShape`    | Propagates. Same rationale.                                                                                                                        |
| `detectLocale`         | Propagates. A locale detector that crashes signals a real fault rather than noise.                                                                 |
| `classifyKind`         | Propagates. Kind classification runs on the QueryShape wecomputed — there's nothing external that can poison it.                                   |
...
`PipelineOpts.signal: AbortSignal` is honored **between stages** rather than within them.
...
Use `mailwoman parse --benchmark <N>` for percentile breakdowns over several iterations against a real input.
...
The fast-path tree is built from QueryShape's format hit alone — useful even without a resolver wired (a consumer whowants the parsed structure for `"10118"` shouldn't pay for the classifier).
...
Expand the **Classification origin** section to see which pipeline stage contributed to each parsed component.

## See also

- [STAGES.md](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/STAGES.mdx) — the full per-stage type interface
- [The staged pipeline](../understanding/our-approach/the-staged-pipeline.mdx) — why these stages exist
- [QUERY_SHAPE.md](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/QUERY_SHAPE.mdx) — the structural-prior sub-system feeding stages 2 and 2.5
