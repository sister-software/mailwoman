...
[inverse-address-frequency](./spatial-expectation-and-density.mdx): down-weight a shared address by how many distinct
entities sit on it, so a lonely address is strong evidence of identity and a
crowded one is nearly none. It's on by default.

But rarity is only meaningful against a population. The change counts how often each
