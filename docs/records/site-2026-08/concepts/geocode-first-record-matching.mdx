...
This property addresses why the first attempt failed. Messy clinic CSVs come with **no ground truth**, so a supervised matcher has nothing to learn from at the start. Fellegi-Sunter still produces a _calibrated_ matcher, and its scores are interpretable. A total match weight is a prior plus a sum of per-field log-Bayes-factors (`M = M_prior + Σ log₂(mᵢ/uᵢ)`), so each decision can be traced to the fields that drove it. The match-weight threshold is the single precision/recall setting. A clerical-review band between the thresholds holds the ambiguous pairs, and the matcher abstains on them, as the geocoder does when its radius grows too large.
