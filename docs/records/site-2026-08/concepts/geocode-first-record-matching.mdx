...
Mailwoman started as the answer to a record-matching problem rather than a parsing one.
...
Every clause warrants its place:
...
and they are **650 meters apart** (Isaj et al., SSTD 2019). String similarity has
...
this two-axis view omits. Generator:
...
Every record can become a rooftop coordinate (± calibrated meters) plus a
...
- **the WOF ancestry** — spelling-invariant blocking by locality and region,
  because you're keying on resolved IDs rather than text;
- **structured components with confidence** — you compare field-by-field, and you
  _know which fields to trust_.

You never match on the formatted string. You match on the resolved place, and
the lossy-template problem disappears with the string it lived in.
...
3. **Cluster** — pairwise scores are _not transitive_ (A~B at 0.6 and B~C at 0.6
   ...
   entities. Skip it and your "entities" without output fracture or fuse (Dedupe; the
   non-transitivity is essential rather than a footnote).
...
2. **Geocode quality weights the evidence.** Two records sharing a _rooftop_ point
   ...
   PO-box/rural-route flags, a Match/Review/Non-Match triage). It maps directly onto
   our situs → interp → admin tier cascade and conformal radii. We have the quality
   class already; the matcherhas to spend it.

Calibrate the thresholds against mailwoman's _own_ measured error distribution rather than the literature's older-geocoder constants. Our rooftop tier is tighter; the
shape of the measured pattern remains supported, the numbers are ours to measure.

## Where a model warrants its place

The classical core is the v1. Models are selective changes bolted on where the
...
- **Org-name embeddings** for fuzzy blocking and similarity. Regex can't see
  ...
  embedding + ANN index can. Embedding/ANN blocking warrants its keep _specifically_
...
- **A gradient-boosted matcher** over the comparison vector once labels exist. A
  ...
  in production (Grab) — a strong, interpretable upgrade from hand-weights rather than a
  ...
  states it never trained on (TX→CA, TX→NY), reducing the co-located over-merge — so
  it warrants the default slot, shipped with a calibrated link threshold (its logit
  ...
  where recall rather than over-merge precision, is what matters. Same model, opposite
  objective: pick the scorer by the question you're asking.
- **An LLM in the gray zone, never on the easy 95%.** Framed as in-context
  _clustering_ rather than all-pairs classification, an LLM reduces API calls ~5× and
...
- **An LLM for ingest.** The messy-data reality — "it came as a SQLite dump" — is
  ...
  samples, infer the column→field mapping, cache it. One call per _source schema_ rather than per row. The most painful onboarding step, deleted.
...
- **Calibrate, then abstain.** A link is a probability; the ambiguous middle goes
  to review rather than to a coin flip.
- **Grade the pipeline against truth, never a stage in isolation.** The reconcile
  ...
  entities versus reality rather than on a scorer's pairwise F1.
...
providers who share a hospital campus. The matcher's strongest precision change is
[inverse-address-frequency](./spatial-expectation-and-density.mdx): down-weight a shared address by how several distinct
entities sit on it, so a lonely address is strong evidence of identity and a
crowded one is nearly none. It's on by default.

But rarity is only meaningful against a population. The change counts how frequently each
address appears across the records you hand it — so when you dedupe a _whole_
dataset, the input is the corpus and it works. Hand it a thin sample and
there's nothing to be rare against: every address looks unique, the signal
flattens, and the change degrades to baseline with no warning. The fix is cheap — the frequency
table is a parse-free string scan, no geocoding — so when you're matching a subset,
...
- [Record-matcher sources (reference)](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/record-matcher-sources.md) — the machine-readable source registry the catalog narrates
