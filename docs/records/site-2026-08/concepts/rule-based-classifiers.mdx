...
Probably the simplest classifier. Postcodes are a regex problem rather than a machine learning problem.
...
- **Confidence is moderate** (0.6) because dictionary hits can be misleading. "New" is the start of several city names, so a 1-token match on "New" is weaker than a 3-token match on "New York City".
...
- **No data required.** A rule classifier ships in code. The dictionary classifiers ship a few MB of dictionaries rather than a multi-GB model.
...
This is the Ship-of-Theseus pattern. The neural classifier warrants each component one at a time. If a neural model regresses, flipping back to `rule_only` is one line of config.
