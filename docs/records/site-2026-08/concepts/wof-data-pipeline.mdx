...
1. **Name aliasing**: "Saint Petersburg" not found via bare `name` lookup — [a known WOF gotcha](./whosonfirst-gotchas.mdx) — WOF stores it as "St. Petersburg". The `variant`/`short` columns in `PlacetypeDataSource` are designed for exactly this; they just need to be populated.
