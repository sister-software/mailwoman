...
- **Want the exact tag union the model emits?** [The component schema reference](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/SCHEMA.mdx) is the single authoritative record.

One correction, since old copies of the outdated claim are still linked from elsewhere: Mailwoman trains **one** model on a combined corpus rather than a separate model per locale. The `@mailwoman/neural-weights-en-us` and `@mailwoman/neural-weights-fr-fr` packages currently ship the identical binary — the publish workflow copies the en-us artifacts into the fr-fr package. Locale-specific weights are future work rather than something shipped today.
