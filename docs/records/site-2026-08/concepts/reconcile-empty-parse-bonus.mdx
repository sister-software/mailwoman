...
First implementation, all 14 interface tests passed. Then every kryptonite fixture failed with `expected … to be defined`: the winning beam was **empty**. The reconciler returned no parse at all, notan incorrect one.
...
### Why per word rather than per slot
...
1. **Fragments beat coverage.** The per-token logit aggregation gives interior fragments inflated confidence — `City` aggregates the I-locality mass of a token that sits _inside_ a locality, scoring `locality=0.92`, while the full `New York City` span scores `0.60`. With a flat per-slot bonus, the beam paid no penalty for leaving `New York` unexplained: the bare `locality="City"` won at `+0.475` over the full span's `+0.199`, and the grouper-audit then promoted the orphaned `York` proposal to a spurious `region` node. Final output: `{region: "York", locality: "City"}` — for an input the plain argmax path parses correctly.
...
Scaling the bonus by the slot's word count makes _coverage_ the thing being rewarded. Equal-coverage interpretations collect equal bonus (the evidence factors alone decide between them), and a covering interpretation out-scores one that silently drops words unless the extra evidence is weak. On the query above, the full span now wins at `+2.03` (three words' bonus) vs `+0.48` for the bare fragment.
...
`INCLUSION_LOG_BONUS` is a **module-level constant** rather than a configurable option. Reasons:
...
- The sensible range is narrow. Below `log(2)` (≈ 0.693) the empty parse re-dominates on most inputs; above `log(4)` (≈ 1.386) the search starts accepting noise spans. Operators who want to tune this should be reading the reconciler's tests rather than the public API.
...
The right semantic is **per-start-position**: top-k overlapping proposals at each anchor rather than a global cap across the whole input. Phrase proposals at offset 0 and offset 18 are not competing for the same slot; pruning them together throws away independent information.

This is the same shape of bug — a natural-reading parameter (`kSpan = 3`) that silently violates the interface requirement — and the same fix (anchor by structure rather than by ranking).
...
- The v0.5.1 learned span proposer ([`PHASE_8_E_learned_span_proposer.md`](https://github.com/sister-software/mailwoman/blob/main/docs/records/plan/phases/PHASE_8_E_learned_span_proposer.mdx)) will produce its own confidence distribution. If it feeds into a search with multiplicative scoring, the inclusion bonus needs to be applied again — adding a layer of factors does not change the trap.
...
- [v0.4.0 ablation campaign](https://github.com/sister-software/mailwoman/blob/main/docs/records/retrospectives/v0-4-0-ablation-campaign.mdx) — the campaign whose mixed results made the missing reconcile rung visible
