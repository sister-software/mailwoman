...
- **Layer 1** is the polish-pass fix for v0.6.1's `dependent_locality` hallucinations on US-pattern addresses. ~2h to build. Solves the "model has no signal telling it `Avenue` is street-typing" problem. Conservative estimate: catches 600-800 of the 1066 dep_locality hallucinations from v0.6.1.
