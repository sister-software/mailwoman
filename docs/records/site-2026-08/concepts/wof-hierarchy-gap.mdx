...
## What WOF's placetype graph looks like

WOF ships 35 placetypes. The graph is a DAG (most placetypes have multiple parents), but projected from `planet` outward, the relevant chain is:
...
**There is no `street` node anywhere in this graph** — not as a child of locality, a sibling of neighborhood, or a parent of address. The WOF DAG only does not represent streets.

## Why WOF skips streets

The omission is principled rather than an oversight:

1. **Streets are linear rather than point or polygonal.** WOF stores geometries. A locality has a polygon, an address has a point. A street is a line — closer to OSM ways than to WOF places.
...
WOF's decision to stop at `neighbourhood → address` is correct _for a place gazetteer_. Itleaves a gap _for an address parser_.
...
flowchart TB
    ...
    LAYER2 -.per-metro extract routing, FST Phase 2+.-> COVERAGE2["Closes the identity vacuum"]
...
- **Layer 2** is the v0.7+ architectural change. It's the FST roadmap's [Phase 2](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/FST_GAZETTEER_LM.mdx) — "Streets would add ~3-5M unique names and require metro-area extract routing for the browser." First POC is NYC-only.
...
- [FST gazetteer LM reference](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/FST_GAZETTEER_LM.mdx) — Phase 2 (streets) is the layer 2 work
...
- [v0.6.1 error analysis](https://github.com/sister-software/mailwoman/blob/main/docs/records/evals/model-versions/v0.6.1-error-analysis.mdx) — the empirical regression that surfaced the gap
