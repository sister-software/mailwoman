...
The corpus draws from several open data sources. Each one contributes a different kind of signal:
...
| **HRSA-FQHC**                    | 19K                       | US federally-qualified health centers                                        |
...
| **state-ia-builders**            | 17K                       | Iowa contractor names + addresses                                            |

Skipped in v0.3.0:

- **openaddresses** — waiting on per-row license filter (PR #56).
...
The diversity of sources matters more than the absolute count. NPPES contributes 9.2M rows but it is the single largest source of `venue + address` co-occurrence. Without it, the model would not have learned that a venue precedes a street more frequently than it follows one.

## Stage 1 — Per-adapter row emission

Each adapter is a TypeScript async generator that yields `CanonicalRow` objects. The interface is straightforward:
...
Real-world addresses appear in several surface forms — abbreviated, capitalized, punctuated, separated. To make the model resilient to these variations, Mailwoman runs a **synthesis** pass over the aligned rows.
...
- **Whitespace normalization** (collapse multiple spaces, add tab separators)
...
Final output is Parquet files with about 1 million rows per extract. Parquet is columnar — the training data loader streams one column group at a time without loading the whole file. Total v0.3.0 output: ~30 train extracts + 1 val + 1 test = 32 files, ~30 GB.
...
- **Wall-clock cost scales with synthesis.** The 5x synthesis multiplier doubles the alignment + extract routing time vs. a synth-off build. v0.3.0 added more sources without bumping max_steps, so the corpus is now under-utilized: a 50K-step training run sees only 6.4 million examples (~1% of the corpus per pass).
...
- **Extracter:** `corpus/src/parquet.ts`
...
- **Plan doc:** [PHASE_1_corpus.md](https://github.com/sister-software/mailwoman/blob/main/docs/records/plan/phases/PHASE_1_corpus.mdx)
