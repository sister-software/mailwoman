...
When a place is missing from the tiers, the fog, and the panels, that absence is the answer: nothing has measured it, so Mailwoman makes no claim about it. The tier table moves when new data and new evals land, and the scope declaration is where it moves first.
