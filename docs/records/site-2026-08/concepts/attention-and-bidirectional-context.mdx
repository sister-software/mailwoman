...
[How the model reasons](./how-the-model-reasons.mdx):
...
positions both before and after itself), notearlier ones. There's no
causal mask — this is a BERT-style encoder rather than GPT-style.
...
representation captures more than a generic "this could be a locality"
guess — it represents the specific structural signal that the following
tokens have the shape of a US state abbreviation + ZIP code, which
strongly supports the locality interpretation.
...
grammar — but expressed as continuous-valued weights rather than categorical
...
docs (see
[corpus-poisoning-vulnerability.md](./corpus-poisoning-vulnerability.mdx))
...
- [How the model reasons](./how-the-model-reasons.mdx) — the central pipeline
  doc this article expands
- [FST priors as shallow fusion](./fst-priors-as-shallow-fusion.mdx) — what
  external world knowledge gets added to the encoder's emissions
- [Viterbi and BIO validity](./viterbi-and-bio-validity.mdx) — what happens
  AFTER the encoder's per-token emissions are computed
- [Corpus poisoning vulnerability](./corpus-poisoning-vulnerability.mdx) — the
