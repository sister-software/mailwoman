...
([see those sub-articles](./how-the-model-reasons.mdx)), the last step turns
...
The piece is small but critical. It's where the schema's parent/child
...
- `region` takes no parent (root in this address)
...
first listed tag that has a labeled span in this address.
...
## What the projection provides you
...
the tree rather than in per-tag recall. A tree validator (planned rather than yet
shipped — see
[corpus-poisoning-vulnerability.md](./corpus-poisoning-vulnerability.mdx)'s
deferred items) would check the AddressTree's structural validity as a
final quality check.

### It's the schema's authoritative record
...
treats this as the schema's authoritative record — extending it (e.g. adding
...
The
[WOF hierarchy gap](./wof-hierarchy-gap.mdx) doc explores how WOF's
...
- **Cross-address linkage.** The tree is per-address. If you parse a
  ...
  locality, that's a resolver concern rather than a decoder one.
...
- [Viterbi and BIO validity](./viterbi-and-bio-validity.mdx) — what
  produces the BIO sequence this article consumes
- [WOF hierarchy gap](./wof-hierarchy-gap.mdx) — the relationship between
