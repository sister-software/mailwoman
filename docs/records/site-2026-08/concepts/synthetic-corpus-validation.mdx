---
sidebar_position: 18
title: Synthetic corpus — alignment validation is essential
tags:
  - concepts
  - corpus
  - training
...
Both v0.5.0 corpus threads (B kryptonite and B2 transliteration) used an LLM ([DeepSeek](./deepseek-reasoning-budget.mdx)) to generate annotated training rows. Both surfaced the same lesson: **the substring-match alignment check is structural infrastructure rather than a quality filter you can drop later.** This article explains why.
...
The implementation is unglamorous (`generate_deepseek_corpus.py::align_or_reject` if you want to read it) but the interface it enforces is what makes the corpus trustworthy.

## What goes wrong without it

LLMs hallucinate around component boundaries. The B and B2 reject logs catalogue the common failure modes:

- **Component embedded in another component**. `house_number=350, street=350 5th Avenue` — the street contains the house number. A naïve trainer would see `350` as both a house number and as the first token of a street, and learn an incoherent BIO labeling.
...
For LLM-generated corpora, alignment problems are inherent to the generation process. The model is producing both the surface and the annotation in the same forward pass; no constraint forces internal consistency. Even with temperature=0 and `reasoning_effort=low`, the 1–2% rate persists.

This shifts the validator from "quality filter" to "trust boundary" — the difference between training on data the LLM only thinks it generated correctly, and training on data that's provably consistent with its own annotation.
...
2. **A reject-rate floor expectation** in the pipeline's README. If you see reject rate drop to zero, that is a bug in the validator rather than a quality breakthrough.
3. **A `corpus-audit` pass before declaring done** — the validator is necessary but not sufficient.

The cost is low (a few hundred lines of Python); the value is high (you can trust that the corpus matches its own annotations).

## See also

- [`CORPUS_V0_4_0_GENERATION.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/CORPUS_V0_4_0_GENERATION.mdx) — the operational record for the v0.5.0 corpus generation, including the actual prompts used
- [v0.5.0 — as shipped](https://github.com/sister-software/mailwoman/blob/main/docs/records/plan/v0-5-0-shipped.mdx) — context for where corpus-v0.4.0 fits
- [The knowledge ladder](../understanding/our-approach/the-knowledge-ladder.mdx) — why we validate generation at this layer rather than trying to fix it downstream
