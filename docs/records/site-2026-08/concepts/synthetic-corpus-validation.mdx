...
Both v0.5.0 corpus threads (B kryptonite and B2 transliteration) used an LLM ([DeepSeek](./deepseek-reasoning-budget.mdx)) to generate annotated training rows. Both surfaced the same lesson: **the substring-match alignment check is structural infrastructure, not a quality filter you can drop later.** This article explains why.
