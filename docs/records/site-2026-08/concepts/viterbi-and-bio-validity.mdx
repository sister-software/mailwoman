...
If youtake per-token argmax, you get whatever each token's
...
Viterbi searches for the **highest-scoring label SEQUENCE** rather than
per-token argmaxes:
...
always followed by `postcode` (`MA 02101`) rather than another locality. The
...
refinement on top of that rather than a replacement for it.
...
[fp32-CRF diagnostic](https://github.com/sister-software/mailwoman/blob/main/docs/records/evals/model-versions/2026-05-28-fp32-crf-diagnostic.mdx).
...
active plan to revisit it — the structural mask alone already provides the
concrete win (no orphan-I), and the encoder's attention covers most of
the rest.

## What a learned CRF would provide (if one ever ships)
...
- `B-postcode` rarely precedes anything (it's typically the last
...
- Better encoder representations. The encoder is the upstream learner;
  a CRF layer is downstream of it rather than a substitute for it.
