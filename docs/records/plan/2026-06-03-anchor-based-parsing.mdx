...
The v0.8.0 German order-shard experiment (postmortem)
...
5. **Global.** Script-shard collection + the coarse-placer tier; the generalized place-anchor gazetteer
   for distinctive streets / landmarks; subdivide a shard by address-system only if its 20k regression check
   refuses to clear.

## Related

- German order-shard postmortem — the failure that motivated
  this.
