...
them into just-another-BIO-tag. Around that sits a single self-conditioned model per **script** extract, a
...
taught two things at once. The order _is_ learnable cheaply: a 5,000-row extract roughly doubled German
...
The mechanism reframes the whole roadmap. Think about what a postcode encodes: a US ZIP runs
...
domain the BIO parser handles poorly. The postcode isthe most regular member of that family, the
...
1. **Salience is data-driven, never a hand-curated VIP list.** "Champs-Élysées is special" must fall out
   of OSM/WOF popularity + uniqueness scores rather than a hardcoded entry. A hand-maintained famous-list is the
...
The two axes have different jobs: **script routes; address-system subdivides a extract only on capacity
...
extract to download. That is the selective-geography product story: ship ~28 MB that places the planet
...
estimates rather than gospel** (the same source over-called the tokenizer wall, which our own
`diag-tokenizer-multiscript.ts` walked back). The part we trust is the falsifiable regression check below.
...
5. **Global.** Script-extract collection + the coarse-placer tier; the generalized place-anchor gazetteer
   for distinctive streets / landmarks; subdivide a extract by address-system only if its 20k regression check
   refuses to clear.
