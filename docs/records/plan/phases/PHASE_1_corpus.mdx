...
**Goal:** build a reproducible, versioned dataset pipeline. End state is `corpus-v0.1.0` on disk, containing aligned BIO-labeled Parquet extracts for US + FR, with a hand-labeled golden eval set.
...
- [ ] Create `packages/corpus-python/` (Python rather than in workspaces — has its own `pyproject.toml`)
- [ ] `packages/corpus/lib/types.ts` — `CanonicalRow`, `LabeledRow`, `CorpusAdapter`, `AdapterOptions` per `reference/INTERFACES.md`
- [ ] Add to root tsconfig references, root package.json scripts

### 2. Adapter framework

- [ ] `packages/corpus/lib/adapter.ts` — base `CorpusAdapter` interface, helper utilities (streaming, checksum, dedup)
- [ ] `packages/corpus/lib/runner.ts` — drives an adapter, writes intermediate JSONL, reports progress, handles backpressure
...
- [ ] `packages/corpus/lib/format.ts` exposes `formatAddress(components: ComponentDict, country: string): string`
...
The tokenizer is trained on the corpus rather than picked off the shelf.
...
- [ ] `packages/corpus/lib/synthesize.ts`
...
- [ ] `packages/corpus/lib/parquet.ts` — write labeled rows to Parquet extracts, ~1M rows per extract
...
- [ ] `packages/corpus/lib/split.ts`
- [ ] Split strategy: **hold out by locality** rather than random row sampling
...
- [ ] Check into git. This is the interface for "what good looks like."
...
- [ ] `corpus-v0.1.0/` exists on disk with Parquet extracts
- [ ] MANIFEST.json has checksums for every extract
...
When you can run `npx mailwoman corpus build --version 0.1.0` from a clean state, get a complete corpus on disk, and run a small Python script that loads an extract and prints `(tokens, labels)` pairs that look correct.
