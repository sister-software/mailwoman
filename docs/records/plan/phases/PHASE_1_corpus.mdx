...
When you can run `npx mailwoman corpus build --version 0.1.0` from a clean state, get a complete corpus on disk, and run a small Python script that loads a extract and prints `(tokens, labels)` pairs that look obviously correct.
