...
- [ ] `onnxruntime-node` installs directly in the dev environment
...
- [ ] Lazy loading: model loads on first `infer` call rather than on construction (unless `warmup: true`)
...
- [ ] If anything regresses, revert the policy change and investigate. The migration is blocked on metrics rather than on intent.
...
- [ ] Golden set: country F1 improved (or held), no metric regressed
