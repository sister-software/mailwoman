...
- **v0.1.0** (shipped 2026-05-18, PR #42) — first Tier 1 ship. F1 below targets (~0.03 macro) due to positional-heuristic overfit; calibration 0.337 in conf>0.9 bucket. direct below-target ship with `v0.2.0 retrain recipe` in session-notes.
...
- **v0.4.0** (shipped to packaged artifacts 2026-05-23 rather than npm-published — issue [#116](https://github.com/sister-software/mailwoman/issues/116)) — six planned work areas: (1) per-token CRF NLL normalization; (2) longer training (reach step-5000+ before judging); (3) class-weighted CE biased toward coarse classes; (4) source-weight rebalance away from NAD-heavy fine-label mix; (5) JS-side Viterbi decode + model-card label loading (landed 2026-05-22, see entry above); (6) reuse `corpus-v0.3.0`. **Shipped recipe is §4 + §5 only**; §1 and §3 deferred to v0.4.1 after destabilizing the dual-loss training.
  ...
  **Ablation campaign (2026-05-23, 5 divergence runs + 3 verdict smokes + 1 full 50K + 1 successful full run):**
  1. The full §1+§3+§4 recipe diverged at **all three tested LRs** (lr=5e-4 step 750, lr=3e-4 step 1000, lr=1.5e-4 step 2000). LR delays divergence proportionally — destabilizer is in the recipe rather than LR.
  ...
  5. **Math sanity-check** of `model.py` + `crf.py` found no implementation bug — `per_token` reduction is `nll.sum() / total_tokens.clamp(min=1)`; class_weights enter via `cross_entropy(weight=...)`. Destabilization is a real recipe interaction rather than a coding artifact.
  ...
  **Post-hoc regression diagnostic (categorized, 4535 entries, 1217 postcode FNs + 194 country FNs):**
  ...
  - Decoder span-trim sidecar (commit `c72ab4c`, no retrain required) addresses the 6% bio_slip class + an unmeasured share of FP rate. Material impact on the headline numbers is smaller than the initial diagnostic suggested but the trim is still a correct decoder bug fix.
...
- **v0.5.0 thread C-s — scaffold only** (shipped 2026-05-23, see [Phase 8 fresh-slate plan § Thread C](./PHASE_8_v0_5_0_fresh_slate.mdx)) — v0.4.1 was [explicitly skipped](./PHASE_8_v0_5_0_fresh_slate.mdx) in favor of a fresh-slate iteration. Thread C-s lands the **model code path** for the v0.5.0 architecture changes — _no training run_ as part of this ship; the training kicks off after Threads A (tokenizer retrain) and B (corpus-v0.4.0 with adversarial expansion) complete.
  ...
  - **Hidden-size knob kept at v0.3.0/v0.4.0 baseline (256).** The plan doc mentions a 256 → 384 or 512 bump "likely paid for by rented GPU." For the scaffold, hold the bump — validate the new architecture trains directly at the current size first; bump becomes an orthogonal `v0_5_0-classifier-large.yaml` follow-up once the baseline lands clean.
  ...
  - **Recipe parked:** `configs/v0_5_0-classifier-smoke.yaml` defines the short verdict-smoke (constant-LR per Thread F's [VERDICT_SMOKES.md](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/VERDICT_SMOKES.mdx), `--smoke-mode constant`). Invocable once Threads A + B + F have all landed. not executed as part of this PR.

  Forward-pass smoke (`tests/mailwoman_train/test_v0_5_0_forward_pass.py`, 14 cases) verifies wiring on stub batches in seconds — no loss.backward, no optimizer step. It is the integration check that decides whether the new architecture is ready for verdict-smoke training rather than whether it converges.
...
- **Each iteration ships an artifact**, target or no target. Below-target ships are OK if they're direct about it (model card + eval ledger entry) and the Ship-of-Theseus coexistence model means the rule classifiers still run alongside.
