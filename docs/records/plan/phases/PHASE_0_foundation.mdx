...
**Goal:** lock the interfaces (schema, classifier interface, policy, locale) before writing any model code. Refactor existing rule classifiers to use the new interface with zero behavior change.
...
**Success:** schema is the single authoritative record, importable as `import { ComponentTag } from '@mailwoman/core'`.
...
Before declaring Phase 0 done, verify the design handles Phase 6 (Japan) directly:
...
- [ ] Does any core code break? It shouldn't. If it does, the abstraction is wrong — fix it now rather than in Phase 6.
...
When you can demonstrate: "the system runs identically to before, but every classifier output flows through a typed, locale-aware, policy-conditional pipeline ready for a neural classifier to plug into."
