...
**Goal:** ship two new workspaces, `@mailwoman/normalize` and `@mailwoman/query-shape`, that implement runtime-pipeline stages 1 + the QueryShape boundary per [`STAGES.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/STAGES.mdx) and [`QUERY_SHAPE.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/QUERY_SHAPE.mdx). Both use pure functions without ML and add no runtime dependencies beyond `@mailwoman/core`.

**Branch:** ad-hoc on `main`; ship via incremental commits.

**Depends on:** [`STAGES.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/STAGES.mdx) (the stage interface) and [`QUERY_SHAPE.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/QUERY_SHAPE.mdx) (the QueryShape sub-system design).

**Why these two together:** QueryShape consumes `NormalizedInput.normalized` and produces structural priors that downstream stages thread through. Building them in the same release tests the boundary between them before it becomes hard to change.

## Order of operations

Per STAGES.md "What's next" suggestion: **QueryShape first**, then Normalize. Reasons:

- QueryShape can run on raw input directly (treating raw as already-normalized) for a v0, which helps early integration.
- QueryShape's interface is smaller, so there is less to get wrong on the first pass.
- Normalize's offsetMap is essential, and building it second lets us see QueryShape's actual access patterns before committing to the offsetMap shape.

## Tasks

### Increment A — `@mailwoman/query-shape`
...
#### 4. Tokenization — internal rather than exported
...
- [ ] One commit, one push. Do not publish to npm; the package is internal until Increment B lands.

### Increment B — `@mailwoman/normalize`
...
- [ ] Do not build the `runPipeline` coordinator yet, because it is a separate phase
...
…and the lint/test/compile checks are all green.
