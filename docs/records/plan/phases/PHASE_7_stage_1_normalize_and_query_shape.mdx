...
**Goal:** ship two new workspaces, `@mailwoman/normalize` and `@mailwoman/query-shape`, that implement runtime-pipeline stages 1 + the QueryShape boundary per [`STAGES.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/STAGES.mdx) and [`QUERY_SHAPE.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/QUERY_SHAPE.mdx). Pure functions, no ML, no new runtime dependencies beyond `@mailwoman/core`.

**Branch:** ad-hoc on `main`; ship via incremental commits.

**Depends on:** [`STAGES.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/STAGES.mdx) (the interface interface) and [`QUERY_SHAPE.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/QUERY_SHAPE.mdx) (the QueryShape sub-system design).
...
- Normalize's offsetMap is essential; building it second lets us see QueryShape's actual access patterns before committing to the offsetMap shape.
...
- [ ] One commit, one push. No npm publish — internal use only until Increment B lands.
