...
**Goal:** retire the inherited debt from v0.1.0 by rebuilding tokenizer + model + corpus + pipeline architecture together, in one coordinated iteration. This release pays one large cost to remove several structural limits at once, rather than working around them across incremental releases.
...
**Depends on:** v0.4.0 shipped (done 2026-05-23) and the issue #116 retrospective written. v0.4.1 is deliberately skipped: the operator decided on 2026-05-23 to skip the incremental warm-start in favor of fresh-slate.

**Language:** Python for training (`packages/corpus-python/`), TypeScript for runtime pipeline + new stages.

## Why fresh-slate

v0.4.0 shipped a `§4-only` recipe after the full §1+§3+§4 destabilized at every tested learning rate. The campaign's main finding concerned more than §1 or §3. The training-side improvements we were adding to v0.3.0 weights ran into architectural constraints inherited from v0.1.0:

- **Tokenizer locked at v0.1.0:** byte-fallback on non-Latin scripts drives 92% of country FN and 18% of postcode FN. No model-side fix is possible without retraining the tokenizer, which forces a fresh model train.
- **Single classifier head doing both boundary discovery and type classification:** BIO labeling couples these two problems, and v0.4.0's bio_slip class (6% of postcode FN) is the symptom. A phrase-grouping layer (new Stage 2.7) decouples them, but its outputs need to flow into the classifier's conditioning, which requires a classifier retrained to use them.
- **Stage 5 reconcile is structural-only today:** it sorts spans and attaches them via PARENT_OF. Real concordance matching (`NY-NY Steakhouse, Houston, TX`, `Paris, Texas`, `Saint Petersburg`) needs top-k from the classifier, top-k from the resolver, and concordance scoring. The classifier therefore needs to emit top-k sequences by design.

Each of these three changes would require either a fresh train or a substantial runtime rewrite. Doing them together costs roughly the same as the most expensive one alone and delivers much more.

## Scope decomposition

The plan has six work areas, each with its own thread. Cross-thread dependencies are noted.
...
- **Specifics:**
  - Target: < 5% byte-fallback on a balanced multi-script eval set
  - Hand-crafted "must keep whole" rules for known postcode formats (5-digit ZIP, UK postcodes, JP `100-0005`, etc.), using sentencepiece user-defined symbols
...
- **Why:** golden v0.1.2 evaluates against adversarial transliterations the model was never trained on. Either we close the train/eval gap, or we treat the eval as out-of-distribution and weight the regression denominator accordingly.
- **Specifics:**
...
- **Output:** `corpus-v0.4.0` (added to `corpus-v0.3.0` rows rather than replacing them). The change only adds adapters, so existing weights remain valid.
- **Blocks:** Threads A, C.
- **Blocked by:** nothing. It can start immediately.
...
- **Why:** unblocks Stage 5 concordance work. The model becomes a candidate generator instead of a single-answer predictor, which matches the architecture's interface.
- **Specifics:**
  ...
  - Use the lessons from v0.4.0: verdict smokes with constant-LR or long max_steps (not cosine-decay). Make one change at a time within this fresh-slate release, and do not combine §1 per-token CRF norm, §3 class weights, and phrase priors in a single run. The phrase priors are the headline change; everything else stays as-close-to-v0.3.0 as possible.
...
- **Why:** closes the kryptonite catalogue. Currently no layer checks whether a parse is internally consistent, and reconcile is meant to be that layer.
- **Specifics:**
...
- **Output:** a runtime change without model retraining. It ships in `@mailwoman/core` as part of the v0.5.0 npm package family.
- **Blocks:** none.
...
- **What:** new pipeline stage between kind classifier and neural classifier. Proposes coherent input units with confidence scores. Ships in two flavors:
  - **Rule-based** first (port of v1's section/sub-section logic): proximity, punctuation, capitalization, hyphenation. It is deterministic and fast, and it needs no training.
  - **Learned** later (small 1-2M param span proposer trained on segmentation labels derived from corpus). It tests whether learned generalization beats rules.
- **Why:** decouples boundary discovery from type classification, which addresses v0.4.0's bio_slip class at the source rather than through a decoder post-trim. It feeds both Stage 3 (as input conditioning) and Stage 5 (as span candidates).
- **Specifics:**
  - New workspace `@mailwoman/phrase-grouper/` alongside `@mailwoman/locale-hint` + `@mailwoman/kind-classifier`
...
- **Blocked by:** nothing. The rule-based version can start immediately.
...
- **What:** carry the v0.4.0 sidecars + diagnostic tools into the v0.5.0 process from day one.
  - `corpus-audit` is already in tree (`corpus/scripts/audit.ts`) and runs directly against `corpus-v0.3.0`. Use it to verify Thread B's corpus mix before tokenizer training and before classifier training.
  - `diagnose_regression.py` is already in tree (`corpus-python/scripts/diagnose_regression.py`). Use it for v0.5.0 eval bucketing during the run rather than after the fact.
  - The verdict-smoke framework is redesigned: **constant LR for the smoke window**, or `max_steps >= 10000` so the cosine tail does not dominate. Documented in [`VERDICT_SMOKES.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/VERDICT_SMOKES.mdx). Code enforcement: `--smoke-mode constant|long-tail` on `python -m mailwoman_train train` and `smoke` subcommands; defaults to constant for end-to-end smokes.
  - Decoder span-trim sidecar (commit `c72ab4c`, `core/decoder/build-tree.ts:58`) stays in main and covers the long tail of bio_slip cases the phrase grouper might miss.
- **Why:** v0.4.0's process meta-bug (cosine LR masking divergence) cost real iteration cycles. Documenting it keeps v0.5.0 from repeating it.
- **Output:** new [`VERDICT_SMOKES.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/VERDICT_SMOKES.mdx) + `--smoke-mode` CLI flag + updated TODO.md.
...
These metrics differ from v0.4.0's "≥2 of 4 axes" frame because v0.5.0 changes the architecture. Per axis:

- **Coarse F1 (country / region / locality)**: recover to ≥ v0.3.0 baseline (country ≥ 0.28, region ≥ 0.18, locality ≥ 0.27) on the non-adversarial subset of golden v0.1.2. Stretch: improve via better phrase boundaries.
- **Fine F1 (street / house_number / venue)**: hold v0.4.0's small wins (street ≥ 0.30, house_number ≥ 0.78, venue ≥ 0.39).
- **Non-Latin adversarial set (new eval split)**: country F1 ≥ 0.50 (vs current ~0, because these are mostly byte-fallback empty preds). This result is measured directly.
...
- [ ] `mailwoman corpus-audit` runs directly against the planned corpus-v0.4.0 mix
...
- A (tokenizer retrain): 1-2 days once the corpus is ready, because Sentencepiece training is fast.
- E rule-based (phrase grouper): 2-3 days, mostly fixture-writing + tuning.
- C (classifier retrain): 3-5 days. A single full 50K run on a rented H100 should converge with the v0.4.0 process improvements in place (verdict-smoke + full-train).
- D (Stage 5 reconcile): 5-7 days, mostly correctness work on the kryptonite catalogue.
- E learned (phrase grouper, if pursued): 3-5 days. This is a stretch goal that can defer to v0.5.1.
...
- 2026-05-23: Operator chose fresh-slate (v0.5.0 fork) over incremental (v0.4.1 warm-start) after the v0.4.0 mixed-result postmortem. Rationale: bundling the tokenizer retrain, phrase grouper, and reconcile expansion costs one big iteration instead of three medium ones, and the resulting architecture has a meaningfully higher ceiling. Rented GPU + DeepSeek-generated adversarial corpus make the previously-prohibitive parts tractable.
...
- [The knowledge ladder](../../site-2026-08/understanding/our-approach/the-knowledge-ladder.mdx) — conceptual framing for what the missing rungs do
- [The pipeline interface](../../site-2026-08/concepts/staged-pipeline-interface.mdx) — runtime mechanics
- [v0.4.0 ablation campaign retrospective](../../retrospectives/v0-4-0-ablation-campaign.mdx) — what made the fresh-slate decision necessary
