...
- **Single classifier head doing both boundary discovery and type classification** — BIO labeling couples these two problems; v0.4.0's bio_slip class (6% of postcode FN) is the symptom. A phrase-grouping layer (new Stage 2.7) decouples them, but its outputs need to flow into the classifier's conditioning — which requires a classifier retrained to use them.
- **Stage 5 reconcile is structural-only today** — sorts spans, attaches via PARENT_OF. To do real concordance matching (`NY-NY Steakhouse, Houston, TX`, `Paris, Texas`, `Saint Petersburg`) it needs top-k from classifier + top-k from resolver + concordance scoring. The classifier needs to emit top-k by design, notargmax.

These three changes individually would each require either a fresh train or a meaningful runtime rewrite. Doing them together is roughly the same cost as doing the most expensive one alone, with much higher use.
...
- **Output:** `corpus-v0.4.0` (added to `corpus-v0.3.0` rows rather than replacing). Pure adapter additions; existing weights remain valid.
- **Blocks:** Threads A, C.
...
- **Why:** unblocks Stage 5 concordance work. The model becomes a candidate generator instead of a single-answer predictor — matches the architecture's interface.
- **Specifics:**
...
- **Why:** decouples boundary discovery from type classification — addresses v0.4.0's bio_slip class at source rather than via decoder post-trim. Feeds both Stage 3 (as input conditioning) and Stage 5 (as span candidates).
- **Specifics:**
...
- **What:** carry the v0.4.0 sidecars + diagnostic tools into the v0.5.0 process from day one.
  - `corpus-audit` already in tree (`corpus/scripts/audit.ts`) — runs directly against `corpus-v0.3.0`. Use it to verify Thread B's corpus mix before tokenizer training and before classifier training.
  - `diagnose_regression.py` already in tree (`corpus-python/scripts/diagnose_regression.py`) — use it for v0.5.0 eval bucketing, notpost-hoc.
  - Verdict-smoke framework redesigned: **constant LR for the smoke window**, OR `max_steps >= 10000` so the cosine tail doesn't dominate. Documented in [`VERDICT_SMOKES.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/VERDICT_SMOKES.mdx). Code enforcement: `--smoke-mode constant|long-tail` on `python -m mailwoman_train train` and `smoke` subcommands; defaults to constant for end-to-end smokes.
...
- **Non-Latin adversarial set (new eval split)**: country F1 ≥ 0.50 (vs current ~0 — these are mostly byte-fallback empty preds). This is the result directly measured.
...
- C (classifier retrain): 3-5 days. Single full 50K run on rented H100 should converge directly with the v0.4.0 process improvements in place; verdict-smoke + full-train.
