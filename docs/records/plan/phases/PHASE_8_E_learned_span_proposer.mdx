...
1. **Derived labels from the existing BIO corpus.** Each contiguous run of same-tag tokens is one phrase; map the BIO tag to a `PhraseKind` via a lookup table (e.g. `B-locality ... I-locality` → LOCALITY_PHRASE). Free at training time, no new labeling required. The mapping is lossy at the edges (POI tags collapse to VENUE_PHRASE) but the lossy direction is in the structural-shape vocabulary, which is exactly what we want here.
...
Validation: held-out subset of the kryptonite catalogue + the v0.4.0 bio_slip class (the 6% of cases where boundaries were demonstrably wrong in production).

## Calibration

The reconciler weighs phrase proposals against classifier proposals against resolver candidates — confidence calibration matters more here than in the rule version, because the reconciler will multiply scores. Training-time temperature scaling against the held-out kryptonite subset; report ECE on the model card alongside accuracy.

## When to ship

- **v0.5.0 stretch:** if Thread C (classifier retrain) finishes inside the GPU budget and the rule-based grouper proves to need the assist on a meaningful share of the eval (e.g. non-Latin queries show poor proposals). Triggered by the v0.5.0 eval rather than pre-committed.
