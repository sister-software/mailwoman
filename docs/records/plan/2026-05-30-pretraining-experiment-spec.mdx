...
corpus mixture — not the thing that's absent.

## The key property: pre-training adds zero bytes to the shipped model

This is a **training-time phase rather than a runtime component.** The lifecycle changes
...
pre-training — only the discriminator (= our encoder) is kept. No new artifact ships.
...
scenario where CLM warrants a place is if mailwoman later wants a **generative repair
/ rewrite / canonicalization head** — the AddrLLM pattern (JD Logistics combined
address _parsing_ + _rewriting_ in one model, reducing parcel re-routing ~43%). If
...
today, that's speculative future-proofing rather than a current need.

**Note on calibration specifically:** no study isolates CLM-vs-MLM _downstream
calibration_ — the result is from _pre-training presence_ rather than the
objective. The objective-independent calibration change is **label smoothing +
...
for a few hours on one A100 — eventhe ~10M strings we can cheaply assemble.
...
4. Harness pass-rate as a regression check (not the promote bar — see the
   harness-lineage doc).
...
pre-training isn't the change at this scale.
...
share one direct scoreboard, and **re-run the arbiter eval after any new model**
