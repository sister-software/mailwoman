...
The third and largest step of the system-conditioning work. The consult and the risk-probe that motivate it are written up in the research blog post "Does a postcode know what country it's in?" and the consult notes under `.agents/skills/deepseek-consult/session-notes-2026-06-03-system-conditioning.md`. PR1 (the codex inverse-mapping + the membership-conditional anchor, #264) shipped. PR2 (an anchor-only locale prior replacing `--default-country`) was killed by the risk-probe before it was built. This is the piece the probe pointed at: teach the model to infer the country from the whole address and condition its own labeling on it.

This document is the plan rather than a launch. It costs GPU time and changes the model, so the run waits on explicit sign-off. The decisions that need an operator call are collected at the end.
...
The pre-registered regression check is "per-locale, the rate of city-start tokens mis-tagged as `postcode` falls below 1% by 20k steps." That cross-pollution metric does not exist yet: the current evals (`scripts/eval/per-locale-f1.ts`, `corpus-python` `eval.py`) measure component F1 and calibration rather than this specific confusion. It needs writing: over a held-out per-locale set, count how frequently a token whose gold label starts a `locality`/`region` span gets predicted `postcode`. This is the direct readout of the Saint-Albans collapse, and it is the check that tells us self-conditioning is doing its job before we spend a full run.

## The checks (pre-registered, resolver is the judge)
...
- **20k cross-pollution check:** city-start-as-postcode under 1% per locale by 20k steps. If it will not clear, that recipe has hit its interference ceiling; stop the run.
...
8. `corpus-python/src/mailwoman_train/export_onnx.py` — export the locale-posterior output (wire into the resolver later rather than in the pilot).
...
4. **Conditioning mechanism:** start with head-level FiLM (cheapest) and escalate to back-half FiLM only if the cross-pollution check does not move, or go straight to back-half. Recommendation: start cheap, escalate on evidence.
