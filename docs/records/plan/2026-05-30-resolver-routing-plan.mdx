...
- **Confidence bands:** `p > 0.8` → v0; `p < 0.2` → neural; the narrow ambiguous
  ...
  resolver-confidence result). Caps the 2× parse cost to a small share of traffic.
...
**Phase 2 — build routing (only if the check passes):** 5. **Auto-label** a real unlabeled corpus (OpenAddresses US strings): run both,
...
for external credibility. Not on the check's critical path.

## First PR scope (Phase 1, steps 1–2)

The eval harness + WOF-bootstrap generator + the v0→tree adapter (with its
preliminary 85% check). That unblocks the baselines + the kill/continue check. No
