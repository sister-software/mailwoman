...
**Direction C.** Route each input to the parser that performs best on it, as the
capability map indicates, and feed the result to the already-shipped WOF
resolver. Build the first end-to-end "address → correct place" benchmark to
measure the result. The plan targets the US first. DeepSeek signed off on it (consult: `.agents/skills/deepseek-consult/session-notes-2026-05-30-resolver.md`).
...
Neural-only routing therefore gives up v0's advantage on clean input. We also
have **no end-to-end accuracy number**. The resolver has unit tests, but no test
measures whole-stack correctness.
...
- **Cheap lexical "canonical-ness" scorer** (pre-parse, O(n) on the raw string):
  ...
  `p(v0-wins)`. It is interpretable and debuggable, and it adds no parse cost.
- **Confidence bands:** `p > 0.8` → v0; `p < 0.2` → neural; the narrow ambiguous
  ...
  resolver-confidence result). The bands limit the 2× parse cost to a small share of traffic.
- **resolver-as-arbiter** is the core mechanism. It makes
  _resolvability_ (gazetteer support) the routing signal, which optimizes the
  end goal directly. It serves three purposes: online fallback (ambiguous band), offline
  auto-labeler for the scorer, and eval oracle.
- **No fusion** in v1. Merging v0's flat record with neural's tree is brittle: a
  bad `PARENT_OF` nesting corrupts the resolver's parent-constraint inheritance,
  which is its main strength. Revisit only if the data requires it.

### The output-interface constraint
...
(`PARENT_OF` containment). This adapter is on the critical path for every
baseline that involves v0.
...
1. **Eval harness + ground truth.** WOF-bootstrap: sample stratified US WOF
   ...
   perturbed** variants (lowercase, comma-stripped, glued `NY14201`, mis-split ZIP,
...
2. **v0→tree adapter** (`PARENT_OF` + tree builder). _Preliminary check:_
   v0-via-adapter must reach **≥85% of v0's standalone component accuracy** on
   canonical golden. A lower score means the adapter is losing information, and
...
**KILL/CONTINUE CHECK (the point of Phase 1):**
...
**Phase 2 — build routing (only if the check passes):**

5. **Auto-label** a real unlabeled corpus (OpenAddresses US strings): run both
   parsers, resolve both, and label by resolver-score delta. Drop rows where
   both results fall below the calibrated min-score, and drop marginal rows
   (delta < win margin), which belong in the online ambiguous band. Calibrate
   min-score and win-margin on the WOF-bootstrap set first.
6. **Lexical quality scorer** (LR on the auto-labels). It approximates the
   arbiter cheaply, so the 2× parse cost applies only to the ambiguous band.
7. **Online router** with confidence bands and thresholds set from the eval
   suite. The ambiguous band runs dual-parse + arbiter (reusing the shipped
   resolver).
8. **Tune + monitor:** log 1% dual-parse samples, compare router vs arbiter,
   and retrain on decay.

**Follow-on (parallel, droppable):** an OpenAddresses eval track (~10k real US
`{address, lat/lon}` points) would give an independent great-circle
coordinate-error number for external credibility. It is not on the check's
critical path.

## First PR scope (Phase 1, steps 1–2)

The eval harness + WOF-bootstrap generator + the v0→tree adapter (with its
preliminary 85% check). That unblocks the baselines and the kill/continue check. It
makes no production-pipeline changes, because all of Phase 1 is offline scripts plus one adapter.
...
- **Eval circularity:** WOF-bootstrap resolves WOF-rendered strings back to WOF.
  ...
  check. Do not describe WOF-bootstrap as "real-world" until OA lands.
- **Admin-level ceiling:** the resolver resolves locality/region/postcode rather than
  street/house. "Correct place" = right city/ZIP rather than right building. Street-level
