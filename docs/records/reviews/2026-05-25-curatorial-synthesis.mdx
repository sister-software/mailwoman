...
Every review, from three different models across two dates, reaches the same assessment. The agreement is unusually close for independent reviews:
...
| **Blog posts**     | Unusually strong and direct, but serial-narrative assumptions and missing search-engine preambles make them hostile to inbound readers.                                                                              |
...
These docs-only changes resolve the status confusion immediately. They support every later phase by giving contributors and reviewers one shared picture of what has shipped.
...
| Tokenizer           | v0.1.0 (16K vocab) — used by v0.4.0 weights. A1 (48K vocab, trained on corpus-v0.4.0) — trained but not yet used in a stable classifier.                                                                                                                                                                                                                                     |
...
| Training state      | CE-only smoke passed with val_macro_f1=0.444 and without divergence past step 2000. Full 50K CE-only run in progress. Target: first v0.5.0 weights that train stably.                                                                                                                                                                                                        |
...
**Update rule**: this page is updated on every release. Blog posts link here for current status rather than to each other.
...
- Direct caveats: model quality, resolver precision (admin/postcode-level rather than rooftop), browser load time
...
- Link to `STAGES.md` for per-stage interfaces
...
2. **Locale check default**: `how-it-works-now.md` says "factory default falls back to caller-trust" — `STAGES.md` says "wired as default" — reconcile to "workspace shipped but not yet wired as factory default"
...
- `STAGES.md` — "Implementers working on pipeline stage interfaces"
...
This work closes the integration gap. It tests the architectural thesis independently of training stability.
...
- **v0.4.0 weights may be too weak**: if kryptonite Δ < +15pp, check whether the correct tag was in the classifier's top-3 for each failed example. If not, the bottleneck is classifier quality (need new weights) rather than reconciler algorithm.
...
### Phase C — Training resolution (this week, blocked on CE-only full run)
...
- If that trains directly: transliteration data is the destabilizer → cap B2 at low batch weight, retrain
...
**[→ #155](https://github.com/sister-software/mailwoman/issues/155)** — Product-level eval matrix: comparison script as release check
...
Everywhere the docs say "geocoder" or show coordinates, add a note that the current WOF resolver returns administrative/postcode-level place candidates rather than rooftop/address-point coordinates. This is the current resolver scope rather than a bug, but the docs must state it explicitly.
...
Update `ARCHITECTURE.md` with a dated status block and audience header. Do not rewrite it. Preserve the historical design rationale and label it as historical.
...
| Product eval matrix       | All comparison modes produce numbers, per-failure-class breakdown works  | Release check established                         | Fix eval infrastructure                                 |
...
Through all of this, guard the core architectural shape:
...
5. **World hierarchy from the gazetteer** — WOF SQLite rather than model memory

That decomposition is the project's intellectual contribution, and none of the five reviews contradicts it. The work ahead is to demonstrate it end to end rather than redesign it.
