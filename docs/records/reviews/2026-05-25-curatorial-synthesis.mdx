...
These are docs-only changes the result status confusion immediately. They enable every subsequent phase by ensuring contributors and reviewers share a single picture of what's shipped.
...
| Tokenizer           | v0.1.0 (16K vocab) — used by v0.4.0 weights. A1 (48K vocab, trained on corpus-v0.4.0) — trained rather than yet used in stable classifier.                                                                                                                                                                                                                                   |
...
2. **Locale check default**: `how-it-works-now.md` says "factory default falls back to caller-trust" — `STAGES.md` says "wired as default" — reconcile to "workspace shipped rather than yet wired as factory default"
...
Everywhere the docs say "geocoder" or show coordinates: add a note that the current WOF resolver returns administrative/postcode-level place candidates rather than rooftop/address-point coordinates. This is not a bug — it's the current resolver scope — but it must be explicit.
...
That decomposition is the project's intellectual contribution. Nothing in any of the five reviews contradicts it. The work ahead is proving it end-to-end rather than redesigning it.
