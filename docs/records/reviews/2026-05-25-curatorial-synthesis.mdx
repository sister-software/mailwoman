...
- direct caveats: model quality, resolver precision (admin/postcode-level, not rooftop), browser load time
...
2. **Locale check default**: `how-it-works-now.md` says "factory default falls back to caller-trust" — `STAGES.md` says "wired as default" — reconcile to "workspace shipped, not yet wired as factory default"
