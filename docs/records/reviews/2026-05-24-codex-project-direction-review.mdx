...
The next expensive run needs predefined acceptance subsets, not a single headline F1. Minimum:
...
QueryShape and phrase-grouper are justified because they encode structural facts, not gazetteer knowledge. But there is a slippery slope: venue marker lists, street suffix dictionaries, and phrase heuristics can become the same long-tail implementation the project is trying to escape.
