...
The current work is a measured step forward, but mostly as architecture and instrumentation rather than yet as model-quality proof. The v0.5.0 scaffolding - QueryShape, kind classification, phrase grouping, JS Viterbi, resolver alternatives, top-k training support, and opt-in joint decoding - is the right shape. The expensive next step should not be "run the full fresh-slate train again and hope." It should be a tighter validation program that proves the destabilizer, closes the top-k-to-Stage-5 integration gap, and establishes per-subset checks before spending H100/LLM/human-labeling budget at scale.
...
This is also where Mailwoman can be meaningfully better than old geocoders: only "more correct top-1," but more direct about ambiguity. `Springfield`, `Paris`, and postalcode placeholder failures should surface as candidate sets and diagnostics.
...
5. Eval compares fallback argmax vs joint decode on the kryptonite set and normal golden sets.
...
- `runtime-pipeline.ts` comments still describe locale check/kind classifier as stubbed even though kind is wired and locale-hint exists as a workspace.
...
The next expensive run needs predefined acceptance subsets rather than a single headline F1. Minimum:
...
The substring validator is correctly treated as critical. But it only proves annotated values appear in raw strings. It does not prove the labels are complete, semantically correct, or distributionally sane.
...
LLM data is useful here, especially for kryptonite cases, but it should enter as a weighted adversarial curriculum rather than as an unexamined corpus expansion.

### 6. Rule-based structural layers must stay bounded

QueryShape and phrase-grouper are justified because they encode structural facts rather than gazetteer knowledge. But there is a slippery slope: venue marker lists, street suffix dictionaries, and phrase heuristics can become the same long-tail implementation the project is trying to escape.
...
- Component schema as a guarded interface.
...
- Make `docs/articles/plan/v0-5-0-shipped.md` the temporary authoritative record, then reconcile `TODO.md`, `STAGES.md`, `ARCHITECTURE.md`, and concept pages against it.
- Move from "model F1" checks to "pipeline outcome" checks: parse exact match, top-k resolver recall, graceful failure, and calibration.
- Treat `reconcileSpans` as an integration priority rather than a future idea.
- Promote `predict_top_k` from Python training support into TypeScript/browser runtime support.
- Use synthetic data as a controlled source with explicit weights, notextra rows.
...
- If A1 + v0.3 trains directly, ship tokenizer win without B2 mass and defer transliteration training.
...
Check it behind `forceJointReconcile` or similar. Run it against:
...
This report should become the release check for any new weights.
...
- keep kryptonite rows as high-value adversarial validation and small training source
