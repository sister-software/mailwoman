...
- `resolver-wof-sqlite/unified-schema.ts` - current schema/index helper. Use or extend this as the authoritative record.
- `resolver-wof-sqlite/schema.ts` - downstream Kysely interface for WOF-style tables.
...
- `node:sqlite` has zero native dependencies. It needs neither `node-gyp` nor prebuilds, and it avoids Electron/Bun build problems downstream.
- Mailwoman already uses `node:sqlite` in resolver code and build scripts.
- Current status: it was added in Node 22.5.0 and unflagged in Node 22.13.0+, where it is still experimental. Release-candidate status starts in the newer Node 25/26 lines. Confirm the runtime with `node --version` and expect warnings on some Node versions.
- The API is intentionally close to `better-sqlite3`: synchronous `prepare()` / `run()` / `get()` / `all()`, and `exec()` for batches. The two APIs differ in places (see §6).

### Why Piscina?

- Built-in `worker_threads` works, but Piscina handles pool lifecycle, queueing, and `destroy()` semantics.
- Workers are useful for reading and parsing several tiny GeoJSON files.
...
Keep the phases separate. Do not interleave them.
...
### `BEGIN IMMEDIATE` after parsing rather than before
...
### `journal_mode` is per-file rather than per-connection
...
Every query blocks the event loop of whichever thread called it. That is acceptable in workers. On the main thread, keep setup and freeze explicit and bounded.
...
If durability across power loss matters on macOS, consider `PRAGMA fullfsync = ON`. For this pipeline, re-ingest is typically cheaper.

---

## 7. Design decisions — resolved 2026-05-28

Reviewed with DeepSeek, grounded in existing codebase behavior and downstream consumer interfaces.

### 1. Table interface

**Ship `spr`, `names`, `concordances`, `place_population`, and `ancestors`. Keep `geojson` only during the build. Omit `geometries`.**

`ancestors` is required because the FST builder's parent-chain resolution (`fst-builder.ts:88`) falls back to the ancestors table when `spr.parent_id` is a sentinel (-1, -4). The build needs the `geojson` table (raw `body TEXT`) for `wof:population` extraction via `json_extract()`. It is dropped before VACUUM because it accounts for ~95% of DB size and is never read at query time. A separate `geometries` table adds no resolver value, since bounding boxes and centroids already live in `spr`.
...
**Build both. Per-country extracts are canonical, and the monolith is a convenience.**

The resolver already supports multi-extract ATTACH (`extract routing.ts`, `multi-extract.test.ts`). Per-country extracts give clean separation, independent update cadences, and smaller downloads. The monolith is a rollup (ATTACH + INSERT SELECT + VACUUM) for consumers that don't want to manage multiple files. Build per-country in parallel, merge on demand.

### 4. Alt geometries

**Skip.**

The existing builder already excludes `-alt-*` GeoJSON files. Alt geometries are alternate polygon representations for the same WOF ID. They are useful for cartography but irrelevant for address parsing. Including them would add duplicate rows or clobber the primary lat/lon/bbox via INSERT OR REPLACE.

### 5. Resumability

**Manifest-based for GeoJSON ingestion; rebuild-from-scratch for aux tables.**

For ~1M global files, a lightweight manifest (JSONL: `{path, wof_id, mtime, status}`) lets resumed builds skip already-processed files. The manifest fits the existing spliterator batching pipeline: the glob emits candidates, and the manifest filters out completed ones. Aux tables (FTS5, R\*Tree, population) are always rebuilt from scratch since they depend on final `spr` state.

### 6. Worker responsibility

**Workers return parsed data to a single main-thread writer.**

This design fixes the database lock bug that blocked the current `wof/prepare` command. Workers read files and parse JSON in parallel, then return `ParsedFeature` structs to the main thread. The main thread runs INSERTs in batched transactions (~500 rows). WAL mode allows concurrent readers during writes, but concurrent writers still serialize, so the design uses a single writer. The bottleneck is file I/O rather than SQLite writes.
...
- **Downstream resolver interface:** `resolver-wof-sqlite/schema.ts`, `resolver-wof-sqlite/lookup.ts`, `resolver-wof-sqlite/fts.ts`.
...
- Existing WOF resolver tests still pass, especially FTS, extract routing, population, proximity, and slim-build tests.
