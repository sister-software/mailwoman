...
Neural models for address parsing are unproven in production. Shipping a policy registry that defaults to `rule_only` and lets the registry assign per-component, per-locale authority, blocked on golden-set metrics, is the correct migration strategy. It means the model can ship below parity (which it currently is on several components) without breaking anything.
...
- The Locale check exists as a workspace but is not wired as the factory default

The architectural thesis — "candidate generation + joint coherence beats per-stage argmax" — has not been proven end-to-end with real model output. This is the largest product risk. Until the reconcile path runs against real classifier output and shows a measured improvement on the kryptonite catalogue, the project is shipping architectural scaffolding without validating its core hypothesis.
...
This observation is not fatal. The project is early-stage and the architecture is additive. It does mean the project's "what you can use today" story is limited, and the docs do not state that limitation.
...
- Component schema as guarded interface (`SCHEMA.md`)
...
- **Synthetic data as curriculum rather than expansion.** DeepSeek-generated data should enter with explicit batch-weight caps, source-specific gradient dashboards, and semantic spot-check sampling before corpus promotion.
- **Integration check before training check.** The reconcile path should be proven against v0.4.0 weights before any new training run is promoted. If joint decode doesn't beat argmax on the kryptonite catalogue with existing weights, retraining won't fix it.
- **Status page as check.** No new training run should begin until the status page reflects the current state. Stale docs produce stale decisions.
...
- **Direct about ambiguity.** "Springfield" should return candidates rather than a confident wrong answer.
...
This direction is coherent, and the architecture supports it. The project's primary risk is that the training instability consumes enough iteration cycles that the integration gap (reconcile as default, real top-K, end-to-end measurement) never closes.
...
- If that trains directly, the transliteration data is the destabilizer — cap it at low batch weight and retry
...
- Check: +15pp kryptonite exact-match AND ≤1pt golden macro_F1 regression
...
### 3. Establish product-level eval checks
...
See the [docs/audience review](./2026-05-25-docs-audience-review.mdx) for the detailed plan. The most valuable items are:
...
- CE-only local GPU smoke must pass the divergence check
...
Spend on the next training run only after all four conditions hold.
...
The thesis is sound. The architecture is the best version of this idea that exists in open-source geocoding. The corpus pipeline and diagnostic infrastructure are industrial-strength. The blog posts are unusually direct and readable.

The project's primary risk comes from process rather than architecture. Five things changed at once, training destabilized, and the integration that would validate the architecture was deferred behind the training problem. The fix is a sequence of concrete steps: stabilize training, wire the pipeline end to end, measure the architectural thesis, and then expand scope.

The thing to guard is the decomposition: rules for high-precision bounded problems, a small neural model for contextual ambiguity, structural priors from the phrase grouper, joint coherence from the reconciler, world hierarchy from the gazetteer. That decomposition is correct, and no item in the current evidence contradicts it.
