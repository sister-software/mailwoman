...
Four release-blocking findings survived adversarial verification, collapsing to three fixes — the root README is wrong in two distinct ways. All of them live in documentation, packaging, and licensing rather than in the model or the architecture. That is the most encouraging shape a pre-launch review can have.
...
This is a mile marker, so measure it. The 2026-06-10 deep-dive named four operational weaknesses; three are now closed. The reproducibility crater is documented directly in `REPRODUCIBILITY.md`, the promotion checks are consolidated and rigorous, and the `CONTRIBUTING_MODEL_WORK` runbook exists. The stale-entry-points problem it flagged is the one that persists, now sharper: the READMEs didn'tdrift, they describe the wrong architecture. The 2026-06-23 review's open items have largely landed: AU word-order shipped in v4.14.0 (G-NAF, @25km 65→87), span-rescore v2 is merged and default-on, and the `haversineKm` duplication is consolidated. The strategic picture it drew — competitive in Europe, direct about the centroid-vs-rooftop gap — still holds at v4.15.0.
