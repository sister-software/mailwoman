...
- **Deferred — model-conditional (need a model/training-available session to behavior-diff):** the neural-path clean-install ONNX assertion; the Python→TS ports (`jsonl-to-parquet` is also tangled — its python can't be cleanly removed, so a port would add duplication); the `mailwoman eval`/`gazetteer` command-ification.
