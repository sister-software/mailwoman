...
The main risk is not that the architecture is wrong. The risk is that the docs and roadmap let too several states coexist: historical plan, shipped code, in-progress training, feature-flag scaffolding, and public product story are interleaved without a single canonical status layer. A newcomer can understand the philosophy, but cannot reliably answer "what can I install today, what works, and what is still experimental?"

The documentation needs curatorial direction more than prose rewrites. Preserve the strong essays. Add a status page, getting-started path, API reference, audience labels, and a current-state reconciliation sweep. Move operator material out of the default reader path. Treat blog posts as narrative explanations rather than canonical status.
...
- The "Two voices arguing inside a model" blog post is the best ML primer in the repo: concrete, direct, and non-condescending.
...
- `STAGES.md`, `how-it-works-now.md`, `the-staged-pipeline.md`, and code comments disagree on locale check default wiring and candidate-list state.
...
Avoid positioning Mailwoman as a rooftop geocoder or Google competitor. The resolver currently resolves places and administrative/postcode-level candidates rather than address points. That limitation should be explicit and repeated wherever "geocoder" appears.
...
The small transformer is a sound fit for token/span labeling. A generative LLM would be slower, harder to audit, and worse aligned with "return spans from the input." The docs should keep explaining that the model labels existing tokens; it does not generate addresses or memorize the world.

### Rules plus neural rather than rules versus neural

The Ship-of-Theseus policy registry is the right migration model. Postcodes, state abbreviations, and bounded format recognizers should remain deterministic. Neural should gain authority per component and per locale.

### Parse/resolve separation

This is foundational. The parser should not memorize WOF; the resolver should not infer grammar. WOF parent chains are a constraint source for reconciliation rather than model training targets.
...
Check should compare argmax vs joint reconcile on golden plus kryptonite cases. This is CPU/TypeScript work rather than a GPU-training blocker.
...
LLM-generated transliteration/kryptonite rows are useful, but substring validation only proves alignment rather than semantic correctness or distributional safety. Synthetic rows need:
...
This is the highest-changeage docs fix. Create `/docs/status` and make it the first docs link after the demo.
...
- Retrospectives/research: narrative history.
...
- `how-it-works-now` says locale check workspace exists but factory default falls back to caller trust; `STAGES.md` says it is wired as default; `mailwoman/runtime-pipeline.ts` still does not wire `@mailwoman/locale-hint` by default.
...
- `v0-5-0-shipped.md` says B2 is still cooking; later docs/researchs say B2 landed.
...
5. Fix the most visible state contradictions: CRF runtime status, locale check default, candidate-list status, v0.5.0 B2 status, `ARCHITECTURE.md` layout/label count.
...
- The current usable-product story must be direct: yes, you can run pieces today, but the neural model's standalone component F1 is limited and the strongest architecture path still needs end-to-end default-path validation.

## Bottom line

Continue. Narrow.

The decomposition is right: deterministic rules for bounded facts, small neural classifier for contextual ambiguity, structural priors before classification, joint reconcile after candidate generation, and WOF/open gazetteers for world knowledge. Guard that shape.
