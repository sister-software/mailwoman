...
The current work is a measured step forward as architecture and instrumentation, but the model-quality story is still unproven. The v0.5.0 scaffolding — QueryShape, kind classification, phrase grouper, JS Viterbi, resolver alternatives, top-k training support, and opt-in joint decoding — is the right shape. The expensive next step should not be "run the full fresh-slate train and hope." It should be a tighter validation program that proves the destabilizer, closes the top-k-to-Stage-5 integration gap, and establishes per-subset product checks before spending H100/LLM/human-labeling budget at scale.

**Recommended posture: continue, but narrow the next cycle.** Treat the next spend as validation of the pipeline hypothesis rather than as a release train.
...
| 2. Locale check      | Language / script family                                         | Yes (rule-based workspace; not wired as factory default) |
...
This decomposition is consistent with the bitter lesson. The bitter lesson says "general methods that use computation win over hand-engineered domain knowledge." It argues against hand-engineering what the system could learn from data, and it does not require one model to do everything. Mailwoman decomposes where decomposition is correct (the gazetteer is a lookup rather than a learning problem; boundary discovery is structural rather than semantic) and learns where learning is correct (per-token type distribution). The important distinction is "learn distributions, look up the world, compose constraints."

### 2. Parse/resolve split stays intact

Keeping parser and resolver separate is the most important architectural decision. It avoids asking the model to memorize WOF/BAN/TIGER, and it gives the resolver room to return candidates instead of pretending one answer is certain. The `alternatives` field and `--candidates` CLI direction are aligned with graceful failure. This is where Mailwoman can be meaningfully better than old geocoders: it can be more direct about ambiguity as well as "more correct top-1."
...
The policy registry and component-by-component migration are conservative in the best sense. Postcodes, state abbreviations, and bounded format rules should remain deterministic. Neural should gain authority by component and by locale. This is how to avoid replacing a debuggable rule parser with an opaque model that is worse in production.

### 5. The corpus pipeline is a moat

11 TypeScript adapters (WOF admin/postcode, BAN, TIGER, NAD, NPPES, HRSA, IMLS, three state sources), ~677M aligned rows at a 0.03% quarantine rate, locality-aware splits, Parquet extracts with license tags, SNAPPY compression. The `CanonicalRow` → alignment → BIO labeling → Parquet extract routing pipeline is industrial-scale data engineering. Reproducing it from scratch takes months.
...
This is the largest product risk. Before expensive validation, wire the real path behind a feature flag and run it against the kryptonite catalogue. If joint decode does not beat fallback on kryptonite without hurting the normal subsets, fix scoring before retraining.
...
- Learning rate (factor-2.3 change shifts collapse by factor-1.3 — controls when rather than whether)
...
### 5. Synthetic corpus validation catches alignment rather than semantics
...
LLM data is useful for kryptonite cases, but it should enter as a weighted adversarial curriculum rather than as an unexamined corpus expansion.
...
| `runtime-pipeline.ts` comments | Locale check/kind classifier are **stubbed**          | Kind is wired; locale-hint exists as workspace                 |
...
- Next spend should be validation rather than release. Preflight checklist before H100 hours.
- Eval metrics must graduate to product-level checks: calibration, exact match, graceful failure, top-k resolver recall, per-failure-class breakdowns.
- Synthetic data needs curriculum weights rather than blind corpus expansion.
...
| locale-hint      | workspace shipped | not wired in factory | tests             | wire or document stub          |
...
- If A1 + v0.3.0 trains directly: ship tokenizer win without B2 mass; defer transliteration training.
...
Check behind `forceJointReconcile` or similar. Run against:
...
Compare to current fallback. If joint decode does not beat fallback on kryptonite without hurting the normal subsets, fix scoring before retraining.
...
This report becomes the release check for any new weights.
...
- Keep kryptonite rows as high-value adversarial validation and a small training source.
...
Spend on a full C-train only after every item passes.
...
- Easy to deploy (npm install without C compilation).
- Direct about ambiguity (candidate surfaces, calibration, graceful-failure signals).
...
The architecture is correct, and the corpus pipeline is a durable advantage. Training stability is the current blocker. The divergence pattern across 9 runs is now the central technical question, and it has two unexamined hypotheses (from-scratch initialization, model capacity) that the current bisect ladder does not test. Run those control experiments before the next full train.
...
5. Establish product-level eval checks.
