...
**Explicitly not the top-line:** resolver top-1 accuracy. If the parse is wrong, the resolver will be wrong regardless. Exact-match measures what the reconciler actually changes; resolver accuracy checks a later "is our WOF data good?" question that joint decode can't answer.
...
**Mitigation:** if golden regression lands in the 0.5–1.5pt band, manually inspect all disagreement entries before deciding. If >half are debatable or clearly wrong in golden, expand the check tolerance. If they are genuine (concordance damage), the matrix stands.
