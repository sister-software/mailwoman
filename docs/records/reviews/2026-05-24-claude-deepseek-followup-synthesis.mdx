...
A follow-up to the [Codex review](./2026-05-24-codex-project-direction-review.mdx) and the [DeepSeek synthesis review](./2026-05-24-deepseek-project-direction-review.mdx). Captures a six-turn technical back-and-forth between Claude (host-claude / operator's Jarvis) and DeepSeek-v4-pro, conducted via `pi --continue` on the same session that produced the DeepSeek review.
...
| Go/no-go decision rule for joint-reconcile | Both reviews listed 10+ metrics but no decision rule                                                       | Two-axis matrix: **+15pp kryptonite exact-match** AND **≤1pt golden v0.1.2 macro_F1 regression** as the blocking conditions. Combined exact-match on `kryptonite ∪ golden` as the single headline number.                                        |
...
- **vs porting beam decoder to TS**: half the work (~100 lines vs 200-300), and sequence-level n-best decoding was the wrong abstraction for the reconciler anyway. The reconciler takes per-span confidence rather than BIO-sequence-level confidence.
...
- **vs waiting for PR #128's top-k inference**: code path matches the eventual production runtime — when the top-k classifier eventually trains, the TS runtimeswaps "per-token softmax aggregation" for "classifier's top-k API," same downstream interface.

## Go/no-go decision matrix for joint-reconcile vs argmax

Two checks, applied after running joint-reconcile against the kryptonite catalogue (4,771 rows) and golden v0.1.2 (4,535 entries) using v0.4.0 weights + Option-C top-k:
...
**Why golden ≤1pt as a guard rail:** concordance scoring should be a no-op on already-correct parses. A >1pt regression on golden means the scoring function is making correct → wrong flips, which indicates a scoring bug rather than an architectural failure.

**Explicitly not the top-line:** resolver top-1 accuracy. If the parse is wrong, the resolver will be wrong regardless. Exact-match measures what the reconciler changes; resolver accuracy checks a later "is our WOF data good?" question that joint decode can't answer.
...
**Mitigation:** if kryptonite Δ &gt; +15pp, before concluding "reconciler broken," audit each failed example. Was the correct tag in the classifier's top-3? If no, the bottleneck is classifier quality (need new weights) rather than reconciler algorithm. That's a different conclusion entirely.

### 3. Golden v0.1.2 is too small to be a trustworthy regression guard

4,535 entries. A 1pt macro_F1 regression is ~45 entries flipping. At typical 1% annotator error rate in human-labeled NER data, that's another ~45 entries of noise. The "regression" signal has a ~10% false-positive rate from annotation noise alone — you could ship joint decode that's better and still "fail" the golden check.

**Mitigation:** if golden regression appears in the 0.5–1.5pt band, manually inspect all disagreement entries before deciding. If >half are debatable or the result shows wrong in golden, expand the check tolerance. If they are actual (concordance damage), the matrix stands.
...
### Track C — blocked on Track A + B results
...
- **Agreement across reviewers ≠ correctness.** Both Codex and DeepSeek converged on similar conclusions, but converging reviewers can share blind spots — the three named above are evidence that pushing past consensus is sometimes the only way to find what's missing.
...
The gradient-norm ratio probe ran a few hours after this synthesis was written. The result falsified the dual-loss decoupling hypothesis at its center — and falsified it in an unexpected direction.
...
| ---------------------- | ------------------------------------- | -------- | --------- | --------------- |
| `v3-ablation/step-500` | reached loss 0.63, right before climb | 7.2–17.0 | 149–275   | **median 16.2** |
...
- **Cooperative regime, loss &gt; 0.41.** Model starts at high entropy; both CE and CRF point downhill toward the same broad basin ("become less random"). Training descends directly.
...
The 0.41 boundary isn't a magic number — it's where the cooperative-vs-conflict transition happens on this specific architecture + corpus. The structure of the failure is the key observation rather than the threshold.

### Why standard repairs don't fix this

- **Lowering `crf_loss_weight`** scales loss values rather than gradient magnitudes. At `crf_loss_weight=0.05` with the observed ratio, the effective optimiser mix is still roughly 1:0.8 CE:CRF.
- **Per-token CRF normalization (§1)** made the _loss_ magnitudes comparable. The probe shows _gradient_ magnitudes are still wildly asymmetric.
...
If all three pass, promote to a full 50K-step CE-only training run. **Stability is the result**; any quality improvements come from recipe knobs (class weights, source rebalance, longer schedules) that were unsafe under dual-loss but become safe once the aggressive loss term is gone.
...
| Phase                        | Time    | Action                                                                                                                                     |
| ---------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| 1 (now, ~2 h, parallel)      | t0      | WOF parent_id spot-check + model.py one-line check + CE-only smoke YAML + launch CE-only smoke (~2 h on the iGPU)                          |
...
| 3 (check at smoke step 2000) | ~t+2 h  | Read CE-only smoke result; both stability + quality checks pass → promote                                                                  |
| 4 (blocked on Phase 3)       | ~t+2 h  | Full 50K-step CE-only C-train (~6–8 h); parallel: act on reconciler matrix result                                                          |
...
- **Hypotheses are tested in the direction the data points rather than the direction you expected.** The probe predicted ratio &lt; 0.01 (CRF collapsed) and found ratio &gt; 8 (CRF dominant) — opposite direction, same repair. The original hypothesis was a useful scaffold for designing the experiment; falsifying it directly was more valuable than confirming it would have been. Build experiments around what would change your mind rather than around what would confirm what you already believe.

The full technical write-up of the diagnostic, the cooperative-vs-conflict regime model, and the repair is at [`docs/articles/concepts/dual-loss-curvature-conflict.md`](../site-2026-08/concepts/dual-loss-curvature-conflict.mdx).

## See also

- [`./2026-05-24-codex-project-direction-review.md`](./2026-05-24-codex-project-direction-review.mdx) — the first independent review
- [`./2026-05-24-deepseek-project-direction-review.md`](./2026-05-24-deepseek-project-direction-review.mdx) — the synthesis review this conversation continues from
- [`docs/articles/plan/v0-5-0-shipped.md`](../plan/v0-5-0-shipped.mdx) — as-shipped state of v0.5.0
- [`docs/articles/plan/reference/VERDICT_SMOKES.md`](https://github.com/sister-software/mailwoman/blob/main/docs/engineering/reference/VERDICT_SMOKES.mdx) — the verdict-smoke discipline doc (pending update with the eff_batch lesson + ratio-probe lesson)
- [`docspathname:///research/2026-05-24-v0-5-0-c-train-bisect.md`](pathname:///research/2026-05-24-v0-5-0-c-train-bisect) and [`docspathname:///research/2026-05-24-bisect-by-elimination.md`](pathname:///research/2026-05-24-bisect-by-elimination) — the public retrospectives the reviews respond to
