...
- **vs waiting for PR #128's top-k inference**: code path matches the eventual production runtime — when the top-k classifier eventually trains, the TS runtimeswaps "per-token softmax aggregation" for "classifier's top-k API," same downstream contract.
