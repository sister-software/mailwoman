...
The Mailwoman documentation is **strong in depth** but **confused about who it's for and what state it's in**. The domain-education articles (Tier 0, the falsehoods, the alternatives) are thoughtful and well-structured. The blog posts are unusually direct and readable. The architecture documentation (the Knowledge Ladder, the staged pipeline, the Ship of Theseus migration) is the project's best intellectual contribution.
...
The fix is not to rewrite everything. It's to add **curatorial structure**: a status page, clearer audience segmentation, terminology reconciliation, and three specific new pages that bridge the gap between "Ifound this project" and "I understand the deep-dive articles."
...
The "Understanding Mailwoman" track opens with articles that establish the problem before proposing the solution. This is the correct order. The six Tier 0 articles — from "How mail delivery works" through "The 90% trap" — build a careful economic and structural case for why address parsing is harder than it looks and why a neural approach is justified. The "case for simple geocoders" appendix is especially well done: it steel-mans the alternatives rather than dismissing them, which gives the project intellectual credibility.

The Tier 0.5 postal concept articles (postcode, ZIP code, concordance, intersection, dual-address) add domain grounding that most geocoding documentation skips. These are concrete, well-exemplified, and educational.

### 2. The Knowledge Ladder / staged pipeline

The six-stage decomposition — normalize → locale check → kind classifier → phrase grouper → token classify → sequence correct → reconcile → resolve — is the best design artifact in the project. It maps directly to the "learn distributions, look up the world, compose constraints" principle. The Knowledge Ladder article (`the-knowledge-ladder.md`) explains notwhat each stage does but _why it belongs there and not somewhere else_. This is valuable for both implementers and architecture reviewers.

### 3. Ship of Theseus coexistence

The policy registry — per-component, per-locale authority with explicit fallback modes — is conservative in the best sense. The docs explain why postcodes should stay deterministic, why the CRF mask is hand-encoded rather than learned, and why the neural model warrants authority one component at a time. This is how to avoid replacing a debuggable rule parser with an opaque model that regresses in production.

### 4. The blog posts

The five blog posts are an unusually direct public engineering log. The v0.4.0 retrospective doesn't hide the regressions behind aggregate metrics — it buckets false negatives, identifies the cosine-LR meta-bug, and separates real regressions from adversarial eval artifacts. The "two voices arguing" post is the best piece of beginner-friendly ML communication in the repository — it explains gradient dominance, dual-loss conflict, and bisect-by-elimination debugging in terms a curious software engineer can follow.

The "taming WOF" post fills an actual gap: there is no other developer-facing guide to the practical gotchas of using Who's On First as a geocoder component. It's the kind of content that attracts inbound traffic from search.

### 5. Browser-first constraint as a forcing function

The ~60MB cold-load budget is used as an architectural guardrail throughout the docs. It prevents scope creep into server-only research territory and keeps the model size modest. The live demo at `mailwoman.ai/demo` is the project's best proof-of-work, and the docs reference it consistently.

### 6. The schema-as-interface discipline

`SCHEMA.md` as the single authoritative record for `ComponentTag`, with the "written rationale in commit" rule, is excellent infrastructure hygiene. The forward-compatibility listing of JP tags forces the type system to handle future expansion without a rewrite. The BIO labeling explanation is clear and complete.
...
A reader arriving at the docs has no way to answer "what works right now?" The information exists but is scattered across:
...
- Several blog posts — written from a specific moment rather than maintained
...
- **Stage** — a runtime pipeline stage (Stage 1: Normalize, Stage 2: Locale check, ...)
...
### 4. The blog posts are a serial narrative rather than standalone articles
...
- The "taming WOF" post is the project's best explanation of Who's On First's structural gotchas, but it lives in the blog rather than in the concepts articles
...
- **API reference.** The staged pipeline interface and schema are documented, but there's no page that says "here's the `parse()` function signature, here's the return type, here's what `AddressNode` contains."
- **What you can use today.** A reader evaluating the project needs to know: can I install this and parse US addresses right now? What accuracy should I expect? What doesn't work? The answer is "yes, with caveats" — but that answer isn't the result shows stated anywhere.
...
- **Good:** complete, well-cross-referenced, defines terms on first non-obvious use.
...
- **Good:** The writing quality is high. The voice is direct (the v0.4.0 retrospective explicitly says we shipped with regressions and here's why). The technical explanations don't condescend.
...
- **Issue:** The tags system (`tags.yml`) is elaborate (29 tags for 5 posts) but underused — several tags apply to only one post and don't enable useful filtering.

### Review documents — collective notes

- **Good:** The Codex and DeepSeek reviews are unusually thorough. Publishing them is direct and useful for anyone evaluating the project's self-awareness.
...
1. **Create `/docs/status`** — the single operational status page. Use the thread table from `v0-5-0-shipped.md` as the template. Include:
   ...
   - Feature flag status (opt-in, default, scaffold-only rather than started)
   - Active experimentation
   - Known regressions with direct severity ratings
...
5. **Create `/docs/getting-started`** — installation, 5-line code example, pointer to demo, pointer to API reference, direct accuracy notes
...
The most direct sentence in the entire documentation is in `CONTEXT.md`:
...
The second question is "can I use it today?" The docs should answer that too: "yes, with these caveats, and here's what's coming." The status page is the vehicle for that answer.

The third question is "should I bet on this?" The Knowledge Ladder architecture, the Ship of Theseus migration, the direct retrospectives, and the browser-first constraint all make a strong case. The docsneed to present that case more directly to someone who isn't already inside the project's reasoning.
