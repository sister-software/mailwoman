...
consistent** in the gazetteer's containment graph rather than to wherever a whole-string country guess
...
anything checks whether a "Portland" sits beneath it. Then it scopes the locality into the
...
whole assignments and vetoed an impossible parent chain (`Houston → New York` warrants `-Infinity`
...
- **Forward** — format and script propose a system. A CJK script implies CJK address systems; a UK
  ...
  rule-based (`query-shape`, `locale-hint`'s script-class scorer), and deliberately so — it reads
...
The minimum viable form of the backward half, using implementation that already exists. When resolving a
...
veto in isolation rather than the whole beam.
...
locality missing under any candidate parent — joint consistency returns no result, and the placer's prior
...
measured that widening it changes no outcome, because the placer abstains and it never checks).

## The quick cache reuses the gazetteer rather than a list
...
the gazetteer places an address, we have rebuilt Pelias in a nicer font. (#264.)
...
_under_ Messina, so the locality is not unresolved, and both Messina **and** Maine contain an
...
## Future work, blocked on a measured residual

Build to a measured gap rather than to a good idea. Once descendant-consistency lands (#263), measure where it
...
- A **supplementary address-system model**. The "don't memorize" rule is task-scoped: the open-world job
  ...
  distribution rather than overfitting. Such a model warrants its keep on the fuzzy tier the rules and the graph
  ...
  which gazetteer extract) rather than disambiguation.
...
- No hand-curated country list, safe-list, or famous-place list as the authority. Derived and
