...
| `scripts/build-address-point-shard.ts`                          | Per-state situs address-point DB builder. `--oa-csv` mode reads OpenAddresses for states Overture omits (HI).       |
...
To add an Overture-gap state from OpenAddresses: download the OA conformed CSV(s) from `results.openaddresses.io/latest/run/us/<st>/<source>.zip`, then `build-address-point-shard.ts --state <ST> --oa-csv <csv1>,<csv2>,…`. The new state database is picked up automatically on the next `coverage build`.
