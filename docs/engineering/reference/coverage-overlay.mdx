...
| `scripts/build-address-point-extract.ts`                        | Per-state situs address-point DB builder. `--oa-csv` mode reads OpenAddresses for states Overture omits (HI).       |
...
| ------------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 48 states + DC + VI | Overture addresses `2026-05-20.0` | `build-address-point-extract.ts` (default).                                                                                                                                                                                        |
| **HI**              | **OpenAddresses**                 | Overture omits Hawaii entirely (verified at the S3 source — 0 rows in both releases). Built via `build-address-point-extract.ts --oa-csv <4 county CSVs>` → 348K rooftop points.                                                   |
...
To add an Overture-gap state from OpenAddresses: download the OA conformed CSV(s) from `results.openaddresses.io/latest/run/us/<st>/<source>.zip`, then `build-address-point-extract.ts --state <ST> --oa-csv <csv1>,<csv2>,…`. The new state database is picked up automatically on the next `coverage build`.
...
3. **AK** — the antimeridian guard now DROPS wrapped hexes (so AK partial-renders if its state database is included); a proper fix splits the boundary at ±180 in `coverage-core.ts` instead of dropping.
...
- [Data, locales, and coverage](../../concepts/data-locales-and-coverage.mdx) — the coverage-claims concept page this overlay makes visible on the demo map
