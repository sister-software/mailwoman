...
- **`nexus-public` ≠ `nexus-assets`.** `nexus-public` (`public.mailwoman.ai/mailwoman/`, the `scripts/publish-demo-assets-to-r2.py` target) hosts the demo's model/DBs/data files. The tile worker reads `nexus-assets`. Don't put tiles in `nexus-public`.
