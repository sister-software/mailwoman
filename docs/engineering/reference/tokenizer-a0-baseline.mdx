...
The v0.1.0 numbers were never a valid measurement of multi-script capability; they were a leakage artefact. A0 is the first valid measurement of "what does our tokenizer do on adversarial transliteration input when we have not trained it on any". The answer is: badly. As expected.
...
1. **The harness is validated.** A0 trained end-to-end on real data. Reservoir sampling, the user-defined-symbols normalization step (see [`SP UDS whitespace failure mode`](./sentencepiece-uds-whitespace.mdx) — or, if you are reading on the published docs site, the version in the mailwoman repo at `corpus-python/src/mailwoman_train/tokenizer/uds.py`), the model card writer, the byte-fallback eval — every piece worked. A1 has zero new code to write.
