---
sidebar_position: 16
title: Training recipe settings
---

# Training recipe settings

Every training run is defined by a YAML config with ~30 knobs. Most interact. This document catalogues each setting, what it does, how we arrived at the current value, what happens when you move it, and where we've found ceilings. The goal is to make the reasoning behind the recipe reproducible — not the recipe itself alone.
...
**What it does.** The width of every hidden representation in the transformer encoder. At `hidden_size=256`, every token is a vector of 256 numbers. Wider = more capacity to represent distinctions between address components.
...
**What 256 provides you.** ~17M params, trains at 6.9 sps on A100 (h256 batch=16 ga=8) or ~120 sps (h256 batch=128 direct — untested but estimated). Fits on any GPU including consumer iGPUs.

### `num_attention_heads`

**What it does.** The number of independent "perspectives" each transformer layer uses when deciding which tokens attend to which. Convention: head_dim = hidden_size / num_heads = 64.
...
**What it does.** The number of transformer blocks the input passes through. More layers = more sequential processing but slower.
...
| ----- | ---------------------------------- | ----------------------------------------------------- |
| false | v0.5.0, v0.5.0 bisect (phrase-off) | Diverged (dual-loss issue rather than phrase-priors). |
...
**Next experiment.** label_smoothing=0.1 is the standard starting point. Expected effect: slightly lower peak macro_f1 (the model hedges its predictions) but better-calibrated confidence scores.
...
| 1e-4   | v0.5.0 LR-drop attempt             | Diverged (dual-loss rather than LR). |

**How we got to 1.5e-4.** v0.3.0 empirically found it as the stable LR for this architecture + corpus. v0.4.0 tried higher (5e-4, 3e-4) but the bisect showed divergence was recipe-driven rather than LR-driven. 1.5e-4 has been the constant since.
...
**What it does.** The number of examples the optimizer sees before updating weights. Larger = smoother gradients (less noise), but each step covers more wall-clock time.
...
**The smoke-batch lesson.** Smoke tests at eff_batch=8 passed without error while full runs at eff_batch=128 diverged. At smaller batch sizes, the higher per-step gradient noise paradoxically stabilizes training against curvature conflicts (the model can't settle deep enough into the basin where the conflict manifests). This is now documented in [VERDICT_SMOKES.md](./VERDICT_SMOKES.mdx).
...
**How we got to constant.** v0.4.0's cosine-LR smoke passed a recipe that diverged in the full run. Constant LR keeps the model at sustained peak LR for the entire training window — divergence surfaces immediately if the recipe is unstable. See [VERDICT_SMOKES.md](./VERDICT_SMOKES.mdx).
...
Current values are carried from v0.4.0's §4 source rebalance (the only recipe setting that shipped clean in v0.4.0):

| Source         | Weight  | Why                                                                                                                                                     |
| -------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| tiger          | 4.0     | TIGER carries structured US address patterns. Highest weight to compensate for its small raw file count.                                                |
...
| usgov-nad      | 1.0     | NAD was previously at 2.0 but dominated the sample (52% of files). v0.4.0 dropped it to 1.0 to recover postcode positional exposure from other sources. |
...
4. **Calibration ceiling.** The 54.5% overconfident-wrong rate with v0.5.0 weights is a first-generation model problem. label_smoothing and temperature scaling are unexplored settings. The reconciler masks this at the pipeline level (0.1% overconfident-wrong) but per-model calibration would help all modes.

## See also

- [VERDICT_SMOKES.md](./VERDICT_SMOKES.mdx) — the smoke discipline that catches recipe failures
- [Dual-loss curvature conflict](../../records/site-2026-08/concepts/dual-loss-curvature-conflict.mdx) — why crf_loss_weight=0 works
- [v0.5.0 — as shipped](../../records/plan/v0-5-0-shipped.mdx) — the thread table
- [What the eval numbers mean](../../records/site-2026-08/understanding/our-approach/what-the-eval-numbers-mean.mdx) — plain-English interpretation
