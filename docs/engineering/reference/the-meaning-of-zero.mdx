...
both readings look like a number, so no error fires and no log line appears while the mechanism
takes no action. This page is the invariant and the audit behind it.
...
### 1. The postcode-anchor channel — the bit exists and no code reads it

`neural/postcode-anchor.ts` documents it:
...
- `75008` absolutely is a postcode, and it exists in most countries that the posterior is uniform.
...
`0`. The change looks configured and has no effect. Tracked on #1142.
...
Every instance costs a wrong verdict or a dead change, and none of them announce themselves. Phase 4a's
