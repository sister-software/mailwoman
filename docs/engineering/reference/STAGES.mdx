...
The formal interface for the six-stage runtime pipeline. Pairs with [`INTERFACES.md`](./INTERFACES.mdx) (classifier-level interfaces) and [`SCHEMA.md`](./SCHEMA.mdx) (component vocabulary) — this document covers the **stage boundaries** that compose them into a working pipeline.

For the narrative / why-it-exists framing, read [`concepts/the-staged-pipeline`](../../records/site-2026-08/understanding/our-approach/the-staged-pipeline.mdx). For the bitter-lesson-safe sub-system used by stages 2 and 2.5, read [`QUERY_SHAPE.md`](./QUERY_SHAPE.mdx).

## Status

- **Canonical:** this file is the canonical interface for the runtime pipeline. Every stage implementation must conform to it.
...
- **Implementation state (frozen 2026-05-25):**
  ...
  - **Shipped, soft-signal (#244):** Stage 1.5 (coarse-placer country router). It is off in the bare `core` coordinator (byte-stable) and wired **on by default** in the `mailwoman` package. It runs after normalize and feeds the resolver's `anchorPosterior` re-rank. It never feeds the classifier.
  - **Shipped, opt-in (retired as default 2026-06-14, #566):** Stage 5 (joint reconcile). It was default-on when the phrase grouper and a `parseWithLogits` classifier were wired. Graded against the assembled pipeline, it broke the street + house_number geocode precondition on 77–84% of clean US addresses while fixing 0%, so **argmax is now the default decode**. The code and A/B harnesses remain. `jointReconcile: true` opts back in (`forceJointReconcile` is the deprecated alias).
  - **Shipped but not wired as factory default:** Stage 2. The `locale-hint` workspace exists, but the `createRuntimePipeline` factory does not wire it.
...
flowchart LR
    ...
    S1 --> CP["Stage 1.5<br>Coarse-placer<br>(soft country prior)"]
    CP --> QS["compute QueryShape<br>(boundary)"]
    QS --> S2["Stage 2<br>Locale check"]
    ...
    S4 -->|"default: argmax"| S6["Stage 6<br>Resolve"]
    S4 -.->|"opt-in"| S5["Stage 5<br>Reconcile"]
    S5 -.-> S6
    CP -.->|"country prior"| S6
    S6 --> O["Top-k places"]
```

The fast-path arrow is where the latency saving comes from: when the kind classifier identifies a trivial input (`postcode_only`, `locality_only`) and the QueryShape's known-format match agrees, the coordinator skips stages 3-5 and goes straight to resolve. See [Fast-path routing](#fast-path-routing) below.

## Shared types

These types appear across multiple stages and are defined here.
...
**Purpose.** Stage 1 preprocesses the raw input string deterministically: Unicode NFC, case folding, abbreviation expansion, and whitespace and punctuation normalization. It uses pure functions without ML.
...
**Today.** `@mailwoman/normalize` workspace shipped 2026-05-22 (increment B of [PHASE_7](../../records/plan/phases/PHASE_7_stage_1_normalize_and_query_shape.mdx)). The public `normalize()` function always applies NFC, punctuation, and whitespace normalization, and optionally applies abbreviation expansion and case folding. It composes the `offsetMap` across transforms. The en-US and fr-FR abbreviation dictionaries are small.
...
**Failure classes owned.** Tokenization and whitespace traps (#3); Unicode/transliteration encoding half (#6). See [`addresses-that-break-geocoders`](../../records/site-2026-08/understanding/why-its-hard/addresses-that-break-geocoders.mdx).
...
**Reuse.** Once computed, `QueryShape` passes into every subsequent stage as additional context. It is not its own stage.

**Today.** `@mailwoman/query-shape` workspace shipped 2026-05-22 (increment A of [PHASE_7](../../records/plan/phases/PHASE_7_stage_1_normalize_and_query_shape.mdx)). It uses pure functions without ML and has zero workspace dependencies. It detects character class, punctuation-bounded segments, known postcode formats (US ZIP/ZIP+4, UK, CA, JP + ambiguous 5-digit FR/DE/US), and PO Box / BP. Hyphen, apostrophe, and underscore are "connectors", so `"10118-1234"` and `"Saint-Denis"` stay one token. It accepts both `string` and `NormalizedInputLite` (structurally compatible with `@mailwoman/normalize`'s `NormalizedInput`).
...
```ts
// packages/locale-hint/src/types.ts
...
export interface LocaleHint {
	...
	/** Alternative locales with confidence — possibilities rather than constraints. */
...
export interface LocaleHintStageOpts {
	/** If provided, takes precedence; detector may still emit alternatives. */
	hint?: LocaleTag

	/**
	 * Below this confidence, the check emits an ensemble of likely locales instead of committing to
	 * one.
	 */
	confidenceFloor?: number // default 0.7
}

export interface LocaleHintStage {
	detect(input: NormalizedInput, shape: QueryShape, opts?: LocaleHintStageOpts): Promise<LocaleHint>
}
```

**Today.** `@mailwoman/locale-hint` workspace shipped 2026-05-23. Rule-based: caller-hint precedence at confidence 1.0; otherwise derived from `QueryShape.characterClass` (CJK → ja-JP, Cyrillic → ru-RU, Arabic → ar) + known-format hits (us_zip4 → en-US, uk_postcode → en-GB, ca_postcode → en-CA, jp_postcode → ja-JP). Ambiguous 5-digit → en-US at low confidence with FR/DE as alternatives. It always returns a locale (falling back to en-US at 0.3) and never abstains. `createRuntimePipeline` does not wire it by default, so callers must pass `detectLocale` explicitly. Without it, the coordinator falls back to a stub that trusts the caller's locale.

**Future.** Replace the rule scorers with a small character-level classifier (~100 KB) trained on the first 200 characters of input. Per-locale corpus rows already carry the `locale` field — supervised training data is free. Below the confidence floor, `locale-hint` signals an ensemble run downstream.
...
- Where do non-shipped locales fall back? Recommend: nearest-locale heuristic — a Polish input falls back to `en-US` weights with a low-confidence flag rather than an error.
...
**Future.** Hybrid rule + tiny model. Rule path is shipped; model path remains: when no format dominates, use a small classifier trained on `(QueryShape, label)` pairs from labeled corpus rows (Phase 5 Studio is the source of those labels).
...
- Specialist heads vs single multi-purpose encoder? Recommend: single encoder for v1; revisit when there's enough labeled `intersection` / `po_box` data to justify specialization.
...
**Today.** `@mailwoman/phrase-grouper` workspace shipped 2026-05-23 (v0.5.0 Thread E). Rule-based: per-kind scorers over token-level structural cues (proximity, punctuation, capitalization, hyphenation, format-shape repetition from QueryShape). It is bitter-lesson-safe because it uses no place-name dictionaries. It is wired as the default `groupPhrases` in `mailwoman/runtime-pipeline.ts::createRuntimePipeline`. In v0.5.0 it is a hard dependency rather than an opt-in shim, because there are no v0.4.0 users to migrate. The result is exposed on `PipelineResult.phraseProposals`.

**Future.** Learned 1-2M-param span proposer trained on segment-boundary labels derived from corpus-v0.4.0 + the kryptonite catalogue. Same `PhraseProposal[]` output interface; the consumers (Stages 3, 5) don't need to know which implementation produced the proposals. See [`PHASE_8_E_learned_span_proposer.md`](../../records/plan/phases/PHASE_8_E_learned_span_proposer.mdx) for the scope.

**Failure classes owned.** Boundary discovery on mid-position postcodes (`Paris 75008`), hyphenated compounds (`NY-NY Steakhouse`), multi-word localities (`Saint Petersburg`), repetition tokens (`Buffalo Buffalo`). Addresses v0.4.0's bio_slip class at source rather than via decoder post-trim.
...
**Purpose.** Per-token BIO labeling. The current Mailwoman model (`@mailwoman/neural` + `@mailwoman/neural-weights-*`).

**Interface.** The `Classifier` interface from [`INTERFACES.md`](./INTERFACES.mdx), with `QueryShape` and `LocaleHint` added to `ClassifierContext`:
...
**Future.** Larger context window (128 → 256 tokens), more locales, continued corpus expansion. Optionally, use `QueryShape.segments` as an encoder input feature (segment-id per token concatenated to the input embedding). This is cheap, requires no architecture change, and gives the encoder a structural prior.
...
Today's implementation. [Linear-chain CRF](../../records/site-2026-08/concepts/crf-decoder.mdx) over a frozen BIO transition mask. Eliminates orphan-`I` sequences ("Saint Petersburg → Petersburg" bug).
...
**Today.** Used at training time and in the Python eval. The JavaScript runtime has shipped Viterbi with the BIO structural mask since 2026-05-22. The mask makes orphan-`I-*` sequences impossible at runtime, even without learned CRF transitions in the model bundle. Learned transitions will compose on top once a future weights release ships them via `crf-transitions.json` or the model card.

### Stage 4b — Span re-reader (future)

When Stage 3 emits a span with low confidence, or two overlapping high-confidence proposals, re-run the encoder on that span alone with extra structural conditioning. **The re-reader is additive: it proposes new alternatives and never deletes existing ones.**
...
**Future.** v0.6.0+ at earliest. Needs a corpus recipe of ambiguity-flagged spans (Studio can produce this).
...
export interface ReconcileInputs {
	...
	classifierTopK: ReadonlyArray<ClassifierCandidate> // from Stage 3 (Thread C-s interface)
...
The coordinator is the orchestration function that composes all stages. It lives in `@mailwoman/core/pipeline` (shipped 2026-05-23). It is generic over its stage implementations: each stage is structurally typed and injected through the `RuntimePipelineStages` record. This keeps core free of dependencies on the concrete neural, normalize, query-shape, and resolver packages. The user-facing convenience factory `createRuntimePipeline()` lives in the `mailwoman` workspace and pre-wires the defaults.
...
export async function runPipeline(raw: string, opts?: PipelineOpts): Promise<PipelineResult> {
	...
	const locale = await localeHintStage.detect(normalized, queryShape, { hint: opts?.locale })
	t.mark("locale-hint")
...
| 2 `locale-hint`                | `@mailwoman/locale-hint` + `@mailwoman/locale-hint-weights`       | new                              |
...
The packaging is intentional: a consumer can pull just `@mailwoman/core` + `@mailwoman/normalize` + `@mailwoman/query-shape` for cheap parsing without the neural runtime. Browser-side, the `locale-hint` weights and the kind-classifier together should fit under 1 MB so a fast-path-only deployment is feasible.
...
- **Adding a new stage**: requires editing this file with new interface + Today/Future block + failure classes + open questions, then an interface test in `@mailwoman/core` proving the new stage composes with the existing ones.
- **Removing a stage**: requires the operator's written sign-off in `DECISIONS.md`, and should be avoided.
...
3. **Telemetry.** `PipelineTiming` is per-call. Should we also expose per-stage success rates and per-stage error counters? Recommend: yes, but as a plug-in observability hook rather than baked into the core interface.
...
5. **Server vs browser parity.** Every stage must run in both. The `locale-hint` and kind classifier should be small enough to bundle; the encoder is already isomorphic. Document each stage's bundle-size budget when implementations land.

## How this relates to existing files

- [`INTERFACES.md`](./INTERFACES.mdx) defines the **classifier-level** interfaces (`Classifier`, `ClassificationProposal`, `PolicyRegistry`, etc.). This file is the **stage-level** interface that composes them.
...
- [`concepts/the-staged-pipeline`](../../records/site-2026-08/understanding/our-approach/the-staged-pipeline.mdx) is the **public-facing narrative** version. This file is the implementer's interface.

## What's next

The staging picture is now formalized. Implementation order, in suggested priority:

1. **Stage 1 + boundary QueryShape together.** Cheap to build; every downstream improvement depends on it.
