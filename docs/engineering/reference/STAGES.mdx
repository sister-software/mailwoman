...
- **Canonical:** this file is the canonical interface interface for the runtime pipeline. Any stage implementation must conform.
...
- **Implementation state (frozen 2026-05-25):**
  ...
  - **Shipped, soft-signal (#244):** Stage 1.5 (coarse-placer country router). Off in the bare `core` coordinator (byte-stable); wired **on by default** in the `mailwoman` package. Runs after normalize, feeds the resolver's `anchorPosterior` re-rank — never the classifier.
  ...
  - **Shipped but not wired as factory default:** Stage 2 (`locale-hint` workspace exists rather than in `createRuntimePipeline` factory).
...
**Today.** `@mailwoman/normalize` workspace shipped 2026-05-22 (increment B of [PHASE_7](../../records/plan/phases/PHASE_7_stage_1_normalize_and_query_shape.mdx)). Public `normalize()` function with NFC + punctuation + whitespace (always) + abbreviation expansion + case-fold (opt-in). Essential `offsetMap` composed across transforms. Small en-US + fr-FR abbreviation dictionaries.
...
**Today.** `@mailwoman/query-shape` workspace shipped 2026-05-22 (increment A of [PHASE_7](../../records/plan/phases/PHASE_7_stage_1_normalize_and_query_shape.mdx)). Pure functions, no ML, zero workspace dependencies. Detects character class, punctuation-bounded segments, known postcode formats (US ZIP/ZIP+4, UK, CA, JP + ambiguous 5-digit FR/DE/US), PO Box / BP. Hyphen / apostrophe / underscore are "connectors" so `"10118-1234"` and `"Saint-Denis"` stay one token. Accepts both `string` and `NormalizedInputLite` (structurally compatible with `@mailwoman/normalize`'s `NormalizedInput`).
...
- **Adding a new stage**: requires editing this file with new interface + Today/Future block + failure classes + open questions, then a interface test in `@mailwoman/core` proving the new stage composes with the existing ones.
