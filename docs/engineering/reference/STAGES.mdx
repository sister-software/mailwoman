...
**Today.** `@mailwoman/normalize` workspace shipped 2026-05-22 (Slice B of [PHASE_7](../phases/PHASE_7_stage_1_normalize_and_query_shape.mdx)). Public `normalize()` function with NFC + punctuation + whitespace (always) + abbreviation expansion + case-fold (opt-in). Essential `offsetMap` composed across transforms. Small en-US + fr-FR abbreviation dictionaries.
...
**Future.** Hybrid rule + tiny model. Rule path is shipped; model path remains: when no format dominates, use a small classifier trained on `(QueryShape, label)` pairs from a labeled corpus slice (Phase 5 Studio is the source of those labels).
