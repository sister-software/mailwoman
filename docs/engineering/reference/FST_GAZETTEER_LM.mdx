...
**Goal:** Pre-compute a finite-state transducer from the WOF SQLite gazetteer that maps token sequences → `(placetype, wof_id, parent_chain, importance)` entries. Use it as an [emission prior in the neural Viterbi decoder](../../concepts/fst-gazetteer-prior.mdx), as a CLI introspection tool, and as the autocomplete backend.
...
- [`TRAINING_RECIPE_CHANGES.md`](./TRAINING_RECIPE_CHANGES.mdx) — training-side fixes (complementary)
