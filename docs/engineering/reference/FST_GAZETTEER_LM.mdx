...
**Goal:** Pre-compute a finite-state transducer from the WOF SQLite gazetteer that maps token sequences → `(placetype, wof_id, parent_chain, importance)` entries. Use it as an [emission prior in the neural Viterbi decoder](../../records/site-2026-08/concepts/fst-gazetteer-prior.mdx), as a CLI introspection tool, and as the autocomplete backend.

**Principle:** Pay down the combinatorial cross-product of "all valid place-name paths through the WOF hierarchy" at build time. At query time, walking the FST is O(depth) rather than O(gazetteer_size).
...
The FST prior gets STRONGER as the prefix grows longer. A bare "S" spreads bias across the candidate places; "San Fran" concentrates it on San Francisco.
...
**With streets (Phase 2+):** ~200-500 MB for full US streets (3-5M unique street names). Too large for browser — split into per-metro-area files (~5 MB each) for browser deployment. Server-side loads the full set.
...
1. **FST is an emission PRIOR rather than a replacement.** The neural model remains the authority for non-gazetteer components. The FST handles what the model can't: knowing which place names exist and their hierarchies.
...
6. **Autocomplete is a prefix walk rather than an index scan.** O(depth × branching) vs Elasticsearch's O(matches). The FST IS the autocomplete index.

---

## Relationship to existing architecture

The FST is the "pay-it-forward" implementation of the mail-carrier philosophy: each carrier's knowledge is pre-compiled into a structure that narrows the search space with each token consumed. The deliberative assembly (phrase grouper + neural model + reconciler) still adjudicates — the FST gives them much better priors to work with.
...
- [`QUERY_SHAPE.md`](./QUERY_SHAPE.mdx) — the existing structural-prior system this extends
- [`DEMO_PRESET_DIAGNOSIS.md`](./DEMO_PRESET_DIAGNOSIS.mdx) — the locality/region confusion this addresses
- [`TRAINING_RECIPE_SETTINGS.mdx`](./TRAINING_RECIPE_SETTINGS.mdx) — training-side fixes (complementary)
- `docs/records/site-2026-08/understanding/exotic-poi/` — venue detection that benefits from FST negative evidence
