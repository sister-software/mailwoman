...
The canonical union of address component types. Defined in [`reference/SCHEMA.md`](./SCHEMA.mdx). Single source of truth. Adding a tag requires a written rationale.
...
- **Locale profiles** prevent the architecture from being Anglocentric by default. If the abstractions can't express a non-Anglo locale cleanly, we fix the abstractions, not the locale.
