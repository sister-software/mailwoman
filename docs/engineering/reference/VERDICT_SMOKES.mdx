...
The smoke was reporting "this recipe is stable at this hyperparameter setting" when what it had demonstrated was "this recipe is stable when LR is decaying off". Two different statements; the smoke conflated them.
...
When in doubt: pick constant-LR. The false-positive cost (a stable recipe gets flagged at sustained peak LR when it would survive cosine decay) is small — you promote it to the full run anyway. The false-negative cost (a divergent recipe passes the smoke and burns a full run) is what the v0.4.0 campaign paid.
...
- **`corpus-audit`** ([`corpus/scripts/audit.ts`](https://github.com/sister-software/mailwoman/blob/main/corpus/scripts/audit.ts)) — per-source parquet-file-count × source-weight diagnostic. Run before training to verify the sampled mix matches intent. Catches the v0.3.0 "NAD = 35% effective sample" class of misuse hazard before it costs a training cycle.
...
The v0.5.0 ablation-smoke at `batch_size=8, grad_accum=1` (eff_batch=8) passed without error — loss descended, val_macro_f1 climbed, no divergence through 1500 steps. The full train at `batch_size=16, grad_accum=8` (eff_batch=128) diverged at step 800.
...
**Rule:** smoke `batch_size` × `grad_accum_steps` must equal the full-run's product. If the full run uses eff_batch=128, the smoke must too — even if that means fewer steps per wall-clock minute. The smoke's job is to reproduce the full run's dynamics rather than to be fast.
