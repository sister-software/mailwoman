...
The model learns the frequency-dominant pattern (Washington = region) because the region label appears more frequently in isolation. The positional signal from OSM full-address rows exists but is diluted.

## BIO boundaries are correct

The model emits `B-region` for "Washington" and a separate `B-region` for "DC" (not `I-region`). This is structurally correct — they are separate entities separated by comma+space (O-tokens). The CRF transition mask is working. The problem is tag assignment on the first entity rather than boundary detection.
...
This fits the bitter-lesson framing test: the regex is locale-bounded (one pattern per locale's abbreviation convention) rather than a gazetteer lookup. The WOF verification is a safety rail rather than the primary signal.
