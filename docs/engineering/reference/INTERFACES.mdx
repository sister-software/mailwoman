...
Every boundary in the system has an explicit TypeScript interface. Stick to these. If you need to extend, extend; do not silently widen.
...
Default policy table is in `packages/core/lib/policy/defaults.ts`. Initial state: every component is `rule_only`. Migrations to other modes happen via explicit commits with accompanying metric evidence.
...
```ts
// packages/corpus/lib/adapter.ts
...
```ts
// packages/corpus/lib/labeled.ts
...
- `LabeledRow.tokens` is the SentencePiece output rather than raw words. This is the model's native input. Tokenization happens once, in the corpus stage rather than at training time. Determinism.
