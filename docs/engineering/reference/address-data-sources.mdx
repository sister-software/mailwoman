...
This page catalogs public-domain and openly licensed data sources that contain facility names and addresses, for use as raw material when training a token-classification (BIO-tagging) postal address parser. It complements the federal and aggregator sources (OpenAddresses, OSM, NAD, TIGER, NPI, IRS BMF, etc.). This list focuses on **state-level** sources, which add component-level variety, residential coverage, and venue-name-paired records that the federal sources lack.

The canonical agency URLs were verified by web search. Some per-state entries in the Regulated Facilities section are based on known agency patterns rather than direct URL verification, and those entries say so inline.

## Why state sources matter for an address NER model

1. **Component supervision is automatic.** State directories are typically already field-structured, with separate columns for street, city, postcode, etc. Each token's BIO tag comes from the column it was taken from, so no manual labeling is required.
2. **Formats vary by state.** 50 states plus DC give 50+ schema variants, abbreviation conventions, and transcription styles, which are useful for training format robustness.
3. **Venue+address pairs are common.** Every facility, professional, or establishment listing pairs an organization name with a structured address, which gives natural `B-venue` / `I-venue` training spans.
4. **Residential coverage.** Real estate, contractor, cosmetology, notary, and license-lookup tools expose home and office addresses that federal facility datasets miss.

## Formats to watch for beyond CSV

State agencies distribute address data in geospatial formats. The DBF attribute table inside a shapefile is essentially a CSV with geometry attached. Extract it with `ogr2ogr -f CSV`, and the records are ready for training. Common formats:
...
- **Site address points / E911 / NG911 address points**: every addressable location as a point with a full component breakdown (housenumber, prefix, street, suffix, unit, city, ZIP, parcel ID). Counties publish these, and states increasingly do so through ArcGIS Hub. This is the most useful single layer type for component-supervised training.
- **Parcel polygons with situs + owner addresses**: two addresses per record (the property situs and the owner's mailing address), which can differ structurally (P.O. boxes for owners, street addresses for situs). MA, NC, MN, UT, MT, VT, RI, and a handful of other states aggregate them statewide.
- **Road centerlines with address ranges**: a current state-level analogue to TIGER. Each segment has from/to house numbers on each side.
- **Critical infrastructure / facilities layers**: state equivalents of HIFLD covering hospitals, schools, fire/police, jails, courts, and public buildings. Each record is a `B-venue` + address pair.
- **EPA/state-DEP regulated sites**: Superfund, brownfields, RCRA, NPDES, and leaking USTs as shapefile/GeoJSON.
- **NCES, NREL AFDC, FCC ASR/ULS**: federal sources that are distributed as shapefiles as well as CSV.

Most state ArcGIS Hubs publish data under CC-BY or equivalent open terms. Check each dataset.

## Licensing reality

State records are not "public domain" in the IP sense, but factual data (name + address) is not copyrightable under _Feist v. Rural Telephone_. Some caveats apply:

- **Washington OSPI school directory**: state law forbids commercial use of directory data.
- **OSM (ODbL)**: share-alike obligations may extend to models trained on it. The question is unsettled, but it is a risk if the corpus is republished.
- **CC-BY state ArcGIS Hub data**: attribution is required if the corpus is redistributed.
- **G-NAF, CC-BY-4.0**: safe to use for AU coverage.

For the safest licensing on published model weights, restrict training to US-federal sources (NCES, NAD, TIGER, NPPES, IRS BMF) and state sources confirmed as public records without use restrictions. That choice reduces coverage and gives unambiguous redistribution rights.

---

## Gazetteer / resolver coordinate sources (companion layer)

Everything after this section is **corpus** material: address _strings_ and name/address pairs to train the BIO parser. This section covers the other half, the **gazetteer** that the resolver and the postcode anchor consume. These datasets turn a parsed span into a place and a coordinate. The requirements differ from the corpus, and this is where WOF's gaps cause problems (see `resolver-wof-sqlite/POSTCODE-ANCHOR.md` and the 2026-06-03 postcode-anchor postmortem).

WOF is an excellent **admin** gazetteer. Its in-DB hierarchy and ancestors table let us resolve without Elasticsearch, and it is **CC0**, which made it an easy choice. It has never been complete for every token type, however. It has [no street node](../../records/site-2026-08/concepts/wof-hierarchy-gap.mdx) (`neighbourhood` → address directly). Its postcode geometry is thin and uneven outside a few countries: US/NL carry their own coordinates, DE is ~66% placeable by borrowing from ancestors, and ES/IT are mostly orphans and effectively unplaceable. Its POI/venue coverage is limited. The standard fix is the rest of the Pelias four-layer stack, supplemented for each gap: **WOF** (admin), **OpenAddresses** (addresses/postcodes), **OpenStreetMap** (streets/POIs), and **GeoNames** (places/postcodes).

### By gap layer

- **Postcode → centroid.** **GeoNames postal** (`download.geonames.org/export/zip`, CC-BY 4.0, ~80+ countries) is a ready-made table of postcode → place + admin + lat/lon. It is the cheapest way to fill the DE/ES/IT gap and needs no point-cloud math, although it is coarser than OA in places and some records are locality-level. **OpenAddresses** point aggregation (median centroid per postcode) is more precise where we have the points. National authorities are the most accurate sources but are fragmented: UK **Code-Point Open** (OS, OGL), France **BAN**, and NL via PDOK.
- **Street / rooftop coords.** **OpenAddresses** (primary), **OSM** `addr:*` on buildings (strong in DE/NL, patchy elsewhere), and national registries: France **BAN**, Australia **G-NAF** (CC-BY; backlog #31), and Sweden **Lantmäteriet** (the national address register). Lantmäteriet access requires **Geotorget** approval, so it is shelved until that approval arrives. It would fill the SE gap.
- **POIs / landmarks / salience** (the generalized-anchor and exotic-POI layer). **OSM** is the richest open POI source. **Wikidata / Wikipedia** is the right source for the data-driven _salience_ signal that the anchor guardrail requires (notability, coordinates, and multilingual names). We already pull `wikimedia-importance.csv` in `build-importance`, so this path has a precedent.
- **Admin (the anchor).** Keep WOF. If it is ever reconsidered, the alternatives are GeoNames admin, **Overture Divisions**, and geoBoundaries.

### Probe: GeoNames vs the WOF postcode gap (2026-06-03)

This probe measured GeoNames postal against the gap in the WOF postcode data (the postcodes WOF carries as members but cannot place). GeoNames has coordinates for **100%** of its DE/ES/IT records:
...
GeoNames postal is therefore the cheap CC-BY fill for ES/IT and for roughly half of DE's remaining gap, and it also corrects WOF's Italian mis-links. The integration follows the rules in the Integrity discipline section below: match on the postcode string, keep the WOF id, and write the GeoNames lat/lon as the centroid. (Overture was not probed. For _postcodes_ it includes OpenAddresses, and we already use OpenAddresses directly. Overture's additional value is therefore the POI layer. A POI probe needs a DuckDB + S3-parquet setup and is the right next step for the generalized-anchor work, but not for postcodes.)

### The modern entrant: Overture Maps

Overture Maps is a Meta/Microsoft/AWS/TomTom consortium (2023+, newer than the corpus catalog below). Its **Places** theme is a large open POI dataset, and its **Addresses** theme includes OpenAddresses. It therefore covers two of our three gaps in one source, with stable **GERS** entity IDs. It is distributed as cloud parquet (DuckDB/S3). Probe it before committing to per-source OSM imports.

### Licensing gradient

For an AGPL product, license terms matter more than coverage:
...
| OpenStreetMap | ODbL                    | attribution + share-alike on derived databases — the highest risk; can impose copyleft |

The licenses favor **GeoNames + Overture + OpenAddresses** as the supplements. Use OSM with care, for derived signals rather than in the shipped DB.

### Integrity discipline

Attach supplemental data as **attributes on WOF-keyed entities** (such as a centroid or a popularity score) rather than importing entities with foreign ids. WOF remains the anchor and provides the eval keys, while the coordinates and the long tail come from other sources. Using Overture's GERS ids (or any parallel id space) as primary keys would break the WOF-id-keyed resolver evals. That is the reason for the rule "extend the custom WOF build, never a prebuilt dump". OA centroid aggregation meets this requirement because the postcode keeps its WOF id, and only its coordinate comes from OA.
...
- **NCES Common Core of Data (CCD) — Public School & District Universe** — https://nces.ed.gov/ccd/ccddata.asp — Bulk CSV/SAS downloads of every public school + district in the US with address. Default CCD home: https://nces.ed.gov/ccd/ — public-use without restriction.
...
- **Idaho** — Idaho SDE Schools / Districts — https://www.sde.idaho.gov/school-districts/ — Alphabetical district list. No published single bulk download from SDE; consider NCES CCD for bulk.
...
- **Maryland** — MSDE Directory — https://marylandpublicschools.org/about/pages/directory.aspx — Online directory; bulk download not published from MSDE. Maryland State Archives list by county: https://msa.maryland.gov/msa/mdmanual/01glance/html/edschools.html.
...
- **Nevada** — NDE Schools and Districts — https://doe.nv.gov/Schools_Districts/ — Schools/Districts landing. Resource page: https://doe.nv.gov/school-and-district-information/resources. Bulk download not published; query CCD or NDE directly.
...
- **North Dakota** — NDDPI Districts/Schools — https://www.nd.gov/dpi/districtsschools — LEA/School directory MIS01/MIS02 framework. Bulk download not linked from main page; investigate STARS reports.
...
- **Utah** — USBE Utah Schools / District Directory — https://schools.utah.gov/schooldistricts — Districts list. Data/Directories landing: https://schools.utah.gov/parent/dataschoollookupeducatorlookupcalendars. Single bulk CSV not linked; investigate manually.
...
- **IMLS Public Libraries Survey (PLS)** — https://www.imls.gov/research-evaluation/surveys/public-libraries-survey-pls — Annual national bulk dataset (CSV/SAS/SPSS) of every administrative-entity public library and every outlet (branch), with addresses. Public-use without restriction. Catalog mirror: https://catalog.data.gov/dataset/public-library-survey-pls-2022. Open Data: https://data.imls.gov/.
...
- **Arkansas** — AR State Library Directory — https://library.arkansas.gov/services/arkansas-public-library-data/library-directory/ — Statewide public library directory grouped by city/county/system.
...
This section lists state-level public databases of licensed professionals, with practice or office addresses, that are suitable as training data for a postal address parsing NER model. All URLs were verified by web search. Most state license-lookup tools are query-only (search by name or number). The NPPES NPI Registry and several state portals offer bulk CSV exports.
...
(Some states route all professions through a single search, and the per-profession entries below refer to it where applicable.)
...
- **Florida DBPR License Search** — covers real estate, contractors and other trades (health professions are separate at FL DOH MQA) — https://www.myfloridalicense.com/wl11.asp?mode=2&search=Name
...
- **Virginia DPOR License Lookup** — covers real estate, contractors and other occupations (medical/nursing handled by VA DHP) — https://www.dpor.virginia.gov/LicenseLookup
...
- **Kansas License Verification Portal** — covers professions across sectors — https://prolicenseverify.ks.gov/
...
- **Ohio eLicense** — covers Medical Board and others; OCILB + Commerce real estate on separate eLicense subsites — https://elicense.ohio.gov/oh_verifylicense
...
- **Tennessee Verify (Commerce & Insurance / DCI)** — covers real estate, contractors and others; health pros separate at TN DOH — https://verify.tn.gov/
...
- Medical: Oklahoma State Board of Medical Licensure & Supervision — https://www.okmedicalboard.org/search — query-only (separate osteopathic (DO) board at https://oklahoma.gov/osboe.html)
...
- Charity registry: Rhode Island Department of Business Regulation (DBR) — https://dbr.ri.gov/banking-securities-and-charitable-organizations/securities-and-charities/charitable-organizations — query/file-only; no public search endpoint
...
- Business entity search: Wisconsin Department of Financial Institutions (DFI) — https://apps.dfi.wi.gov/apps/corpsearch/search.aspx — query-only (note: DFI rather than SoS)
...
- **Nevada** — State of Nevada — https://open.nv.gov/ — Socrata (limited). No unified portal; GIS scattered.
...
- **Wisconsin** — N/A unified — multiple specialized portals; GeoData@Wisconsin (https://geodata.wisc.edu/) is the largest geospatial catalog.
...
License terms: most state ArcGIS Hub deployments default to Esri Open Data terms (effectively public-domain or attribution). Confirm the terms for each dataset. Socrata-hosted state portals generally publish under each state's open-data terms, which are typically attribution-only.

---

### Regulated establishments per state

Each state entry lists the following, where available: Child care licensing, Assisted living/nursing home (state-level), Alcohol/ABC, Cannabis (where legal & public), Auto dealer licensing.
...
This list includes only states that aggregate parcels statewide, beyond per-county data. License notes are included where the terms were visible.
...
- Regulator websites (DMV dealer lookups, liquor license search portals) are query-only; bulk acquisition requires a FOIA/public records request unless an open-data dataset is also published.
