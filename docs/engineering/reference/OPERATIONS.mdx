...
You are an autonomous agent. Optimize for **legibility to a human checking in once a day** rather than for cchange one-shot completions.

### Pacing

- Work in small, complete units. A "unit" is anything that ends with a green test suite or a working command.
- Commit at every unit boundary. Better more commits than too few.
...
4. Never block waiting for input. If a decision feels blocking, write it to `DECISIONS.md` under a `## BLOCKED` heading and continue with adjacent work.
...
Only created if you have a blocker. One entry per blocker.
...
2. License check — must be MIT, Apache-2.0, BSD, or ISC. AGPL-compatible licenses fine for the package itself (Mailwoman is AGPL) but verify each package.
...
- `parquet-wasm` + `apache-arrow` — Parquet read/write in TypeScript
...
/data/
  corpus/
    ...
    aligned/           # post-alignment Parquet extracts
...
- **Package / release versions** (the published `mailwoman` npm package + the live demo) are a SEPARATE, independent line — the user-facing product version (currently `4.4.0`). It does not share a counter with the corpus or model versions: the package marched 4.0 → 4.4 through the parity campaign while the corpus stayed on its own `corpus-v0.x` line. A release pins which `corpus-v<…>` and `model-v<…>` it trained on (via `ModelCard.trainedOn`), but its own number belongs to the product rather than the data.
...
- All code
- Corpus manifests (file lists + checksums rather than the files)
