...
How to bring a fresh lab container to the point where it can train a classifier or run a tokenizer build. Captures the gap that bit v0.5.0 Thread C-train.
...
- **`amdgpu.ids: No such file or directory`**: also harmless. The file is shipped with `mesa-amdgpu-vulkan-drivers` which isn't installed in the lab base image. Nothing fails as a result.
...
1. **Image size**: torch + transformers + onnxruntime add ~5 GB to the container image. Multiplied across every lab container (most of which never train), this is wasteful.
...
- [`tokenizer-a0-baseline.md`](./tokenizer-a0-baseline.mdx) — the lighter-weight alternative path (CPU-only, no torch needed)
