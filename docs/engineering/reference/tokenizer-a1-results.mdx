...
A1 is the v0.5.0 plan ([`PHASE_8 §A`](../../records/plan/phases/PHASE_8_v0_5_0_fresh_slate.mdx)) tokenizer retrain on `corpus-v0.4.0`. Same harness as [A0](./tokenizer-a0-baseline.mdx); the only intended differences are the corpus (v0.4.0 = v0.3.0 + Thread B kryptonite + Thread B2 transliteration) and the country list (widened to cover Thread B2's target locales).
...
| ----------- | --------- | --------- | ------------ | -------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **overall** | **36.7%** | **18.2%** | **−18.5 pt** | 18.0%          | +0.2 pt     | halved vs A0; lands on the v0.1.0 leakage floor. Misses the plan's `<5%` target — pulled by thai + residual cjk.                                                                                        |
| cjk         | 80.0%     | 45.2%     | −34.8 pt     | 51.6%          | −6.4 pt     | **stretch hit** — beats v0.1.0 leakage floor. Largest non-Latin script class; this is the headline result.                                                                                              |
| cyrillic    | 2.2%      | 2.4%      | +0.2 pt      | 0.0%           | +2.4 pt     | within noise of A0. Eval fixture's Cyrillic rows are short / common-vocabulary; not enough surface for the B2 cyrl training file to change the number.                                                  |
...
- **A1 beats v0.1.0 leakage on cjk** — the largest non-Latin script class and the most important target. v0.1.0's CJK number was always a WOF admin-name artefact (see [tokenizer-a0-baseline.md](./tokenizer-a0-baseline.mdx#why-a0-looks-worse--the-wof-admin-name-leakage-hypothesis)); A1's 45.2% stands without that leakage.
- **Latin improved rather than regressed** — the anti-goal in the plan held. 5.8% → 3.4% is a meaningful Latin gain, likely from the kryptonite rows sharpening US/FR sub-pieces.
...
The follow-up work for `<5%` overall is a **Thread B3**: extend `TRANSLIT_SCRIPTS` + `KNOWN_SOURCE_PREFIXES` to `grek`, `arab`, `hebr`, `deva`, `thai` (per [`CORPUS_V0_4_0_GENERATION.md`](./CORPUS_V0_4_0_GENERATION.mdx#scope) — these are noted as additive / deferred), regenerate the deepseek rows, retrain as `v0.5.0-a2`. Treating this as a separate thread keeps C-train unblocked.
...
This document was produced by an autonomous AI agent operating in a Mailwoman playpen container. The byte-fallback numbers are mechanical measurements emitted by the harness; the interpretation and the C-train recommendation are the agent's reading rather than the operator's.

## See also

- [Tokenizer A0 — baseline](./tokenizer-a0-baseline.mdx) — what A1 is measured against
- [PHASE_8 §A](../../records/plan/phases/PHASE_8_v0_5_0_fresh_slate.mdx#a-tokenizer-retrain--multi-script--adversarial-coverage) — the plan
- [`CORPUS_V0_4_0_GENERATION.md`](./CORPUS_V0_4_0_GENERATION.mdx) — how the corpus A1 trained on was assembled
- Harness fix that unblocked the run: commit `51b77d5` (`harness: read MANIFEST.json for train extract resolution`)
