...
| cyrillic    | 2.2%      | 2.4%      | +0.2 pt      | 0.0%           | +2.4 pt     | within noise of A0. Eval fixture's Cyrillic slice is short / common-vocabulary; not enough surface for the B2 cyrl training file to change the number.                                                  |
...
- **A1 beats v0.1.0 leakage on cjk** — the largest non-Latin script class and the most important target. v0.1.0's CJK win was always a WOF admin-name artefact (see [tokenizer-a0-baseline.md](./tokenizer-a0-baseline.mdx#why-a0-looks-worse--the-wof-admin-name-leakage-hypothesis)); A1's CJK win is honest.
...
The follow-up work for `<5%` overall is a **Thread B3**: extend `TRANSLIT_SCRIPTS` + `KNOWN_SOURCE_PREFIXES` to `grek`, `arab`, `hebr`, `deva`, `thai` (per [`CORPUS_V0_4_0_GENERATION.md`](./CORPUS_V0_4_0_GENERATION.mdx#scope) — these are noted as additive / deferred), regenerate the deepseek slice, retrain as `v0.5.0-a2`. Treating this as a separate thread keeps C-train unblocked.
