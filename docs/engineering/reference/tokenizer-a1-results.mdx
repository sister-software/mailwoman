...
This document was produced by an autonomous AI agent operating in a Mailwoman lab container. The byte-fallback numbers are mechanical measurements emitted by the harness; the interpretation and the C-train recommendation are the agent's reading rather than the operator's.
