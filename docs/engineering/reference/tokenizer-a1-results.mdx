...
A1 is the v0.5.0 plan ([`PHASE_8 §A`](../phases/PHASE_8_v0_5_0_fresh_slate.mdx)) tokenizer retrain on `corpus-v0.4.0`. Same harness as [A0](./tokenizer-a0-baseline.mdx); the only intended differences are the corpus (v0.4.0 = v0.3.0 + Thread B kryptonite + Thread B2 transliteration) and the country list (widened to cover Thread B2's target locales).
...
- [PHASE_8 §A](../phases/PHASE_8_v0_5_0_fresh_slate.mdx#a-tokenizer-retrain--multi-script--adversarial-coverage) — the plan
