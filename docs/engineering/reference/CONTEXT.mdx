...
- **Phase 2 training metrics** must capture confidence calibration and graceful-degradation rate, not per-component F1 alone. A model that scores well on F1 while reporting overconfident wrong answers on ambiguous inputs fails the success metric.
...
- **Bug-triage process**: when a parser bug is reported, add a **training row**, not a rule or a confidence demerit. The bug report becomes a corpus entry; the model learns from the example without anyone touching the parser implementation. If a bug surfaces a missing schema tag, that's a Phase 0 revisit (see `reference/SCHEMA.md`), not a confidence patch.
