...
Hence Ship of Theseus: migrate component-by-component, conditioned on metrics, never flip a single "neural on" switch.
...
If the architecture handles FR correctly, it will handle DE/IT/ES/PT trivially.
...
- `Saint-Denis` vs `Saint Denis` vs `St. Denis` — same place, three orthographies; the model must learn the alternation rather than pick a canonical and reject the rest

These are not noise. They are the inputs a model must learn to handle, and the inputs a rule parser cannot. The corpus pipeline that produces training data must include them — generated alongside their correct labels (compositional synthesis) rather than relied on the alignment layer to discover them after the fact.

## Success metric: graceful failure

From slide 20 of the same talk:

> People want what their local post office expects. And, that means failing gracefully.
...
Historically the confidence levels and several phrase constraints, confidence boosts, and demerits in rule-classifier-era geocoders are all part of a greater problem. As one builds more and more exceptions and adjustments to a domain-specific machine, one occludes the true nature of how addresses work. And worse, one's mental confidence in how accurately the domain has been mapped reinforces the belief — not with a platonic ideal of well-formed addresses, and not even of the platonic ideal of what a badly formatted address might look like, but with only the addresses which the system happens to struggle with the most. At the beginning, luck is in one's favor and meaningful exceptions surface naturally. But each bug report of a badly parsed address entrenches the system deeper. And if one then tries a new way of parsing, regressions are guaranteed — meaning even more implementation is required.

The principle that inverts this trap, and that should be quoted verbatim wherever this section is referenced:

> _We want the opposite: every additional address should give us confidence in the **possibilities** rather than the **constraints**._

### Operational implications

- **Phase 2 training metrics** must capture confidence calibration and graceful-degradation rate rather than per-component F1 alone. A model that scores well on F1 while reporting overconfident wrong answers on ambiguous inputs fails the success metric.
...
- **Bug-triage process**: when a parser bug is reported, add a **training row** rather than a rule or a confidence demerit. The bug report becomes a corpus entry; the model learns from the example without anyone touching the parser implementation. If a bug surfaces a missing schema tag, that's a Phase 0 revisit (see `reference/SCHEMA.md`) rather than a confidence patch.
- **Golden eval set** must include intentionally-ambiguous addresses where the _correct_ answer is "partial parse + low-confidence flag" — not the unambiguous cases alone.
- **No `confidence_boost` / `confidence_demerit` layer** in the synthesis pipeline or downstream of the model. That's the rule-era implementation the bitter lesson rejects. The model's softmax probability is the confidence; calibration is a training-time concern.
