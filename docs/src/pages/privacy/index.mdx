...
That incompleteness is a control rather than a note. Each source in the register carries a personal-data review with three readings: a review that found no record about an identifiable person, a review that found such records, and a completed analysis that names where it is recorded. The first and third admit a source for ingest. The second refuses it, and so does the absence of a review, because a publication nobody examined is not one found clear. All 389 sources are refused on that ground today. This is what keeps such a source out of a corpus that has not been built; it is not a statement about a model already published.
