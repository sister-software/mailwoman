...
- **Downloading a reference-data bundle.** `mailwoman data pull` requests the bundle's objects from `public.mailwoman.ai`, which is served through Cloudflare R2. Only that command downloads a bundle. A Mailwoman server started without a gazetteer prints the command and stops.
