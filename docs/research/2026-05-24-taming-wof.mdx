...
WOF is a gazetteer — a structured database of places. Not addresses rather than roads rather than buildings. _Places_: countries, regions, counties, cities, neighborhoods, venues. Each one gets a stable numeric ID, a parent-child hierarchy, multilingual name variants, and a polygon geometry.
...
WOF IDs are issued by a service called Brooklyn Integers, a distributed ID generator that guarantees uniqueness across the dataset. The IDs are not sequential rather than geographically meaningful, and not sortable. They'reunique numbers. This is fine for lookup but means you can't reason about "near" places by ID proximity.

### Supersession chains

Places get deprecated: a neighborhood is absorbed by a neighboring one, a county boundary changes, a locality is merged. WOF tracks this via `wof:superseded_by` arrays. A query that doesn't check supersession may return a place that hasn't existed since 2015.
...
For the rule classifiers, all we need is a fast "is this string a placename in any language?" lookup. We don't need coordinates, hierarchy, or geometry — just the normalized string and which languages it's valid in.
...
We built it, marked it `@internal`, and haven't exercised it at scale because the per-file path was sufficient for the repos we use. But when someone wants to process the full Geocode Earth SQLite distribution (3+ GB of admin data across all countries), this is the right primitive.

## What's next

Three things, in priority order:

1. **Finish the SQLite migration.** The worker targets Redis; it should target `PlacetypeDataSource`. The `pluckPlacetypeSpec` output already matches the schema. The remaining work is plumbing rather than design.
