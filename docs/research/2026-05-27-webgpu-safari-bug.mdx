...
The deeper lesson: test infrastructure that lacks GPU access will never catch GPU-specific bugs. Headless browsers are not real browsers for hardware acceleration. If your product runs on GPUs, you need at least one test that exercises the GPU path on a device that has one.

## References

- [Technical reference: the two WebGPU providers](https://github.com/sister-software/mailwoman/blob/main/docs/records/site-2026-08/concepts/onnxruntime-web-webgpu-failure mode.mdx)
