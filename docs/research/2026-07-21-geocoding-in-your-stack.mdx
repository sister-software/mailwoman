...
Mailwoman is a parser and geocoder that runs from a SQLite file and a ~40&nbsp;MB model, entirely on your own machine ([it makes no outbound network request](/research/geocoding-that-never-phones-home)). None of that helps if wiring it in is its own week, though. So the work of the last stretch wasn't the model. It was making the thing install the way the rest of your dependencies do.
...
Same call you already had. It now resolves against a rooftop-level result on your own box, The API returns an OpenCage-style `annotations` block — timezone, coordinate formats, the things upstream Nominatim leaves empty — hanging off `loc.raw`.
...
You bring the gazetteer. The model is in the image, but geocoding needs the admin database, and that's a download or a build. The worldwide candidate file is about 1.4&nbsp;GB, and it's read-only, so it mounts `:ro` and you never think about it again. Until it's in place, the server parses but won't geocode, which is a degrade with a clear message rather than a crash. If you're not sure what's present, `mailwoman doctor` checks the weights, the data root, the gazetteer, and the runtime and tells you the one command to fix each gap.

The other cost is the shape. This is SQLite rather than a cluster, so one instance is one node. That's the point on a single box or inside an app, and it means you scale by running more copies behind a balancer rather than by tuning a shared database. If your plan was a managed Postgres fleet, this isn't that, on purpose.
...
None of this changes what the geocoder does — it's the same parser and the same resolver from a month of eval posts. What changed is the distance between reading this and having an endpoint, which is now a `docker run` or an `npm install` rather than a migration. The address still turns into a coordinate. Itdoes it inside the stack you already have.
