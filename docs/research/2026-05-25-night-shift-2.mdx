...
- **The same training script** ran locally (for smoke tests) and remotely (for the full run) without modification: the configpoints at `/data/`, which is either the local mount or the Modal Volume.
...
- **Class-weighted cross-entropy disabled** — the v0.4.0 recipe change that addresses the 956-FP hallucination problem is now safe to use.
