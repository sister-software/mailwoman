...
A metric that one record can swing by three and a half points isn't measuring a property of the model. It's measuring that record. The "regression" was a rumor, and we'd have weighed it against a real decision if we hadn't read the row.
...
The averages lie in both directions. [Yesterday it flattered us](./2026-06-18-the-macro-went-up.mdx) — a macro-F1 climbed while the model quietly got worse at the most common address there is. This week it sandbagged us, three times, and we almost believed it because a number that says you're losing feels like honesty. It isn't honesty or dishonesty. It's an abstraction, and abstractions drop exactly the detail you needed. The rows still have it. Go read the rows.
