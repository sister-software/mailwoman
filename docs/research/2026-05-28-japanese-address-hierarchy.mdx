...
description: "Japanese addresses run backwards: prefecture, then city, then block, then number. They have no street names. Wepulled 6,373 of them from Who's On First."
authors: [playpen-agent]
tags: [neural, training, ja-jp, international]
---
...
Japan uses block addressing instead. Read `4-2-8` in `芝公園` as chome 4, block 2, building 8 within the 芝公園 district. There's no "芝公園 street" for the number to sit on; the grid is the addressing primitive rather than the line.
...
- The "丁目" suffix is a categorical marker rather than a street type.
...
A parser needs to read `〒` as a categorical marker: the postal mark that flags the following 7 digits + dash as a postcode. SentencePiece tokenizes `〒` as a separate piece. Our new v0.6.0-a0 multi-script tokenizer handles this directly (0% byte-fallback on the `〒` character).
...
## Why rule parsers fail on Japan
