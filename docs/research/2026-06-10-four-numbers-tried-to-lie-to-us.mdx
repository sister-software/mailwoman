...
This is the story of shipping v4.2.0: a consolidation, a bar that lowered itself, a
consultant who was confidently wrong twice, a capacity wall with a number on it,
...
We consolidated every proven change of the parity campaign into one training run: the
unit extract, the affix extract, the country gazetteer anchor, the multi-locale balance.
The core tags soared. Locality up thirteen points, region up eleven, country from nothing
to 87. And the street-affix split, which a solo run hadtaught to 78, crashed
to 27.
...
While writing up the check criteria for the fix attempts, we restated the targets from
...
a interface becomes a vibe, and a vibe always drifts toward whatever you already built.
The operator caught it by asking one question about one number. The fix cost ten
minutes, and the rule it produced is now non-negotiable: **the training config is the
canonical check, and any change to a bar is a stated decision with a reason, in
writing, before the eval runs.** When a model misses the bar, you confront the miss:
re-baseline it with reasons, or iterate. Re-describing it as a pass is how you
wake up six weeks later wondering when street tagging got worse.

## Lies two and three: the consultant's confidence

We consult an external model on consequential forks, and it warrants its keep frequently
enough that we keep asking. This time it was sure, twice, with numbers. The affix
crash was a scheduling problem rather than a capacity problem. Five-times density would clear 72. And the 75 we'd seen in a quick diagnostic? Not a transient.
...
wrote months ago that the first model-size increase would have to warrant its way in with
evidence, and this is what earning it looks like. The escalation now sits in an issue
with a pre-registered check and a cheap probe to try first — a dedicated affix head —
...
_next_ regression has a recorded baseline to measure against. The cards that only
...
whole-parse accuracy. Plausible, right? We'dtraded street precision away, and we
...
Four numbers lied to us in 24 hours: a check that softened itself in retelling, a
...
The implementation that caught them is boring on purpose. Bars written down before looking.
...
or condemns what youbuilt, go read how the measurement was taken before you
react to it.
