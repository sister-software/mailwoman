...
The string-similarity tool gets both of these backwards:
...
Same place, low score — different punctuation, "Street" versus "St", reordered unit.
...
Different places by 650 metres. One character apart. Score: 0.96.

This isn't a tuning problem you can threshold your way out of. It's the wrong coordinate system.
...
Resolving within the provider registry, we used the registry's own entity ID as held-out ground truth. The result was an honest, unflattering number — and then a surprise about that number, which gets its own section below. We also have detail on which levers move real errors and which backfire. Honest negatives are still results.
