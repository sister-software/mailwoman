...
authors: [teffen]
tags: [geocoding, evals, record-matching, motivation]
date: 2026-06-24
---
...
That's the gap we set out to close. Every geocoder chases accuracy, be-right-more-frequently, and so do we. The piece almost nobody hands you is the one underneath it: a number on each answer that tells you _which_ ones to trust, so you can keep the good ones and send the rest to a human.

{/* truncate */}

## The forecaster's interface

Think about a weather forecaster. A good one who says "70% chance of rain" is right on about seven of every ten days they say it. That's the whole interface: the number means what it says. A neural model makes the same promise implicitly every time it hands you a softmax probability, and for a long while ours without output broke it, confident in some bands and timid in others, with no single number you could lean on.

We fixed that part first (it's [its own story](https://github.com/sister-software/mailwoman/blob/main/docs/records/site-2026-08/concepts/confidence-calibration.mdx)). The short version: a small post-hoc correction maps the model's raw confidence onto a calibrated one, so when a parsed span reads `conf="0.94"` it's right about 94% of the time, measured on addresses the fit never saw. When the model says it's sure, now you can believe it.

A calibrated number per _span_ still isn't a number you can act on per _answer_, though. So we carried it the last step, to the resolved coordinate.
...
It's a change rather than a wand. Crank the threshold and you hand more rows to humans; that's the cost, stated directly, and it's yours to set against your own tolerance for a wrong match. What we've done is make the trade legible and put a real number on both sides of it.
