...
But we have one cheap experiment that gets us most of the way there. Hold the tokenizer constant (keep A1), andswap the corpus back to v0.3.0 (the old data, no transliteration mass). That tests whether the transliteration data is the destabiliser, while preserving the result.
