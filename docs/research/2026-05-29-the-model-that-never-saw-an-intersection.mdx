...
And the metric we ship on — **harness pass rate** — didn't move. 14.6% → 13.8%. If anything, slightly down. Two tags (house numbers, streets) _regressed_.

## Following the evidence

A well-calibrated model that's no better at the job is a clue rather than a victory. So instead of tuning the smoothing knob again, we asked a blunter question: **of everything the harness gets wrong, what kind of wrong is it?**

We categorized every failure. The answer reframed the whole project:

- **55% of the gap was missing labels** — the model emitted _no tag at all_ where one belonged. Not a wrong value rather than a fuzzy boundary. Silence.
...
Uncertainty doesn't look like that. A model that's merely unsure still puts _some_ probability on the right tag; ~0.0001 means the model has _no representation of intersections whatsoever_. The labels existed in its output vocabulary; it had never learned to use them.
...
Calibration's one actual win (a small postcode bump) pointed at a _second_ coverage story, this one about tokenization.
...
So the fix for intersections is mundane: a couple thousand synthetic `X & Y St` examples, labeled and dropped into the corpus as a small targeted supplement, plus a retrain that finally gives the model something to learn from. That run is training as we publish this.

We'll report what the model does once it has, for the first time, seen an intersection.
