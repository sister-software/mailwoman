...
Yes, on the labels — and in a way that drove the lesson home twice. We built exactly what the probe pointed at: teach the model bare "City, STATE" rows, so it stops needing a street to recognize a town. Held-out locality F1 climbed from 66 back up to 80, past where it started before any of this. A French house-number tag we'd been stuck on for months came along for the ride, up ten points. The macro-F1? It barely twitched.
