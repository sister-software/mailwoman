...
One honest footnote. The four hard boundary cases we set out to improve, the comma-less cities and the trailing house numbers, barely moved. We chased them, rebalanced for them, and they sat there. More data won't move them. They're a thirty-million-parameter model bumping its own ceiling on the genuinely ambiguous cases, and knowing when to stop pushing a lever is worth as much as the fix. We banked the locality win and left the boundary shapes for a bigger model. That's a different post.
