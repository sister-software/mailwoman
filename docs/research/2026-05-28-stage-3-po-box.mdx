...
The label IDs are stable: STAGE3 appends to STAGE2 without reordering. Old parquet extracts work unchanged — theydon't emit the new tags. Models trained on STAGE2 IDs would still decode correctly against a STAGE3 classifier head; the new logit slots never get picked.
...
2. **Whole-phrase spans** ("PO Box 123" as one `po_box` span rather than "123" alone). Matches the existing golden eval convention.
