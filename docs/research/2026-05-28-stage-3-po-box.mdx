...
The label IDs are stable: STAGE3 appends to STAGE2 without reordering. Old parquet extracts work unchanged — they just don't emit the new tags. Models trained on STAGE2 IDs would still decode correctly against a STAGE3 classifier head; the new logit slots just never get picked.
...
The same recipe scales to street decomposition, intersection, unit, and the JP-specific Phase 6 tags. The schema is already declared. Each new tag is the same kind of work as PO box was tonight.
