...
tags: [neural, rule-based, resolver, geocoding]
---
...
So "neural scores 20.7%" measures one thing: **how frequently neural disagrees with Pelias's exact
conventions** — where to split a multi-word street, where a venue ends and a locality begins, the
dozens of micro-decisions addressit happened to encode. It makes no statement about how frequently neural is
_wrong_. Useful as a regression check (did a retrain break something we used to match?); useless
as a verdict on which parser is better.

## Decomposing the 20%

To judge quality moderately we need benches drawn from _outside_ the Pelias lineage. We score both
parsers on three:
...
- **Messy input → neural wins, decisively** (61% vs 39%) — and this is the biggest bench by
  ...
  reordering, weird casing. It's the closest proxy we have to what people type, and
...
A geocoder's job is to put a real address on the map. So the test that matters is end-to-end:
...
Measure on the distribution you serve. For us that's messy, abbreviated, real-world
addresses — and there, the learned model is ahead.

---

The full breakdown is in the
[v0.7–v0.8 retrospective](https://github.com/sister-software/mailwoman/blob/main/docs/records/retrospectives/v0-7-v0-8-neural-vs-rules-retrospective.mdx): every
arena, the neural deficits (it does truncate `Belle Fourche` to `Belle`), the masked-LM
