...
The federal release blankets the state. New York City sits in its own color because the city publishes its own address file, and OpenAddresses carries it. No guess was made about which is which — the source rode along on every point, from ingest all the way to this map. That's the whole trick, and it's a boring one: a `source` column that nobody dropped.
