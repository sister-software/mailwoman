 ...
 *
 *   This overlay ships exactly ONE artifact — `pair-index-de.bin` — because it declares
...
const PKG_DIR = String(repoRootPath("neural-weights-de-de"))
const CLI = String(repoRootPath("mailwoman", "out", "cli.js"))
const PAIR_INDEX_BIN_DEST = resolve(PKG_DIR, "pair-index-de.bin")
/**
 * Checked-in WOF-derived (Ortsteil, Gemeinde) pairs — the same posture as the GB secondary sources.
 */
const DE_PAIRS_JSONL = String(repoRootPath("data", "gazetteer", "de-pairs-v1.jsonl"))
/**
 * Calibrated magnitudes — the pair the R9 bars were measured at (0/70 confound FPs, 60/60 tag-correct).
 */
...
if (!existsSync(CLI)) {
	console.error(`WARNING: ${CLI} not built — run \`yarn compile\` first, then re-run for pair-index-de.bin.`)
} else if (!existsSync(DE_PAIRS_JSONL)) {
	console.error(`WARNING: missing ${DE_PAIRS_JSONL} — pair-index-de.bin not built.`)
} else {
	...
	if (existsSync(PAIR_INDEX_BIN_DEST)) {
		try {
			...
			if (header.delta !== PAIR_INDEX_DELTA) {
				console.log(`rebuilding pair-index-de.bin — delta ${header.delta} → ${PAIR_INDEX_DELTA}`)
			} else if (header.transitionBeta !== PAIR_INDEX_TRANSITION_BETA) {
				console.log(
					`rebuilding pair-index-de.bin — transitionBeta ${header.transitionBeta} → ${PAIR_INDEX_TRANSITION_BETA}`
				)
			} else {
				needsRebuild = false
			}
		} catch (error) {
			console.log(`rebuilding pair-index-de.bin — header unreadable (${(error as Error).message})`)
		}
	}

	if (!needsRebuild) {
		console.log(`skipped pair-index-de.bin build — ${PAIR_INDEX_BIN_DEST} is current`)
	} else {
		...
		if (result.status !== 0 || !existsSync(PAIR_INDEX_BIN_DEST)) {
			console.error(`FAILED: gazetteer pair-index --country de (exit ${result.status})`)

			process.exit(1)
		}

		console.log(`built pair-index-de.bin ← ${DE_PAIRS_JSONL}`)
	}
}
