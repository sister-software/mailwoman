 ...
 *
 * Every exports entry carries a `node` condition pointing at the source `.ts`
 * (the first-class-TS migration), which is precisely what the alias list used to restate by hand.
 * Restating it meant the two could disagree, and they did: the generic
 * `@mailwoman/core/(.+) -> core/$1/index.ts` rule assumed every subpath was a directory, so each
 * bare-file subpath needed its own earlier entry (`objects`, `fs`, `crypto`, `api/disk-storage`, …)
 * and a new one was a silent `Cannot find package` in unrelated suites until someone added it.
 *
 * The mirror-image gap on `@mailwoman/corpus/(.+) -> corpus/src/$1.ts` mis-resolved
 * the exported `./tools` directory subpath (#1523).
 *
...
export default defineConfig({
	resolve: {
		alias: [
			...
			// onnxruntime-web's `/webgpu` subpath ships browser-only bundles: under Node they
			// fetch() their Emscripten loader as a file:// URL (undici rejects the scheme)
			// and then import() a blob: URL (Node's ESM loader rejects that too).
			// The root export carries a `node` condition with a Node-ready build
			// (fs-based wasm loading), so tests resolve to it.
			// Production imports keep `onnxruntime-web/webgpu`.
			// This alias lives only in the vitest module graph, where WebGPU is unavailable anyway.
			//
			// ASK the package, don't hand-assemble the dist path (2026-08-06 triage).
			// This read `resolvePath(here, "node_modules/onnxruntime-web/dist/ort.node.min.mjs")` —
			// a literal that says the same thing the comment above says, except it says
			// it in a form the package cannot correct.
			// `import.meta.resolve("onnxruntime-web")` applies the `node` condition of
			// the exports map the comment is describing, so an ORT upgrade that renames
			// or relocates the Node bundle keeps resolving.
			// The literal would have silently missed.
			// Verified 2026-08-06 against onnxruntime-web 1.x: it resolves to
			// `dist/ort.node.min.mjs`, the same file the literal named.
			// `exports` has no `./dist/*` subpath, so the root specifier is the only one that reaches it.
			{
	...
	test: {
		...
		// Every test file used to re-transform and re-import the entire aliased workspace graph on its own.
		// Now each fork pays that once.
		...
		exclude: [
			...
			// pick them up and crash on the unfamiliar `test.describe` API. (The build/ entry — the `docusaurus build` health check — was missing here, so it surfaced the moment CI could reach the test phase again.)
			"**/docs/test/browser/**",
			...
			"**/docs/test/build/**",
			"**/docs/test/e2e/**",
			// Agent worktrees under .claude/worktrees/ are isolated git checkouts.
			// Each contains a full copy of the repo's test files.
			...
			// They normally surface as an unexplained `1 skipped`, and at `--maxWorkers=4` one
			// failed the run outright with "No test suite found in file .../legend.test.js".
			// CI never saw it — .venv is not checked in, so a fresh checkout collects 316 files
			// against a working tree's 321 — but it is a real local and agent-worktree flake,
			// and the failure mode is a red run nobody can attribute.
			"**/.venv/**",
			// scratchpad/ holds staged release trees, probe output, and copied source.
			// Same class: not checked in, so invisible to CI, and a landmine locally.
			"**/scratchpad/**",
			// @mailwoman/react's tests are Vitest browser mode only (playwright/chromium via the
			// workspace's own vitest.config.ts + `test:browser`). Importing vitest/browser inside this root forks-pool sweep is a hard error ("can be imported only inside the Browser Mode"), which is exactly what broke CI's Test leg from #1215 onward. CI runs the browser leg as its own step (test.yml "Test react (browser mode)").
			"**/react/**/*.test.{ts,tsx}",
			// @mailwoman/license-worker's tests import `cloudflare:test` and run under the Workers pool through the
			// workspace's own vitest.config.ts (`yarn test:license-worker`); this forks pool cannot load them. CI runs them as their own step, beside the react browser leg.
