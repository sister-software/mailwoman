...
} from "./fst-freshness.ts"
import { FSTMatcher } from "./fst-matcher.ts"
import { FST_FORMAT_VERSION, serializeFST } from "./fst-serialize.ts"
import type { FSTProvenance } from "./fst-types.ts"

const TMP = mkdtempSync(join(tmpdir(), "fst-freshness-"))
...
function writeFST(name: string, provenance?: FSTProvenance): string {
	const path = join(TMP, name)
	writeFileSync(path, serializeFST(fixtureMatcher(), provenance))
...
const SOURCE = join(TMP, "source.db")
...
describe("md5FileSync", () => {
	...
	it("hashes a file LARGER than one read chunk identically", () => {
...
describe("readWOFSourceIdentity", () => {
	it("computes and caches the digest in an md5sum-format sidecar", () => {
		const path = join(TMP, "sidecar-source.db")
		...
		expect(readFileSync(`${path}.md5`, "utf8")).toBe(`${identity.md5}  sidecar-source.db\n`)
	})

	it("recomputes when the sidecar is OLDER than the source", () => {
		const path = join(TMP, "stale-sidecar.db")
		writeFileSync(path, "one")
		writeFileSync(`${path}.md5`, "00000000000000000000000000000000  stale-sidecar.db\n")
		// Sidecar predates the source: the guard must not trust it.
		utimesSync(`${path}.md5`, new Date(1000), new Date(1000))

		expect(readWOFSourceIdentity(path).md5).toBe(md5FileSync(path))
	})

	it("trusts a sidecar at or after the source's mtime without reading the source", () => {
		const path = join(TMP, "trusted-sidecar.db")
		writeFileSync(path, "two")
		const lie = "11111111111111111111111111111111"
		writeFileSync(`${path}.md5`, `${lie}  trusted-sidecar.db\n`)
...
describe("peekFSTStampFields", () => {
	it("reads the format version and the provenance trailer", () => {
		const path = writeFST("stamped.bin", provenanceOf({ sourceDBMD5: "deadbeef", sourceDBBytes: 12 }))
	...
	it("reports no provenance for an artifact serialized without one", () => {
		const fields = peekFSTStampFields(writeFST("unstamped.bin"))

		expect(fields?.formatVersion).toBe(FST_FORMAT_VERSION)
		expect(fields?.provenance).toBeUndefined()
	})

	it("returns undefined for a non-FST file, a stub, and an absent path", () => {
		const notFST = join(TMP, "not-an-fst.bin")
		writeFileSync(notFST, Buffer.alloc(64, 0x7f))
		expect(peekFSTStampFields(notFST)).toBeUndefined()

		const tooSmall = join(TMP, "tiny.bin")
		writeFileSync(tooSmall, Buffer.from("FST\0"))
		expect(peekFSTStampFields(tooSmall)).toBeUndefined()

		expect(peekFSTStampFields(join(TMP, "nope.bin"))).toBeUndefined()
	})

	it("survives a truncated trailer instead of throwing", () => {
		const path = writeFST("truncated.bin", provenanceOf({ sourceDBMD5: "deadbeef" }))
		const bytes = readFileSync(path)
		// Cut into the JSON: the declared length now runs past EOF.
		writeFileSync(path, bytes.subarray(0, -20))
...
describe("fstStaleReason", () => {
	...
	it("passes an artifact stamped with the current source", () => {
		const path = writeFST(
			"current.bin",
	...
	it("flags a source md5 that has moved, and names the build date", () => {
		const path = writeFST(
			"moved.bin",
	...
	it("distinguishes 'no stamp at all' from 'stamp without a checksum'", () => {
		...
		expect(fstStaleReason(peekFSTStampFields(writeFST("bare.bin")), expected)).toBe("carries no build stamp")

		const unchecksummed = writeFST("no-md5.bin", provenanceOf({ sourceDB: "/some/where.db" }))
		expect(fstStaleReason(peekFSTStampFields(unchecksummed), expected)).toContain("no source checksum")
	})

	it("flags a format older than the tree writes even when the source matches", () => {
	...
	it("flags a size disagreement at a matching md5", () => {
		const path = writeFST(
			"size-skew.bin",
	...
	it("flags a changed exclusion policy only when the caller supplies one", () => {
		const path = writeFST(
			"policy.bin",
			provenanceOf({
...
describe("fstFreshnessWarning", () => {
	it("names the artifact, the reason, and the rebuild command", () => {
		const path = writeFST("warn.bin", provenanceOf({ sourceDBMD5: "0".repeat(32) }))
	...
	it("is silent for a current artifact", () => {
		const path = writeFST(
			"quiet.bin",
			provenanceOf({ sourceDBMD5: SOURCE_IDENTITY.md5, sourceDBBytes: SOURCE_IDENTITY.bytes })
		)
	...
	it("is silent when either side is absent — a missing file is a different report", () => {
		expect(
			fstFreshnessWarning({ fstPath: join(TMP, "gone.bin"), sourceDBPath: SOURCE, rebuildCommand: "x" })
		).toBeUndefined()

		expect(
			fstFreshnessWarning({
				fstPath: writeFST("orphan.bin", provenanceOf()),
				sourceDBPath: join(TMP, "gone.db"),
				rebuildCommand: "x",
			})
		).toBeUndefined()
	})
