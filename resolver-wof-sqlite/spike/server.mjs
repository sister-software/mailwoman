/**
 * Tiny static HTTP server with explicit Range-request support. Necessary because sql.js-httpvfs
 * issues byte-range fetches against the .db file; without proper Range handling the VFS errors out
 * or silently downloads the whole file (defeating the entire premise).
 *
 * Logs every request to stdout (one JSON line per fetch) so the orchestrator can aggregate the
 * traffic pattern. Designed for spike use; not production-grade.
 */
...
function safeJoin(root, urlPath) {
	...
	if (!joined.startsWith(root)) return null // path traversal guard
	return joined
}
