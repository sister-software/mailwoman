 ...
 *
 *   The CLI proper — everything `cli.ts` defers until its process-wide preamble has run. Reached only through
 *   `cli.ts`'s dynamic import, never as an entry point of its own: the bin is `out/cli.js`, and importing this
 *   module directly skips the V8 compile cache and the React build selection that make the CLI usable.
 */
