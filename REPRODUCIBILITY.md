...
modal run -d corpus-python/launch/train_remote.py --config corpus-python/src/mailwoman_train/configs/v1.0.2-consolidation-runB.yaml --resume auto

# 2. Export ONNX (on Modal — local onnxruntime can trip ShapeInferenceError on dynamo graphs)
modal run corpus-python/launch/train_remote.py::export_onnx --output-dir=/data/output-v101-runB-s42 --step=020000
