...
npx tsx packages/corpus/scripts/fetch-sources/fetch-state-sources.ts
npx tsx packages/corpus/scripts/fetch-sources/fetch-hrsa.ts
...
| --------------------------- | --------------------------------------------------------------------------------- | -------------------------- |
| `fetch-hrsa.ts`             | HRSA Health Center Service Delivery Sites (federal)                               | A (US PD)                  |
| `fetch-imls-pls.ts`         | IMLS Public Libraries Survey — outlet-level (~17K library branches, FY 2023)      | A (US PD)                  |
| `fetch-nppes.ts`            | NPPES NPI registry — full monthly dissemination (~7M provider venue+address rows) | A (US PD)                  |
| `fetch-state-sources.ts`    | NY/TX/DE/OR notaries, IA contractors, WA health providers, HI lobbyists           | A (state PD-equivalent)    |
| `fetch-state-hi-schools.ts` | Hawaii DOE school directory (XLSX → CSV via openpyxl)                             | A (state PD-equivalent)    |
| `fetch-openaddresses.ts`    | OpenAddresses country collections (default: Canada / `ca`)                        | B/C mixed — per-row filter |
...
are all Tier A except `fetch-openaddresses.ts`, which is a **Tier-mixed**
...
`fetch-openaddresses.ts` reads `OA_BATCH_TOKEN` from the environment:
...
OA_BATCH_TOKEN=$OA_BATCH_TOKEN npx tsx corpus/scripts/fetch-sources/fetch-openaddresses.ts --country fr
```
