...
export const weights = {
	...
	overFar: 8,
	/**
	 * The last line is charged this fraction of the shortfall, so a paragraph
	 * ends balanced rather than stranded.
	 */
	lastShort: 0.25,
	/**
...
function measure(tokens: readonly string[], tabWidth: number): TokenFacts[] {
	...
	return tokens.map((token, index) => {
		...
		const clause =
			/[,;:][)"'\]`]*$/.test(token) ||
			DASH.test(token) ||
			(next !== undefined && CONJUNCTIONS.has(next.toLowerCase().replaceAll(/[^a-z]/g, "")))
...
export function reflowBlockComment(
	...
	if (original.length === 1) {
		...
		if (middle.every((line) => /^\s*\*(?: |$)/.test(line))) {
			...
			plainPrefix = indents.toSorted((a, b) => a.length - b.length)[0] ?? indent
