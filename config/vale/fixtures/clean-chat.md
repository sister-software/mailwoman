...
row, which asserts that a southeast direction narrows the candidate set; the
direction is dropped at tokenization, before the resolver runs. The fix
belongs in the tokenizer, and the row stays red until then.

The compass bearing is 42 degrees east of north.
