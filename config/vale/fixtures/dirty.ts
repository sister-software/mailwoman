...
// Here it is: `the gate` and `a seam`. The loader reads each row, which validates the fields
// and records the result, so the report can compare runs.

// A dosage is not a metric; describe the measured rows. Anchored, not bare.

// `ShellNoun.yml` runs over source comments through the same config, and each clause
// below must trip it: the probe confirmed the guard, the guard holds, we recover the win
// and keep the affix win, the cost was carried entirely by one class, 3-digit is the
// reduce, and we take this to 8k. The probe validated the guard, the result stands,
// the gain held, and the regression was carried by postcode tokens.

// The provenance grade stored on a zoning row. Exactly one per row, and the two never merge.
// ONE ARTIFACT HOLDS ONE GRADE.

// `EmphasisCapitals.yml` refuses an ordinary word set in capitals between lowercase
// neighbours, and each shape below must trip it: the parser is RESTRICTED to one locale,
// the row is ABSENT. NOT a ship. The prior is ON by default, and the run was a PURE WIN.
