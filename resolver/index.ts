 ...
 *
 *   `@mailwoman/resolver` — the address resolver implementation, lifted out of `@mailwoman/core` (#215)
 *   so it can depend on `@mailwoman/spatial` (haversine) + `@mailwoman/codex` (USPS directionals)
 *   instead of reinventing them. The TYPE contract stays in `@mailwoman/core/resolver` (so the
 *   `core/pipeline` composes the resolver structurally without a package cycle); this barrel re-exports
 *   it, so `@mailwoman/resolver` is a complete drop-in for what used to be `@mailwoman/core/resolver`.
 */
