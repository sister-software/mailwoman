...
// identical canonicalization rule to tell apart which target node's edge names a given family_id,
// and re-deriving it a second time independently would be exactly the "two definitions
// that can drift" hazard guards.ts's own extraction already closed for assertISODate.

/**
 * The one `filer_family` row {@linkcode insertFamilyMembership} writes, described in the
 * builder's own terms rather than the table's: `memberNodeID`/`namingNodeID` are the
 * two ends of the `HoldingCompany`/`ManagementCompany` edge this row accompanies, and
 * `identifierType`/`name` are what {@linkcode mintFamilyID} canonicalizes into the `family_id`.
 */
export interface FamilyMembershipFact {
	...
	assertion: string
	/**
	 * The inferred match's score; `null` on an authoritative membership, where nothing was matched
	 * (the schema's own check constraint rejects a score there — see `createFilerFamilyTable`).
	 */
