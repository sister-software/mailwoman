...
export interface ParsedSubsidiary {
	name: string
	/**
	 * Omitted (never an empty string) when Exhibit 21 gave no jurisdiction for this row/line —
	 * decision 6: absence of a jurisdiction is not itself something to guess at.
	 */
	jurisdiction?: string
}
...
export interface ParsedExhibit21 {
	...
	/**
	 * Rows/lines this parser recognized as an entry (a table data row, a list item, a non-blank text line)
	 * but could not confidently reduce to a subsidiary name — decision 6: counted and dropped,
	 * never guessed at, and never thrown as an error either (criterion 3).
	 */
	unparseable: number
}
...
function subsidiariesFromTable(
	...
	for (const [rowIndex, row] of rows.entries()) {
		...
		// A header/decoration row using <td> instead of <th> — recognized the same way,
		// but counted, since this is a content judgment rather than markup certainty
		// (decision 6: count a judgment call, don't discard it for free).
		...
		if (mapping) {
			...
			if (!name && mapping.name < mapping.jurisdiction) {
				// An indented corporate tree: the child's name sits in a column to the right of
				// the labelled name column but still left of the labelled jurisdiction column.
				// The nesting depth is discarded (an Exhibit 21 row is a registrant→subsidiary edge either way);
				// the name itself is not in doubt, because the header says the jurisdiction is to its right.
			...
			// Otherwise fall through to the generic rules rather than abstaining:
			// a ragged table (`anterix-2025.htm`'s 5- and 6-cell rows under a 6-cell header)
			// misaligns the mapping without making the row unreadable.
		}
...
/**
 * Classifies every top-level table in document order and concatenates the results, carrying
 * each table's column mapping forward to its siblings until another header row replaces it —
 * edgar splits one logical table across page-break tables constantly, and only the first
 * carries the header (`att-2025.htm`'s second table holds AT&T Mobility, Cricket Wireless,
 * Teleport Communications America and BellSouth Telecommunications with no header of its own).
 */
...
/**
 * Shared by the list and plain-text strategies: strip a leading list marker,
 * abstain on a title/heading line or a line too long to be one name,
 * split every remaining candidate line, count a blank result as `unparseable`
 * (the only abstention these two strategies ever make on the name — see the module docstring),
 * apply the same header/decoration-row check the table strategy uses
