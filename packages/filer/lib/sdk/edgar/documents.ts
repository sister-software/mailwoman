...
/**
 * One document {@linkcode parseFilingDocuments} reads out of a filing's sgml manifest —
 * `type` and `filename` exactly as edgar's own `&lt.type&gt.`/`&lt.filename&gt.` manifest
 * lines spell them (never normalized/uppercased — see {@linkcode EXHIBIT_21_TYPE_PATTERN} for
 * why matching stays case-insensitive instead of relying on a canonical spelling), plus the
 * absolute archive `url` this module derives ({@linkcode accessionArchiveURL} + `filename`).
 */
 ...
 * (`EX-21`, `EX-21.1`, `EX-21.01`, lowercase `ex-21.2`, …) while rejecting a
 * type that merely starts the same way.
 *
 * `EX-2`, `EX-2.1`, `EX-210`, `EX-23`, `EX-21A` are all distinct exhibits
 * rather than a spelling variant of Exhibit 21.
 *
...
/**
 * Fetches one filing's accession manifest (`{@linkcode accessionArchiveURL}(filing.cik, filing.accessionNumber)`
 * joined with `${filing.accessionNumber}-index-headers.html`, through the shared
 * {@link SECDocumentClient} — `exhibit21.ts`'s one-method structural type rather than the concrete
 * SEC client, so a test never needs an axios harness) and returns its Exhibit 21 documents.
 *
