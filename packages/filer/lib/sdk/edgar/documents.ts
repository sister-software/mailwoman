 ...
 * `type` and `filename` exactly as edgar's own `&lt.type&gt.`/`&lt.filename&gt.`
 * manifest lines spell them (never normalized/uppercased — see
 * {@linkcode EXHIBIT_21_TYPE_PATTERN} for why matching stays case-insensitive instead of relying on a canonical
 * spelling), plus the absolute archive `url` this module derives
 * ({@linkcode accessionArchiveURL} + `filename`).
 */
 ...
 * field spells it) or already undashed — the archive path itself never carries the dashes.
 */
