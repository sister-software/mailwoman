...
/**
 * One `company_tickers.json` row, exactly as `parseCompanyTickers` reads it off the wire — every value
 * optional/ `unknown`-typed here because this is untrusted third-party JSON rather than yet validated.
 */
 ...
 * structural mismatch (decision 8's "malformed input must be loud" discipline,
 * carried from `form499.ts`/`provider-list.ts`) — this is SEC's own canonical reference file,
 * so a row that doesn't fit the documented shape is a real signal something changed
 * upstream rather than a case worth silently skipping.
 */
 ...
 *
 * There is no ticker column (the empty third field is always blank). ~1,054,085 entries
 * covering 40 MB. read the file once and keep the result rather than reparsing it per query.
 *
 * An entry whose CIK won't parse is skipped without throwing — this is a flat file rather than
 * SEC's documented API shape, and a malformed line is the rule rather than the exception.
...
export interface CIKCandidate {
	 ...
	 * means the two names are identical once legal designations are stripped — which is
