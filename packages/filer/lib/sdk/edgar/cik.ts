 ...
 *
 * Branded over `string`, mirroring {@linkcode FRN}'s (`frn.ts`) identical rationale.
 * A bare, unpadded numeric CIK would collide with itself under a naive string
 * comparison once padding is inconsistently applied.
 */
 ...
 * {@linkcode fetchTenKFilings} take this rather than the concrete class so a test can
 * substitute a trivial stub instead of building a full axios harness.
 *
 * A real `createSECClient()` instance already satisfies this structurally.
 * The production caller always passes one, so this stays "go through the existing
 * SEC client", never a second fetcher.
 */
...
/**
 * One `company_tickers.json` row, exactly as `parseCompanyTickers` reads it off the wire.
 *
 * Every value optional/ `unknown`-typed here because this is untrusted third-party JSON
 * rather than yet validated.
 */
...
export interface CIKCandidate {
	...
	ticker: string
	/**
	 * Similarity in `[0, 1]` between the query name and this candidate's `companyName`,
