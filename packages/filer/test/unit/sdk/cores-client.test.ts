...
describe("parseCORESRegistration — real CORES detail pages", () => {
	it("reads every stated field off the Knology/WOW! record", async () => {
	...
	it("reads the `ContactPhone:` label, which the page ships without its space", async () => {
		expect(parseCORESRegistration(COMCAST_FRN, await fixture("frn-0003768165-comcast.html"))?.contactPhone).toBe(
			"(215) 286-4040"
		)
	})
...
describe("parseCORESRegistration — abstention", () => {
	it("returns null, never a stub record, for a page carrying no registration table", async () => {
		expect(parseCORESRegistration(UNKNOWN_FRN, await fixture("frn-0000000000-no-record.html"))).toBeNull()
	})
