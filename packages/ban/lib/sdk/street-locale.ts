...
/**
 * ISO-3166 alpha-2 (lowercase) → the street-normalization locale a BAN extract was built with.
 * FR-only: BAN is the French national register. Adding a country here means shipping
 * that country's national register on the shared schema and having a matching branch in
 * `normalizeStreetForKeyLocale` — never a silent fold with the wrong rules.
 */
 ...
 * until a probe misses. Add the country to {@link BAN_COUNTRY_TO_STREET_LOCALE}
