...
export class BANRegionDatabaseProvider implements Disposable {
	 ...
	 *
	 * Awaits `pathExists` for each supported country's rooftop extract and its derived
	 * street-centroid tier, recording the paths that exist so `for` never touches the filesystem.
	 * Safe to call more than once: the probe promise is cached, so every caller
	 * (and {@linkcode BANRegionDatabaseProvider.create}) shares one pass.
	 */
