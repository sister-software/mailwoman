...
test.describe("Demo — structural render", () => {
	test("paints the page shell, map, about box, and form on load", async ({ demo, page }) => {
		...
		// Address form: label and field. The pill carries no submit control — a `type="search"`
		// field submits on Enter. `exact` is required: `getByLabel` matches by substring,
		// and the chrome names the search wrapper "Search addresses" and the chip row
		// "Example addresses", so a bare "Address" resolves to three elements.
		await expect(page.getByLabel("Address", { exact: true })).toBeVisible()
		await expect(page.locator("#mw-pipeline-input")).toBeVisible()

		// Example chips row. `MapChipRow` renders no caption — the row carries its name
		// as `aria-label` where the pre-chrome `PresetChips` wrote a visible "Try:"
		// span, so the row is read by role and name.
		...
		// in the sheet that control opens — `MapControlButton` carries its name
		// as `aria-label` on an icon button, so the name is a label and not text.
