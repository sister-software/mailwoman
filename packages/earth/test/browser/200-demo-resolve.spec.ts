...
test.describe("Demo — resolution cascade", () => {
	...
	test("ZIP-only example drops a marker", async ({ demo }) => {
		...
		// The cascade may resolve via postcode or fall back to locality if the postcode
		// has placeholder coords. Either way: a marker.
		expect(markerCount).toBeGreaterThan(0)
	...
	test("German address — postcode 10115 country-restricts into Berlin rather than New York", async ({ demo }) => {
		// Regression for the candidate-table cascade: 10115 is both a Berlin DE postcode
		// and a New York US ZIP, and the gazetteer now carries US + DE/FR/EU postcodes.
		// The locality must resolve first (Berlin → DE by population) and country-restrict
		// the postcode, so it resolves to the DE 10115 point — IN Berlin — never the NYC ZIP.
	...
	test("Canadian address — postcode M5H 2N2 resolves into Toronto (the -20f CA coverage)", async ({ demo }) => {
		// -20f folds Canada's Overture divisions into the admin gazetteer
		// (Toronto, Montréal, … — absent before) plus 843k CA postcode centroids.
		// So "Toronto" resolves to Ontario (top by population), the cascade country-restricts to CA,
		// and the CA postcode is reachable. Grade the coordinate: downtown Toronto ≈ 43.6,
		// -79.4 — not Toronto, Ohio (40.46), where it landed pre-CA-admin.
	...
	test("Russian address — Moscow resolves to Russia rather than Moscow, Idaho (-20g world coverage)", async ({
		...
		// So the English "Moscow" (an alias of Москва) resolves, and the 10.4M-pop RU city outranks
		// the 26k-pop US homonym. Grade the coordinate: Moscow ≈ 55.7, 37.6 — not Idaho (46.7, -117).
	...
	// countries had no postcode tier on the demo at all. Each case feeds locality +
	// postcode + country and grades the resolved coordinate against the city bbox:
	// the cascade country-restricts and the new postcode (or its locality) is reachable.
	// Pairs with the v4.14.0 AU model (postcode-first format).
