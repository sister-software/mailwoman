...
test.describe("Demo — model-visualizer debug drawer", () => {
	...
	test("/ has no drawer until dev mode is toggled", async ({ demo, page }) => {
		...
		// The trace toggle sits behind the Developer capsule in the map chrome
		// and reads "Trace the decode path" — the two display toggles moved there
		// so they stop heading every result. Open that sheet, then check the box.
