...
// of every command's flags, and a strict parse rejects the first one it has not heard of — `--json`,
// `--locale`, `--debug` — before dispatch, which is the whole CLI rather than one command.
// Unknown options stay in the vector for the command router to interpret.
...
// A root version request, which is why the positional count is required:
// `--version` reached from anywhere in the vector would make the launcher answer
// `mw geocode --version` itself, swallowing a flag that belongs to the command.
