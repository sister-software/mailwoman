...
// `#geocode/worker` is deliberately absent. It is a worker-thread entry rather than a
// library module: its body destructures `workerData` at import, so pulling it through
// this barrel runs it on the main thread and throws before any test reaches an assertion.
// Entries are spawned by path. only `stream.ts` names it.
