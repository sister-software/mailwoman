...
export interface GeocodeDeps extends LayerDesignationRoutes {
	...
	/**
	 * OPT-IN venue tier (#1684's POI half): upgrade a venue-led address's admin/street answer
	 * to the poi.db entity with the venue's exact name-key near the resolved anchor.
	 *
	 ...
	 *
	 * The street-miss fallback retries a mis-tagged bare toponym as a locality,
	 * and a bare-locality retry must run under the #912 posture — the inferred scope
	 * withheld — while an explicit scope stays supreme.
	 * Only the caller knows which.
	 * The CLI threads it.
	 */
	defaultCountryIsInferred?: boolean
	/**
	 * Lexicon-aware kind classifier (#1649) — `createKindClassifier({ poiLexicon })`
	 * from `@mailwoman/kind-classifier`.
	 *
	 * When present it gets first refusal on the query: a top-slot POI/category/near-me
	 * verdict abstains from the address lanes with the intent markers attached,
	 * instead of manufacturing a confident wrong answer from a thing-query.
	...
	resolver: Resolver
	/**
	 * Explicit input register (Decision A / GTM B10 — canonical docs on `@mailwoman/core/pipeline`'s `InputMode`).
	 *
	 * Unset → derived per parse from the kind classifier's verdict.
	 ...
	 *
	 * Consulted only when no US per-state situs database matched (a non-US parse), and ahead
	 * of {@link osmDatabases}: a national register is denser + coordinate-authoritative,
	 * so it outranks the community OSM fallback.
	 * Inject from `@mailwoman/ban`'s `BANRegionDatabaseProvider`; absent = no national tier.
	 *
	 * Licence Ouverte/Etalab (permissive) — see `ban/readme.md`.
	 * The shape generalises to other national registers.
	 */
	nationalDatabases?: (country: string) => RegionDatabases
	/**
	 * OSM rooftop databases keyed by ISO-3166 alpha-2 country (#247) —
	 * the opt-in international precision tier.
	 *
	 * Consulted only when no US per-state situs database matched (a non-US parse)
	 * and no {@link nationalDatabases} register covered the country, so the US path
	 * is untouched and BAN wins where it exists.
	 * Inject from `@mailwoman/osm`'s `OSMRegionDatabaseProvider`; absent = no OSM tier.
	 ...
	 *
	 * Absent = the geocode result is byte-identical to a run without the field existing
	 * (the consult never happens, the `authoritative` block never appears).
	 ...
	 * (`--locale en-US "Zürich"` scoped to US answers Zurich, Kansas).
	 *
	 ...
	 *
	 * The #694 comma-less crater was the space-join rather than the casing,
	 * so on comma-joined input it is a clean win.
	 * Set `false` to restore the legacy raw-case parse.
	 */
	 ...
	 * but the drop-in servers (nominatim/photon) call `geocodeAddress` directly.
	 *
	 * Without it a double-spaced / odd-punctuation query was fragile (`"Damrak 1, 1012 LG"` → unresolved).
	 *
	 ...
	 * (the address's single most expensive step).
	 *
	 * Supply the output of {@link parseForGeocode} when a caller already parsed the same address
	 * for another purpose — a PostalAddress, say — so the inference runs once rather than twice.
	 * Must come from `parseForGeocode` (same input + opts), or the resolved tree won't match the address.
	 *
	 * Omit for the normal one-shot path.
	 */
	...
	/**
	 * Coarse country router (#244, soft prior).
	 *
	 * A `(text) → { country, confidence }` predictor.
	...
	/**
	 * Admin descendant-consistency (#263, `ResolveOpts.adminCoherence`) —
	 * re-pick a (region, locality) pair so the locality descends from the region
	 * ("Portland, ME" → Maine rather than Messina). **Default-on** for the geocode path.
	 *
	 * Only fires when a region's child locality fell through, so the well-resolved path is byte-identical.
	 *
	 * Pass `false` to opt out.
	 */
	...
	/**
	 * Re-probe a resolved-nothing lookup across the other admin bands and record which hold it (#1741/#1756).
	 *
	 * Diagnostic — the answer never changes, only the record of why it was not found.
	 *
	 * Requires `resolveTraceSink`.
	 */
	diagnoseUnreachable?: boolean
	/**
	 * Lineage attachment (#404, `ResolveOpts.includeAncestors`) — stamp each resolved
	 * node's containment chain onto `metadata.ancestors`, which is what puts region-class
	 * ancestry in front of the admin-coherence verdicts (#1717): without it `region`
	 * reads `unverifiable` on every winner. **Default-on** for the geocode path —
	 * the stamp is flag-only metadata (no ranking or re-pick reads it) served from a per-id
	 * memoized backend probe, and it no-ops on a backend/artifact without `ancestors()`.
	 *
	 * Pass `false` to opt out.
	 */
	includeAncestors?: boolean
	/**
	 * Postcode-country coherence (#42, `ResolveOpts.postcodeCountryCoherence`) —
	 * let a (postcode, locality) pair that is geographically consistent in exactly
	 * one country override a wrong {@link defaultCountry}.
	 *
	 * `12 Rue de Rivoli, 75001 Paris` under the en-US locale otherwise lands in Texas,
	 * and with the postal databases attached in Addison. **Default on**
	 * (operator-promoted 2026-08-05 — gauntlet zero newly-failing conditional cases pinned
	 * either way, 56,000 pair evaluations across both backends at zero false positives.
	 * See `docs/records/evals/2026-08-05-postcode-coherence-default-on-evidence.md`).
	 * Pass `false` to opt out.
	 *
	 * On this path it also re-selects the rooftop tier.
	 * Database selection happens before the resolve and keys off `defaultCountry ?? placedCountry`,
	 * so a US-scoped call would pick no national/OSM database and leave the
	 * corrected FR address at its commune centroid.
	 *
	 ...
	 * **Default off** — demotion is the failure mode with teeth.
	 *
	 * Pass `true` to opt in (the pre-registered B1 criterion set lives in `resolver/postcode-shape-coherence.ts`).
	 */
 ...
 * A trace built from re-derived opts would describe a decode nobody ran.
 *
 * Sharing this function is what makes the trace a receipt for the tree
 * instead of a plausible reconstruction of it.
 */
export interface GeocodeParseInputs {
	 ...
	 *
	 * Absent when the caller pinned a register.
	 * Nothing was classified, and reporting a kind here would be inventing one.
	 */
...
export function geocodeParseInputs(
	...
	// #1002: expandAbbreviations with the locale-unknown safe set (Bd/Bvd/Av/Imp → the expanded street type). The model mis-parses undertrained FR abbreviations ("2 Bd du Palais" → house_number "2 Bd", which then fails the point-tier number match); the EN suffixes are deliberately not expanded (the model is trained-robust on them, and St/Dr are ambiguous with Saint/Doctor). The locale isn't known pre-parse, so only the collision-free multi-locale entries apply — see LOCALE_UNKNOWN_DICT.
	const parseInput = deps.normalizeInput === false ? input : normalizeGeocodeInput(input, deps.classifier).normalized

	// #981: apply the query-shape emission prior the runtime pipeline applies (core/pipeline/runtime-pipeline.ts:336 `computeQueryShape` → `safeClassify` → parse with `queryShape`). Without it the geocode path — the drop-in servers (nominatim/photon `/api`) + the geocode CLI — diverged from the pipeline: a detected known-format span (`nl_postcode` → `B-postcode`, …) or a US region abbreviation never biased the emissions here. Computed on `parseInput` (the exact text handed to the model), matching the pipeline (which computes it on the normalized text, before the classifier's internal case-normalization). It is a no-OP whenever the shape carries no known format and no region abbreviation (the bare `street, city` class) — `buildEmissionPriors` returns an all-zeros matrix — so both bare-form and well-formed inputs are byte-stable. It warrants its keep only on the ambiguous digit-span / region-abbrev cases the model isn't already confident about.
	const queryShape = computeQueryShape(parseInput)

	// Decision A: explicit register wins.
	// Otherwise the kind verdict decides (same derivation as the runtime pipeline —
	// the drop-ins + geocode CLI reach parse through here rather than runPipeline).
	...
	return {
		...
		opts: {
			...
			// #1497: the gazetteer prior. `classifier.parse` reads `fst` from opts only, with no config fallback, so a path that never passes it never constructs the prior. It is not a weaker decode, it is a different one. Absent `deps.fst` this spread is empty and the decode is byte-identical to before.
 ...
 * (`normalizeInput`) → `classifier.parse` (postcodeRepair + normalizeCase) →
 * `recognizeUSRegions` → {@link recognizeBarePostcode}.
 *
 * Exposed so a caller can run it once and feed the result to both {@link geocodeAddress}
 * (via `GeocodeDeps.parsedTree`) and another consumer of the parse
 * (e.g. `decodeAsJSON(tree)` → a PostalAddress), instead of parsing the same address twice.
 * The inference is ~3 ms/row — the single most expensive step — so sharing it is
 * a ~1.3× win on a parse-then-geocode pipeline.
 */
export async function parseForGeocode(
	...
	// #22: a bare unambiguous postcode the model read as a street ("N7 0BT" → `{ street: … }`) is retagged before the resolve — see recognizeBarePostcode for why nothing downstream can recover it.
	const tree = recognizeBarePostcode(recognizeUSRegions(await classifier.parse(parseInput, opts)))

	// #1735: the multi-node generalization of #22. A letter-digit postcode span the model split into street + house_number ("KT2 6AB") is repaired from the shape stage's own span. Runs here, before any caller derives scope from the tree: the session's bare-postcode guard must see the repaired tree, or a locale-inferred country filter starves the lookup the repair exists to enable.
	repairPostcodeContradiction(tree, queryShape)

	// #1747: a `street_suffix` with no `street` anywhere, abutting a place name, belongs to that name — `Brixton Hill` parsed as locality `Brixton` + a floating `Hill` and resolved 300 km away. Runs after the postcode repairs so it judges the final tree, and its adjacency guard leaves a genuine one-word street untouched.
...
export async function geocodeAddress(input: string, deps: GeocodeDeps): Promise<GeocodeOutcomeLike> {
	// #1649 first refusal — before the resolve and before the register-flip retry rider, so a refused thing-query can neither resolve nor be retried into nonsense. See intent-refusal.ts.
	if (deps.classifyKind && !deps.inputMode) {
		...
		if (refusal) {
			...
			// `intent_markers` said why we abstained.
			// Nothing said what we understood, and a caller could not tell a refusal from
			// a parse that found nothing without reading the marker.
			//
			// No extra parse on any production path — the session computes the tree once
			// and threads it as `parsedTree` (geocode-session.ts:679).
	...
	// lived here from 2026-07-28 to 2026-08-19 and was retired under the #486
	// repair-retirement policy with a measured record of exactly zero: no effect on
	// the full board, none on its 199-row failure subset (counterfactual sweep),
	// and none on 300 fresh BAN + 300 fresh fdic register records.
	// The misrouted- record class it was specced for. #1694 holds the receipts.
	return geocodeAddressOnce(input, deps)
}

/**
 * Thread the address's country evidence into the walk's options.
 *
 * One path for the whole precedence chain: an explicit caller scope is supreme.
 * An inferred scope yields to the #1684 check.
 *
 * Postcode-format countries (#1589) reach the scoped `postalcode` probe.
 * The fuzzy tier's locale scope (#1585) and the soft locale prior (#27) thread beneath.
 */
function applyCountryEvidence(opts: ResolveOpts, tree: AddressTree, deps: GeocodeDeps): void {
	// #1589: the parsed postcode's format-implied countries. Computed before the scope block so the scope check can read them. Threaded to the resolver either way.
	const formatCountries = countriesFromPostcodeFormat(treePostcodeValue(tree))

	if (deps.defaultCountry) {
		// #1684 conditional scope: a locale-inferred scope yields to the model's own confident contrary read of the text, or to a postcode format that excludes the inferred country (see shouldDropInferredScope). The scope is dropped, never re-pointed. The worldwide race with cross-country primary preference + fame decides, which is the behavior the graded scope=none arm measured on exactly this class ("Nanjing Road, Huangpu, Shanghai" was a West Virginia namesake under the inferred filter). An explicit caller scope never enters here.
		if (shouldDropInferredScope(tree, deps.defaultCountry, deps.defaultCountryIsInferred === true, formatCountries)) {
			// No hard scope.
			// The postcode-format block below may still scope, which is the documented
	...
	// #1589: the format-implied countries also reach the resolver's scoped `postalcode` probe. The resolver applies them only when no explicit country selection applies — which keeps an explicit defaultCountry supreme over format evidence, which in turn outranks a locale hint.
	if (formatCountries.length) {
		opts.postcodeFormatCountries = formatCountries
	}

	// #1585: the locale hint's country scopes the typo-fuzzy tier — always threaded, never a filter on exact matches (the backend's fuzzy block is the only consumer).
	if (deps.fuzzyCountryScope) {
		opts.fuzzyCountryScope = deps.fuzzyCountryScope
	}

	// #27: the locale country as a soft prior, only where no hard scope is in force. Nothing else in the cascade is disturbed — the resolver ignores it under a `defaultCountry` or an `anchorPosterior`.
	...
	// #1880: capital status for the resolver's bounded capital promotion. Threaded verbatim — the resolver applies its own stand-downs (bare-toponym class, no anchor posterior, exact tier).
...
/**
 * The resolver pins a caller opts into.
 *
 * Each defaults to off at the resolver, so an unset dep stays absent rather than becoming an
 * explicit `false`; a pin promoted to default-on leaves this list for an `!== false` read.
 */
...
async function geocodeAddressOnce(input: string, deps: GeocodeDeps): Promise<GeocodeOutcomeLike> {
	...
	// `input` stays raw for the result.
	// The parse + placer see the normalized form.
	...
	// The caller's trace sink, wrapped so the same records also feed the derivation projection at the end.
	// See `traceCollector` for the no-sink guarantee.
	const trace = traceCollector(deps.resolveTraceSink)
	const traceSink = trace.traceSink

	const opts: ResolveOpts = {
		// Admin descendant-consistency (#263) — joint-consistency resolve over the
		// gazetteer's containment graph.
		// Default-on at the core resolver too since #895 (drift D1 settled); the explicit
		// propagation here keeps `deps.adminCoherence: false` an effective opt-out
		// (an unset ResolveOpts field would otherwise re-default on downstream).
	...
	// Coarse country router (#244, soft prior) — default-on (#244 M2).
	// Undefined → the bundled placer.
	// A function → that placer.
	// False → disabled.
	...
	// #928: a distinctive postcode format outranks the language-based placer (which conflates GB/US → US namesakes). When conditioned on and no explicit defaultCountry, set the country prior from the parsed postcode's format. The placer block below then no-ops via its `!opts.anchorPosterior` guard. Confidence 1.0 — a matched format is unambiguous. hardCountry still checks on the safelist (GB isn't on it yet, so this is a soft anchorPosterior re-rank for GB — enough to de-boost the US namesakes. A safelist add would make it hard, see #985).
	if (
	...
	// #912 change 1: the placer abstains on a single bare locality — OOD input, and the wrong soft posterior overrides the resolver's better-informed exact-tier/population ranking (see isBareLocalityTree). Explicit defaultCountry / anchorPosterior from the caller are untouched. #1589 extends the abstention to a bare postcode: the placer's language model reading `SW1A 1AA` as English placed US at safelist confidence, and the resulting hardCountry filtered the GB row out before the format-implied probe could run. The code's own format says more than its script.
	if (placeCountry && placerResult && !isBareLocalityTree(tree) && !isBarePostcodeTree(tree)) {
		...
		if (placed.country && placed.country !== "OTHER" && !opts.anchorPosterior) {
			// #1738's disagreement test, hoisted. It used to sit inside the hardCountry branch below and check the hard filter only, which left the soft posterior unrestricted — and the soft posterior is enough on its own: the within-tier sort key is `(prominence ?? Score) + w·posterior[country]` at w = 1. Therefore, on `Queen Street, Bristol` a 0.9261 posterior gap overturns GB Bristol's 0.884776 prominence lead and the answer moves 5,274 km to Connecticut (#1751). The placer reads the street token — `King Street, Bristol` reads GB 0.5227 and survives, `Queen` does not — and nothing else in the tree names a country.
			//
			// One probe, read by both checks.
			// An unknown or fuzzy bearer is not disagreement, and a resolver without the
			// findPlace passthrough behaves exactly as before.
			...
			// #743/#194: default-on coverage-guarded hard country filter (same check as the runtime pipeline, via the shared helper so the two production paths can't drift). Same safelist precedence as above: per-call override → the artifact's coverage manifest → the code-constant fallback.
			const hardCountry = hardCountryFor(
				placed.country,
				placed.confidence,
				opts,
			...
			if (hardCountry) {
				// #1738: a language read is not a country verdict. The placer's languages are pluricentric — French text licenses {FR, CA, be, CH, …} — and hardening on the language's home country exiled the francophone-CA class ("1001 Boulevard Saint-Laurent, Montréal" answered Montréal-la-Cluse, Ain: the FR hard filter excluded Québec before any race ran). The promotion therefore yields when the tree's own locality names a place whose dominant bearer (population-first, unscoped, exact) lives in a different country: Paris under French text still hardens (the dominant Paris is in FR); Montréal does not, and the placer's posterior stays the soft anchor the worldwide race weighs. An unknown or fuzzy bearer is not disagreement, and a resolver without the findPlace passthrough hardens exactly as before — absence degrades to the prior behavior, never to a crash.
	...
	//
	//   1. National open-register (#1012).
	//      An authoritative government address register (BAN-FR today), denser +
	...
	// The resolved locality's box then scopes the (street, number) probe.
	// Measured safe — zero ambiguous (street, number) pairs across Paris
	// arrondissements in the 2026-05-18 BAN database.
	//
	...
	// An explicit defaultCountry wins.
	// Otherwise the coarse placer's country.
	...
	// The state-slug selection above is country-blind, and AU state codes collide
	// with US postal states — 'Kingsley WA 6026' under an AU scope reads region 'WA'
	// and opens the Washington database, which can only miss.
	...
	// which no rooftop tier can serve — gets a street-level coordinate instead of the commune centroid.
	// The resolver's applyStreetCentroid self-checks on no-house-number (a numbered query is byte-identical)
	// and unions these hints with the resolved-tree countries, because the pre-resolution
	// country of a bare thoroughfare is unreliable (bare-locality tree / placer mis-route).
	// US never supplies a street database, so `provider("us")` is undefined
	// and the US path stays byte-stable.
	...
	if (interpolation) {
		...
		// #374 doctrine: a database that carries its own conformal multiplier (the `interp_calibration` metadata table, read at open — `radiusCalibration`) is self-calibrating. The resolver reads it directly and this path passes nothing. Two carve-outs preserve the ladder:
	...
	// #42 postcode-country coherence, default-on at the core resolver. Propagated explicitly for the same reason `adminCoherence` is. So `deps.postcodeCountryCoherence: false` stays an effective opt-out rather than an unset field that re-defaults on downstream. The resolver owns the verdict, and its answer is read back off the tree.
	...
	// #1717 stage 2 is promoted default-on, so an explicit `false` is the only thing that withholds it. The resolver core keeps its byte-stable `=== true` read, so the default lives here and in the session, where every promotion's does.
	...
	// Second pass, whenever the resolve settled a different country than the databases were selected for.
	// Rooftop + street-centroid databases are selected before the resolve
	// (they have to be — they're resolver inputs), off a country that can be corrected by any
	// of several mechanisms mid-resolve: the #42 coherence override and the #1735 explicit
	// pre-scope stamp receipts, but the placer and the #1684 dropped-scope worldwide race
	// do not — `92 Laurell Road, Gander, NL A1V 0A9` resolved Gander CA with no receipt
	// and sat at the city centroid while the CA rooftop database held the exact point.
	// So the trigger is the resolved tree's own country, with the receipt kept as the
	// fallback for trees whose scope changed without a resolved carrier node.
	// `opts.defaultCountry` is deliberately left alone so a receipt-driven verdict
	// re-derives identically and survives onto the returned tree.
	// Bounded at one extra resolve.
	...
	// ROAD_TO_V9 §4.
	// One `classifyKindSync` over the text the parse actually saw — computed before the
	...
	// ROAD_TO_V9 §4 marker assembly.
	// The verdict itself is computed above the street-miss fallback.
	...
	// #1989 / #1991: what each attached spatial layer designates for the coordinate this answer reached, recorded beside it. Every route reads a finished result and returns a record — no candidate, no ordering, no abstain — and each contributes nothing when absent, so an unconfigured session produces the identical marker list.
	result.intent_markers = [...markers, ...layerDesignationMarkers(deps, result.lat, result.lon, verdict)]

	// #1901: the authoritative consult runs last, over the finished result's evidence, and attaches its answer beside it. Nothing above this line reads the block, so an unconfigured session skips it byte-identically.
