...
/**
 * Tags a stranded affix may be absorbed into — name-containing place tags whose
 * surface can legitimately end in a street-type word: `Brixton Hill` the locality,
 * `Bishop's Stortford` the venue, `Hythe Marina Village` the dependent locality.
 *
 ...
 *
 * Mutates `tree` in place and returns whether anything moved, matching `repairPostcodeContradiction`'s
 * shape so both post-decode repairs read the same way at the call site.
 */
