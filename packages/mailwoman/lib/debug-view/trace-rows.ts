 ...
 *
 * `systemSource` is required and rides in parentheses: `auto` means the locale head chose the system,
 * `pinned` means the bundle or the caller did, and `off` means conventions never ran — three
 * different reasons for the same `us`, and the trace is the only place that distinction survives.
 */
