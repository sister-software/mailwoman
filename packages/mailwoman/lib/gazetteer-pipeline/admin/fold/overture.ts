 ...
 *
 * Consumers store these ids — gold rows, cached results, cross-artifact joins — so an id has
 ...
 * which is a different artifact from the DDL that builds the tables — the two can drift,
 * and the only thing that catches it is binding a row.
 */
