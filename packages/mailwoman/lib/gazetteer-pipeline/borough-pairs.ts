...
/**
 * One borough pair in the pair-index entry shape (`normalizeFSTToken`-folded keys
 * are the builder's job — this module emits raw surfaces so the caller applies the
 * same fold as the PPD path, keeping one normalization owner).
 */
 ...
 *
 * - **GB** takes boroughs only. Its neighbourhood pairs come from a curated,
 *   venue-confound-boarded file (`data/gazetteer/london-pairs-v2.jsonl`, campaign R4b) —
 *   sweeping in all ~20k GB WOF neighbourhoods here would ship an unboarded batch
 *   and skip the law-1 discipline every GB increment has cleared.
...
> = {
	...
	// because it was measured there. Indian cities carry official renames that WOF
	// has not promoted: it stores Bangalore while an address today says Bengaluru
	// (renamed 2014, present as an `eng` variant). Without expansion the pair exists
	// and never fires — "12 MG Road, Indiranagar, Bengaluru" emitted no dependent locality at all.
	//
	// Not enabled globally, deliberately. Applying it everywhere took the US index
	// from 47,878 to 101,560 — more than double, on surfaces no board has ever graded.
	...
	// ES and IT need aliases for the reason India did, in their own languages rather than English:
	// WOF stores `Rome` for a city written `Roma`, and `Cordoba` for one written `Córdoba`.
	// The fold does not strip accents, so `cordoba` and `córdoba` are different keys
	// and the unaccented WOF form would never match a real address.
	ES: {
		children: ["borough", "neighbourhood"],
		parents: ["locality", "localadmin", "borough"],
		expandParentAliases: true,
...
/**
 * Shortest parent alias worth indexing. WOF's `eng` variants include airport
 * and agency codes ("BLR", "bbmp" for Bangalore) — three letters or fewer is
 * overwhelmingly that class rather than a name anyone writes in an address,
 * and a short key is the shape most likely to collide with an unrelated word.
 */
const MIN_ALIAS_LENGTH = 3

/**
 * A surface this Latin-script model could actually read: letters
 * (including accented and extended Latin), digits, spaces and the punctuation place names carry.
...
export function extractBoroughPairs(adminDBPath: string, country: string): BoroughPair[] {
	...
	try {
		...
		// Placetype names are module constants, never caller input — inlining them keeps
		// one prepared statement rather than a variable-arity parameter list.
		...
		// Parent alias expansion. A writer uses the name they know, which is not always WOF's preferred
		// one: WOF stores Bangalore, while an Indian address today almost always says Bengaluru
		...
		//
		// Scoped to `eng` deliberately. The names table is exhaustively multilingual — Bangalore
		// alone carries ~100 language rows — and folding all of them in would bloat the artifact
		// with scripts this Latin model never sees and surfaces no English-written address uses.
		// The existing surface-expansion probe (`pair-index-hierarchy-probe.ts`) checks on
		// `official = 1`; that is right for name-exactness checks but too strict here, because
		// it is exactly the rows a rename leaves behind. Positive evidence only: an extra
		// parent key can only create a match where the writer actually used that name.
		...
		for (const { canonical, alias, language } of aliasRows) {
			...
			// The writer's language decides which alias is worth carrying: the country's own official
			// languages, plus English as the lingua franca. WOF's preferred name is often neither —
			// it stores `Rome` (eng) for a city Italians write `Roma` (ita), and `Bangalore`
			// for one Indians write `Bengaluru`. Restricting on `eng` alone, as this did
			// when India motivated it, misses every Italian and Spanish form.
			//
			// `isOfficialLanguage` is the codex table the WOF ingest already consults for exactly
			// this question, so "which languages does this country write in" keeps one owner
			// instead of gaining a second hardcoded list.
			if (language !== "eng" && !isOfficialLanguage(country, language)) continue

			// latin script only. India has 22 official languages and WOF carries Devanagari,
			// Tamil and Bengali names for its cities. this model never sees those scripts,
			// so indexing them is pure artifact weight. The check is on the alias rather than
