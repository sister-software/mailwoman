 ...
 *
 * A probe that throws (no network, no `gh`, no auth) resolves upstream with the failure
 * in `reason`: an unreachable fork registry is not evidence about the fork either way,
 * and pretending otherwise would silently pull upstream data over a correction we rely on.
 * The caller sees which happened.
 */
