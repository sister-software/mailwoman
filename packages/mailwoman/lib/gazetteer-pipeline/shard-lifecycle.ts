 ...
 *
 *   The shard build lifecycle every sealed gazetteer artifact shares: the staging PRAGMA block, the
 ...
 *
 *   Extracted from the postcode shard builders (`geonames-tail`, `codepoint-shard`, `ni-osm-shard`) and the
 ...
 * planner its statistics. Run after the last write and before {@link vacuumShardInto}.
 */
 ...
 */
export async function vacuumShardInto<DB>(db: DatabaseClient<DB>, out: string): Promise<void> {
...
/**
 * The FTS pass over a freshly published (not yet sealed) shard: open it, build `place_search` + `place_bbox`, close it.
 */
export async function buildShardFTS<DB>(
	out: string,
	openShard: (path: string) => DatabaseClient<DB>,
	onProgress?: (phase: string, detail?: string) => void
): Promise<BuildFTSResult> {
	using outDB = openShard(out)
...
/**
 * What a shard records when a rebuild cannot recover a provenance field. A sentinel STRING rather than an empty one: a
 * consumer reading `source_release: ""` cannot tell "no release label exists" from "nobody looked", and the
 ...
 * caller substitutes {@link UNKNOWN_PROVENANCE} and says so in the shard.
 */
