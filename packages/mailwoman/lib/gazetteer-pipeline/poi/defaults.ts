...
/**
 * Pinned Overture release for the places-theme ingest.
 *
 * Matches `overture-ingest.tsx`'s own `DEFAULT_RELEASE` pin (the addresses-theme ingest)
 * as of this writing — a monthly Overture release covers every theme at once,
 * so the two pins move together in practice.
 ...
 * pulling a value from a `.tsx` file into this pipeline layer would invert that dependency direction.
 *
...
/**
 * `--dominance` default — a QID's modal name must cover at least this fraction of its total rows to qualify.
 *
 * Below the floor, the QID is systematically mistagged
