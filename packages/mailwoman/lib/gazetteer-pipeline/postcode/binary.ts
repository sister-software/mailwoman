 ...
 *   so it can be tested without a shard and a terminal (#1509).
 *
 *   WHY IT MOVED. The command's GB branch derived the outward district by splitting `name` on a SPACE.
 *   That was written against `postalcode-gb.db` (the retired GeoNames-lineage shard), whose `name`
 *   carries the spaced display form. The licence-clean Code-Point Open shard
 ...
 *   reads both shards. {@linkcode keyFloorViolation} makes an empty or catastrophically-degraded build
 ...
 * it. Structural, not a guess — and unlike a space split it holds on a shard that stores the glued form.
 */
...
/**
 * The key a shard `name` enters the binary under. GB is space-stripped to the train painter's form; every other system
 * stores `name` already normalized (DE/FR `68161`, NL `1012LM`, US `94105`), so it serializes verbatim.
 */
...
/**
 * The outward district of a GB unit postcode, derived by SHAPE from either shard's storage form (`AB101AB` and `AB10
...
/**
 * Derive one country's PCB1 entry set from its shard rows. GB gets the unit/outward treatment described in the module
 * docstring; every other country serializes verbatim.
 */
export function buildPostcodeBinaryEntries(
	country: string,
	rows: readonly PostcodeShardRow[],
 ...
 *
 * MEASURED 2026-08-06 against the shipped shards, `SELECT COUNT(*) FROM spr WHERE placetype='postalcode' AND
 ...
 * collapse (a derivation that stopped matching the shard's storage form, a country filter that stopped selecting) — not
 * to pin a count that legitimately moves with every upstream refresh. A build that comes back at 51% of what the shard
 * holds is still wrong, but it is wrong in a way a human reads in the roll-up; a build at 0% is the one that ships
 * silently.
 *
...
export function keyFloorViolation(country: string, keys: number, granularity: GBGranularity): string | null {
	...
	return (
		...
		`shard's \`name\` column carries the key shape this country's derivation expects, and that the ` +
