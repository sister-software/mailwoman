 ...
 * the spec's own wording rather than the full `normalize/abbreviations.ts` dictionary
 * the gauntlet's metamorphic layer uses. Keeping it small and separate means this
 * suite exercises a different, independent perturbation source than the gauntlet —
 * two implementations of the same literature class rather than one shared with an inherited bug.
...
/**
 * Secondary-address designators (unit/suite/floor markers) the look-ahead
 * also treats as "not name-shaped" — these are capitalized like a proper noun
 * but are never what follows a genuine Saint-prefix ("St Apt 4B" isn't a place name).
 ...
 * "St" immediately followed, mid-phrase (no comma), by an ordinary capitalized word that
 * ISN'T a designator or suffix — e.g. a street literally named "St Rose Ave" read out of
 * context. v1 accepts that residual false-exempt (an under-tested row) over a false-swap
 * (a corrupted ground-truth string): mislabeling "St Andrews" as a suffix breaks the test's own
 * fixture, which is worse than skipping a swap. A future tightening could check the following
 * word against a gazetteer of known Saint-prefixed place names instead of a fixed word list.
 */
...
/**
 * Double every literal space character. Applicable only when the input carries a literal space —
 ...
 * Mirrors a real, mundane input shape: an address copy-pasted out of a spreadsheet cell or CSV
 * field that still carries its enclosing quotes. Always applicable (every string can be wrapped).
...
/**
 * Identity — the text is not perturbed. The runner special-cases this id: it parses the
 * original string twice (two independent classifier calls, never reusing a cached result)
 * and compares the two outputs. This is Chen et al.'s original metamorphic identity
 * relation (`f(x)` computed twice must agree), repurposed to catch nondeterminism
 * in the decode path rather than a text perturbation.
 */
...
/**
 * Metamorphic transforms applied to an input. Each one must leave the parse unchanged,
 * so a differing result is a bug rather than a disagreement.
 */
