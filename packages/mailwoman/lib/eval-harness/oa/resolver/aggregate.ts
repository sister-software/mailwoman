 ...
 *
 * Shared rather than repeated because the two views of one row must agree on it:
 * an empty state is as unknown as a missing one, so `||` is the operator rather than `??`.
