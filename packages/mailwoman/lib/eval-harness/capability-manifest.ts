...
/**
 * The union of every tag any codex conventions row forbids — the only tags the loader's delta check reads,
 * so the only tags that need a paired `maskOnF1`. Derived from the codex so a new forbid
 * row automatically widens the manifest the next time it's regenerated.
 */
...
async function buildManifest(paths: ResolvedPaths): Promise<Capabilities> {
	...
	for (const [tier, tierOverrides] of Object.entries(TIERS)) {
		...
		for (const spec of MASK_EVAL_LOCALES) {
			...
			// `inputMode: "formatted"`: certification probes are formatted postal addresses, whose production
			// path disables evidence-bundle channels. The mask-regression check grades the
			...
			for (const t of TAGS) {
				// Skip tags the model never emits and never sees in gold under
				// either mask — a 0/0 F1 is not a capability claim, just noise.
...
export async function generateCapabilityManifest(options: CapabilityManifestOptions = {}): Promise<void> {
	...
	if (WRITE) {
		...
		// surgical insert (not a JSON round-trip): the shipped card hand-formats compact inline
		// objects (`"anchor": { "required": true }`) that a `JSON.stringify` would expand,
		// spuriously reordering a shipped artifact. Instead, append one new top-level key,
		// byte-preserving everything else. The card is validated JSON, so its tail is
		// `…\n}\n` (root close); we splice `,\n\t"capabilities":…` before that final brace,
		// one indent level deep (each block line tab-prefixed).
		...
		// Strict rather than tolerant: a corrupt model card must abort the splice
		// rather than read as "no capabilities block" and append a second one.
