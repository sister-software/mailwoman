 ...
 * raw population desc, weighted score as the tiebreak.
 *
 * Kept here so the comparison is against the code that shipped rather than against a paraphrase of it.
 */
...
if (!wofPaths.length) {
	...
	// Board inputs and their probe surfaces: the input is what a user types,
	// the surface is the token whose namesakes are actually in contention.
	// Both go through the ranking.
