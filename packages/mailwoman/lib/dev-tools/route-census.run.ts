/**
 * Which rows the weights-family routers send to a graph that cannot produce the row's expected tags, and which rows the
 * candidate routers disagree about.
 *
 * `docs/engineering/CONTRIBUTING_MODEL_WORK.mdx` requires a route comparison before a change sends an existing request
 * to a different graph, and a comparison needs truth. The gauntlet board declares no routing label. It does carry one
 * for a subset, in a form that has to be derived: a row's `expectComponents` names the tags a correct parse produces, a
 * family's model card declares the tags its head can emit, and a row whose expected tags only one family emits is
 * labeled for that family by construction. On the current board that is 26 rows of 1,029, and 7 of the 26 route to a
 * family that cannot emit what they ask for.
 *
 * The other 1,003 rows have no label, because their tags are in both families' vocabularies. For those the tool reports
 * where the routers disagree, which is the set a human has to adjudicate.
 *
 * A row states its country, its address kind and its expected components. Nothing says which model graph should read
 * it, and two obvious derivations both fail on rows the board already holds.
 *
 ...
 *
 * The script share does not supply it either. `逊克二分场四队, HEILONGJIANG, CHINA` carries 29.2% Han and routes to the
 * character model. `Far East Chinese 口福羊汤, 13 Gerrard St, London W1D 5PS` carries 10.8% Han and stays on the Latin one.
 * Those distributions overlap, which is why `carriesFamilySegment` reads where the Han sits rather than how much there
 * is. A threshold fitted here would restate one router's rule and then grade that router against itself.
 *
 * Which scripts a family can encode settles nothing either. The Latin SentencePiece model tokenizes `新加坡` as three
 * pieces, `▁新` `加` `坡`, and `neural-weights-cjk`'s `char-vocab.json` carries all 52 ASCII letters among its 4,451
 * entries. Each family represents the other's script.
 *
 * Which tags a family can emit does settle a row, and it is read from the model cards rather than argued. `en-us`
 * declares 33 labels and `cjk` declares 49, and `block`, `district`, `municipality` and `prefecture` are in the second
 * set alone. A row expecting any of those is unreachable on the Latin graph whatever script it is written in. That is
 * what labels the 26 rows, and it reaches rows a disagreement census cannot: `jp-ws-jingumae-4-12-10-google-canonical`
 * is mis-routed and both routers agree on it, so agreement is not correctness and the two sections below answer
 * different questions.
 *
 * This tool claims no confusion matrix over the whole board. It grades the rows the label sets label, and for the rest
 * it reports what each router answered and prints the rows they disagree about.
 *
 ...
 *
 * Comparing the two routers means resolving this first. The script router abstains on every Latin row and the locale
 * hint names the Latin family, and those are the same outcome: a process opened at `--locale en-US` loads the Latin
 * family, so "the caller decides" and "the Latin family" route the row to one graph. Comparing the raw answers instead
 * reported 1,011 of 1,029 rows as disagreements, every one of them a row both routers send to the same place.
 *
 * `en-us` because that is `resolveWeights`' own default locale. A census of a process opened elsewhere would resolve
 * abstentions to that locale's family instead.
 */
 ...
 *
 * The locale travels with the family because the two are different claims and this census turns on the difference.
 * `Rinrin, 3 Chome-57 Tenmanmachi, Takayama, Gifu 506-0025, Japan` is locale `ja-JP`, which the locale hint gets right
 * from `format=jp_postcode`, and the row is written entirely in Latin script. Folding that locale to a family answers
 * `cjk` for a Latin row, and on this row that answer reaches the only graph whose head vocabulary carries the
 * `prefecture` and `municipality` tags the row expects. Whether that graph parses romaji well is unmeasured. The reason
 * column reports which scorer decided, so a reader can tell a postcode-named locale from a script-named one.
 */
 ...
 *
 * A card's labels are BIO-prefixed (`B-locality`, `I-locality`), so the prefix is stripped to leave the tag a row's
 * `expectComponents` is keyed by. A family whose card is absent from the checkout contributes no tags and therefore
 ...
 *
 * `undefined` is the ordinary answer. It means the row's tags are in every family's vocabulary, so the label sets say
 * nothing about which graph should read it, and the row needs a human label instead.
 */
...
interface Misroute {
	...
	input: string
	/**
	 * The expected tags the routed family cannot emit. These are what makes the row unreachable.
	 */
	unreachable: string[]
}
...
interface Disagreement {
	...
	localeHint: string
	/**
	 * The locale the hint named, and which of its scorers named it. A family disagreement whose locale is right is a
	 * different finding from one whose locale is wrong.
	 */
	hintLocale: string
	hintReason: string
	/**
 ...
 *
 * Copied from `cases/load.ts`'s `COUNTRY_DIR` rather than widened, so this census reads the population the board reads.
 * A recursive glob over the whole tree instead returns 1,308 rows across 230 countries, because it picks up
 * `cases/generalization/country-sweep-2026-08-05-passes.jsonl` — 279 rows the loader's directory rule excludes and
 * `mwdev_compare` does not run. Measuring a router on rows the board never grades would report a mis-route nothing
 * checks.
 */
...
for (const dir of countryDirs) {
	...
	for (const file of files) {
		for await (const row of JSONSpliterator.fromAsync<BoardRow>(file)) {
			...
			// The label sets grade this row when they name exactly one family. The shipped router is the arm graded, because
			// it is the one deciding today.
