...
async function main() {
	...
	const runLocale = async (name: string, slice: string, cc: string, tag: string): Promise<string> => {
		...
		await oaResolverEval(
			...
			(line) => evalLines.push(line),
			(line) => logLines.push(line)
		)

		const evalMarkdown = evalLines.join("\n") + "\n"

		await writeLocalFile(evalMarkdown, `${TMP}/${tag}.eval.md`)
		await writeLocalFile(logLines.join("\n") + "\n", `${TMP}/${tag}.log`)
		const row = evalLines.find((l) => l.startsWith("| **neural** |")) ?? ""
		...
		const pip = await $({
			nothrow: true,
		})`node ${PIP_CONTAINMENT_PATH} ${resolved} --label ${name} --json ${pipJson}`
