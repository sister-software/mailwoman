...
/**
 * Draw `n` register rows, seeded through DuckDB's own generator so a re-run with the
 * same seed draws the same rows. The building-led quarter is drawn from the rows
 * whose `unit` is a building name. the rest from every row.
 */
