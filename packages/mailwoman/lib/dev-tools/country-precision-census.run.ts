 ...
 *
 * `UK` is the one that matters and it is not a parser error: the alpha-2 for the United Kingdom is
 * `GB`, `UK` is exceptionally reserved, and every British address in the wild writes the second.
 * The first version of this census scored `14 New St, London EC2M 4HE, UK` and `West End, Woking, UK`
 * as contradictions, which charged the parser for the difference between a standard and the language.
 */
...
for (const seed of cases) {
	...
	// the population is the question.
	// "Did we get a named country wrong" is only askable where the input names one, and
	// where the one it names is the row's own — 361 of the 982 rows.
