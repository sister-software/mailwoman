...
/**
 * Jurisdictions this project models separately from any ISO 3166-1 code, each because its addresses are not expressible
 * in the code's own grammar. Adding a row here is a claim that the jurisdiction needs its own parser behaviour, so each
 * carries the reason.
 */
...
const DIMENSIONS: ReadonlyArray<{ name: string; held: (r: JurisdictionRow) => boolean }> = [
	...
	// Admission is part of the predicate. Street rows the config does not admit train nothing, and reporting them
	// as held would put this row above the one it depends on.
