...
describe("wilson", () => {
	...
	it("matches a hand-computed value — 3/15 is the Paris cell that motivated this board", () => {
		// Standard Wilson, z=1.96, computed independently: p=0.2, denom=1+3.8416/15=1.25611,
...
describe("the FR fragment board fixture", () => {
	...
	it("never repeats a street surface across classes", () => {
		...
		const surfaces = fixtures.map((f) => f.surface).filter((surface) => surface != null && surface.length)
