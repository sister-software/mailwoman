 ...
 *
 *   Tests the ablation expectation model against a fake gazetteer.
 *
 *   The expectation must depend only on the components that survive a deletion, never on the
 *   variant's output. The fake gazetteer holds everything else fixed so the tests can vary the output.
 */
...
/**
 * Builds a gazetteer place.
 *
 * `negRank` is `-log10(population + 1)`, so a larger place has a smaller value.
 */
...
/**
 * The Springfield, IL ladder with rooftop, locality, county, region and country rungs.
 */
...
describe("bboxRadiusKm — a zero-extent bbox is ABSENCE, not a radius of zero", () => {
	...
	// `spr` stores an unset bbox with `min == max`.
...
describe("rungRadiusKm — no rung without a radius", () => {
	...
	// A rung without a radius is dropped and reported as a gap.
...
describe("ablationLadderFromChain", () => {
	...
	// An ancestor rung tighter than its child would fail a point that the child passes.
...
describe("deriveExpectedRung — computed from what REMAINS", () => {
	...
	// The surviving region and street still support the rooftop rung.
	...
	// A bare ambiguous name without other evidence should produce an abstention.
	...
	// The model cannot evaluate a venue, so it cannot require abstention.
...
/**
 * Checks that the expectation never depends on the variant's output.
 *
 * An expectation derived from the output would grade the pipeline against itself and pass everything.
 */
...
describe("residualWords — the corpus types less than the input carries", () => {
	...
	// A row whose only asserted component is deleted leaves an empty component set.
	// The words left in the input still identify a place, so the model must not require abstention.
	...
	// Residual words are evidence of unknown strength.
	// They can lift an abstention and never deepen a rung.
...
describe("gradeAgainstLadder", () => {
	...
	// An expected abstention and a lost answer need different operator actions, so they get different grades.
	...
	// An unconstrained expectation still fails an answer off the ladder.
	...
	// A slot filled with a different token fails even when the coordinate is correct.
	it("fails a SUBSTITUTION even when the coordinate is perfect", () => {
		...
		// The achieved rung is still reported.
	...
	describe("the anchor floor — a deletion is charged only for what IT cost", () => {
		it("reads a variant that matches its already-coarse anchor as held, not as a loss", () => {
			// The anchor and the variant both land on the locality rung.
			const graded = gradeAgainstLadder({
				...base,
				anchorRungDepth: 1,
				expected: rung(0),
		...
		it("still fails a variant coarser than that anchor", () => {
			...
			// The county rung is one step coarser than the anchor's locality rung.
...
describe("per-case overrides", () => {
	...
	// A pin for a missing rung returns null so the caller can report it.
