 ...
 * street-name audit, plus 24 `operator:world-structures-2026-08-10` cases — real-world
 * addresses from eleven countries whose structure the `ComponentTag` union cannot fully express
 * (Brasília's sector/quadra hierarchy, Colombian cross-street nomenclatura, GhanaPostGPS digital codes,
 * plus codes, Irish townland recursion, Mongolian district/khoroo nesting, Nicaraguan relative addressing).
 * Country dirs went 29 → 121 in the country-sweep batch, and 121 → 125 with
 * this one (gh, mn, ne, ng are new).
 */
 ...
 *
 * Changing the corpus changes this.
 * That is the point: an edit to a `.jsonl` row now needs a matching edit here,
 * and the diff says "the corpus changed" rather than "a 3,500-line file changed".
 *
 * Moved 2026-08-06 (#1507) — `ab541bba…` → `848548e6…` — when seven country-sweep
 * family-A rows gained `expectPlaceName`.
 * Row count was unchanged there, which is why the board id below did not move:
 ...
 *
 * Moved again 2026-08-11 — `02026054…` → `f87db0a9…` — when
 * `mn-ws-gandantegchinlen-dual-script` gained the per-row `expectComponentRenderings`
 * interface (#34: the global dual-script grader relaxation became a per-row opt-in).
 * Row count and every `id`+`input` were untouched, so the board id there stayed.
 *
 ...
 * row flips improvement_target → pass with its measured receipt.
 * Row count and every `id`+`input` untouched, so the board id stays.
 *
 * Moved 2026-08-11 (sixth) — → this — by the NZ re-pins after the linz database
 * promote (#1617): the two Stanmore Bay rows flip improvement_target → pass with the
 * database's real coordinates replacing their pre-database abstain pins.
 * Row count and every `id`+`input` untouched, so the board id stays.
 *
 ...
 *
 * Moved 2026-08-11 (eighth) — → this — by the fork→entity wire landing: the comer row
 * flips improvement_target → pass with its measured receipt (tier `venue`, 6 m).
 * Row count and every `id`+`input` untouched, so the board id stays.
 *
 * Moved 2026-08-12 (night) — → this — by the 2-row `bare_capital_street_miss` board
 * (Wellington, Antwerpen): the model tags the lone token `street`, the street tier nulls,
 * and the locality walk provably answers (the resolver bisect is in each row's note).
 * 532 → 534, so the board id moves too.
 *
 * Moved 2026-08-12 (second) — → this — by the street-miss fallback landing:
 * both bare-capital rows flip improvement_target → pass with measured receipts.
 * Row count and every `id`+`input` untouched, so the board id stays.
 *
 ...
 * retry keys the register's NUMBER_FIRST). 537 → 538, so the board id moves too.
 *
 * Moved 2026-08-12 (fifth) — → this — by the 2-row `au_rooftop` board riding the G-NAF lane:
 * the WA state-slug collision witness ('47 Renegade Way, Kingsley WA 6026' —
 * the parsed region 'WA' opened the US Washington database and starved the AU rooftop
 * until the non-US pre-resolve country learned to outrank the state-slug match)
 * and the rural LOT class ('LOT 373 Clifton Street, Sandstone WA 6639').
 * Both land status=pass with register-point receipts. 535 → 537, so the board id moves too.
 *
 * Moved 2026-08-12 (fourth) — → this — by the #1614 word-level fuzzy measure landing plus the 1-row
 * `situs_wrong_village` board (#1631 Teichstraße, the wrong-village rooftop match panel-v2 surfaced):
 * the aucklnad measure-misalignment witness flips improvement_target → pass with its measured
 * receipt (Auckland NZ, word-level 0.975 vs auckley 0.868), the board-flagged newly-passing
 * `gb-street-name-savile-row` promotes to pass, and the Savile Row regression check's
 * ledger closes its fuzzy residual (namesake corrections die under WORD_FUZZY_MIN 0.85)
 * while opening #1632 for the span-rescore alias door its board print actually
 * traces to. 534 → 535, so the board id moves too.
 *
 ...
 * common noun (Poisson FR, Pesce IT, Dimanche CF, Tó PT, Tô BF, Laç AL, Vogel CH) whose
 * folded keys are shared with translation-gloss alt-name rows on unrelated US places.
 * Ten rows pin the primary-preference rescue that holds today; `bf-gloss-to-accent` lands
 * improvement_target for the live Toledo-as-region miss. 558 → 569, so the board id moves too.
 *
 * Moved again the same day — by the label-centroid swap (#1726): nine bare-country/region tolerances
 * tighten to the label-point era (fr 700→100 km, jp 1000→50 km, de 500→100 km, cn 1500→600 km, tx/ca/ga),
 * and `us-athens-ga-30601` re-pins its expectation to the Athens label centroid
 * at city scale — the old expectation was the artifact's own math centroid,
 * and no rooftop is coverable (the GA database holds no `1 Broad St`).
 * Tolerance and note edits only, so the board id holds — content-addressing working as intended.
 *
 ...
 * (the Kent alternative sits flagged `regionScopeMiss` — the re-admission class),
 * found when a live GB POI query over the Kent Rochester came back empty
 * and read as a data gap. 569 → 570, so the board id moves too.
 *
 ...
 * and the fully-French form lands improvement_target (the francophone-CA scope confound:
 * the locale head's FR 1.00 hard-scopes the walk and Montréal-la-Cluse answers).
 * 570 → 572, so the board id moves too.
 *
 * Moved again the same day — → this — by the #1738 dominant-bearer guard's promotions:
 * `ca-qc-boul-st-laurent-full` (5,858 km → the attested rooftop at 0 m) and
 ...
 * text carries the Latin '1' — the oracle's canonical rendering leaked into the component
 ...
 * pair (#1747): a `street_suffix` with no `street` anywhere in the tree, so the resolver
 * was handed a bare `Brixton` and answered Brixton, devon, 300.3 km away. 573 → 575,
 * so the board id moves too. * Moved 2026-08-23 — → this — by the operator's Google
 * Maps batch (57 rows, 36 conditional) plus the #1764 accented-commune row.
 * Truth is each place's own map pin, resolved from the supplied short link.
 *
 * First VE and PG coverage the board has had, and the first rows for punctuation-led venue names
 ...
 *
 * It still has to move, because `note` is in {@link SEED_CASE_KEY_ORDER}
 * and the hash is a function of the whole canonical row.
 * That is the design working: a corpus edit cannot reach the board without someone re-pinning it on purpose.
 *
 * Every built `regression.db` carrying the old stamp is now stale and a runner will
 ...
 * The country sweep is the opposite kind of change.
 *
 * It adds 114 rows — so this one moves, and it should: the ablation board is genuinely a different board.
 *
