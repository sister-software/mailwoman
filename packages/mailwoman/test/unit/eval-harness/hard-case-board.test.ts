...
describe("hard-case board schema", () => {
	it("accepts a row with no coordinate at all", () => {
		expect(HardCaseSchema.safeParse(VALID).success).toBe(true)
	})

	it("accepts a complete coordinate triple", () => {
		const parsed = HardCaseSchema.safeParse({
			...VALID,
			expectLat: 46.7306,
			expectLon: -116.999,
	...
	it.each([
		...
		expect(HardCaseSchema.safeParse({ ...VALID, ...partial }).success).toBe(false)
	})
...
describe("hard-case board fixture", () => {
	it("loads, validates, and is sorted by id", async () => {
		const board = await loadHardCaseBoard()
