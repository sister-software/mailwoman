 ...
 *   shape (`corpus/src/database-recipes/locale.ts`'s `districtAsLocality` check) without touching the CSV
 *   read path — CSVSpliterator itself is exercised elsewhere (`locale.test.ts`).
 */

import {
	PairIndexBuilder,
	applyPairIndexHoldout,
	nearestRankPercentile,
} from "mailwoman/gazetteer-pipeline/pair/index/index"
...
describe("PairIndexBuilder", () => {
	...
	it("records the caller's parent tag per row, not one derived from the child tag (PIX2)", () => {
		...
		// The US WOF source's two parent shapes on the same child tag: a locality parent and a borough parent.
		// `WESTERN_PARENT_OF.dependent_locality` is `["locality"]`, so a derivation could only
		// ever produce the first — the second is the edge PIX2 exists to be able to state.
