...
// Exact key-set parity — see file header.
// Fails to compile on any field added, removed, or renamed on either side
// (GeocodeResult or GeocodeOutcomeSchema).
export type _KeysMatch = Expect<Equal<keyof GeocodeResult, keyof KnownInferred>>

// Direction 1 — schema-too-narrow guard, narrowed to the schema's declared fields
// (see file header for why the literal `.loose()` form is impossible).
...
// Direction 2 — schema-too-wide guard, literal form.
// The schema must never promise more than a real GeocodeResult actually guarantees.
