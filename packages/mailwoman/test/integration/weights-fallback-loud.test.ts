 ...
 *
 * `pt-BR` has no carrier package today.
 * If one ever ships, this breaks the same way and wants the same one-line move.
 */
...
describe("#1108 loud weights fallback — weights ABSENT (non-interactive / piped stdin)", () => {
	test("plain parse: warns on stderr, still emits parseable JSON on stdout, exit 0", async () => {
		...
		// The encoder-less structural fallback still produced output rather than staying silent or empty.
