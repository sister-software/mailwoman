 ...
 *
 *   Build `postalcode-ni-osm-<date>.db` — the Northern Ireland `BT` unit-postcode shard from
 *   OpenStreetMap, and the only coverage that exists for the hole Code-Point Open leaves.
 *
 *   ## Why this shard exists, and why it is partial ON PURPOSE
 *
 ...
 *
 *   ## Why a partial shard is strictly additive
 *
 *   Since #1480 an unknown postcode ABSTAINS rather than fuzzy-matching, so a `BT` code this shard does
 *   not carry behaves exactly as it does today — no answer — while a code it does carry now resolves.
 *   There is no input for which adding this shard produces a WRONG answer where it previously produced
 ...
 *   operator's machine and picked up because `DEFAULT_POSTCODE_SHARDS` is `existsSync`-filtered, which
 ...
 *   two #920 laws), `populateAncestors`, `buildFTS`, `createShardMetaTable`, `sealDatabase`. This file
 ...
 * - `9.7e12` — `CODEPOINT_ID_BASE` (`./codepoint-shard.ts`)
 * - `9.8e12` — **this shard**
 *
...
/**
 * Total NI postcode SECTORS — an outward code plus one inward digit (`BT3 9`). The coarser denominator: a shard can
 ...
 * denominator, and the one the OSM shard saturates: 80 of 80.
 */
...
export interface BuildPostcodeNIOSMOptions {
	 ...
	 * endpoint to rebuild the same shard is what the dated directory exists to avoid.
	 */
	offline?: boolean
	/**
...
export interface BuildPostcodeNIOSMResult {
	...
	/**
	 * Distinct districts and sectors the shard covers, against the national totals.
	 */
	districts: number
	sectors: number
	/**
 ...
 * bounds that a broken validator would blow through, and those are worth more than they look: the `codepoint-shard.ts`
 * gate learned the hard way that a tolerance derived from the failure it is meant to catch catches nothing, so every
 * check here is against a fixed number.
 *
...
async function writeShardMeta<DB extends ShardMetaDatabase>(
	...
	const rows: Array<[string, string]> = [
		...
		[
			...
			"A miss on a BT postcode in this shard means NOT ATTESTED IN OPENSTREETMAP. It does NOT mean the postcode " +
				...
				"shard existed — the shard is strictly additive and cannot turn a right answer into a wrong one.",
		],
		[
			"coverage_gap_reason",
