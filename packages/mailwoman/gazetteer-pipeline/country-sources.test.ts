...
import { planCountryMove, servingSources } from "./country-plan.ts"
import {
...
describe("the id-band literals in country-plan.ts", () => {
	it("agree with the folds that mint them", async () => {
		...
		const { OVERTURE_ID_BASE } = await import("./admin/fold-overture.ts")
		const { GEONAMES_ID_BASE } = await import("@mailwoman/resolver-wof-sqlite/geonames-aliases")
		const source = readFileSync(new URL("./country-plan.ts", import.meta.url), "utf8")
