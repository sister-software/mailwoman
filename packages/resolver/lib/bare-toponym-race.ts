 ...
 * the bare-toponym shape whose country-placetype sibling race `#lookupAndPick` runs.
 * A bare name the parser tagged `locality` can name a country
 * ("Japan", "China" — single country names are out of the parser's training distribution),
 * and the locality placetype filter makes the country row unreachable regardless of
 * how the ranking would order it. Any second value-containing node makes the input
 * address-shaped and the race stays off; `dependent_locality` maps to the same placetype
 * but is an address-interior tag, so only a literal `locality` qualifies.
 */
...
/**
 * The log-population dominance a bare region namesake must hold over the locality winner before
 * the bare-toponym race promotes it. The value is DECISIVE_MARGIN_LOG10 from the ablation-expectation
 * model (0.5 — below it the top-ranked place is the intended one 52.4% of the time, above it 89.1%);
...
/**
 * Alias roles the side races refuse to answer through (#1730): a lone bare token
 * that only reaches a place via an abbreviation row ("Tó" folds onto Toledo's "TO")
 * or a translation-gloss row did not name that place — while the role-NULL exonym tier
 * stays open, which is what lets 格鲁吉亚 win the country race through its display-name alias.
 ...
 * Exact matches only: the fuzzy tier exists for typo recovery on address
 * spans, and a fuzzy country is a guess this race must never promote.
