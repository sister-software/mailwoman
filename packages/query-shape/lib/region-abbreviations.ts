...
export function detectRegionAbbreviations(
	...
	for (const seg of segments) {
		...
		// `k` rather than `t`: a token belongs to one segment, but the outer loop needs the pointer
		// parked at this segment's first token so the next iteration resumes in the right place.
