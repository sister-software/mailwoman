...
test.each([...PLANETARY_BODIES])("%s: the config names its own body, host, tilesets and identity", (body) => {
	...
	// `-terrain`, not `-hillshade`: the archive carries terrarium-encoded elevation rather than a
	// shaded picture, and the two are indistinguishable to a cache, so the new content took a new name.
