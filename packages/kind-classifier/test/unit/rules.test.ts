...
test("scoreLandmark: fires when the text leads with a relative-location phrase", () => {
	...
	expect(scoreLandmark(input("Near the park"), shape())).toBe(0.9)
...
test("scoreVenueLandmark: short capitalized non-address phrases; rejects addresses", () => {
	expect(scoreVenueLandmark(input("Pier 39"), shape())).toBe(0.88)
	expect(scoreVenueLandmark(input("Grand Central Terminal"), shape())).toBe(0.88)
...
test("ScoreLocalityOnly: short, alpha, ≤2 segments without format hits", () => {
