 ...
 *
 * Measured additions, same as the heads above: `city`, `town` and `valley` each
 * came off a real corpus row that was forking wrongly.
 */
 ...
 * No unbounded-whitespace-then-literal prefix, which is the `js/polynomial-redos` shape
 * (see the `ANCHOR_SEPARATOR` docstring in `poi.ts` for the same analysis).
 */
 ...
 * That guard is what keeps the fork off the common case.
 */
export function scoreRoutePair(input: NormalizedInputLite, shape: QueryShapeLike): number {
	...
	// Reduplication — "Pago Pago", "Baden-Baden", "Walla Walla", "Bora Bora".
	// Nobody travels from a place to itself, so a repeated token is a universal
	// single-name signal and needs no lexicon at all.
 ...
 *
 * Requires a non-empty subject before the locator, so a bare "near me" stays with the `landmark`
 * leaders rule rather than claiming to be a category search with a missing focus point.
 */
