...
the registry does not list. `scriptsUnreferenced` keeps the pathspec the baseline was recorded against, `scripts/**/*.ts`
as git reads it, which requires a directory between `scripts/` and the file (`lib/tracked-sources.ts` says why).
