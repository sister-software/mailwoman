...
| `module-surface`              | a module's count of top-level interfaces, constants, functions, lines and divider comments against per-metric limits; advisory, and it claims no decomposition on the author's behalf                  | —       |
| `module-cohesion`             | a large module's top-level declarations partitioned by which reference which, reported when two communities share no imported dependency; advisory, and the warning names both groups                  | —       |
