 ...
 *
 * `interpolated` is not a hedge here.
 * It is the mechanism.
 *
 ...
 * `apache-arrow`, `@mailwoman/ban`, `spliterator` and the rest of the training-corpus pipeline behind
 * it, for one regular expression, into a package whose entire job is to make two http calls.
 *
 ...
 * - `street_suffix` ← `suffixType` + `suffixDirection`.
 *   Mailwoman has no separate suffix-directional tag, and the two are adjacent
 *   and in this order on the envelope (`123 N main ST E` → suffix `ST E`).
