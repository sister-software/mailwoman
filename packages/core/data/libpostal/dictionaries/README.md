...
- **ambiguous_expansions.txt**: e.g. "E" could be expanded to "East" but could
  be "E Street", so if the string is encountered, it can either be left alone or expanded. In general, add single-letter abbreviations in most languages to ambiguous_expansions.txt because single letters also serve as initials
...
- **concatenated_suffixes_inseparable.txt**: Things like "...bg." => "...burg"
  ...
  abbreviated equivalent or can help identify the token in parsing as,
...
- **entrance.txt**: string indicating an entrance, such as the word "entrance" and its appropriate abbreviations.
- **given_names.txt**: a dictionary of given names derived from Wikipedia has been provided in the special "all" language directory. Western given names are commonly abbreviated using the first letter but specifying them all would create too much variation. Instead of trying to expand "J" to every possible J name, it might be better to abbreviate possible given names and add those versions as alternate forms of the string for matching purposes.
...
- **personal_suffixes.txt**: post-nominal suffixes, including generational forms such as Junior/Senior in English or der Jungere in German.
...
- **staircase.txt**: strings indicating a staircase, such as the word "staircase" or "stair".
...
- **unit_directions.txt**: phrases to indicate which side of the building the apartment/unit is on, such as "left", "right", "front", or "rear".
