 ...
 *
 * A fault is not an error return: `runPipeline` still resolves, and `tree` still carries
 * whatever the remaining stages could prove. What the fault provides the caller is the ability
 * to tell "the model faulted and the rule-based stages filled the tree back in" apart from
 * "the model ran and found nothing" — which, before #40, was impossible from the outside.
...
export interface PipelineFault {
	...
	/**
	 * The thrown value's `name` (`TypeError`, `RangeError`, …), or `"Error"` when something that
	 * isn't an `Error` was thrown. Machine-stable enough to branch on. the `cause` carries the rest.
	 */
	name: string
	message: string
	/**
...
export interface PipelineResult {
	 ...
	 * Empty array when the coordinator ran with no grouper (pre-v0.5.0 callers) or
	 * when the fast-path skipped Stage 2.7. Stage 3 consumes this as conditioning.
	...
	/**
	 * Every stage crash the coordinator caught and degraded past, in the order they happened.
	 * **Always present** — an empty array is the coordinator stating that no stage faulted,
	 * which is a different claim from a missing field. Non-empty means the tree you are
	 * holding was produced with at least one stage down. see {@link PipelineFault}.
	 */
	 ...
	 *
	 * Nothing here changed which answer won. The markers are advisory by construction: the two
	 * intent kinds that could have displaced a structural incumbent are scored below it, and the two
	 * that do win the top slot (`near_me`, `poi_category`) route exactly as their incumbents did.
	 */
