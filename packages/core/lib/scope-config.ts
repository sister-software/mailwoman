...
/**
 * The declared tiers, keyed by the number scope.mdx prints.
 * Keys are strings because JSON has no integer keys, and the tier numbers are labels
 * rather than a scale you would do arithmetic on.
 */
...
/**
 * `scope.config.json`, as read. `$comment`-prefixed keys carry the reasoning
 * and are ignored here. a `Record` value is country code → the reason that entry exists,
 * and an empty reason is refused by the `locale-scope` check rather than by this reader,
 * so a malformed register still parses and reports rather than throwing at every call site.
 */
export interface ScopeConfig {
	tiers: Record<string, string[]>
	/**
	 * Country code → why the arc blocks on it beyond rule 6's tier-1 floor.
	 * A tier-1 country needs no entry.
	 */
	dRuleProtected: Record<string, string>
	/**
	 * Country code → why a shipping locale sits in no tier.
	 * Each is a debt with a stated reason.
	 */
...
export interface ShippedTrainingConfig {
	...
	graphPackage: string
	/**
	 * Which field of that package's `model-card.json` the path was read out of.
	 * Recorded because the two cards disagree about which of their own fields tracks the current version.
	 */
	readFrom: string
}
 ...
 *
 * Throws when the family has no entry. A family whose config nobody recorded is a family
 * whose admission numbers cannot be produced, and answering with another family's
 * config would report one graph's admissions under another graph's name.
 */
 ...
 *
 * The reason travels with the country because the arc reports a D-rule block at the moment a
 * reader is deciding whether to believe it, and "GB" alone does not answer "why is GB on this list".
 * Tier-1 membership is the reason for a tier-1 country, so those entries say so.
 */
...
/**
 * The countries a default-on change may not regress: tier 1, which iron rule 6
 * guards unconditionally, plus every country the register guards explicitly.
 * Sorted by country code so an arc's reasons read the same on every run.
 */
