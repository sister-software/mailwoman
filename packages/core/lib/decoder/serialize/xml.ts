...
function serializeNode(node: AddressNode, indent: string, opts: Required<SerializeXMLOpts>): string {
	...
	const inner = [childrenStr, altsBlock].filter((part) => part.length).join(nl)
...
export function decodeAsXML(tree: AddressTree, opts: SerializeXMLOpts = {}): string {
	...
	// Source-ordered XML for the root components.
	// When includeUnknown is set, interleave the all-O gaps as `<unknown>` elements by
	// start offset so the children tile the raw input losslessly.
