...
export interface DeclaredFeatureCountInput {
	 ...
	 * reports a layer's total and nothing narrower.
	 *
	 * Wins when present, even at zero: a chunk that declares nothing about its
	 * size passes 0 and the parent checks the sum.
	 */
	declared?: number
	limit?: number
	layerCount: number
