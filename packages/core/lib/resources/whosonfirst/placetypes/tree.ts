 ...
 *
 * WOF placetypes form a DAG (a placetype can have multiple parents — `borough`, for example,
 * is a direct child of both `country` and `macroregion`), so a tree projection rooted
 * at a single placetype can list the same descendant under multiple branches.
