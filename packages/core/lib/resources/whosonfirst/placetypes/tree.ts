 ...
 *
 * WOF placetypes form a DAG (a placetype can have multiple parents — `borough`,
 * for example, is a direct child of both `country` and `macroregion`), so a tree projection
 * rooted at a single placetype can list the same descendant under multiple branches.
 ...
 *
 * The traversal mirrors {@linkcode Placetype.findChildren} — direct children only
 * at each level, recursively. The DAG-to-tree projection may repeat descendants
 * under multiple parents. if you need each placetype to appear exactly once,
 * use {@linkcode Placetype.findDescendants} for a flat de-duplicated set instead.
 */
