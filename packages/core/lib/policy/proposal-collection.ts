 ...
 *
 * Classifiers that throw are isolated — their failure logs but does not block other
 * classifiers' proposals from being collected. (Per the `ProposalClassifier` interface,
 * implementations are supposed to swallow errors, but defense-in-depth lives here.)
 */
...
/**
 * Optional policy filter. An explicit `policy` registry is authoritative. without
 * one the input is returned unchanged.
 */
