 ...
 *
 *   The synchronous street-tier lookup interfaces the resolution ladder is built on — address point,
...
export interface AddressPointHit {
	...
	release: string
	/**
	 * The point's own scope tags, when the extract row carries them — the register's
	 * locality (normalized key form) and postcode.
	 *
	 * A rooftop answer can then be decorated with the commune/postcode the register attests,
	 * which a query that never named them cannot supply.
...
export interface AddressPointLookup {
	find(query: {
		 ...
		 *
		 * The Taiwanese register scopes a point by 縣市 and 鄉鎮市區 (`臺北市` / `中正區`), which the parse tags `region`
		 * and `subregion`; a reader built on that register composes its locality key from the pair.
		 ...
		 * (OSM addr nodes often don't) but do carry a coordinate — the resolved locality's
		 * bounding box scopes the `(street, number)` probe instead.
		 * US situs never passes it (byte-stable).
		 */
 ...
 * a subset of that shape so the concrete `StreetInterpolator`/`AddressPointInterpolator`
...
export interface InterpolationLookup {
	...
	/**
	 * The artifact's own conformal radius multiplier for `uncertaintyM` (#374),
	 * read from the extract's `interp_calibration` metadata table at open time
	 * (the pair-index δ/transitionBeta header precedent): the multiplier is a property of the calibration
	 * set the artifact was built against, so it ships in the artifact rather than in caller code.
	 *
...
/**
 * One street-centroid hit (#1042) — the street-level tier below the exact
 * address-point tier and above admin-centroid resolution.
 *
 * A street's centroid + an honest extent-derived radius, for a street-only query
 * (no house number) that an address-point tier cannot serve by definition.
 * Derived from a national register's rooftop points (`street-centroids-<cc>.db`, a `group BY street` roll-up).
 *
 ...
 * (NI's 80 BT districts) carries `ancestors` and no `lat`/`lon` — representable
 * as absence, never as `0,0` (the meaning-of-zero rule).
 * `radiusP95Km` is mandatory whenever a coordinate is present (M-3's receipt: a 1-digit US band
