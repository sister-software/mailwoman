...
/**
 * #928: distinctive postcode formats that unambiguously indicate a country — a stronger country signal than the language-based coarse placer, which conflates GB/US (both carry English street patterns) and mis-routes GB addresses to US namesakes (`London E4 9AZ` → London, Ohio) at 0.94–0.96 confidence.
 *
 ...
 *
 * Unlike the {@link POSTCODE_FORMAT_COUNTRY} singles, this shape implies a SET: no single
 * country owns it, so it can check a locale-inferred scope but never name one country outright.
 */
