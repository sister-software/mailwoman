 ...
 *
 * Upstreams that publish a flat rate (SEC edgar's 10 req/s, verified enforced) and actively police it
 ...
 * (N callers racing `acquire()` in the same synchronous turn), grants are issued in
 * whatever order the synchronous reservation happens to run in.
 * This never affects the rate — every grant is still exactly `intervalMs` after the last —
 * only which specific caller waits how long under backlog.
 *
 * A caller needing fifo fairness would need a real queue on top of this.
 */
