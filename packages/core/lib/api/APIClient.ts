...
/**
 * Milliseconds in a minute — the numerator when turning
 * {@linkcode APIClientConfig.requestsPerMinute} into an interval.
 */
...
export interface APIClientConfig {
	...
	displayName: string
	/**
	 * Where the client's own lines go. Defaults to a console logger prefixed with `displayName`,
	 * which writes debug lines to stdout. a caller that owns stdout passes `silentLogger()` or its own.
	 */
	 ...
	 *
	 * This cannot express a flat per-second rate, which is what most fair-access
	 * policies actually publish. For that use
	 ...
	 *
	 * Set this when an upstream publishes a flat rate (SEC edgar: 10 requests/second,
	 * enforced): `1000 / rate`. Unlike
	 ...
	 * Defaults to {@linkcode systemClock}; tests inject a fake clock so timing
...
export class APIClient<C extends APIClientConfig = APIClientConfig> extends EventTarget implements AsyncDisposable {
	...
	constructor(config: C) {
		...
		//
		// `axios-cache-interceptor` short-circuits a cache HIT by replacing `config.adapter`
		// with its own `cachedAdapter`, so anything installed here is reached only when the
		// request is actually going to the network. Restricting in `fetch()` instead put the
		// cache interceptor downstream of the check and made every cache hit burn a full pacer
		// sleep: measured 1 dispatch, 5 hits, five 111ms sleeps for zero network traffic.
		// `/Archives/` documents are cached for a century by design, so warm re-runs are the
		// expected mode for a bulk crawl — at 100k cached documents that is ~3 hours of sleeping
	 ...
	 * The pacing/cooldown limit deliberately does not happen here — it sits in the
	 * adapter (see the constructor), downstream of the cache, so a hit costs nothing.
	...
	public fetch = async <T>(options: AxiosRequestConfig): Promise<AxiosResponse<T>> => {
		...
		// and the pacing/cooldown limit lives in that instance adapter — so passing one here
		// would dispatch with no grant at all. Reproduced against the un-stripped form: a client
		// at `minRequestIntervalMs: 5000` issuing three concurrent `fetch({ url, adapter })`
		...
		// The difference is that it swaps on the merged config from inside the
		// interceptor chain, after this method has already handed the request over.
		// Stripping it here closes the caller-supplied door without touching the interceptor's.
		//
		// Latent when found — no shipped client passes an adapter —
		// which is exactly how the cooldown/pacer composition bug survived too.
		// Closing it before `bdc/sdk/client.ts` is written against this interface.
	 ...
	 *
	 * The bug this replaced: `fetch()` awaited a single `$cooldown` read and the request was
	 * only counted by a response interceptor. N callers invoked in the same turn all cleared
	 * the limit before any response came back to set a cooldown — measured at 40 dispatches
	 * inside 3ms against a configured budget of 2/minute, and 40 against 10/minute.
	 *
	 * The pacer is re-acquired on every pass of the loop rather than taken once up front.
	 * A grant is a claim on a specific instant. blocking on a cooldown after taking one leaves
	 ...
	 * when both limits were configured together. Re-acquiring discards the stale grant
	 * (the pacer under-issues by one per cooldown wait, which is the safe direction)
	 * and takes a fresh one for the instant we actually dispatch.
	 */
	protected acquireDispatchSlot = async (): Promise<void> => {
		for (;;) {
			...
			if (!pending) {
				// Check and reserve in the same synchronous step. Splitting them — "await the limit,
				// then count" — is the whole bug: every caller queued behind the same microtask
				// turn passes a limit that only closes once one of them has already counted,
				// so the (N-1) callers behind the budget-spending one sail straight through.
	 ...
	 * opening a cooldown once the budget is spent. Synchronous by construction:
	 * it must run to completion before the next caller can observe the counter,
	 * which is precisely what the response-interceptor version could not guarantee.
	 */
	#reserveCooldownSlot(): void {
		...
		if (this.#requestCountWithinCooldown >= requestsPerMinute) {
			...
			//
			// The original computed `(60000 / N) - elapsed`, which is the spacing between two requests
			// rather than the length of the budget window . Therefore, N dispatches went out back to back
			// and the client waited 60/N seconds before releasing another N. Measured on a bare
			// client at `requestsPerMinute: 10`, 20-call fan-out: arrivals `[0 x10, 6000 x10]` —
			// 20 inside one sliding minute against a budget of 10, a sustained 100/minute.
			// A caller trusting the docstring would have hammered an upstream at 10x its stated limit.
