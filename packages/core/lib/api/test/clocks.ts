...
export interface FakeClock extends ClockLike {
	...
	sleepCalls: number[]
	/**
	 * Advance the clock without recording a `sleepCalls` entry — simulates wall-clock time passing
	 * between two calls (e.g. "a day later") without the code under test having awaited anything.
	 */
...
export class VirtualClock implements ClockLike {
	...
	public async runUntilSettled<T>(work: Promise<T>): Promise<T> {
		...
		for (;;) {
			...
			// `settled` is flipped by the callbacks above rather than by this loop body — hence the
			// explicit break rather than a loop condition (which reads as unmodified to a static analyzer).
			if (settled) break
...
/**
 * The maximum number of `timestamps` falling within any sliding window of `windowMs`,
 * using a trailing half-open window `(anchor - windowMs, anchor]` at each timestamp —
 * the conventional "requests in the last N ms" definition.
 */
