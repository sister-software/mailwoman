 ...
 *
 * This is effectively a wrapper around `JSON.stringify` that returns a branded type to indicate
 * that the output is valid JSON. It also allows for optional newline and indentation settings.
 *
 * @param input The object to be pretty-printed.
 * @param newline Whether to append a newline character at the end of the
 *   output. Defaults to `true`.
 * @param space The indent: a string to repeat, or a count of spaces.
 *   Defaults to a tab character (`"\t"`). Both forms are accepted because both are
 *   what the builtin accepts, and a caller converting `2` to `" "` at the call site
 *   would be doing the conversion this parameter exists to hold.
 *
 ...
 *
 * This is effectively a wrapper around `JSON.stringify` that returns a branded
 * type to indicate that the output is valid JSON. It is also one of two places in
 * the codebase that is allowed to use `JSON.stringify`
 *
 * @param input The object to be stringified.
 * @param keys An allowlist of property names, in the order they should print.
 *   Two call sites need it and neither is cosmetic: a cache key that must omit the API key
 *   and fix the order of what remains, and a regenerated seed file that must diff only
 *   where a value changed. Passing the list here keeps both on the branded printer.
 *
 ...
 *
 * Non-throwing: invalid JSON — and any non-string input, `Buffer` included — returns the
 * fallback (`null` unless one is given). Callers that need a throw on corrupt input,
 * or `JSON.parse`'s reviver parameter, use `JSON.parse` directly behind a scoped lint disable.
 */
...
/**
 * A JSON array carried inside a string field. `undefined` answers `[]`; a value that
 * parses to anything but an array throws, because a non-array there is a schema change
 * at the source rather than something to coerce — an empty array would read as "none",
 * which is not what a differently-shaped value means.
 *
