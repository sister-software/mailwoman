 ...
 *
 * Typed `never`, so a caller gets definite-assignment narrowing after the call.
 */
...
/**
 * The one blessed way to build a child-process environment: the current environment with explicit overrides.
 *
 * Everything outside `core/env` + this module is forbidden from touching `process.env` directly
 * (enforced by `scripts/lint-raw-env-argv.ts`) — read config through `$public`/`$private`.
 */
