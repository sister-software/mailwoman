 ...
 * They were previously `other` outlier exposure (PL/PT/CZ) or simply unrepresentable. here
 * they become first-class in-map countries so the soft country prior can pin them.
 */
export const NEW_EU = [
	"AT",
	"BE",
 ...
 * country are Overture format. Without an Overture sample of their own,
 * their Overture-format eval rows scatter to the Overture-trained neighbours
 * (measured: only 63% of ES eval rows routed ES, ~26% leaked to CH/PT/HR/IT/FR/CZ). supplement
 * their corpus rows with an Overture sample so each owns its own format shape. the format
 * then stops being discriminative and the model falls back to the linguistic n-grams.
 * GB excluded — its Overture parquet is empty.
 */
