...
/**
 * The rungs a generic can name, coarsest first.
 *
 * `headquarters` is not a rung: `场部` names the central settlement of the unit it follows,
 * a point inside the unit rather than a further level, and stays inside the span.
 */
...
/**
 * The ordinals an organizational unit is numbered with: Chinese numerals
 * (`三`, `二十九`, `一零三`, `十五`) or Arabic digits.
 *
...
/**
 * One ordinal unit: `三分场`, `二十九队`, `一零三团`, `十五分场`.
 *
 * A headquarters marker carries no ordinal.
 */
 ...
 * `红卫大队` is a brigade-era toponym that survives as a village name, so it carries
 * no chain and the whole run stays the head.
 *
 * `null` when the run carries no chain at all.
 */
