...
describe("clean", () => {
	test("allows only declared generated roots and their descendants", async () => {
		...
		const output = scratch.resolve("packages/core/out")

		expect(() => assertDirectoryIsUntracked(output, [output])).not.toThrow()
		expect(() => assertDirectoryIsUntracked(scratch.resolve("packages/core/out/generated"), [output])).not.toThrow()

		expect(() => assertDirectoryIsUntracked(scratch.resolve("packages/core/outside"), [output])).toThrow(
			/outside declared generated directories/u
		)

		expect(() => assertDirectoryIsUntracked(scratch.resolve("packages/core/lib"), [output])).toThrow(
			/outside declared generated directories/u
		)

		expect(() => assertDirectoryIsUntracked(scratch.resolve("packages"), [output])).toThrow(
			/outside declared generated directories/u
		)
	})
	...
	test("dry-run validates without changing directories or files", async () => {
		...
		const output = scratch.resolve("packages/core/out")
		const artifact = scratch.resolve("packages/core/out/stale.js")
	...
	test("cleans existing artifacts and creates an allowed missing directory", async () => {
		...
		const missing = scratch.resolve("packages/core/dist")

		await cleanDirectory(missing, { allowedRoots: [missing] })
		expect(await pathExists(missing)).toBe(true)

		const buildMetadata = scratch.resolve("packages/core/tsconfig.tsbuildinfo")
