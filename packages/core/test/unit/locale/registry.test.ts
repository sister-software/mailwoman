...
import { enUS, frFR, jaJP } from "@mailwoman/core/locale/profiles"
import { InMemoryLocaleRegistry } from "@mailwoman/core/locale/registry"
...
describe("LocaleProfile — bundled profiles", () => {
	...
	test("ja-JP omits street but carries house_number (CJK Phase 2 / D4)", () => {
		// JP remains forward-compatible: it has no `street` role (district/block carry that ladder) and
		// the core abstraction must accept the omission. D4 added house_number —
		// the compact number ("2-3-16") is one whole-span house_number. the fine
