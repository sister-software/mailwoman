 ...
 *   pair. It is here for the same reason the readers are: `@mailwoman/platform/fs` is the `node:fs` mirror, and
 *   `packages/core/fs/*` is the only place that reaches it.
 *
...
import type { ReadStream, WriteStream } from "@mailwoman/platform/fs"
import type { PathBuilderLike } from "path-ts"

/**
 * The runtime's own stream types, re-exported for the same reason the readers re-export theirs.
 */
export type { ReadStream, WriteStream } from "@mailwoman/platform/fs"
