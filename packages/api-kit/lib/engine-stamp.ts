...
/**
 * `Server` names the engine and its license branch; `Link: rel="license"` is the registered relation
 * (RFC 8288) that lets a proxy, a browser, or `curl -I` find the terms without a body change.
 *
 * Set before the handler runs, so the headers are on the context when any `c.json` —
 * the route's or the error net's — builds its response.
 */
