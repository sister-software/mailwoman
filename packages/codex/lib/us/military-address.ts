...
export const US_MILITARY_POST_OFFICE_CODES = [
	/**
	 * Army Post Office — domestic USPS gateway for Army and Air Force overseas mail.
	 *
	 * Also used for some diplomatic addresses (DPO is preferred for State Dept posts since 2011).
	 */
...
/**
 * City-line regex: APO/FPO/DPO <region> <zip> USPS military ZIP assignment per
 * Pub 28 and the Armed Forces zip code list: - AA (Americas): 340xx range -
 * AE (Europe/ME/Africa/Canada): 09xxx range - AP (Pacific): 962xx-966xx range The regex
 * accepts any 5-digit or 9-digit ZIP code in combination with a valid region code —
 * validating the specific numeric range for each region is left to the caller
 * (region+ZIP co-validation is operational policy rather than structural syntax).
 */
