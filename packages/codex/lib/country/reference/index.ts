...
/**
 * The flag emoji for an ISO 3166-1 alpha-2 country code, formed from the two Regional
 * Indicator Symbols (`US` → 🇺🇸). Returns `""` for anything that is not two ascii letters.
 */
...
/**
 * Fill the country-reference fields (ISO 3166 alpha-2, flag, calling code, currency) from
 * a resolved country code. Abstains on anything that isn't two ascii letters.
 */
