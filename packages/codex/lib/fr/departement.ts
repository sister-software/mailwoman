 ...
 */
// `unknown`, not `string | null | undefined`: this function's whole job is to answer
// null for anything that is not a departement code, and its callers hand it values off
// a CSV row or a JSON body. A narrower parameter did not make those callers safer —
// it made the guard test assert its way past the signature.
