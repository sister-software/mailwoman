...
describe("GB's single-line form", () => {
	...
	it("moves the postcode's separator when the post town is absent, and nothing else", () => {
		// The mark names the break preceding the postcode line, so it applies to whatever line survives above it. This
		// case moved with the mark: it read `27 Minories, EC3N 1DE` before. Nobody has measured which form dominates a
		// GB address written with no post town — the 10,282,560-to-3,265,642 count is over rows carrying both — so this
		// case pins the behavior and claims nothing about the register.
		expect(
...
${SLOTS.postcode}`,
	...
	it("keeps the mark on the right break when a line above it renders nothing", () => {
		// `evaluateLines` drops the empty locality line and renumbers what survives. The mark names the line a break
		// precedes, so it travels with the postcode rather than landing on the break above it.
