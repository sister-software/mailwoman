 ...
 *
 * The elevation archive is `<body>-terrain`, not `<body>-hillshade`, and the rename
 * is the point rather than a tidy-up. Its content changed — from a shaded greyscale
 * picture to terrarium-encoded height — and the two are indistinguishable to a cache.
 * Publishing the new bytes under the old name would leave the edge free to serve a cached
 * picture to a client that reads it as elevation, which renders as relief that is wrong
 * rather than absent. A new name also leaves the old archive in place to roll back to.
 */
