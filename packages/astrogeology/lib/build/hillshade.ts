 ...
 *
 * A source that does not span the whole body is refused: the extent assigned below is the whole body's.
 */
 ...
 *
 * The usgs mosaics are integer metres, so it is constant zero and the encoding keeps
...
/**
 * Build a body's hillshade archive.
 *
 * Answers the exact tool invocations, in order, for the manifest.
 */
export async function buildHillshadePMTiles(
	...
	// 2. Encode terrarium: R is the high byte of the offset height, G the low byte, B the fractional metre.
	//    Each band is computed separately because `gdal_calc.py` writes one band per run,
	//    then the three are stacked.
