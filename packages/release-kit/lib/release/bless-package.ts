 ...
 * prompting for a code — without this operation standing in the middle of it. stdin and stdout stay inherited (npm
 * keeps a real TTY for any prompt); only stderr is piped, because that is where npm writes the approval URL and we want
 * to read it on the way past. zx forwards piped output to the terminal itself, so nothing here re-emits it.
 */
