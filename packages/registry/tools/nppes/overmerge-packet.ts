...
export function writeOvermergePacket(path: string, input: OvermergePacketInput): number {
	const { entities, rows, recordCount, maxNpis, orgNameLabel } = input
	...
	const out: string[] = [
		...
		"",
		`TX --max-npis ${maxNpis} · ${recordCount} records · generated by \`registry scorer-eval nppes-benchmark --dump-overmerges\``,
		"",
