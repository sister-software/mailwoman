 ...
 * a predicted pair is two records in the same entity. `totalRecords` is the labelled population
 * and only feeds the adjusted-Rand expectation, so it must be the whole record set
 * rather than the clustered subset — a smaller value inflates ARI.
 */
