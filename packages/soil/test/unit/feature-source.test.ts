...
import { createShapefileFeatureSource } from "@mailwoman/soil/sdk/ingest"
...
describe("createShapefileFeatureSource: the declared count", () => {
	it("is the layer's count when no limit is given", async () => {
		const source = await createShapefileFeatureSource({ shapefilePath: "/nowhere/soil.shp", areaSymbol: "IA001" })

		expect(source.declaredFeatureCount).toBe(5)
	})

	it("is the limit when the limit is below the layer's count", async () => {
		const source = await createShapefileFeatureSource({
			shapefilePath: "/nowhere/soil.shp",
			areaSymbol: "IA001",
			limit: 3,
		})
