...
export class SoilDataAccessClient extends APIClient<APIClientConfig> {
	...
	public async query(sql: string): Promise<string[][]> {
		const { data } = await this.fetch<string>({
			...
			// text rather than JSON, and that is the whole trap.
			// A JSON response type hands a failure body to a JSON parser, which either throws
			// something unrelated to what went wrong or — on a 200 — yields nothing at all.
			responseType: "text",
	...
	/**
	 * The published survey areas whose symbol starts with `prefix` — a state code for
	 ...
	 *
	 * This is the second path the built artifact is checked against: same authority,
	 * different distribution channel, and geometry this package has never touched.
	 * Measured at 1.807 s per point, so a few hundred points is minutes.
	 */
...
/**
 * Build a {@link SoilDataAccessClient} with the disk cache and pacing this
 * package's acquisition path expects.
 */
