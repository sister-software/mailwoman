 ...
 *
 *   The ring blob — how `flood_zone_area.rings` stores one authority polygon's UNSIMPLIFIED coordinates,
 *   and the point test that reads it without materializing them.
 *
 *   THE GEOMETRY IS THE TRUTH TABLE. The H3 index above it answers a cell that lies wholly inside one
 *   zone; a cell the boundary crosses falls through to a ray cast against the few polygons the index
 *   already named. A rooftop answer at a zone boundary — which is where a flood answer matters most — has
 *   no other honest source, so the rings are stored at the resolution the authority published them and
 *   never simplified.
 *
 ...
 *   `pointInPolygon` in `@mailwoman/spatial` uses), but it still has to know which rings share a polygon:
 *   an island sitting inside another polygon's hole is inside the layer and would cancel to "outside" if
 *   every ring were pooled into one list.
 */
 ...
 */
export const FLOOD_RING_BLOB_VERSION = 1
...
/**
 * One decoded polygon: `[exteriorRing, ...holes]`, each ring a flat `[lon, lat, lon, lat, …]` run.
 */
 ...
 */
export function encodeRings(polygons: ReadonlyArray<ReadonlyArray<ReadonlyArray<readonly number[]>>>): Uint8Array {
		...
		for (const ring of rings) {
			if (ring.length < MINIMUM_RING_POSITIONS) {
				throw new RangeError(
					`flood rings: a linear ring needs at least ${MINIMUM_RING_POSITIONS} positions, got ${ring.length}`
				)
			}
	...
	if (!entries.length) {
		throw new RangeError("flood rings: geometry carries no ring")
	}
	...
	view.setUint32(0, FLOOD_RING_BLOB_VERSION, true)
 ...
 *
 * @throws {Error} When the blob's version is not {@linkcode FLOOD_RING_BLOB_VERSION}, or its declared ring table does
 *   not account for the bytes present. A blob that is silently mis-read answers a containment question wrongly, and a
 *   wrong containment answer is indistinguishable from a real one.
 */
function openRings(blob: Uint8Array): {
	...
	if (version !== FLOOD_RING_BLOB_VERSION) {
		throw new Error(`flood rings: blob version ${version}, expected ${FLOOD_RING_BLOB_VERSION}`)
	}
	...
	if (expected !== blob.byteLength) {
		throw new Error(`flood rings: blob declares ${expected} bytes of geometry, holds ${blob.byteLength}`)
	}
 ...
 * check the build's fixtures assert.
 */
 ...
 */
export function ringAreaReadings(polygons: ReadonlyArray<ReadonlyArray<ReadonlyArray<readonly number[]>>>): {
	nested: number
	allExterior: number
} {
	let nested = 0
