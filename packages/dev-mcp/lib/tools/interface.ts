...
import { runInterfaceCensus } from "#interface/report"
import type { DevTool, DevToolDeps } from "#tool-kit"
import { ENGINE_CONFIG_SCHEMA, INPUT_SET_SCHEMA } from "#tool-kit"

export const interfaceTool = ({ registry }: DevToolDeps): DevTool => ({
	name: "mwdev_interface",
	description:
		"DOES THE TREE OBEY ITS OWN INTERFACE — `validateTree`'s two structural invariants, run at board scale. A parse " +
	...
	inputSchema: z.object({
		inputs: INPUT_SET_SCHEMA.optional().describe(
			"Defaults to the full board. Needs no truth of any kind — the interface is a claim a tree settles about " +
	...
	handler: async (args) => runInterfaceCensus(registry, args),
})
