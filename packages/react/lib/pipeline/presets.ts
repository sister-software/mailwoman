 ...
 *
 * Chosen because it exercises every stage — house number, street, directional,
...
export const PIPELINE_PRESETS: ReadonlyArray<Preset> = [
	...
	// GB dependent_locality (placetype-pair-prior arc) — "Henbury" flips to
	// dependent_locality via the en-gb pair-index prior.
	// The UK postcode is also structurally detectable (parity with docs EXAMPLE_ADDRESSES).
	{
	...
	// Postcode deliberately omitted: a trailing "Porirua 5026" folds "porirua 5026" in
	// segment mode and misses the index's bare "porirua" key (tracked as #1308).
	// The `country: "nz"` pin is required — locale-hint can't structurally detect NZ
	// (4-digit postcode isn't distinctive), so only the pin selects the nz index.
	// Free-typed NZ stays unfired.
