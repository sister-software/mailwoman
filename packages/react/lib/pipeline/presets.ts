...
/**
 * Address the pipeline explorer opens on. Chosen because it exercises every stage —
 * house number, street, directional, locality and region — in one line.
 */
export const PIPELINE_DEFAULT_ADDRESS = "1600 Pennsylvania Ave NW, Washington, DC 20500"

// `country` mirrors the docs `EXAMPLE_ADDRESSES` set (kept in parity) — the placetype-pair
// country pin (#1278 phase 2's `{country}` override). A host that wires the pair prior
// pins the country while the input equals the preset text, so a locale structural routing
// can't detect (NZ) still fires. free-typed input drops to structural detection.
/**
