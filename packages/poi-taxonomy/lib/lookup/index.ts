 ...
 *   classifier's job rather than this package's.
 *
 ...
 *
 * `locale` selects locale-restricted synonyms with the variant-aliases semantics: exact locale
 * 1.0, language-only 0.5, otherwise no match. unrestricted phrases always match at 1.0.
...
/**
 * Resolve a canonical seed category id to the Overture `taxonomy.primary` leaf
 * ids a built `poi.db` stores for it.
 *
 * Seed ids that declare no `overtureCategories` resolve to `[seedID]` (identity);
 * an unknown seed id resolves to `[]`.
 * The POI executor fans a category query out over this list and re-tags the hits with the canonical seed id.
 */
