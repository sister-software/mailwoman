...
export const AddressRole = {
	...
	Mailing: "mailing",
	/**
	 * Where a licensed person or organization practices. A provider's practice location,
	 * a notary's place of business.
	 */
	Practice: "practice",
	/**
...
/**
 * The role a row carries when it does not say. Frozen corpora written before the
 * field existed read as premise, which is what the sources that produced them emit.
...
export interface SourceProvenance {
	...
	/**
	 * Stable id within the adapter's source. For SQLite-backed adapters this is
	 * the row's primary key. for CSV/GeoJSON, a hash of the canonical components.
	 * Must be stable across reruns so that dedup and holdout manifests are reproducible.
	 */
	source_id: string

	/**
	 * Corpus version string. Stamped by the runner rather than the adapter.
	 * Locked together with the tokenizer version: `corpus-v0.1.0` ships with `tokenizer-v0.1.0`.
	 */
	corpus_version: string

	/**
	 * Short license label or spdx id for _this_ row. Defaults to the adapter's
	 * `defaultLicense`, but per-row sources (OpenAddresses) override.
	 */
	license: string
}

/**
 * Marker placed on rows produced by `synthesize.ts`. Allows downstream code to weight,
 * stratify, or exclude augmentations.
 */
 ...
 * mapped to its surface form _as it appears in `raw`_. Alignment uses this to assign BIO labels.
 *
 * Country is ISO 3166-1 alpha-2 (`"US"`, `"FR"`). Locale is BCP-47 (`"en-US"`, `"fr-FR"`)
 * and is optional. adapters that can't be sure leave it empty and let the runner default by country.
 */
export interface CanonicalRow extends SourceProvenance {
	...
	/**
	 * Component-by-tag ground truth. Surface forms must occur in `raw`
	 * (within the alignment edit distance threshold) or the row will land in the quarantine pile.
	 */
	...
	/**
	 * What this address is the address of. The runner stamps the adapter's `addressRole`
	 * on every row that omits it, so an adapter sets this per row only when one source
	 * carries more than one role — Taiwan's company register holds the registered address
	 * and the tax office's business address in separate columns of the same row.
	 *
...
/**
 * Output of `align.ts`. Carries everything `CanonicalRow` does, plus parallel `tokens`
 * and `labels` arrays of identical length (`labels[i]` is the BIO tag for `tokens[i]`) and —
 * as of the v0.5.0 char-offset migration (#519) — parallel char-span arrays addressing `raw` directly.
 *
...
export interface LabeledRow extends CanonicalRow {
	...
	/**
	 * Char-offset label spans over `raw` (parallel arrays, per the #519 ruling): `span_starts[i]`
	 * is the inclusive start offset (UTF-16 code units) of span `i`, `span_ends[i]` its exclusive
	 * end, `span_tags[i]` its component tag. Invariants — enforced loudly by `alignRow`,
	 * documented for every other producer: sorted ascending by start, non-overlapping.
	 * `raw` must be NFC-normalized or the offsets are ambiguous (also enforced by `alignRow`).
	 *
	 * Optional during the v0.4.x → v0.5.0 transition only: alignment always emits the
	 * triple. frozen historical corpora and not-yet-migrated synthesis paths may lack it.
 ...
 * `"edit-distance-exceeded:<tag>"`, `"raw-empty"`. Re-running alignment after a fix
 * should re-emit the quarantined rows. the runner keys them by `source_id`.
 */
 ...
 * files, or even an https URL. Each adapter documents its own expected shape in its readme.
 *
 ...
 *
 * Adapters are async generators: they yield `CanonicalRow`s one at a time,
 * the runner consumes them (writing jsonl + maintaining checksums + driving alignment).
...
export interface CorpusAdapter {
	/**
	 * Stable, machine-friendly id used in paths and CLI
	 * args. E.g. `"wof-admin"`.
	 */
	readonly id: string
	 ...
	 * Required, and with no default, because the answer is a property of the source that only
	 * the adapter's author has read: a health-provider registry carries practice locations,
	 * a tax-exempt-organization file carries mailing addresses, and a national address
	 * register carries premises. A field that defaulted would record premise for all three.
	 *
