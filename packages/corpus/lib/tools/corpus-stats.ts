...
async function discoverParquetFiles(pathArg: string): Promise<string[]> {
	...
	if (stat.isDirectory()) {
		const root = PathBuilder.from(pathArg)
		const names = await Globerator.files("parquet", { cwd: root, recursive: false }).toSorted()

		return names.map((name) => root(name).toString())
	}
...
export async function buildCorpusStats(args: CorpusStatsOptions): Promise<void> {
	...
	// Token stats stay complete.
	// They're cheap and we need accuracy at the long tail for label-vacuum detection.
	let prunedBigrams = 0
