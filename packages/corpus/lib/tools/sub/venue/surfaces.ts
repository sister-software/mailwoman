 ...
 * see {@link SubVenueSurface.phrase}.
 */
const CASE_FOLDING_SCRIPT = /^[\p{Script=Latin}\p{Script=Cyrillic}\p{Script=Greek}\d\s\p{P}]+$/u

/**
 * Scripts written without spaces between words, where a token split cannot find a designator
 * and a substring test is the correct operator. Han, Hiragana, Katakana.
...
/**
 * Normalize a surface for the table: trim, collapse internal whitespace,
 * and lowercase only when the string is entirely in a bicameral script.
...
/**
 * A phrase → record index, keyed on the normalized phrase.
 * Built by {@link buildSurfaceIndex} from the surfaces present before the harvest runs,
 * and the reason a matched phrase can be attributed to the record it actually names.
 */
export type SurfaceIndex = ReadonlyMap<string, { recordID: string; recordKind: "designator" | "modifier" }>

/**
 * Index the surfaces accumulated so far by phrase. First writer wins, so a seed record
 * beats a Wikidata alias that happens to collide — `terminal` stays the `terminal`
 * designator even though it is also an Italian alias for it.
 */
 ...
 *
 * Token-boundary matching for Latin script rather than substring: `Nordterminal` is a real
 * German compound in which `terminal` is a suffix, and a substring test would also fire
 * on `Terminalstraße`. The compound case is a genuine miss and it is the right miss —
 * admitting suffix matches would fire on every `-hall`/`-gate` compound in Germanic
 * and Nordic street naming, which is exactly the confound class `Briggate`/`Kirkgate` represents.
 *
 * For Han/Kana names that rule finds nothing at all, because the script has no
 * word boundaries: `第1ターミナル` splits into one token that matches no surface.
