...
/**
 * The key a manifest written before the 2026-09-01 vocabulary rename uses for its file list, spelled the way the reader
 * spells it: by concatenation, because the word is banned in this tree and the ratchet's baseline is zero.
 */
...
describe("baseManifestFiles", () => {
	it("reads a manifest written before the 2026-09-01 rename", () => {
		...
		// `v0.28.0-reviewed-postcode-tail` declaring 706 train files and resolving ONE. An overlay assembled from a
		// base read as empty carries no base files at all, which is a corpus of only the overlay.
	...
	it("REFUSES a base that lists no files under either key", () => {
		...
		// would be written with its own files and nothing else, and read as a complete corpus.
