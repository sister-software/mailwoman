...
describe("usgov-irs-bmf adapter", () => {
	...
	it("tags a PO-box street line as po_box (not street)", async () => {
		const rows = await collect(
			await writeCSV("010674605,IGLESIA FUENTE DE AGUA VIVA,PO BOX 3869,CAROLINA,PR,00984-3869")
		)
	...
	it("splits a numbered street into house_number + street", async () => {
		const rows = await collect(await writeCSV("123456789,COMMUNITY FOUNDATION INC,1234 MAIN ST,SAN JUAN,PR,00901"))
		const r = rows[0]!
	...
	it("drops the +4 from a ZIP+4 postcode", async () => {
		const rows = await collect(await writeCSV("111,ORG A,PO BOX 1,PONCE,PR,00731-1234"))
	...
	it("skips rows missing city or zip", async () => {
		const rows = await collect(
			await writeCSV(
				"1,NO CITY,PO BOX 1,,PR,00901",
				"2,NO ZIP,PO BOX 2,SAN JUAN,PR,",
				"3,OK ORG,PO BOX 3,SAN JUAN,PR,00901"
			)
		)
	...
	it("honors the row limit", async () => {
		const rows = await collect(await writeCSV("1,A,PO BOX 1,SAN JUAN,PR,00901", "2,B,PO BOX 2,SAN JUAN,PR,00901"), {
			limit: 1,
		})
