 ...
 *
 *   Dev-weights linker for `@mailwoman/neural-weights-ja-jp`.
 *
 ...
 *   family's and this links only the per-locale FST (`fst-ja-jp.bin`) from the shared build area, so
 *   `resolveWeights({locale: "ja-jp"})` surfaces `fstPath` in local dev.
 */
