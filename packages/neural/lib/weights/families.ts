...
/**
 * How a family's graph reads its input. `sentencepiece` consumes subword ids from `tokenizer.model`; `char` consumes
 * code points indexed by `char-vocab.json`. The value matches the `encoder` field a model card declares.
 */
...
/**
 * The vocabulary artifact each encoder requires beside `model.onnx`. A standalone family package ships both files. An
 * overlay ships neither and resolves them from its base.
 */
 ...
 *
 * `caller` is the ordinary path: no predicate claimed the input, so it runs on the locale the caller opened the process
 * with. The two script readings are distinguished because they disagree on the case the segment rule exists for — a Han
 * address line beside a Latin province folds to `mixed` and only `script-segment` reaches it.
 */
 ...
 *
 * `family` is `undefined` when no predicate claimed the input, and {@linkcode RoutingDecision.abstainedBecause} then
 * says which reading declined. A caller that wants only the answer reads `family`; a caller measuring a router —
 * confusion matrix, abstention rate, false routes by country — needs the reason, and reconstructing it from the answer
 * alone is impossible once two predicates can name the same family.
 */
export interface RoutingDecision {
	...
	source: RouteSource
	/**
	 * How strongly the reading claims the input, in `[0, 1]`. The script predicates are rules rather than scorers and
	 * answer `1` when they fire, so this carries information only once a scored predicate exists.
	 */
	confidence: number
	/**
	 * Which reading declined, when `family` is `undefined`. Absent on a decision that named a family.
	 */
...
export interface WeightsFamily {
	/**
	 * The family id. It is also the locale segment `resolveWeights` uses for the graph package, so `cjk` resolves
	...
	/**
	 * Every locale this family serves, including the graph package's own. A locale appears in exactly one family, which
	 * the `weights-family` repository check enforces against the installed `neural-weights-*` manifests.
	 */
	...
	/**
	 * Locales that route here when a locale predicate is read. Declared for completeness of the contract and read by no
	 * shipping router. Enabling one changes which graph serves an existing request, and
	 * `docs/engineering/CONTRIBUTING_MODEL_WORK.mdx` requires a route comparison and a full regression board before that
	 * ships.
	 */
...
/**
 * The Latin SentencePiece family. Eight overlays declare `mailwoman.baseWeights` on its graph package and ship
 * retrieval artifacts only.
 *
 * It declares no routing predicate. Every Latin request reaches it through the caller's locale, which is the behavior
...
/**
 * The character family. Japanese, Chinese and Korean share one graph and one character vocabulary, so the four scripts
 * below name one family rather than three.
 */
