...
export interface PostcodeAnchor {
	...
	confidence: number
	/**
	 * `exact` — the string is a real postcode; `outward` — a GB unit (`SO4 3RX`) resolved
	 * to its outward district (`SO4`), the granularity the GB gazetteer is aggregated at
	 * (no penalty — it is a real, confident GB match); `fuzzy` — only an edit-distance-1 variant exists
	 * (a likely typo / OCR slip), so the confidence carries a penalty; `none` — in no gazetteer.
	 */
...
/**
 * Normalize a shaped span to the canonical gazetteer key: uppercase, collapse internal whitespace to a
 * single space, and strip the German `D-` courtesy prefix (the extracts store `68161`, not `D-68161`).
 */
