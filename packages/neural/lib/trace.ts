...
export interface NeuralParseTrace {
	...
	pieces: TracePiece[]
	/**
	 * Postcode-anchor channel exactly as fed (post-choreography).
	 * Absent = channel not fed.
	 */
	anchor?: SoftFeatureChannel
	/**
	 * Gazetteer channel exactly as fed (post-suppression).
	 * Absent = channel not fed.
	 */
	...
	spanScores?: number[][][]
	/**
	 * The locale-head axis: the country code each `localeLogits` index means, serialized
