 ...
 * `createScorer` throws on the same condition (the eval path fails closed),
...
export function buildAnchorFeatures(
	...
	if (options.spanMode === "shaped") {
		for (const match of collectMatches(asciiUpper(text))) {
			...
			// Not `\s+`, not the `D-` strip `normalizePostcode` does — those would diverge from what trained.
