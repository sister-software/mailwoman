...
/**
 * Delta threshold for the capability-manifest check (#718/#719): a conventions row may forbid a tag
 * only if the mask does not provably destroy a real capability — i.e. `maskOffF1 − maskOnF1 ≤ 5pp`.
 *
 * A delta rather than an absolute floor: a tag the model emits at 0.80 is guarded if the mask drops it
 * to 0.0, but a tag the mask leaves intact (small/zero delta) is legal regardless of its absolute F1.
 */
 ...
 * else the soft-feed sibling the weights package ships, so eval + serving read the same artifact.
 *
...
async function resolveAnchorSource(
	...
	// A model that declares `shaped` was trained against a lookup with letter-containing
	// keys, so preferring the pilot file for it grades the candidate against a
	// lookup that cannot carry the spans it learned.
	// Silent, and the failure looks like "the retrain did nothing".
...
/**
 * The card-scoped repo candidate for the street-type lexicon (#1510):
 * the card-declared generation when the card names one, else the legacy literal
 ...
 *
 * Card-declared only — never inferred from the graph, because the ONNX inputs are identical either way.
 * An undeclared card (every bundle shipped before that date) yields `undefined`,
...
export interface CreateScorerOpts {
	 ...
	 *
	 * Why this exists (#1497).
	 * `loadFromWeights` sets `fstPath` from the resolved weights package; `createScorer` builds
	 ...
	 * except for a card declaring `span_mode: "shaped"`, which inverts the order.
	 * See {@link resolveAnchorSource}.
	 */
	anchorLookupPath?: PathBuilderLike
	/**
	 * Gazetteer-anchor lexicon path.
	 *
	 * Default {@link DEFAULT_GAZETTEER_LEXICON} when it exists, else the soft-feed
	 * sibling shipped in the weights package (#718 D1).
	 */
	...
	/**
	 * Country-surface lexicon path (#1104).
	 *
	 * Default {@link DEFAULT_COUNTRY_LEXICON} when it exists, else the soft-feed
	...
	strict?: boolean
	/**
	 * Serving tier whose certified capabilities the load-time delta check
	 * (#718/#719) reads from the card's `capabilities` block: `"server"`
	 * (anchor+gazetteer — the production default) or `"pocket"` (anchor-only).
	 *
	 * Default `"server"`.
	 * A tier the card doesn't certify → the check has no capability claims to consult and is a no-op (legal).
	 */
	tier?: string
	/**
	 * Deliberate, declared ablations (warn-not-throw).
	 *
	 * See {@link ScorerOverrides}.
	 */
...
/**
 * The load-time delta check (#718/#719).
 *
 * Iterate the codex `ADDRESS_SYSTEM_CONVENTIONS`; for every `forbiddenTags` entry,
 * look up the loaded tier's certified capability for that (system, tag).
 ...
 * mask-on measurement → forbidding it is rejected at load time.
 *
...
async function assertConventionsRespectCapabilities(
	...
	if (!manifest) {
		// No certified capabilities → nothing to guard.
		// Old cards still load (warn once per process).
...
export async function createScorer(opts: CreateScorerOpts): Promise<NeuralAddressClassifier> {
	...
	if (overrides.anchor === false) {
		...
		// Fail closed: declared-required but the lookup is missing or parsed empty → the
		// model would be fed zeros (the anchor-off identity) and silently go OOD.
