...
import { readDeclaredArtifactFile, unfedAnchorDetail } from "./weights.ts"
...
describe("readDeclaredArtifactFile", () => {
	...
	it("reads the SHIPPED cards: en-us/fr-fr/en-gb declare their binaries, en-nz declares none", () => {
		...
		expect(readDeclaredArtifactFile(workspacePath("neural-weights-en-us"))).toMatchObject({ file: "postcode-us.bin" })
		expect(readDeclaredArtifactFile(workspacePath("neural-weights-fr-fr"))).toMatchObject({ file: "postcode-fr.bin" })
		expect(readDeclaredArtifactFile(workspacePath("neural-weights-en-gb"))).toMatchObject({ file: "postcode-gb.bin" })
		expect(readDeclaredArtifactFile(workspacePath("neural-weights-en-nz"))).toBeUndefined()
	})
})
