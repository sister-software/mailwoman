...
const readCard = (rel: string) => parseJSONStrict<ModelCard>(readFileSync(workspacePath("neural", "test", rel), "utf8"))
	...
	for (const overlay of OVERLAYS_WITH_CARDS) {
		for (const channel of EVIDENCE_CHANNELS) {
			test(`${overlay} declares ${channel} as the base does, or declares no block at all`, () => {
				const card = readCard(`../../neural-weights-${overlay}/model-card.json`)
