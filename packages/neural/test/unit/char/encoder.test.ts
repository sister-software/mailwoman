...
interface Fixture {
	interface: { max_units: number; max_unit_width: number; ctx_chars: number; pad_id: number; unk_id: number }
...
const encoderInterface = {
	maxUnits: fixture.interface.max_units,
	maxUnitWidth: fixture.interface.max_unit_width,
	ctxChars: fixture.interface.ctx_chars,
}

describe("encodeCharUnits", () => {
	it("agrees with the Python encoder on every fixture row", () => {
		expect(fixture.interface.pad_id).toBe(PAD_CHAR_ID)
		expect(fixture.interface.unk_id).toBe(UNK_CHAR_ID)
		expect(fixture.rows).toHaveLength(56)

		for (const row of fixture.rows) {
			const encoded = encodeCharUnits(row.raw, vocabulary, encoderInterface)
	...
	it("treats an astral code point as one unit and reads it back out of the input by UTF-16 offsets", () => {
		...
		const encoded = encodeCharUnits(raw, vocabulary, encoderInterface)
		...
		expect(encoded.charIDs[encoded.units.indexOf(astral)]![encoderInterface.ctxChars]).toBe(UNK_CHAR_ID)
	})

	it("truncates to S units and pads a short row with attention 0", () => {
		const long = encodeCharUnits("東京都千代田区丸の内一丁目".repeat(8), vocabulary, encoderInterface)
		expect(long.units).toHaveLength(encoderInterface.maxUnits)
		expect(long.attentionMask.every((bit) => bit === 1)).toBe(true)

		const short = encodeCharUnits("東京", vocabulary, encoderInterface)
		...
		expect(short.charIDs[2]).toEqual(new Array(encoderInterface.maxUnitWidth).fill(PAD_CHAR_ID))
	})
})
