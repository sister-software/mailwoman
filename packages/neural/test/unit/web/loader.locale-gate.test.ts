		...
		test(`@mailwoman/${pkg}: no node:* / fs / path / process runtime import in the transitive source`, async () => {
			...
			for (const file of files) {
				...
				for (const line of TextSpliterator.from(text)) {
					...
					// A value import of @mailwoman/core would drag the ~9MB data package into
					// the bundle. only a type-only re-export (erased) is allowed.
...
describe("detectPairIndexCountry — structural country from the input shape", () => {
	...
	test("bitter-lesson-safe: a bare place name with NO postcode is NOT read as gb — it falls through to the us fallback", () => {
		...
		// The pair prior is additive, so a conservative miss (no bias) is the safe failure mode. a
		// caller who knows the posture uses the `{ country }` override on resolvePairIndexForText.
