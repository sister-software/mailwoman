...
// Maine (exact alias "ME", no population) vs a populous state whose name token
// also matches "ME" but is not an exact match.
// "ME Plains" stands in for the real states that surfaced as non-exact "ME" candidates.
// Maine has no population so the boost asymmetry is unambiguous: with a large `populationBoost`,
// ME-Plains gets a big lift and Maine gets +0 — so without tiering the populous non-match strictly
// outranks the exact match (the bug), and with tiering the exact match wins regardless (the fix).
...
// Maine (no population) gets +0.
// Makes the tiering-off case reliably pick the populous non-match.
...
describe("findPlace — exact-match tiering", () => {
	...
	test("short-query over-fetch rescues an exact-abbrev region below the normal window (NY → New York)", async () => {
		...
		// Without widening the window for short queries it never enters the candidate pool, so exact-match
		// tiering can't promote it and a token-matching decoy wins (the real-DB "NY → Highland, GB" bug).
		// With the widening, New York is in the pool and tiering lifts it.
		// No `country` hint — this is the bare, no-context path.
