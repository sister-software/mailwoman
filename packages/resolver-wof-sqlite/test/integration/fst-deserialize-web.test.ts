...
import { deserializeFSTWeb, readFSTProvenanceWeb } from "@mailwoman/resolver-wof-sqlite/fst-deserialize-web"
...
test("web reader roundtrips crossCountryBranches under header flags bit0, undefined without it", async () => {
	const { serializeFST } = await import("@mailwoman/resolver-wof-sqlite/fst-serialize")
	const { FSTMatcher } = await import("@mailwoman/resolver-wof-sqlite/fst-matcher")
