...
describe("rankByPrimaryPreference — capital-status bonus (#1880, bounded in log10-population units)", () => {
	// The pure arithmetic under the CapitalIndex-backed callback: national capital −2, admin-1 seat −1,
	// SUBTRACTED from neg_rank so `prominence` (derived from effectiveNegRank downstream) carries the
	// preference through the resolver walk's re-sort. Population figures are the real board rows.
	const CR = 1
	const US = 2
	const BM = 3
	const CA = 4
	...
	const nationalOf = (country: number) => (r: { country_id: number | null }) => (r.country_id === country ? 2 : 0)

	test("a national capital within the 100x margin outranks the more-populous namesake (San José → CR, not CA)", () => {
		// San José CR 342,188 vs San Jose US 969,655 — gap 0.45 log10 < 2 → the capital wins with the bonus…
		const rows = [pop(969_655, 1, US), pop(342_188, 1, CR)]
		const on = rankByPrimaryPreference(rows, 5, undefined, undefined, nationalOf(CR))
		expect(on[0]!.country_id).toBe(CR)
		// …and WITHOUT the callback the population order stands, bit for bit (the default-off contract).
		const off = rankByPrimaryPreference(rows, 5)
		expect(off[0]!.country_id).toBe(US)
		expect(off.every((r) => r.effectiveNegRank === r.neg_rank)).toBe(true)
	})

	test("the margin binds: a 100x-more-populous namesake keeps winning (Hamilton → ON, not Bermuda's 902)", () => {
		// Hamilton BM 902 vs Hamilton CA 519,949 — gap 2.76 log10 > 2 → the bonus cannot flip it.
		const ranked = rankByPrimaryPreference(
			[pop(519_949, 1, CA), pop(902, 1, BM)],
			5,
			undefined,
			undefined,
			nationalOf(BM)
		...
		expect(ranked[0]!.country_id).toBe(CA)
	})

	test("an admin-1 seat carries the 10x margin (Springfield → IL over MO, but not over a 10x peer)", () => {
		const seatOf = (r: { country_id: number | null }) => (r.country_id === CR ? 1 : 0)

		// Springfield IL 116,250 (the seat, staged as CR) vs Springfield MO 169,176 — gap 0.16 < 1 → seat wins.
		const near = rankByPrimaryPreference([pop(169_176, 1, US), pop(116_250, 1, CR)], 5, undefined, undefined, seatOf)
		expect(near[0]!.country_id).toBe(CR)

		// Against a 12x peer the seat yields — the seat margin is 1 log10, not a dominant tier.
		const far = rankByPrimaryPreference([pop(1_400_000, 1, US), pop(116_250, 1, CR)], 5, undefined, undefined, seatOf)
		expect(far[0]!.country_id).toBe(US)
	})

	test("a row with no recorded population never receives the bonus (population 0 is NO EVIDENCE)", () => {
		// A pop-0 duplicate at the capital's coordinates must not ride the bonus over a real city.
		const ranked = rankByPrimaryPreference([pop(50_000, 1, US), pop(0, 1, CR)], 5, undefined, undefined, nationalOf(CR))
		expect(ranked[0]!.country_id).toBe(US)
		// toBeCloseTo, not toBe: pop 0 makes neg_rank the -0 of -log10(1), and the pass-through arithmetic
		// normalizes it to +0 — Object.is would fail on the sign of zero, not on the contract.
		expect(ranked[1]!.effectiveNegRank).toBeCloseTo(ranked[1]!.neg_rank, 10)
	})

	test("the bonus rides effectiveNegRank — the value prominence is derived from — not a side-channel sort", () => {
		// The #861 write-back lesson: a re-order that leaves effectiveNegRank untouched is silently undone by
		// the resolver walk's own `prominence ?? score` sort. So the CONTRACT is the value, not the order.
		const ranked = rankByPrimaryPreference([pop(342_188, 1, CR)], 5, undefined, undefined, nationalOf(CR))
		expect(ranked[0]!.effectiveNegRank).toBeCloseTo(-Math.log10(342_189) - 2, 5)
	})
})
