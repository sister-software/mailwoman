...
beforeAll(async () => {
	...
	const lines = [
		// Tuen Mun, a Hong Kong district. Its `alternatenames` column carries the Chinese form beside the
		// romanizations, exactly as the shipped HK dump does.
		row({
			0: "1818446",
			1: "Tuen Mun",
			2: "Tuen Mun",
		...
		// The packing noise the admission rule still has to refuse — parenthesized asides and bracketed
		// qualifiers GeoNames puts in the same column.
		row({
			0: "1818999",
			1: "Noise Town",
			2: "Noise Town",
