...
/**
 * Name of the FTS5 virtual table this module owns. Centralized so `WOFSQLitePlaceLookup`
 ...
 * FTS5 assigns token positions to tokens only — separator characters never consume a position —
 * so any character the tokenizer treats as a boundary leaves the two aliases' tokens adjacent
 * and the false phrase match intact. The separator must therefore be an indexed token
 * that sits between the aliases and breaks positional adjacency.
 *
 ...
 *   everything outside `\p{L}\p{N}` from token bodies, and U+E000 is neither — no user query
 *   can ever address the separator token. The demo's `sanitizeFTS` strips it explicitly.
 * - **Never in place names**: PUA codepoints are unassigned by definition. real-world
 *   WOF names don't carry them. Defensively, the insert below also strips any embedded
 *   U+E000 from source names so a poisoned row can't forge an alias boundary.
 ...
 * Only if #189 were instead built as one-row-per-alias would both become
 * moot (per-alias rows give exact equality and phrase isolation for free).
 ...
 *   aliases space-joined, boundaries lost. Falls back to the historical padded-containment
 *   check, conditioned on `anyStrictExact` — unrestricted containment would
 ...
 *   (canonical name or region abbreviation). Only consulted for legacy bags.
 */
...
/**
 * Name of the R*Tree virtual table that indexes WOF places' bounding boxes for proximity
 * / bbox lookups. Built alongside `place_search` by the CLI and `buildFTS: true`.
 ...
 * Powers the population-weighted ranking boost. Sparse — WOF only populates this
 * field for ~15% of localities (and mostly larger ones); missing means no boost,
 * never a penalty. Built upstream by `scripts/build-unified-wof.ts` at ingest
 * (and copied through by `build-slim`) — this module consumes it, never builds it.
 *
 ...
 *
 * The lookup reads only `encyclopedic` from it, and only to carry the value onto the
 * result — referential is derived from the population already joined, and no order
 * BY anywhere touches this table (ROAD_TO_V9 §2). Sparse and schema-versioned:
 * a pre-split gazetteer has this table with a single conflated `importance` column
 * instead, which the lookup's column probe deliberately refuses to read.
 */
export const PLACE_IMPORTANCE_TABLE = "place_importance"

/**
 * Counters for a single `buildPlaceSearchFTS` run. Exposed so callers
 * (CLI, lazy-build) can render progress to the user.
 */
 ...
 *
 * The FTS5 index is used for name-based `match` queries. the R*Tree is used for bbox +
 * proximity filtering. Both are pure SQLite — no extensions required.
 *
 * Returns a `BuildPlaceSearchFTSResult` summary. Idempotent when `drop: false` —
 * re-running against an already-indexed DB skips whichever indexes already exist.
 */
export function buildPlaceSearchFTS<DB>(
	...
	if (!ftsExisting || opts.drop) {
		...
		//
		// Aliases join on the boundary-preserving ALIAS_SEPARATOR token (#523) —
		// space-padded so each alias still tokenizes normally — and any U+E000 embedded in
		// a source name is defensively flattened to a space so it can't forge a boundary.
	...
	// note: `place_population` is not built here. `scripts/build-unified-wof.ts` extracts `wof:population`
	// straight into that table at ingest (the canonical source carries no `geojson` table),
	// and `build-slim` copies it through. This function only owns the two
	// FTS-derived virtual tables, both of which build from `spr` + `names` alone.
