...
export function candidateFromSearchRow(
	...
	// Missing population → no contribution.
	// Never penalizes.
	...
	// Carried for consumers (annotations / API surfaces).
...
export function rankCandidates<DB>(
	...
	if (weights.exactMatchTiering && candidates.length) {
		...
		if (exactIDs.size) {
			...
			// replaces raw population as the within-tier key — the 48026 rule: the map view
			// or the user's location breaks a cross-country postcode tie.
			// Without hints, referential ordering decides.
			//
			// ROAD_TO_V9 §2: this is the site that "orders namesakes", so it is the
			// site that has to say what it orders by.
			// `compareReferential` is referential desc with raw population as the tiebreak,
			// which is provably the same order as the `(b.population ?? 0) - (a.population ?? 0)`
			// it replaces — referential is strictly increasing in population below saturation
			// and constant above it, and the tiebreak restores the order in the saturated tail.
			// Measured zero-delta rather than assumed: see `place-importance-schema.test.ts`
			// and `resolver-referential-ranking.test.ts`.
