...
/**
 * The placetype taxonomy used by Who's On First. Ordered roughly from coarsest (country)
 * to finest (address). See https://github.com/whosonfirst/whosonfirst-placetypes
 * for the authoritative definitions of each.
 *
 ...
 *
 * `score` is the post-boost ranking number — higher is better, but the scale is
 * implementation- defined. Callers should treat it as ordinal rather than absolute.
 *
 * `id` is the WOF place id. It's named generically (not `wof_id`) so the shape stays
 * structurally compatible with `@mailwoman/resolver`'s `ResolvedPlace` — `WOFSQLitePlaceLookup`
 * satisfies the generic `ResolverBackend` interface without an adapter shim.
 *
...
export interface PlaceCandidate {
	 ...
	 *
	 * It is what lets a consumer see that two candidates in one answer denote one settlement
	 * at two admin tiers. The gazetteer carries 285,478 populated localities that share
	 * a folded name with a `localadmin` within 5 km, and 264,523 of those (92.7%) name
	 * that twin as their depth-1 ancestor — so the pair is legible from this field alone.
	...
	prominence?: number
	/**
	 * Population from WOF's `wof:population` property. Only present when the candidate has
	 * it on record — WOF carries population for ~15% of localities (mostly larger ones).
	 ...
	 * via `compareReferential`, which restores the megacity order with a population tiebreak —
	 * is the same order as ordering by population. That equivalence is the point:
	 * naming the ranking key costs nothing at the ranking.
	 *
	 ...
	 *
	 * Reserved slot awaiting a strict-channel source: present only when a extract's `place_importance`
	 * table carries the split columns, and no shipped extract does — the FTS lookup's clauses
	 * emit NULL for everything today. `undefined` means either "no encyclopedia entry for
	 * this place" or "this gazetteer predates the split"; both are absence, and neither is 0.
	 ...
	 * Emitted only by the candidate-table backend, and only when the artifact measured this place —
	 * absent is unmeasured, never zero. Consumed by `rankByImportance` (`resolver/toponym-prior.ts`)
	 * for the bare-toponym class. See `candidate-schema.ts` → `CandidateTable.importance`
	 * for why the blend rather than the strict channel, is what ships.
	 */
	importance?: number
	/**
	 * Bounding box from WOF's `spr.{min,max}_{latitude,longitude}` columns.
	 * Coarse outline for the place — a city's bbox is the city's full extent, a postcode's
	 * is roughly the postcode polygon's envelope. Optional because not all callers ask for
	 * it. implementations are free to omit when the underlying schema lacks the columns.
	 */
	bbox?: GeoBbox
	/**
	 ...
	 * in `@mailwoman/core`: present only when this candidate's row would have
	 * taken the cross-country alias penalty and the exemption prevented it.
 ...
 *
 * `near` and `bbox` are independent. `near` is a soft signal — candidates close
 * to the point get a ranking boost but distant candidates aren't dropped.
...
export interface FindPlaceQuery {
	...
	primaryOnly?: boolean
	/**
	 * Alias-row name roles the probe refuses to answer through (#1730) — the bare-toponym
	 * side races pass `abbr`/`gloss`. Role-NULL alias rows (the exonym tier) stay
	 * open. backends/artifacts without a role column ignore it.
	 */
	 ...
	 * also carries `postcode`, candidate rows within `POSTCODE_CONTAINMENT_THRESHOLD_KM`
	 * of the postcode's own centroid sort by distance first, the rest appended in their
	 * original order. Set by the resolver from `ResolveOpts.postcodeContainmentCoherence`;
	 * absent → the population-first order is untouched (byte-identical).
	 */
	postcodeContainmentCoherence?: boolean
	/**
	 * The tree's parsed region qualifier (#1717 stage 2) — set by the resolver
	 ...
	 * stamps `containedByQualifier`, ranks contained candidates first, and may ADD
	 * contained same-key candidates a country scope hid — additive only, never a filter.
	...
	/**
	 * Ordered proximity-bias points (viewport center, user location, …),
	 * each optionally weighted (default 1.0, first entry strongest by convention).
	 * Soft — a re-rank signal, never a filter: with bias present, exact-tier candidates order by
	 * combined prominence (population + the best decayed-distance term over these points) instead
	 * of population alone, which is how an ambiguous bare postcode ("48026": Fraser MI vs Russi IT)
	 * follows the map view / the user. Absent (and no `near`) → ranking is byte-identical to today.
...
/**
 * The pull-based lookup surface. Implementations resolve a `FindPlaceQuery` to a ranked
 * list of `PlaceCandidate`s. The interface is async even though `node:sqlite` is sync —
 * leaves room for `Worker`-backed implementations later without a public API break.
 */
