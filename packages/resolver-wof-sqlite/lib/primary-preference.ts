...
/**
 * The row shape the re-rank needs.
 *
 * `is_primary` is optional: a reader over an artifact vintage that predates the column omits
 * it from the select, no row reads as primary, and the re-rank no-ops by construction —
 * the exact degradation the browser reader's vintage guard relies on.
 */
 ...
 *
 * The raw candidate order is population-first (`neg_rank ASC`) and treats an alias row
 * (a place's alt-name / exonym, `is_primary=0`) and a primary-name row (`is_primary=1`) on equal footing.
 * So a foreign place whose transliterated exonym coincidentally normalizes to a query —
 * Changchun CN stores the Turkish exonym "Çançun" (`name_key="cancun"`), 4.19 M pop —
 * outranks the primary-name place the query actually means (Cancún MX, 0.89 M pop).
 *
 * This penalty makes a same-key alias have to clear a population margin over
 * a foreign primary before it wins.
 *
 * It is deliberately not a dominant sort key (no `order BY is_primary desc`, which would
 * make every primary outrank every alias and break the alt-names users depend on —
 ...
 * (all same-name places worldwide) are re-ranked in-process, so the probe fetches this many
 * (population-ordered) before the re-rank rather than the caller's small `limit`, ensuring the
 * intended primary isn't pushed below the fold by a cluster of more-populous foreign aliases.
 *
 * Bounded and small — a single contiguous B-tree scan.
 */
 ...
 * `rows` arrive population-ordered (`neg_rank ASC`); an alias (`is_primary=0`) is
 * pushed back by `delta` in log10-population units only when the top-population primary
 * sharing the key is in a different country, and is `demoted` out of the exact tier
 * when that penalty leaves it behind the primary.
 *
 * Returns the top `limit` after the re-rank, each annotated.
 *
 ...
 * same population, so `neg_rank` is equal to the bit and `referential` follows it
 * (`referentialFromPopulation` is a pure function of population).
 *
 ...
 *
 * Where the term decides, and where IT cannot (#1729).
 * It binds inside `findPlace`, so it orders every row set that actually contains the tie —
 * but the resolver walk's probes all carry a placetype filter, and `PLACETYPE_FILTER_GROUPS`
 * (core/resolver) never mixes `locality` with `county`: the `Of`-shape locality/county pair
 * is partitioned before this ranker runs, the locality probe fetches one row, and the walk's
 * own `locality` request selects the seat by construction — the same winner, decided upstream.
 *
 ...
 * of which only 722 were the seat/district duplicate.
 * The rest were contests between genuinely distinct places that merely tie —
 * 2,885 `locality → neighbourhood` (a bare city name losing to a same-named hood),
...
export function rankByPrimaryPreference<R extends PrimaryPreferenceRow>(
	...
	// The primary the alias actually competes with for the top slot: highest population (min neg_rank).
	// Undefined when the set has no primary → nothing to prefer, penalty is 0,
	// order stays population-first (today's behavior).
	...
	const annotate = (r: R): RankedRow<R> => {
		...
		// prevented it — conditions the winner-level receipt needs, computed where the decision is made.
		// A variant row that was in-country, primary, or keyed under a set with no
		// foreign primary never fired, and stays unmarked.
