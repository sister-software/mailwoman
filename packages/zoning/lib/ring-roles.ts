...
export interface ResolvedRingRoles {
	 ...
	 *
	 * Measured at 9 of 3,516 nationally, every one a sliver under 1.7 m² sitting on its parent's boundary.
	 * They are carried rather than dropped: the publisher's own area accounting subtracts them,
	 * so dropping one would add ground the plan carved out, and a feature with exactly
	 * one exterior — 8 of the 9 — has no ambiguity at all.
	 */
	adjacentHoles: number
	/**
	 * `1` where the feature's exterior was chosen by magnitude because no ring read as one by orientation.
	 *
	 * Measured AT one feature OF 85,330, and its size is the reason the fallback exists rather than a refusal.
	 * `objectid` 74040 — Galway County Council, `Agriculture` — is a single three-vertex
	 * ring enclosing 3.0 × 10⁻⁷ m², a third of a square micrometre.
	 *
	 ...
	 * rather than being implied to be zero.
	 */
	 ...
	 *
	 * The ingest's own receipt, stored on the row.
	 * It is what the build compares against the Department's `Shape__Area` sum, and its sign is
	 * the record that the orientation was read: a source that started publishing counter-clockwise
 ...
 *
 * Deliberately discards the arriving nesting.
 * Both encodings reach here — a properly nested polygon and a pile of single-ring parts —
 * and the orientation is the only signal that means the same thing in both.
 *
 * Keeping the nesting for the features that have it and inferring it for the ones
 * that do not would put two rules over one artifact.
 */
...
export function resolveRingRoles(polygons: MultiPolygonRings, featureID: string): ResolvedRingRoles {
	...
	for (const ring of rings) {
		...
		// clockwise is exterior under this service, and `ringSignedAreaM2` signs
		// clockwise positive — see its docstring.
		// A zero-area ring is degenerate rather than either role, and is carried as a hole
		// so it can never enclose anything.
		if (signed > 0) {
	...
	// no ring read AS an exterior, SO magnitude decides — measured at exactly one feature of 85,330.
	// The largest ring is the one that encloses the area, which is the correct reading both for a
	// degenerate sliver whose winding is floating-point noise and for a feature published wholly inverted.
	// Refusing here would fail the build on the publisher's own data,
	// and skipping the feature would invent an absence.
	if (!exteriors.length) {
		let largestIndex = 0
	...
	// smallest containing exterior, so a hole inside an island inside a hole lands
	// on the island rather than on the outer ring.
	// Sorting once is cheaper than choosing per hole, and it makes the choice deterministic on a tie.
	...
	for (const hole of holes) {
		...
		// A hole no exterior contains a majority of sits on its parent's boundary —
		// measured at 9 of 3,516 nationally, all under 1.7 m².
		// It goes to the smallest exterior of the same feature, which is the only
		// exterior at all on 8 of the 9, and is counted.
