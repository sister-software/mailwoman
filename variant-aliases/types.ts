 ...
 * - `brand` aliases resolve to a canonical brand name. Multiple regional variants of the same
 *   brand ("Macca's", "McDo", "Mickey D's", "マクド") all map to `McDonald's`.
 */
...
export interface VariantAliasBase {
	...
	variant: string
	/**
	 * BCP-47 locale tags where this variant is in active use. Used to gate lookups: only consider
	 * an alias when the detected locale matches one of these. A query in `en-US` won't match
	 * Australian "servo" because `en-AU` is not in `["en-US"]`.
	 */
	locales: ReadonlyArray<string>
	/**
	 * Free-form regional refinement within the locale (e.g. "NYC", "Kansai", "Quebec"). Not used
	 * for matching today; informational. A future enhancement could combine this with a coarse
