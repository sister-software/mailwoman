...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptSnippet:
			"Validate mailwoman release readiness (version, weights, git, compile, tests)",
		promptGuidelines: [
		...
		parameters: Type.Object({
			...
			skipCompile: Type.Optional(
				Type.Boolean({
					description:
						"Skip running yarn compile (default: false). Skip if you know it was already compiled.",
				}),
			),
		}),
		async execute(toolCallId, params, signal, onUpdate) {
			...
			if (!releaseConfig) {
				result.passed = false;
				result.issues.push(
					"release.config.json not found or unparseable",
				);
			} else {
				...
				if (!releaseConfig.version) {
					result.passed = false;
					result.issues.push(
						"release.config.json missing version field",
					);
				}
			}
			...
			onUpdate?.({
				content: [
					{ type: "text", text: "Checking weights files..." },
				],
			});
			...
			if (skipWeights) {
				result.issues.push(
					`Weights checks skipped (MAILWOMAN_SKIP_WEIGHTS_COPY/MAILWOMAN_SKIP_WEIGHTS set)`,
				);
			} else {
				if (weights.missing.length > 0) {
					result.passed = false;
					result.issues.push(
						`Missing weights: ${weights.missing.join(", ")}`,
					);
				}
				if (weights.symlinked.length > 0) {
					result.passed = false;
					result.issues.push(
						`Symlinked weights (will break npm publish): ${weights.symlinked.join(", ")}`,
					);
				}
			}
			...
			if (gitResult.code !== 0) {
				result.passed = false;
				result.issues.push(
					`Git status failed (code ${gitResult.code}): ${gitResult.stderr.trim() || "unknown error"}`,
				);
			} else {
			...
			if (!params.skipCompile) {
				onUpdate?.({
					content: [
						{ type: "text", text: "Running yarn compile..." },
					],
				});
				...
				if (!result.compileSuccess) {
					result.passed = false;
					result.issues.push(
						`yarn compile failed (code ${compileResult.code})`,
					);
				}
			} else {
			...
			if (!params.skipTests) {
				onUpdate?.({
					content: [
						{ type: "text", text: "Running yarn ci:test..." },
					],
				});
				...
				if (!result.testSuccess) {
					result.passed = false;
					result.issues.push(
						`yarn ci:test failed (code ${testResult.code})`,
					);
				}
			} else {
			...
			if (result.weightsMissing.length > 0) {
				lines.push(
					`  Missing: ${result.weightsMissing.join(", ")}`,
				);
			}
			if (result.weightsSymlinked.length > 0) {
				lines.push(
					`  Symlinked: ${result.weightsSymlinked.join(", ")}`,
				);
			}
			lines.push(
				`Git clean: ${result.gitClean ? "yes" : "no"}`,
			);
			lines.push(
				`Compile: ${result.compileSuccess === null ? "skipped" : result.compileSuccess ? "passed" : "failed"}`,
			);
			lines.push(
				`Tests: ${result.testSuccess === null ? "skipped" : result.testSuccess ? "passed" : "failed"}`,
			);
	...
	pi.registerTool({
		...
		promptSnippet:
			"Parse a postal address string into components (house_number, street, locality, region, postcode)",
		promptGuidelines: [
		...
		parameters: Type.Object({
			address: Type.String({
				description:
					"Postal address string to parse (e.g. '1600 Amphitheatre Parkway, Mountain View, CA 94043')",
			}),
			locale: Type.Optional(
				Type.String({
					description:
						"Locale tag matching a weights package (en-US, fr-FR). Default: en-US.",
				}),
			),
			defaultCountry: Type.Optional(
				Type.String({
					description:
						"ISO-3166 country to scope the WOF resolver (e.g. 'US'). Pass 'none' to disable.",
				}),
			),
		}),
		async execute(_toolCallId, params, signal) {
			...
			const truncated = pretty.length > maxBytes
