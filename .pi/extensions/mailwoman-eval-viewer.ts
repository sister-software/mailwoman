 ...
 *
 * Registers /eval-viewer command that opens an interactive overlay displaying
 * the latest parity scorecard as a sortable table with regression highlighting.
 *
...
class EvalViewerComponent {
	...
	render(width: number): string[] {
		...
		for (let i = startIdx; i < Math.min(startIdx + maxRows, this.rows.length); i++) {
			...
			const valueStrs = row.versions.map((v) =>
	...
	pi.registerCommand("eval-viewer", {
		...
		handler: async (_args: string, ctx: ExtensionCommandContext) => {
			...
			if (!scorecard || scorecard.rows.length === 0) {
				ctx.ui.notify(
					`Failed to parse scorecard: ${scorecardPath}`,
					"error",
				);
				return;
			}
			...
			await ctx.ui.custom<undefined>(
				(_tui, theme, _keybindings, done) =>
					new EvalViewerComponent(scorecard, theme, () => done(undefined)),
				{
					overlay: true,
					overlayOptions: {
						width: "85%",
