...
interface DebtCounters {
	...
	rawNULBytes: number
	/**
	 * `toISOString().slice(0, 10)` re-typed outside its home. `isoDate()` in `@mailwoman/core/utils/time` is the one
	 * definition; twenty call sites had re-typed it before the counter existed.
	 */
	reTypedISODate: number
	/**
	 * An Earth-radius literal — the mean radius in kilometers or meters, with or without digit separators — outside
	 * `packages/spatial`, which is the one home of the haversine. A copy is a second Earth to keep in agreement with the
	 * first.
	 */
...
for (const path of paths) {
	...
	if (!path.endsWith("packages/core/lib/utils/time.ts") && /from "@mailwoman\//.test(text)) {
		counters.reTypedISODate += text.match(/\.toISOString\(\)\.slice\(0, ?10\)/g)?.length ?? 0
	}

	if (!path.includes("packages/spatial/")) {
		counters.earthRadiusLiterals += text.match(/\b6_?371(?:_?000|e3)?\b/g)?.length ?? 0
	}
