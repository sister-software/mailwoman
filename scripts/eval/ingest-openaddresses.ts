 ...
 *
 *   Ingest-openaddresses.ts — build the OpenAddresses (OA) US evaluation sample for the
 ...
 *   ## FLAGS
 *
 *   --out <path> output JSONL (default data/eval/external/openaddresses-us-sample.jsonl) --cache<dir>
 *   where source zips are cached (default /tmp/oa-cache) --target <n> overall cap on emitted
...
interface SourceStats {
	read: number
	kept: number
	droppedNoCityOrPost: number
	droppedBadGeo: number
	droppedBadStreet: number
	droppedDup: number
	sampled: number
}
...
// ---------------------------------------------------------------------------
const US_BBOX: BBox = { minLat: 15, maxLat: 72, minLon: -180, maxLon: -60 }
const DE_BBOX: BBox = { minLat: 47, maxLat: 56, minLon: 5, maxLon: 16 }
const FR_BBOX: BBox = { minLat: 41, maxLat: 51.5, minLon: -5.5, maxLon: 9.7 } // metropolitan France
const NL_BBOX: BBox = { minLat: 50.7, maxLat: 53.6, minLon: 3.3, maxLon: 7.3 }
...
function zipPathFor(cache: string, source: Source): string {
...
function cleanPostcode(s: string): string {
	if (!s) return ""
...
function main(): void {
	...
	const perSourceRecords: Array<{ source: Source; records: OaRecord[] }> = []
	const report: Array<{ source: string; state: string; packagedLicense: string | null } & SourceStats> = []
	const blocked: Array<{ source: string; reason: string }> = []
