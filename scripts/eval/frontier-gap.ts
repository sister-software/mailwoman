 ...
 *
 *   Run: node --expose-gc scripts/eval/frontier-gap.ts [--per-country 3] [--min-pop 50000] [--out
 *   <md>]
 */
...
interface CountryRow extends CountryEntry {
	cc: string
	bareRate: number
	ccRate: number
}
...
// --- aggregate ---
const rows: CountryRow[] = [...perCountry.entries()]
	.map(([cc, e]) => ({ cc, ...e, bareRate: e.bare / e.n, ccRate: e.withCc / e.n }))
	.sort((a, b) => a.ccRate - b.ccRate || a.bareRate - b.bareRate)
type NumKey = "n" | "bare" | "withCc" | "namesake" | "hintEmpty"
const sum = (k: NumKey): number => rows.reduce((s, r) => s + r[k], 0)
const totalN = sum("n")
