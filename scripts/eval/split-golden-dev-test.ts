 ...
 *
 * Splits the golden eval set into a held-out DEV/TEST partition so the calibration experiment (#31) can report per-tag
 * recall on data the model's recipe never tuned against. The methodology contract (v0.7 plan):
 *
 * - 90/10 dev/test, stratified per source file (us / fr / adversarial) so each split preserves the country/category mix.
 * - DETERMINISTIC: a fixed seed + mulberry32 + Fisher-Yates means re-running reproduces byte-identical splits. The TEST
 *   partition is "read exactly once per release" — it must therefore be stable and committed.
 * - BACKWARD-COMPATIBLE: writes `dev/` and `test/` SUBDIRECTORIES under the golden dir. `loadGolden()` in
 *   harness-postcode.ts / eval-matrix.ts uses a non-recursive `readdirSync` that only picks up `*.jsonl`, so the
...
/**
 * Returns a shuffled COPY of indices [0..n). Seed is mixed per-file so the three files don't share an identical
 * permutation.
 */
