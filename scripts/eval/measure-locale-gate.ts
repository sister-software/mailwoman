 ...
 *
 * #33 asks for a "locale pre-classifier (lexical features → MLP)". Before building an MLP, measure what the EXISTING
 * rule-based `@mailwoman/locale-gate` (`detectLocaleSync`: postcode-shape + script scoring over `QueryShape`) already
 * achieves at predicting an address's country from lexical cues alone. If the rule baseline is strong, #33 needs no new
 * model — it's already shipped, and the remaining system-aware work is #38 (per-system containment) + #39 (tree
 * `system` field).
 *
