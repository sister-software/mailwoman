 ...
 *
 *   The shipped `openaddresses-de-sample.jsonl` renders every German row US-style —
 *   `27 Straußstraße, Berlin, Berlin 12623` (house-number FIRST, postcode TRAILING after the city).
 *   Real German addresses are written `Straußstraße 27, 12623 Berlin` (house AFTER street, postcode
 *   BEFORE city). A model trained on German order scores ~46% locality on the US-order rendering and
 *   ~84% on the native rendering — the "German collapse" was substantially this rendering mismatch.
 ...
 *   re-runnable: parse house# + street out of the US-order `input`, take locality + postcode from the
 *   gold `expected`, and re-emit in German order. Everything else (lat, lon, expected, state, source)
 *   is preserved verbatim, so `oa-resolver-eval --eval <output>` measures the same points, only
 *   rendered the way Germans write them.
 *
...
/**
 * `"27 Straußstraße, Berlin, Berlin 12623"` → `"Straußstraße 27, 12623 Berlin"`. House number is the
 * leading `\d+` (optionally a unit letter, `27a` / `27 A`) of the first comma-segment; the rest is the
 * street. Locality and postcode come from the gold `expected` so we never depend on the US-order
 * string's trailing layout. Rows with no leading house number render street-only (still German order).
 */
