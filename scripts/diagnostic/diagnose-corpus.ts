...
/**
 * `str(PurePosixPath(input))` — the normalized string form Python's `pathlib` prints. Collapses
 * redundant `/`, drops `.` segments and a trailing slash; preserves `..` (Python does not resolve
 * it). Used so this script's printed paths and the sort order of resolved shards match the Python
 * original byte-for-byte on the realistic (clean, absolute) inputs.
 */
...
/**
 * Resolve train/val/test shard paths via MANIFEST.json (adapter-addition corpora) or legacy glob
 * fallback (monolithic corpora). Faithful port of `_shard_paths` from the Python data loader.
 *
 * Per shard: use the manifest path AS-IS when it exists (overlay cross-dir refs, or a corpus on its
 * build machine); otherwise RE-ROOT it under `corpusDir` (take the `<split>/<basename>` tail). A
 * manifest that declares shards this loop cannot find is BROKEN (#480) — fail loud with the full
 * missing list. Falls back to a glob over `corpusDir/split` only when the manifest yields nothing.
 */
...
/**
 * Return the `source` value of the first row in a parquet shard. Faithful port of
 * `_shard_first_source`. Corpus v0.2.0 shards are 100% source-segregated (one source per shard), so
 * the first row's source identifies the shard's source. `threads=1` + `LIMIT 1` returns the first
 * physical row (matching PyArrow's `read_row_group(0)[0]`).
 */
