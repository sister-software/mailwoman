 ...
 *   bounded-concurrency primitive — same one `build-unified-wof.ts` uses to fan out file reads
 *   behind a single writer). Each state is an isolated child process (its own DuckDB + SQLite
 *   heap), so N states run at once with no shared-memory risk. To avoid oversubscribing cores, each
 *   child's DuckDB scan is capped at `--threads` (default: cores / concurrency), so concurrency ×
 *   threads ≈ cores. The per-state steady-state bottleneck is the single-threaded SQLite insert
 *   loop, not the scan, so N concurrent inserts is the real win. Sequentialising is still available
 *   via `--concurrency 1`.
 *
 ...
 *   regardless.
 *
 *   Usage: node scripts/build-national-situs.mjs [--out-dir <path>] [--release <tag>] [--states
 *   CA,FL,...] [--concurrency 4] [--threads N] [--license-filter NAD] [--force]
 */
...
const states = (args.states ? args.states.split(",").map((s) => s.trim().toUpperCase()) : STATES_BY_COVERAGE).filter(
	Boolean
)
...
const manifest = {
	...
	builtAt: null,
	licenseFilter: args["license-filter"] ?? null,
	states: {},
	datasetTotals: {},
}
let built = 0,
