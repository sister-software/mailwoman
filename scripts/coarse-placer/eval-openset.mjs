 ...
 *
 *   Usage: node scripts/coarse-placer/eval-openset.mjs [--model <dir>] [--out-md <path>] Requires
...
const { featurize, COARSE_CLASSES } = await import(new URL("core/out/coarse-placer/featurize.js", root).href)
...
const W = new Float32Array(readFileSync(path.join(args.model, "weights.bin")).buffer)
...
function load(file) {
	return readFileSync(path.join(args.data, file), "utf8")
		.trim()
		.split("\n")
		.map((l) => JSON.parse(l))
}
...
for (const [country, raws] of byClass) {
	...
	for (const raw of raws) {
		...
		for (let k = 0; k < nIn; k++) mu[k] += v[k]
	}
	for (let k = 0; k < nIn; k++) mu[k] /= raws.length
...
for (const [country, raws] of byClass) {
	...
	for (const raw of raws) {
		...
		for (let a = 0; a < nIn; a++) for (let b = 0; b < nIn; b++) Sigma[a][b] += d[a] * d[b]
		nTot++
	}
}
for (let a = 0; a < nIn; a++) for (let b = 0; b < nIn; b++) Sigma[a][b] /= nTot
// Ridge for invertibility.
for (let a = 0; a < nIn; a++) Sigma[a][a] += 1e-3
...
// ---------------------------------------------------------------------------
console.error("scoring in-map test + off-map heldout…")
const SCORES = ["maxprob", "p_inmap", "energy", "maxlogit", "maha"]
...
function scoreRow(raw, trueCountry) {
	const z = logits(raw)
...
function pointAt(scoreKey, t, inSplit, heldSplit) {
	let keepCorrect = 0
...
function paretoFor(scoreKey) {
	...
	// Summaries.
	let balanced = { val: -1, pt: null } // max of min(inMapAcc, heldCaught) on the FULL probe
	let atHeld90 = null // highest inMapAcc with heldCaught >= 90
	let atIn90 = null // highest heldCaught with inMapAcc >= 90
	for (const p of pts) {
...
const results = Object.fromEntries(SCORES.map((k) => [k, paretoFor(k)]))
...
lines.push("")
lines.push(
	`_Frozen shipped model (\`${path.basename(args.model)}\`), NO retrain. In-map test ${inmapTest.length} rows ` +
