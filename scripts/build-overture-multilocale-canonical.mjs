 ...
 *   JSONL (#149, release 2026-06-17.0) for a set of EU locales, capping rows/locale for balance + a
 *   bounded local build, and writes ONE combined canonical jsonl ({raw, components, country,
 *   source, source_id}). That feeds the standard overlay path:
 *
 *   Node scripts/build-overture-multilocale-canonical.mjs --cap 150000 --out
 *   /tmp/ovl/overture-ml.canonical.jsonl node scripts/align-canonical-shard.mjs --input <canonical>
 *   --output <labeled> --corpus-version 0.5.0 python3 scripts/jsonl-to-parquet.py --input <labeled>
 *   --output <NEW>/train/part-overture-multilocale-train.parquet python3
 *   scripts/assemble-fr-admin-split-overlay-manifest.py ... (overlay onto v0.8.0)
 *
 *   WHY: the model is en-us/fr-trained; the 8-locale coordinate panel (2026-06-22) showed resolve
 *   rate tracks training representation (FR/IT ~80%, mid-tier PT/PL/AT/CZ ~50%). This shard gives
 *   the model the non-en/fr street + locality formats it never saw. Gated on the coordinate panel.
 */
