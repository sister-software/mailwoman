 ...
 *   in `learned-scorer-clustering-eval.ts`). The held-out-NPI A/B showed the GBT beats the FS spine
 ...
 *   used to cluster the eval state's records through the same `resolveEntities` pipeline (FS spine
...
async function main(): Promise<void> {
	...
	console.error(`[E] clustering ${EVAL_STATE} records (FS spine vs GBT vs LR, trained on ${TRAIN_STATE})…`)
	interface ArmScore {
		precision: number
		recall: number
		f1: number
	...
	lines.push(
		`_Generated by \`scripts/record-matcher/learned-scorer-crossstate-eval.ts\`. The GBT + LR are trained on ` +
			...
			`never saw — through the same \`resolveEntities\` pipeline (FS spine / GBT scorer / LR scorer), best F1 over a fine ` +
	...
	const verdict =
		dGbt > 0.02
			? `**The GBT win GENERALIZES across states** — trained on ${TRAIN_STATE}, it still beats the FS spine on ${EVAL_STATE} ` +
			...
			: dGbt < -0.02
				? `**The GBT win does NOT generalize** — trained on ${TRAIN_STATE}, it is WORSE than the FS spine on ${EVAL_STATE} ` +
				...
				: `**The GBT roughly TIES the FS spine cross-state** (${pct(gbtArm.f1)}% vs ${pct(fs.f1)}%, ` +
	...
	lines.push(
		`A single train/eval state pair (${TRAIN_STATE}→${EVAL_STATE}), one geocoded sample each, a compact pure-Node GBT ` +
			`(120 rounds, depth 3). The FS arm is the benchmark spine (same model), so the comparison is fair. Absolute F1 ` +
