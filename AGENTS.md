...
Mailwoman is a postal-address parser shipped as the unscoped entry package `mailwoman` (CLI + library) plus **36 scoped `@mailwoman/*` workspaces**. The repo root is the private orchestration package `@mailwoman/universe` (not published). 35 of the 37 workspaces publish to npm — `docs` and `tile-worker` stay private. The table groups them by role:
