...
Mailwoman is a postal-address parser shipped as the unscoped entry package `mailwoman` (CLI + library) plus **38 scoped `@mailwoman/*` workspaces**. The repo root is the private orchestration package `@mailwoman/universe` (not published). 36 of the 39 workspaces publish to npm — `docs` and `tile-worker` stay private; `osm` isn't published yet (ODbL counsel sign-off pending, see `osm/README.md`). The table groups them by role:
