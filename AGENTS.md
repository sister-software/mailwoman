...
Mailwoman is a postal-address parser shipped as the unscoped entry package `mailwoman` (CLI + library) plus **40 scoped `@mailwoman/*` workspaces**. The repo root is the private orchestration package `@mailwoman/universe` (not published). 38 of the 41 workspaces publish to npm — `docs` and `tile-worker` stay private; `osm` isn't published yet (ODbL counsel sign-off pending, see `osm/README.md`). The table groups them by role:
...
| **Model + core**                                                             |                                    |                                                                                                                                                                 |
| `core/`                                                                      | `@mailwoman/core`                  | Tokenization, decoder, policy registry. Ships ~9 MB libpostal + WOF + chromium-i18n data under `core/data/`                                                     |
| `api-kit/`                                                                   | `@mailwoman/api-kit`               | API plumbing: Hono node serve wrapper, OpenAPI emit helpers, generic timing metrics, native error envelope                                                      |
| `neural/`                                                                    | `@mailwoman/neural`                | SentencePiece tokenizer + ONNX inference + decoder wiring                                                                                                       |
| `sentencepiece-wasm/`                                                        | `@mailwoman/sentencepiece-wasm`    | google/sentencepiece v0.2.2 → WASM with the native-offsets binding — the tokenizer core (single-file ESM, committed artifact, rebuilt via `build.sh`)           |
...
| `poi-taxonomy/`                                                              | `@mailwoman/poi-taxonomy`          | POI category taxonomy + phrase lexicon — Overture category snapshot, infrastructure extension namespace, synonym table (`drinking fountain` → `drinking_water`) |
| **Resolver + geo**                                                           |                                    |                                                                                                                                                                 |
...
| `ban/`                                                                       | `@mailwoman/ban`                   | BAN (Base Adresse Nationale) FR national address register — fetch/shard provider (`ban/sdk`)                                                                    |
| `osm/`                                                                       | `@mailwoman/osm`                   | OpenStreetMap rooftop address-point ingestion — fetch/shard provider (`osm/sdk`); publish blocked on counsel sign-off                                           |
...
| `api/`                                                                       | `@mailwoman/api`                   | The native HTTP API — engine-agnostic `/v1` surface (parse, geocode, batch, resolve, format) with health/metrics + emitted OpenAPI                              |
...
| `mcp/`                                                                       | `@mailwoman/mcp`                   | MCP server — mailwoman's spatial toolset for agents over stdio (parse, geocode, poi_search, overpass_export, layer_manifest)                                    |
| **Private / infra**                                                          |                                    |                                                                                                                                                                 |
...
- **[`docs/engineering/CONTRIBUTING_MODEL_WORK.mdx`](./docs/engineering/CONTRIBUTING_MODEL_WORK.mdx)** — the model-work runbook: which evals gate a change, the lever-shape taxonomy, how to add a shard. Read before touching the model, a shard recipe, or an eval. Iron rule 6 is the **D-rule**: no default-on mechanism (model OR decode-time) ships with a known regression vs the shipped model on any tier-1 locale — fix, per-locale-gate, or opt-in.
...
- **`sdk/` submodules mean data acquisition** (fetch/extract/shard-provider — see `ban/sdk`, `osm/sdk`, `tiger/sdk`). CLI helpers live in `mailwoman/cli-kit/` (`useCommandTask`, `CheckList`, the command component types), the parser test harness in `mailwoman/test-kit/` — do not grow a new `sdk/cli`. The old `./sdk/cli` + `./sdk/test` subpath shims were removed in v7.0.0 — import `mailwoman/cli-kit` / `mailwoman/test-kit` directly.
- **HTTP clients extend or instantiate `APIClient`** (`@mailwoman/core/api`), not raw `fetch`. It carries what every fetcher otherwise re-derives: strict minimum-interval pacing (`minRequestIntervalMs`) or a `requestsPerMinute` budget, both applied downstream of the cache so a hit costs nothing; bounded retry honouring `Retry-After` (**off unless you set `retry`**); response caching (`@mailwoman/core/api/disk-storage` for an on-disk one); and error mapping to `ResourceError`, so callers branch on `error.status` / `isTransientResourceError(error)` rather than message prose. Inject `clock` and drive it with `@mailwoman/core/api/test-clocks` so timing tests never sleep. `filer/sdk/sec-client.ts` is the worked example; `bdc/sdk/client.ts` is the second, and covers what SEC didn't — a binary path (`responseType: "arraybuffer"` plus a per-request `cache: false`, which is how you turn caching off without touching `core/api`) and a per-minute budget. **`requestsPerMinute` does not deliver N requests per minute**: its cooldown is `60000/N` minus the gap since the last dispatch, so N alone dispatches N requests every `60/N` seconds — measured at 100/min for `requestsPerMinute: 10`. Set `minRequestIntervalMs` too (`60000/N`); that is the gate that holds the rate. **This rule binds API REQUESTS, and a 2026-08-02 audit found 28 raw `fetch(` call sites outside browser and test code.** Reading them split the population in two, and the split is the useful part. **API requests** — small bodies, repeated calls, rate-limited hosts — are where `APIClient`'s pacing, bounded retry, response caching and `ResourceError` mapping earn their keep; ~23 sites qualify and should migrate (`scripts/check-release-parity.ts` is the worked example of the migration itself, verified to produce a byte-identical verdict). **File transfers** are not: `osm/sdk/fetch.ts` and `tiger/sdk/download.ts` stream multi-gigabyte archives straight to disk, where response caching is nonsense, there is nothing to pace, and axios buffers any non-stream response type in memory. Those keep raw `fetch` and say so in place. Do not "finish the job" on them.
...
- **Before writing a small utility, check whether it already has a home.** The 2026-08-02 dedupe audit (`docs/superpowers/specs/2026-08-02-taste-audit-findings.md`) found that most duplication here is not "nobody wrote it" but "the call site couldn't find it, or couldn't use it as shaped". The homes it settled, beyond `APIClient` and `dataRootPath` above:
  - `core/utils/python-random.ts` — `mulberry32(seed)` and `makeLcg(seed)` as `() => number` thunks, plus the `SeededRandom` class. Sixteen files had re-typed one of these. Prefer `mulberry32` for new code; `makeLcg` exists only because its exact stream is baked into shipped corpus rows and eval splits.
  - `core/utils/sealed-db.ts` — `swapDatabaseIntoPlace(tmp, final)`, the atomic publish step this file specifies in prose, next to `sealDatabase` / `openBuiltDatabase`.
  - `core/utils/stats.ts` — `percentile` (nearest-rank, `p` in **[0, 100]**), `median`, `formatPercent`. Watch the unit: local copies elsewhere took a fraction, so a careless swap silently changes a gate number.
  - `core/utils/jsonl.ts`, `core/utils/hash.ts` — JSONL read/write/iterate, `sha256File` / `sha256Hex` / `md5File`.
  - `@mailwoman/spatial` — `haversineKm`, and now `pointInRing` / `pointInPolygon` / `pointInMultiPolygon`. `resolver-wof-sqlite/geo.ts` re-exports both for its own readers.
  - `match/comparators.ts` — `jaro`, `jaroWinkler`, `levenshteinSimilarity`, `nameSimilarity`, `jaccard`.
  - `corpus/src/adapter.ts` — `HOUSE_NUMBER_PREFIX` + `splitStreetLine(line)`, the house-number/street split every US CSV adapter uses.
