...
- **HTTP clients extend or instantiate `APIClient`** (`@mailwoman/core/api`), not raw `fetch`. It carries the machinery every fetcher otherwise re-derives: strict minimum-interval pacing (`minRequestIntervalMs`, concurrency-proof — a token bucket cannot hold a flat per-second cap) or a `requestsPerMinute` budget, opt-in bounded retry honouring `Retry-After`, response caching (`@mailwoman/core/api/disk-storage` for an on-disk one), prefixed logging, `AsyncDisposable`, and error mapping to `ResourceError` — a numeric `status` plus a `(source, kind, reason)` URN, so callers branch with `error.status` / `isTransientResourceError(error)` instead of matching message prose. `filer/sdk/sec-client.ts` is the worked example: rate ceiling, host allowlist, cache-TTL policy, and one bespoke error message, over ~350 lines instead of the ~710 the standalone version needed. `bdc/sdk/client.ts` is still raw `fetch` and is the next migration.
...
**Known gaps (#875) — reconciled 2026-07-25 against a full audit (the PUBLIC/breaking sweep already landed):** the v5.0.0 sweep omitted `Us` and generic `Json`/`Jsonl`, but the public-surface renames are now **done**, not pending: `recognizeUSRegions` (internal), `isUSStateAbbreviation` + the codex `US_STATE_*` family (public `@mailwoman/codex` — swept; `isUsStateAbbreviation` = 0 repo-wide, consumers on the US form), `pyJSONDumps` (core), and `writeJsonl`/`fetchJson`/`GeoJsonFeatureCollection` (0 occurrences / never-exported). A grep for any **exported** lowercase-acronym identifier (`Json|Jsonl|Us[A-Z]|Http|Api|Url`) across all published workspaces returns **zero** — no public breaking rename remains. What's left is **non-breaking internal cosmetic only**: ~11 local variables across 6 files (`corpusJsonl`, `countriesJson`, `metaJson`, `outJsonl`, … in `mailwoman/eval-harness/oa-resolver-eval.ts`, `mailwoman/gazetteer-pipeline/postcode-locality/base.ts`, `core/coarse-placer/coarse-placer.ts`, `corpus/src/tools/lint-shard.ts`, `registry/tools/gold-set-sample.ts`, `resolver-wof-sqlite/fst-serialize.ts`) — no caller/def split, so it does NOT need the major (do it anytime, or skip). The "version-gated batch, bundle with the next major" framing applied to the public renames that are now shipped; the residual is a free-standing cosmetic cleanup.
