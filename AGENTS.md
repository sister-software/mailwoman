...
- `docs/engineering/reference/jurisdiction-coverage.mdx` answers what this repository has for each of
  250 jurisdictions, one row each, with corpus rows, admission, gazetteer places and board rows in
  separate columns. It is generated by
  `packages/mailwoman/lib/dev-tools/corpus/jurisdiction-coverage.run.ts`; regenerate it rather than
