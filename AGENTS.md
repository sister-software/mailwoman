...
- **HTTP clients extend or instantiate `APIClient`** (`@mailwoman/core/api`), not raw `fetch`. It carries what every fetcher otherwise re-derives: strict minimum-interval pacing (`minRequestIntervalMs` — a token bucket cannot hold a flat per-second cap) or a `requestsPerMinute` budget, both applied downstream of the cache so a hit costs nothing; bounded retry honouring `Retry-After` (**off unless you set `retry`**); response caching (`@mailwoman/core/api/disk-storage` for an on-disk one); and error mapping to `ResourceError`, so callers branch on `error.status` / `isTransientResourceError(error)` rather than message prose. Inject `clock` and drive it with `@mailwoman/core/api/test-clocks` — timing tests stay deterministic and never sleep. `filer/sdk/sec-client.ts` is the worked example (711 → 417 lines); `bdc/sdk/client.ts` is still raw `fetch` and is next.
