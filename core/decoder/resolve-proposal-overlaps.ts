 ...
 *   `rule`) and filters them per-component via the policy registry. That per-*tag* filter is blind
 *   to cross-*tag* span overlap: a `neural` street span `[0,11]` ("350 5th Ave") and a `rule`
 ...
 *   have overlapping spans.** It is a greedy interval selection — accept proposals in priority order,
 *   skip any that overlap an already-accepted span.
 *
 *   ## The selection policy (the gate-tunable lever)
 *
 *   Priority is **confidence desc, then shorter span first, then earlier start**:
 *
 *   - *Confidence primary* respects the arbitration that already happened — a source the registry
 *     kept at high confidence wins its span.
 *   - *Shorter-span-first on ties* preserves finer decompositions: given equal-confidence
 *     `street[0,11]` vs `{house_number[0,3], street[4,11]}`, the two finer spans are accepted and the
 *     coarse subsuming span is dropped — keeping the street+house_number precondition intact (the
 *     thing #566 broke). The neural argmax path labels per-token, so it normally emits the finer
 *     decomposition itself; this tiebreak is the safety net when a coarse rule span competes.
 *
 ...
 *   to look. (An alternative — earliest-end-first maximal-tiling, ignoring confidence — maximizes the
 *   *count* of non-overlapping spans but can let a spurious tiny span evict a correct large one;
 *   confidence-primary guards against that.)
 *
...
/**
 * Reduce a set of (possibly overlapping) arbitrated proposals to a coherent, non-overlapping set via
 * greedy interval selection. The output is sorted by span start (the order {@link proposalsToTree}
 * expects). Input is not mutated.
 *
 ...
 */
export function resolveProposalOverlaps(
