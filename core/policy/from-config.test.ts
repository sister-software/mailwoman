 ...
 *
 *   The loud-validation contract for policy-config loading (#478). Every rejection path throws with
 *   the offending JSON path — a config typo must never silently fall back to defaults.
 */
...
describe("policyRegistryFromConfig", () => {
	...
	it("throws on an unknown mode", () => {
		expect(() => policyRegistryFromConfig({ "*": { region: { mode: "neural" as never } } })).toThrow(
			/mode "neural" is not one of/
		)
	})
	...
	it("throws on an out-of-range threshold", () => {
		expect(() => policyRegistryFromConfig({ "*": { region: { mode: "both", confidence_threshold: 1.5 } } })).toThrow(
