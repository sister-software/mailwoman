 ...
 * - `neural_preferred`: keep all proposals, but drop rule proposals when at least one neural proposal
 *   exists for the same component.
...
/**
 * A single policy entry. Locale-less entries are the global default; locale-scoped entries override
...
export interface ClassifierPolicy {
	...
	/**
	 * Minimum confidence for a proposal to be retained. Applied before the policy-mode filter.
	 * Inclusive ([threshold, 1.0]). Undefined means "no threshold."
	 */
	confidence_threshold?: number

	/**
	 * Locale scope. If absent, the entry applies to every locale unless overridden by a
...
export interface PolicyRegistry {
	/**
	 * Look up the effective policy for a (component, locale) pair. A locale-specific entry wins over
	 * a global one; if neither exists, the registry-wide default (`rule_only`, no threshold) is
	 * returned.
	 */
	lookup(component: ComponentTag, locale?: string): ClassifierPolicy

	/**
	 * Apply policy filtering to a flat list of proposals. Output is a new array; the input is not
	 * mutated.
	 */
