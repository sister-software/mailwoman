 ...
 *
 *   Tests for #370 "Lever A" — postcode-disambiguated locality selection (`opts.postcodeConsistency`).
 *   A same-named locality that resolves far from a resolved sibling postcode is either re-picked from
 *   its alternatives (the same-named instance nearest the postcode) or, if none reconciles, has its
 *   coordinate fall back to the postcode point + flagged. Byte-stable when the flag is unset.
 */
...
const PC = { id: 900, name: "75001", placetype: "postalcode", country: "FR", lat: 48.86, lon: 2.35, score: 1, exactMatch: true }
const SP_FAR = { id: 1, name: "Saint-Pierre", placetype: "locality", country: "FR", lat: 44.0, lon: 5.0, score: 8, exactMatch: true } // ~600 km from PC
const SP_NEAR = { id: 2, name: "Saint-Pierre", placetype: "locality", country: "FR", lat: 48.9, lon: 2.4, score: 7, exactMatch: true } // ~6 km from PC
...
describe("resolveTree + postcodeConsistency (Lever A)", () => {
	it("re-picks the same-named locality nearest the postcode (the wrong instance was the top match)", async () => {
		...
		const out = await resolver.resolveTree(tree([postcodeNode(), localityNode()]), { defaultCountry: "FR", postcodeConsistency: true })
