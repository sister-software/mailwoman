...
const v353 = await pc("scratchpad/v353-numsplice23-cache")
const n = v310.scored
const rate = (m: number) => ((n - m) / n).toFixed(3)
console.log(`parity postcode (n=${n})`)
console.log(`  v310:  misses=${v310.miss.length} rate=${rate(v310.miss.length)}`)
console.log(`  v352 (3-digit):  misses=${v352.miss.length} rate=${rate(v352.miss.length)}`)
console.log(`  v353 (10-999):   misses=${v353.miss.length} rate=${rate(v353.miss.length)}`)
const v310in = new Set(v310.miss.map((m) => m.split("  ")[0]))
console.log("\n=== v353 NEW postcode regressions vs v310 (guard) ===")
let r = 0
for (const m of v353.miss)
	if (!v310in.has(m.split("  ")[0])) {
		r++
		console.log("  " + m)
	}
console.log(`v353 new regressions: ${r}  (v352 had 0, v351 had 7)`)
