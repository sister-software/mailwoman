...
const v354 = await pc("scratchpad/v354-numsplice3-full-cache")
...
console.log(`  v354 (2k 3-digit 100-999): misses=${v354.miss.length}  rate=${rate(v354.miss.length)}`)

const v310inputs = new Set(v310.miss.map((m) => m.split("  ")[0]))
console.log("\n=== rows v354 gets WRONG on postcode that v310 got RIGHT (the guard — should be ~empty) ===")
let regressions = 0
for (const m of v354.miss)
	if (!v310inputs.has(m.split("  ")[0])) {
		regressions++
		console.log("  " + m)
	}
console.log(`\nv354 NEW postcode regressions vs v310: ${regressions}  (v351 had 7)`)
