...
def _model(cfg):
    # char_mode never reads the SP table, so a placeholder vocab size mirrors train.py's char-path call.
...
def test_build_optimizer_splits_the_47_label_head_into_its_own_group():
    ...
    # The two classifier tensors — weight (47, 384) + bias (47) = 18,095 params, the number the
    # launch log prints.
...
def test_every_trainable_param_lands_in_exactly_one_group():
    """A carve-out that dropped params would train them at no LR at all."""
    cfg = load_config(FULL)
    model = _model(cfg)
...
def test_the_fourteen_fresh_rows_share_one_tensor_with_the_other_thirty_three():
    """The group is the whole head: `stage3-jp` appends its 7 tags, so the 14 fresh rows are ids 33..46 of a single `nn.Linear` weight."""
    jp = resolve_label_set("stage3-jp")
    fresh = [jp.label_to_id[f"{prefix}-{tag}"] for tag in JP_FINE_TAGS for prefix in ("B", "I")]

    assert len(fresh) == 14
    # The fresh rows are a contiguous tail after the 33 stage3 labels.
    ...
    # The stage3 ids are preserved verbatim, so a 33-label checkpoint would load "aligned" and still
    # be wrong for this corpus.
...
def test_a_47_label_checkpoint_will_not_load_into_a_33_label_head():
    """`strict=False` does not tolerate a size mismatch, so warm-starting the probe checkpoint would crash rather than partially apply."""
