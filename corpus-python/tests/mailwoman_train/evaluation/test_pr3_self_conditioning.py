...
def test_locale_id_map():
    ...
    assert locale_id("de") == LOCALE_TO_ID["DE"]
    assert locale_id("ZZ") == IGNORE_INDEX
...
def test_pilot_config_loads_and_matches_scope():
    ...
    assert cfg.model.crf_loss_weight == 0.0
    assert cfg.model.label_smoothing == 0.1
    assert set(cfg.data.country_weights) == {"US", "FR", "DE"}

    assert "v0.4.1-de" in cfg.data.corpus_dir
    assert (cfg.data.source_weights or {}).get("synth-german", 0) > 0
    assert cfg.train.max_steps == 20000
