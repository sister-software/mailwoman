...
def test_raw_load_state_dict_clobbers_a_changed_classifier_lr(tmp_path):
    ...
    m1 = TinyModel()
    optim1 = build_optimizer(m1, learning_rate=1e-5, weight_decay=0.01, classifier_learning_rate=1e-3)
    ...
    m2 = TinyModel()
    optim2 = build_optimizer(m2, learning_rate=1e-5, weight_decay=0.01, classifier_learning_rate=1e-4)
...
def test_restamp_resume_lrs_recovers_the_live_config_value(tmp_path, capsys):
    ...
    labels = ["base", "classifier_learning_rate"]
...
def test_restamp_resume_lrs_is_silent_when_nothing_changed(tmp_path, capsys):
    ...
    optim2 = build_optimizer(m2, learning_rate=1e-5, weight_decay=0.01, classifier_learning_rate=1e-3)
    live_lrs = [g["lr"] for g in optim2.param_groups]
    labels = ["base", "classifier_learning_rate"]
    sched2 = _build_scheduler(optim2, _scheduler_cfg(warmup_steps=2))
