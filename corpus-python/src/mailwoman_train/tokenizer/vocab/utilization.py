...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "colon-separated FROM:TO prefix remap for manifest parquet paths; defaults to "
    ...
    files = [s["path"].replace(src, dst, 1) for s in manifest["slices"] if s["split"] == "train"]
    missing = [s for s in files if not os.path.exists(s)]

    if missing:
        raise SystemExit(f"{len(missing)} parquet paths missing locally, first: {missing[0]}")

    total_rows = manifest.get("total_rows")
    print(f"[utilization] {len(files)} train files, {total_rows:,} rows, vocab {vocab_size}, workers {args.workers}")
    ...
    with ProcessPoolExecutor(max_workers=args.workers, initializer=_init_worker, initargs=(args.tokenizer,)) as pool:
        futures = {pool.submit(_count_file, s, vocab_size): s for s in files}
        ...
        for future in as_completed(futures):
            ...
            if done % 25 == 0 or done == len(files):
                ...
                print(
                    f"[utilization] {done}/{len(files)} files ({rate:.1f}/s) — fired {fired}/{vocab_size} "
