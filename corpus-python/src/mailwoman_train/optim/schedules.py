...
def linear_cooldown(optimizer: AdamW, cooldown_start: int, max_steps: int) -> LambdaLR:
    ...
    full rerun (Hägele et al. 2024, arXiv:2405.18392; MiniCPM, arXiv:2404.06395; Chinchilla's
...
def constant_with_warmup(optimizer: AdamW, warmup_steps: int) -> LambdaLR:
    # Linear warmup → constant: cosine decay over a short smoke-test window collapses the LR before
    # divergence shows in the loss curve.
    def lr_lambda(step: int) -> float:
        if step < warmup_steps:
            return float(step) / float(max(1, warmup_steps))
        return 1.0
