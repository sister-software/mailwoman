...
def main() -> None:
    ...
    recs = [json.loads(line) for line in Path(args.conf).read_text().splitlines() if line.strip()]
    ...
    lines.append("")
    lines.append(
        f"MCE (bins n≥20) {robust_mce(rel_raw):.4f} → {robust_mce(rel_cal):.4f} · "
    ...
    def reliability_table(title: str, rows: list[dict], label: str) -> None:
        ...
        for r in rows:
            ...
            lines.append(
                f"| [{r['lo']:.2f}, {r['hi']:.2f}) | {r['n']} | {fmt(r['conf'])} | {fmt(r['acc'])} | {gap:.3f} |"
            )
        lines.append("")
